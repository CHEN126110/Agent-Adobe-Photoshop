...
import type {
    EagleCandidateVisualEvidenceRequest,
...
export interface EagleCandidateVisualInsightRequest {
    ...
    requestedEvidence: EagleCandidateVisualEvidenceRequest[];
...
function buildBaseRequest(input: {
    ...
    return {
        ...
        evidence: [{
            source: 'eagle-candidate-visual-insight-request',
            summary: input.evidenceSummary,
            status: input.status === 'ready_for_visual_insight_fill_plan' ? 'needs_review' : 'unknown'
        }],
        boundary: BOUNDARY,
        canRunEagle: false,
        canRunAgentRuntime: false,
...
function buildVisualSamplingPlan(input: {
    ...
    const samplingCandidate: ProjectVisualSamplingCandidate = {
        ...
        priority: 95,
        score: 95,
        reason: `Eagle 候选 ${input.candidate.candidateId} 已显式选择，并由 ${input.projectAssetRef.source} 关联到项目素材；仍需真实视觉证据。`,
        cacheKey,
        cacheStatus: 'miss',
        shouldAnalyze: true,
        requiredEvidence,
        evidence: [{
            source: 'eagle-candidate-visual-insight-request',
            summary: `${input.projectAssetRef.projectRelativePath} queued from selected Eagle candidate metadata; visual evidence still required.`,
            status: 'unknown'
        }]
    };
    ...
    return {
        ...
        evidence: [{
            source: 'eagle-candidate-visual-insight-request',
            summary: `Built bounded visual sampling plan for ${input.projectAssetRef.projectRelativePath}.`,
            status: 'needs_review'
        }]
    };
}
...
export function buildEagleCandidateVisualInsightRequest(
    ...
    const requestedEvidence = handoff?.visualAnalysisRequest?.requestedEvidence || [];
    ...
    if (!handoff || handoff.status !== 'ready_for_visual_analysis_request' || !selectedCandidate) {
        return buildBaseRequest({
            ...
            evidenceSummary: 'Eagle candidate handoff is not ready; no visual sampling or cache fill plan was created.'
        });
    }
    ...
    if (!projectAssetRef) {
        return buildBaseRequest({
            ...
            warnings: handoff.warnings,
            evidenceSummary: 'Selected Eagle candidate is ready, but no explicit project asset ref was provided.'
        });
    }
    ...
    return buildBaseRequest({
        ...
        evidenceSummary: `Built visual insight fill plan request for ${projectAssetRef.projectRelativePath}; execution remains separate.`
    });
}
