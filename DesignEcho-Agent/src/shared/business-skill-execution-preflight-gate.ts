...
function buildMissingContextInputs(
    ...
    return REQUIRED_EXECUTION_CONTEXT
        // 三态语义（治理 2026-08-04）：字段未提供（undefined）视为 unknown，放行；
        // 只有显式 false（确认缺失）才算缺失输入。此前 !== true 会把调用方漏传字段
        // 静默折成缺失，若本 gate 前移为执行门禁会直接阻断（见 docs/agent-gates-definitions.md）。
        .filter((key) => contextState?.[key] === false)
