/**
 * 设计团队评审裁决解析（纯逻辑，主/渲染进程共用，可由现有核心验证复用）
 *
...
/**
 * 把确定性评分卡并进评审裁决（critic 接评分卡的单一并轨口，纯逻辑可复用验证）。
 *
 * 调用方契约：scorecard 应来自 evaluateDeterministicAssertions → scoreDesignAssertions
 * （仅有唯一答案的确定性事实断言，0 token）。主体占比、对齐、对比、字号比等测量信号
 * 不得在这里获得美学否决权，必须结合 Brief 和画面由视觉评审解释。
 *
 ...
 * - 仅携带 blockerKind + proofRef 的确定性失败强制翻 needs_fix——该事实不允许被模型散文结论（pass）或
 *   不可机读（unparseable）抵消；
...
export function mergeDeterministicScorecardIntoCriticVerdict(
    ...
    // R-040 containment：critic 不能旁路 scorecard/verdict 的证据资格，裸 severity=blocker
    // 与审美/VLM finding 只随 issues 提示，不抵消模型的 pass。
    const hasBlockerFailure = scorecard.blockers.some(isQualifiedDesignQualityHardBlocker);
