...
export interface DesignPlannerInput {
    ...
    existingEvidence?: DesignAgentOsEvidence[];
...
export interface DesignPlannerSelectedContext {
    ...
    businessVisualEvidenceGate?: BusinessSkillVisualEvidenceGate;
    knowledge: DesignKnowledgeResult[];
    evidence: DesignAgentOsEvidence[];
    performancePolicy?: {
...
function isChatOnlyIntent(intent: UserIntent): boolean {
    return intent.action === 'chat' || (!intent.requiresPhotoshop && intent.targetScenario === 'unknown');
}

function isSaveOrExportIntent(intent: UserIntent): boolean {
...
function buildSelectedContext(
    ...
    const businessVisualEvidenceGate = buildBusinessSkillVisualEvidenceGate({
        ...
        requiresVisualEvidence: options.requiresVisualEvidence
    });
    const notes: string[] = [];
    if (knowledge.length) {
...
function buildGenericDesignDsl(input: DesignPlannerInput, scenario: DesignAgentOsScenario): DesignDSL | undefined {
    ...
    return {
        ...
        evidence: [{
            ...
            summary: `从当前文档尺寸生成保守 DSL：${width}x${height}。`,
            status: 'needs_review'
        }]
    };
}
...
function makeStep(
    ...
    return { id, operation, target, params, reason, expectedEvidence };
}

function buildPlannerSteps(input: DesignPlannerInput, intent: UserIntent, dsl: DesignDSL | undefined): ExecutionPlanStep[] {
    ...
    if (hasUsableAsset(input)) {
        steps.push(makeStep(
            ...
            ['selected asset path', 'asset dimensions', 'asset role', 'ProjectAssetIndex candidate evidence', 'VisualSamplingPlan evidence', 'VisualInsightCache evidence']
        ));
    }
    ...
    steps.push(makeStep(
        ...
        'photoshop-result',
        {
            requiredEvidence: ['tool result', 'layer bounds', 'verification report']
        },
        '工具成功不等于设计完成；执行后必须回收验收证据。',
        ['VerificationReport', 'bounds or screenshot evidence']
    ));
...
function determineReadiness(
    ...
    if (isSaveOrExportIntent(intent)) {
        if (!input.currentDocument) {
            warnings.push('保存/导出需要当前 Photoshop 文档证据。');
...
function buildVerificationPlan(input: {
    ...
    return {
        ...
        limitations: [
            ...
            '知识搜索、智能缩放和文案 evidence 只能作为上下文，不能跳过验收层。'
        ],
        evidence: [{
            source: 'design-planner',
            summary: `生成 ${input.steps.length} 个 plan-only 步骤。`,
            status
        }]
    };
}
...
export function planDesignTask(input: DesignPlannerInput): DesignPlannerOutput {
    ...
    const intent = buildUserIntentFromText(input.userText, {
        scenario,
        constraints: [
            ...(input.constraints || []),
    ...
    const scenarioForPlan = intent.targetScenario === 'unknown' ? 'general-design' : intent.targetScenario;
    const requiresVisualEvidence = !isChatOnlyIntent(intent) && !isSaveOrExportIntent(intent);
    const selectedContext = buildSelectedContext(input, performancePolicy, {
        scenario: scenarioForPlan,
        requiresVisualEvidence
    });
    const designDsl = isChatOnlyIntent(intent) || isSaveOrExportIntent(intent)
        ? undefined
        : buildPlannerDesignDsl(input, intent.targetScenario === 'unknown' ? 'general-design' : intent.targetScenario);
    const steps = buildPlannerSteps(input, intent, designDsl);
    const readiness = determineReadiness(input, intent, blockers, warnings);
    const brief = buildDesignBriefFromIntent(intent, {
        goal: intent.normalizedText || '未提供明确设计任务。',
        constraints: [
            ...(input.constraints || []),
            `readiness=${readiness}`,
    ...
    const executionPlan: ExecutionPlan = {
        ...
        evidence: [{
            source: 'design-planner',
            summary: `Planner readiness=${readiness}; action=${intent.action}; scenario=${intent.targetScenario}。`,
            status: readiness === 'blocked' ? 'failed' : 'needs_review'
        }],
        limitations: [
...
export function mapPlannerOutputToDesignAgentOsEvidence(output: DesignPlannerOutput): DesignAgentOsEvidence {
    return {
...
function extractVerificationTargets(output: DesignPlannerOutput): string[] {
    ...
    if (targets.size === 0 && output.intent.requiresPhotoshop) {
...
export function buildPlannerExecutionPreflightGate(
    ...
    if (output.readiness === 'blocked') {
        return {
            ...
            evidence: [{
                source: 'design-planner-preflight',
                summary: `blocked; blockers=${output.blockers.length}`,
                status: 'failed'
            }]
        };
    }
    ...
    if (output.readiness === 'needs_context' && !allowNeedsContextExecution) {
        return {
            ...
            evidence: [{
                source: 'design-planner-preflight',
                summary: `needs_context; required=${requiredContext.join(', ') || 'additional-design-context'}`,
                status: 'needs_review'
            }]
        };
    }
    ...
    return {
        ...
        reason: output.readiness === 'needs_context'
            ...
            : 'Planner readiness=ready，可以进入执行并继续收集验收证据。',
        blockers: output.blockers,
        warnings: output.warnings,
        requiredContext,
        verificationTargets,
        ...
        evidence: [{
            source: 'design-planner-preflight',
            summary: `decision=execute; readiness=${output.readiness}; plannedSteps=${plannedStepIds.length}`,
            status: 'needs_review'
        }]
    };
}
