...
export interface BuildAgentTaskPlanningContractInput {
    ...
    /**
     * 只有结构化续跑、受控计划或确定性 Skill 才能把“工具可用”升级为
     * “本轮必须产生真实任务进展”。普通自然语言由主 Agent 自主决定是否调用工具。
     */
 ...
 * 工具可见或可执行不等于本轮必须调用工具。只有上游结构化 owner 显式签发进展义务的
 * 请求，才至少要产生一个非 Harness 控制调用，才能进入完成判定。
 */
export function requiresAgentTaskProgress(
    plan?: AgentTaskPlanningContract
): boolean {
    if (plan?.requiresTaskProgress !== true) return false;
