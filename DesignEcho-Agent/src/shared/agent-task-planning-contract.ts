...
import {
    buildPlanningGeneralArtifactPattern,
    buildPlanningLongFormSignalPattern,
    buildPlanningPrimaryVisualSignalPattern,
    buildPlanningSkuSignalPattern,
    buildPlanningWhiteBackgroundSignalPattern
...
export type AgentTaskProgressObligation = 'none' | 'observation' | 'delivery';
...
export interface BuildAgentTaskPlanningContractInput {
    ...
    /**
     * 由上游高置信生产证据、结构化 owner / 续跑、受控计划或确定性 Skill，把
     * “工具可用”升级为“本轮必须产生真实任务进展”。问答、候选与只读请求不得签发该义务。
     */
    requiresTaskProgress?: boolean;
    /**
     * 进展义务的强度：纯检查只要求可信观察；明确制作要求真实交付动作。
     * 旧调用方只传 requiresTaskProgress=true 时按 delivery 兼容。
     */
...
export interface AgentTaskPlanningContract {
    ...
    /** 工具可用性之外的独立完成义务；旧持久化计划缺失该字段时按 false 处理。 */
    requiresTaskProgress?: boolean;
    /** 观察与交付必须分开；条件检查不能被误签成无条件写入。 */
    taskProgressObligation?: AgentTaskProgressObligation;
 ...
 * 工具可见或可执行不等于本轮必须调用工具。只有上游高置信生产证据或结构化
 * owner 显式签发进展义务的请求，才至少要产生一个非 Harness 控制调用，才能进入完成判定。
 */
export function requiresAgentTaskProgress(
    plan?: AgentTaskPlanningContract
): boolean {
    if (resolveAgentTaskProgressObligation(plan) === 'none') return false;
...
/**
 * 读取 TaskPlan 的结构化进展义务。旧计划只有 requiresTaskProgress=true 时，
 * 继续按历史的交付义务解释，避免持久化任务在升级后被静默放松。
 */
export function resolveAgentTaskProgressObligation(
    plan?: AgentTaskPlanningContract
): AgentTaskProgressObligation {
    if (plan?.taskProgressObligation) return plan.taskProgressObligation;
    return plan?.requiresTaskProgress === true ? 'delivery' : 'none';
}

export function requiresAgentTaskDeliveryProgress(
    plan?: AgentTaskPlanningContract
): boolean {
    return requiresAgentTaskProgress(plan)
        && resolveAgentTaskProgressObligation(plan) === 'delivery';
}
...
function hasSkuSignal(text: string): boolean {
    return new RegExp(buildPlanningSkuSignalPattern()).test(text);
}

function hasDetailPageSignal(text: string): boolean {
    return new RegExp(buildPlanningLongFormSignalPattern()).test(text);
}

function hasExplicitDetailPageDocumentTarget(text: string): boolean {
    const targetTerms = buildPlanningLongFormSignalPattern();
    return new RegExp(`(${targetTerms})\\s*(文档|画布|草稿|设计稿|页面|模板)`).test(text)
        || new RegExp(`(创建|新建|制作|生成|设计|完成|做)\\s*(一个|一张|一版|个|张)?[^。！？!?，,]{0,24}(${targetTerms})`).test(text);
}

function hasMainImageSignal(text: string): boolean {
    return new RegExp(buildPlanningPrimaryVisualSignalPattern()).test(text);
}

function hasWhiteBackgroundSignal(text: string): boolean {
    return new RegExp(buildPlanningWhiteBackgroundSignalPattern()).test(text);
}

function isGeneralDesignArtifactRequest(text: string): boolean {
    const artifact = buildPlanningGeneralArtifactPattern();
...
function usesReferenceMethod(input: { text: string; skillId?: string }): boolean {
    return input.skillId === 'layout-replication'
        || /参考|复刻|仿照|照着|还原|复现|临摹|同款(?:版式|设计|效果|画面)|(?:做|设计|制作|改).{0,8}同款/.test(input.text);
}
...
/**
 * 用户在一句话里并列点名了主图、详情页、SKU 三类交付物。
 *
 * 真机 2026-07-30：「下面你需要帮我开始规划设计主图详情页SKU」被 inferScenario 的先到先得链
 * 判成单一品类（主图信号先命中，后面两类分支根本不执行），于是额外叠加了
 * 「这里的 SKU 表示主图白底图素材来源」——把用户明确要的 SKU 交付物曲解成了主图的素材。
 * 三类同时点名时不存在这种从属关系（详情页不可能是主图的素材来源），必须按并列交付物处理。
 *
 * 判据取三类齐全而非任意两类：「做主图，白底用 SKU 里的图」是真的从属场景，不能一起误伤。
 */
function listsAllPrimaryDeliverableKinds(text: string): boolean {
    return hasMainImageSignal(text) && hasDetailPageSignal(text) && hasSkuSignal(text);
}

function usesSkuAsMainImageSource(input: { text: string; skillId?: string }): boolean {
    if (listsAllPrimaryDeliverableKinds(input.text)) return false;
...
function inferScenario(input: {
    ...
    const declaredScenario = input.manifest
        ? [
            ...
            .map((entrySkillId) => getSkillById(entrySkillId)?.visualSamplingScenario)
            .find(Boolean)
        : undefined;
    ...
    // 普通自然语言走 autonomous-agent 时，计划层只保存中性目标，不在主模型判断前
    // 用品类词虚构场景、交付物或视觉前置条件。结构化 Manifest 已在上方优先解析；
    // 未结构化请求由模型在循环内 declareDesignIntent 或按真实行为选择能力。
    if (input.skillId === 'autonomous-agent') return 'unknown';
...
function buildDeliverables(input: {
    ...
    if (input.route === 'autonomous_agent') return withMethodOutputs(['agent_resolved_result']);
...
function buildConstraints(input: {
    ...
    const constraints = [
        '先明确任务目标、必要上下文和验收目标，再允许有状态工具执行。',
        '工具执行成功只证明调用返回，不等于用户目标或视觉质量已经通过。',
        '不能输出或消费无依据的分数字段。'
    ];
    if (input.manifest) {
        constraints.push(
    ...
    if (!input.manifest && input.skillId === 'sku-batch') {
        constraints.push(
...
export function buildAgentTaskPlanningContract(
    ...
    return {
        ...
        requiresTaskProgress: input.requiresTaskProgress === true,
        taskProgressObligation: input.requiresTaskProgress === true
            ? input.taskProgressObligation || 'delivery'
            : 'none',
        requiredInputs,
