...
import {
    ...
    type LivePhotoshopAcceptanceEvidenceIntake
} from './live-photoshop-acceptance-evidence-intake';

export type AgentAcceptanceEvidenceMatrixVersion = 'agent-acceptance-evidence-matrix/v0';
...
export interface BuildAgentAcceptanceEvidenceMatrixInput {
...
export interface AgentAcceptanceEvidenceModeSummary {
    ...
    livePhotoshopEvidenceIntake?: LivePhotoshopAcceptanceEvidenceIntake;
    evidenceReady: boolean;
    qualityClaimAllowed: false;
    nextAction: string;
}

export interface AgentAcceptanceEvidenceMatrixReport {
    version: AgentAcceptanceEvidenceMatrixVersion;
    generatedAt: string;
    modes: AgentAcceptanceEvidenceModeSummary[];
    totals: {
        modeCount: number;
...
export function buildAgentAcceptanceEvidenceMatrix(
    input: BuildAgentAcceptanceEvidenceMatrixInput = {}
): AgentAcceptanceEvidenceMatrixReport {
    const modes = AGENT_ACCEPTANCE_MODE_IDS.map((mode) => {
        ...
        const livePhotoshopEvidenceIntake = buildLivePhotoshopEvidenceIntakeForMode(mode, input.artifacts?.[mode]);

        return {
            mode,
            controlPlane,
            artifact,
            livePhotoshopEvidenceIntake,
            evidenceReady: artifact.status === 'passed' || artifact.status === 'skipped',
    ...
    return {
        version: 'agent-acceptance-evidence-matrix/v0',
        generatedAt: new Date().toISOString(),
        modes,
        totals: {
            modeCount: modes.length,
            evidenceReadyCount: modes.filter((mode) => mode.evidenceReady).length,
        ...
        boundaries: [
            ...
            'qualityClaimAllowed is always false here because design quality requires separate visual evidence and review.'
        ]
    };
}

export function formatAgentAcceptanceEvidenceMatrixMarkdown(
    report: AgentAcceptanceEvidenceMatrixReport
): string {
    const lines = [
        '# Agent Acceptance Evidence Matrix',
        '',
        `- version: ${report.version}`,
        `- generatedAt: ${report.generatedAt}`,
        `- evidenceReady: ${report.totals.evidenceReadyCount}/${report.totals.modeCount}`,
    ...
    for (const item of report.modes) {
        ...
        if (item.livePhotoshopEvidenceIntake) {
            lines.push(`- livePhotoshopEvidenceStatus: ${item.livePhotoshopEvidenceIntake.status}`);
            lines.push(`- livePhotoshopEvidenceRequiredNext: ${item.livePhotoshopEvidenceIntake.requiredNextEvidence.join(', ') || 'none'}`);
        }
...
function buildLivePhotoshopEvidenceIntakeForMode(
    mode: AgentAcceptanceControlPlaneModeId,
    artifact: AgentAcceptanceRawArtifact | undefined
): LivePhotoshopAcceptanceEvidenceIntake | undefined {
    if (!mode.startsWith('live-photoshop')) return undefined;
    return buildLivePhotoshopAcceptanceEvidenceIntake({
        artifact,
...
function buildNextAction(
    ...
    if (artifact.status === 'not_found') {
        return `Run ${controlPlane.script} to generate evidence for this mode.`;
    }
    ...
    return 'Artifact is available; use it as mode-scoped evidence only.';
}
