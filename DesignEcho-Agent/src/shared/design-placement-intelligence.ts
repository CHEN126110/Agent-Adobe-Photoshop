...
export type DesignAssetDirectUseSuitability = 'suitable' | 'conditional' | 'unsuitable';
export type DesignAssetVisualRole =
    | 'hero_product'
    | 'hero_scene'
    | 'model_context'
    | 'detail_evidence'
    ...
    | 'reference'
    | 'unknown';
export type DesignAssetSourceTreatment =
    | 'direct_full_frame'
    | 'clip_to_container'
    | 'matte_and_recompose'
    | 'supporting_only'
...
export type DesignAssetSelectionMode = 'auto' | 'suggest' | 'force';
export type DesignAssetSelectionAuthority = 'visual_evidence' | 'agent_judgment' | 'harness_receipt';
export interface DesignAssetForceSelectionReceipt {
    authority: 'harness';
    receiptId: string;
    source: 'user' | 'project_fact';
    reason: string;
}
export type DesignAssetAutoPlacementDecisionCode =
    | 'ready'
    | 'no_asset_candidate'
    | 'candidate_confirmation_required'
    | 'design_role_required'
    | 'visual_selection_evidence_required'
    | 'candidate_requires_recomposition'
    | 'candidate_role_mismatch'
    | 'candidate_unsuitable'
    | 'candidate_direct_use_unverified'
    | 'candidate_score_below_threshold'
    | 'candidate_margin_below_threshold';

export interface DesignAssetAutoPlacementCandidateEvidence {
    score: number;
...
export interface DesignAssetAutoPlacementDecision {
    eligible: boolean;
    code: DesignAssetAutoPlacementDecisionCode;
    authority: DesignAssetSelectionAuthority;
    topScore: number;
    margin: number;
}

/**
 * 自动置入是素材选择与 Photoshop 写入之间的最后一道纯逻辑边界。
 * 它只判断是否已有足够证据安全地执行“直接置入”，不评价最终设计质量、不调用模型、
 * 不选择 Tool，也不把需要去底/重组的素材偷偷降级成矩形图片。
 */
export function evaluateDesignAssetAutoPlacement(input: {
    mode: DesignAssetSelectionMode;
    designRole?: string;
    minScore: number;
    minMargin: number;
    candidates: readonly DesignAssetAutoPlacementCandidateEvidence[];
    /** 只能由 Harness/controller 注入；不得从模型 Tool 参数透传。 */
    forceSelectionReceipt?: DesignAssetForceSelectionReceipt;
    /** @deprecated 模型自报来源只记作 agent_judgment，不具有授权效力。 */
    forceDecisionSource?: string;
    /** @deprecated 模型自报理由不能替代 Harness receipt。 */
    forceReason?: string;
}): DesignAssetAutoPlacementDecision {
    const top = input.candidates[0];
    const second = input.candidates[1];
    const topScore = Number.isFinite(Number(top?.score)) ? Number(top?.score) : 0;
    const secondScore = Number.isFinite(Number(second?.score)) ? Number(second?.score) : 0;
    const margin = second ? topScore - secondScore : topScore;
    const forceReceipt = input.forceSelectionReceipt;
    const validForceReceipt = forceReceipt?.authority === 'harness'
        && Boolean(String(forceReceipt.receiptId || '').trim())
        && ['user', 'project_fact'].includes(forceReceipt.source)
        && Boolean(String(forceReceipt.reason || '').trim());
    const defaultAuthority: DesignAssetSelectionAuthority = input.mode === 'force'
        ? 'agent_judgment'
        : 'visual_evidence';
    const decision = (
        code: DesignAssetAutoPlacementDecisionCode,
        authority: DesignAssetSelectionAuthority = defaultAuthority
    ): DesignAssetAutoPlacementDecision => ({
        eligible: code === 'ready',
        code,
        authority,
    ...
    if (!top) return decision('no_asset_candidate');
    if (input.mode === 'suggest') return decision('candidate_confirmation_required');
    if (input.mode === 'force' && validForceReceipt) {
        return decision('ready', 'harness_receipt');
    }
    if (!String(input.designRole || '').trim()) return decision('design_role_required');
    if (!top.visualObserved) return decision('visual_selection_evidence_required');
    if (top.sourceTreatment === 'matte_and_recompose') return decision('candidate_requires_recomposition');
    if (top.sourceTreatment === 'supporting_only') return decision('candidate_role_mismatch');
    if (top.sourceTreatment === 'reject' || top.directUseSuitability === 'unsuitable') {
        return decision('candidate_unsuitable');
    }
    const directPlacementTreatment = top.sourceTreatment === 'direct_full_frame'
        || top.sourceTreatment === 'clip_to_container';
    if (top.directUseSuitability !== 'suitable' || !directPlacementTreatment) {
        return decision('candidate_direct_use_unverified');
    }
    if (topScore < input.minScore) return decision('candidate_score_below_threshold');
    if (margin < input.minMargin) return decision('candidate_margin_below_threshold');
