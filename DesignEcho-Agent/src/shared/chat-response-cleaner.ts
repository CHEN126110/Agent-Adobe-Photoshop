...
function containsInternalThinkingLeak(value: string): boolean {
    ...
    return containsDsmlToolCallMarkup(text)
        ...
        // 过度矫正修复（2026-08-06）：本行原先还拦「用户让我 / 用户要求 / 用户想要 / 我得 /
        // 我需要确保 / 首先，我 / 接下来我」。这些不暴露任何内部机制，只是第一人称中文叙述，
        // 却是设计师说明思路时最自然的开头——"用户要求完成主图、详情页和SKU。首先，我需要
        // 了解项目里有哪些素材" 会同时命中三项，整段被判为思考泄漏丢弃，用户因此看到 Agent
        // 闷头调工具、一句话不说。清洗器要挡的是工程话术（manifest / schema / 工具名 / 状态码），
        // 不是禁止 Agent 用第一人称讲人话。这里只保留「模型在复述系统指令」这类真泄漏。
        || /(?:不能提工具名|不能说已经完成|私有链式思维)/u.test(text);
}
...
function rewriteDesignValidationTerms(value: string): string {
    return String(value || '')
        ...
        .replace(/layout\.regions\[(\d+)\]\.imagePlacement:cover_and_subject_fill_ratio_are_ambiguous/giu, (_match, index) => `第 ${Number(index) + 1} 张图片同时要求铺满区域和按指定主体占比完整放入，需要由设计助手保留其中一种意图`)
        .replace(/layout\.regions\[(\d+)\]\.imagePlacement:cover_conflicts_with_avoid_crop/giu, (_match, index) => `第 ${Number(index) + 1} 张图片同时要求铺满区域和完全不裁切，需要由设计助手保留其中一种意图`)
        .replace(/layout\.regions\[(\d+)\]\.imagePlacement:focal_point_and_subject_fill_ratio_conflict/giu, (_match, index) => `第 ${Number(index) + 1} 张图片同时指定了背景关注点和主体占比，需要由设计助手保留其中一种定位意图`)
        ...
        .replace(/background\.imagePlacement\.cropPolicy\s*[:：][^；\n]*/giu, '背景图片的裁切方式还没有说明清楚')
        .replace(/background\.imagePlacement\.subjectFillRatio\s*[:：][^；\n]*/giu, '背景图片不使用商品主体占比，请改用背景自身的对齐与裁切方式')
        .replace(/background\.imagePlacement\s*[:：][^；\n]*/giu, '背景图片的缩放、对齐或裁切方式还没有说明完整')
...
function rewriteUserFacingOperationTerms(value: string): string {
    return rewriteDesignValidationTerms(rewriteInternalAssetVocabulary(value))
        ...
        // ID 列表整体吃掉（2026-08-06）：原规则只匹配 layerId 后的第一个数字，
        // 「layerId：6, 10, 14, 18」会被替成「对应图层, 10, 14, 18」，裸 ID 照样端给用户。
        // 必须先于单数字规则，否则前者先命中就没机会了。
        .replace(/`?layerId`?\s*(?:为|:|：|=)\s*\d+(?:\s*[,，、]\s*\d+)+/gi, '指定的几个图层')
        .replace(/`?layerId`?\s*(?:为|:|：|=)\s*\d+/gi, '对应图层')
        .replace(/\(\s*layerId\s*[:：]\s*\d+\s*\)/gi, '')
        .replace(/\blayerId\b/gi, '对应图层')
        .replace(/\bdocumentId\b/gi, '文档标识')
        // 门禁指路里会出现的工具原名：这些字符串来自 preflight 给模型的纠错指导，
        // 走到用户可见通道时必须是人话（工具卡片那侧本来就用显示名）。
        .replace(/\bopenProjectFile\b/g, '打开项目文件')
        .replace(/\bcreateDocument\b/g, '新建文档')
        .replace(/\bswitchDocument\b/g, '切换文档')
        .replace(/\blistDocuments\b/g, '查看文档列表')
        .replace(/\bsetLayerVisibility\b/g, '设置图层可见性')
...
export interface UserVisibleTextSanitizeOptions {
    /**
     * 这段文本是否由真实工具结果支撑（技能执行完，模型把结果讲给用户听）。
     *
     * 罐头能力菜单那三条判据针对的是「模型该干活却只做能力推销」的对话路径，靠措辞识别推销。
     * 而工具结果支撑的汇报天然会复述交付范围、分点列出待补输入，措辞与能力菜单高度重合：
     * 真机 2026-07-30，用户同一句「帮我规划设计主图详情页SKU」下，MiMo 与 DeepSeek 两种完全
     * 不同措辞的优质规划回复先后被整段判死，用户侧显示成「当前模型没有生成面向用户的判断，
     * 请切换可用模型」——而模型和措辞都不是原因，换模型也无效。
     *
     * 所以这里按语境区分而不是继续补措辞白名单：有工具结果作事实基础的汇报，不适用推销判据；
     * 内部泄漏、路由载荷、诊断文本等清洗一律照常生效。
     */
    toolResultBacked?: boolean;
}

export function sanitizeUserVisibleAgentText(
    content: string,
    options?: UserVisibleTextSanitizeOptions
): string {
...
export function sanitizeUserVisibleAssistantBodyText(
    content: string,
    options?: UserVisibleTextSanitizeOptions
): string {
    ...
    const agentVisibleText = sanitizeUserVisibleAgentText(content, options).trim();
...
export function sanitizeUserVisibleDiagnosticText(
    ...
    // 只认带上下文的认证信号：裸 401 会被回复正文里任意一个 401（色号/尺寸/数值）点亮。
    const hasExplicitConversationalAuthSignal = /(鉴权失败|认证失败|Invalid\s+API\s+Key|API\s*Key\s*(?:无效|错误|未配置|不可用)|http\s*(?:status\s*)?401|status(?:\s*code)?\s*401|\bunauthorized\b)/i.test(cleaned);
    ...
    if (hasExplicitConversationalAuthSignal && looksLikeConversationalUnavailable) {
        // 留痕：这条替换会把模型的真实回复整个换掉，误判时用户只看到「没有通过认证」，
        // 而调用侧毫无异常可查——必须能回溯是哪段文字触发的。
        console.warn(`[ResponseCleaner] 判定为对话模型认证失败并替换回复：${cleaned.slice(0, 300)}`);
    ...
    // 这两个条件原本各含一个裸 401，回复里任何一个 401 都能同时点亮两边，
    // 把一条正常回复整个换成认证失败文案（真机 2026-08-01：只有某条特定消息复现，
    // 而模型压根没被调用、主进程零错误日志）。改为只认带上下文的认证语义。
    if (/(当前)?对话模型(鉴权失败|连接未通过)|Invalid\s+API\s+Key|API\s*Key\s*(?:无效|错误|未配置|不可用)|http\s*(?:status\s*)?401|\bunauthorized\b/i.test(cleaned)
        && /对话模型|AI\s*(?:对话服务|连接)|conversation|conversational|自然回复|Invalid\s+API\s+Key|\bunauthorized\b/i.test(cleaned)) {
        // 留痕：这条替换会把模型的真实回复整个换掉，误判时用户只看到「没有通过认证」，
        // 而调用侧毫无异常可查——必须能回溯是哪段文字触发的。
        console.warn(`[ResponseCleaner] 判定为对话模型认证失败并替换回复：${cleaned.slice(0, 300)}`);
        return CONVERSATIONAL_MODEL_AUTH_FAILURE_MESSAGE;
...
export function cleanAssistantFailureErrorText(error?: string): string {
    ...
    // 未知错误只进入诊断日志。UI 没有可靠公开映射时宁可保留调用方的自然标题，
    // 也不能把 provider / Runtime / Tool 的原始报文包装成“错误: 原文”交给用户。
    return '';
}
