 ...
 *
 * 关键红线（红线1）：本门禁触发的降级对原任务 mutation 必须是【终态】——不得重放原任务、不得重复
 * mutation（否则会把"复制这个图层"重跑成复制两次，正是项目记忆 system-refactor-432 记的病灶）。
 * decision.terminal 与 downgrade 同真；若 Manifest Profile 同时明确缺少写后结构/视觉读回，调用方只能
 * 续接 R5 补读与复核，不得取得 E1 写权限。本模块不发起降级动作、不改运行时，只做确定性判定。
 *
...
export interface CompletionObservationGateDecision {
    ...
    downgrade: boolean;
    /** 该降级是否终止原任务 mutation 重放。与 downgrade 同真；不排斥上层只续接 R5 补读。 */
    terminal: boolean;
