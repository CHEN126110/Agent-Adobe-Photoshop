...
import {
    ...
    hasSkuExistingTemplateClaimText,
...
const STAGE_DEFINITIONS: Record<SkuWorkflowStageId, SkuWorkflowStage> = {
    ...
    confirm_combos: {
        ...
        label: '确认组合',
        agentOwnsDecision: true,
        userVisiblePurpose: '正式上架组合使用项目配置或用户确认；用户明确委托自主判断时可先完成标注来源的可逆设计草稿，发布前再复核。'
    },
    batch_production: {
        id: 'batch_production',
        label: '批量生产',
...
export function buildSkuWorkflowStagePlan(input: BuildSkuWorkflowStagePlanInput): SkuWorkflowStagePlan {
    ...
    const existingTemplateClaim = hasSkuExistingTemplateClaimText(text);
    const templateReady = input.hasTemplateReady === true;
    ...
    if (existingTemplateClaim) stageSignals.push('existing_template_claim');
    if (templateReady) stageSignals.push('template_ready_observed');
    ...
    let reason: string;
    if (existingTemplateClaim && !templateDesign) {
        reason = '用户说明项目中已有模板，先只读核实项目资源和模板结构，再决定组合确认与批量生产。';
    } else if (templateDesign) {
        reason = 'SKU 模板仍需要设计判断，批处理只能在模板和组合确认后进入。';
    } else if (canEnterControlledBatchSkill) {
        reason = 'SKU 组合已确认，可以在读回资源后进入受控批量生产。';
    } else if (execution) {
        reason = 'SKU 生产请求需要先确认组合，避免脚本自行猜测上架组合。';
    } else {
        reason = 'SKU 请求需要先识别已有资源，再由 Agent 决定下一步。';
    }
