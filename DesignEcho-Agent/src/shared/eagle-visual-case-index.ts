...
function buildVisualCase(
    ...
    const itemId = extractEvidenceValue(result.evidence, 'Eagle item id') || stripEaglePrefix(result.id);
    const dimensions = parseDimensions(extractEvidenceValue(result.evidence, 'Image dimensions'));
    const extension = extractEvidenceValue(result.evidence, 'Extension') || extensionFromName(result.title);
    const folders = splitCsvEvidence(extractEvidenceValue(result.evidence, 'Folders'));
    const filePath = extractEvidenceValue(result.evidence, 'Local file reference');
    const thumbnailPath = extractEvidenceValue(result.evidence, 'Thumbnail reference');
    ...
    return {
        ...
        evidence: result.evidence.map(normalizeText).filter(Boolean),
...
function buildCaseWarnings(result: DesignKnowledgeResult, dimensions?: { width: number; height: number }): string[] {
    ...
    if (!extractEvidenceValue(result.evidence, 'Local file reference')) {
...
function extractEvidenceValue(evidence: string[], label: string): string {
    const prefix = `${label}:`;
    const line = evidence.find((item) => normalizeText(item).startsWith(prefix));
    if (!line) return '';
    return normalizeText(line.slice(prefix.length));
}

function splitCsvEvidence(value: string): string[] {
...
function stripUnsafePayload(index: EagleVisualCaseIndex): EagleVisualCaseIndex {
    return {
        ...
        cases: index.cases.map((item) => ({
            ...
            evidence: item.evidence.map(sanitizeText),
            warnings: [
                ...item.warnings,
