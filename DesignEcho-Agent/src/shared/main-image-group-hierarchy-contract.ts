import type { EvidenceRef, VerificationCheck, VerificationReport } from './design-agent-os-contracts';
import type {
...
function validateProductionStructure(
    production: MainImageProductionDocumentStructureEvidence
): string[] {
...
function buildContract(input: {
    ...
    const evidenceStatus = input.status === 'ready_for_disposable_group_hierarchy_adapter'
        ? 'needs_review'
        : 'failed';
    const evidence: EvidenceRef[] = [{
        source: 'main-image-group-hierarchy-contract',
        summary: `status=${input.status}; documents=${input.documents.length}; blockers=${input.blockers.join('/') || 'none'}`,
    ...
    const contract: MainImageGroupHierarchyContract = {
        ...
        verificationReport: {
            ...
            status: evidenceStatus === 'needs_review' ? 'passed' as const : 'failed' as const,
            scope: 'task' as const,
...
export function buildMainImageGroupHierarchyContract(
    ...
    const production = input.productionDocumentStructureEvidence || null;
