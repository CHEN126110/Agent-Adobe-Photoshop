...
import {
    PHOTOSHOP_CRAFT_RECIPES,
    formatPhotoshopCraftRecipeForKnowledge,
    type PhotoshopCraftRecipe
} from './knowledge/photoshop-craft-recipes';
...
function photoshopCraftRecipeSearchText(recipe: PhotoshopCraftRecipe): string {
    return normalizeText([
        recipe.recipeId,
        recipe.title,
        recipe.visualIntent,
        recipe.useWhen.join(' '),
        recipe.doNotUseWhen.join(' '),
        recipe.requiredObservations.join(' '),
        recipe.toolOptions.map((option) => `${option.toolName} ${option.useWhen}`).join(' '),
        recipe.expectedStructure.join(' '),
        recipe.expectedPixels.join(' ')
    ].join(' '));
}

function photoshopCraftRecipeMatches(recipe: PhotoshopCraftRecipe, queryText: string): boolean {
    if (!queryText) return true;
    const haystack = photoshopCraftRecipeSearchText(recipe);
    return queryText
        .split(/\s+/)
        .filter(Boolean)
        .some((token) => haystack.includes(token));
...
function photoshopCraftRecipeToKnowledgeResult(recipe: PhotoshopCraftRecipe): DesignKnowledgeResult {
    return {
        id: `local-recipe:${recipe.recipeId}@${recipe.version}`,
        title: `Photoshop Craft Recipe｜${recipe.title}`,
        intent: 'recipe',
        sourceType: 'local_recipe',
        summary: formatPhotoshopCraftRecipeForKnowledge(recipe),
        sourceNotes: [
            `版本：${recipe.version}`,
            `来源修订：${recipe.provenance.sourceRevision}`,
            `生命周期：${recipe.provenance.lifecycle}`,
            '边界：仅作可选择的工艺知识，不授予 Tool、Stage、写入、完成或 Release 权限。'
        ],
        tags: [
            'photoshop-craft-recipe',
            recipe.recipeId,
            ...recipe.applicableTaskTypes,
            ...recipe.toolOptions.map((option) => option.toolName)
        ],
        allowedUses: ['prompt_context', 'recipe_hint'],
        sourceLevel: 'curated_recipe',
        sourceRank: 92
...
export function searchLocalDesignKnowledge(query: DesignKnowledgeQuery): DesignKnowledgeSearchResponse {
    ...
    if (includesIntent(query, 'recipe') && includesSourceType(query, 'local_recipe')) {
        results.push(
            ...PHOTOSHOP_CRAFT_RECIPES
                .filter((recipe) => recipe.provenance.lifecycle === 'active')
                .filter((recipe) => photoshopCraftRecipeMatches(recipe, queryText))
                .map(photoshopCraftRecipeToKnowledgeResult),
            ...REFERENCE_STYLE_RECIPES
