...
export function diffAcceptanceSnapshots(
    ...
    options: {
        ...
        allowNoDocumentBefore?: boolean;
    } = {}
): AcceptanceSnapshotDiff {
    const tolerance = options.geometryTolerance ?? 0.5;
    const issues: string[] = [];

    if (before.success !== true && options.allowNoDocumentBefore !== true) {
        issues.push(`before snapshot failed: ${before.error || 'unknown error'}`);
    }
    if (after.success !== true) issues.push(`after snapshot failed: ${after.error || 'unknown error'}`);
    if (!before.hasDocument && options.allowNoDocumentBefore !== true) {
        issues.push('before snapshot has no active document');
    }
