...
function buildRunGuard(
    ...
    return {
        ...
        requireFinalAcceptanceSnapshot: true,
        requireManualReviewBeforeQualityClaim: true,
        failurePolicy: 'Stop immediately on the first failed Photoshop operation, capture available readback evidence, and do not claim design quality.'
    };
}
...
export function buildMainImageLiveExecutorCheckpoint(
    ...
    const verificationReport: VerificationReport = {
        ...
        evidence: [{
            source: 'main-image-live-executor-checkpoint',
            summary: `status=${status}; operations=${operationCount}`,
            status: canStartLiveExecutor ? 'needs_review' : 'failed'
        }]
    };
    ...
    return {
        ...
        evidence: [{
            source: 'main-image-live-executor-checkpoint',
            summary: `status=${status}; scope=${runGuard.executionScope}; operations=${operationCount}; connection=${connection.source}`,
            status: canStartLiveExecutor ? 'needs_review' : 'failed'
        }],
        verificationReport
    };
