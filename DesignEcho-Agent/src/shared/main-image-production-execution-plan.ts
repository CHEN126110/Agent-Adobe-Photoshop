...
import type {
    ...
    MainImageVariantPlacementStrategyEvidence
...
export interface MainImageProductionExecutionPlanInput {
    productionDocumentStructureEvidence?: MainImageProductionDocumentStructureEvidence | null;
...
export interface MainImageProductionExecutionPlanEvidence {
...
function makeBlockedEvidence(input: {
    status: MainImageProductionExecutionPlanStatus;
    blocker: string;
    warning?: string;
    ...
    return {
        ...
        evidence: [{
            source: 'main-image-production-execution-plan',
...
function buildPlacementLookup(
    placementEvidence: MainImageVariantPlacementStrategyEvidence
): Map<string, MainImageVariantPlacementPlan> {
    const lookup = new Map<string, MainImageVariantPlacementPlan>();
    for (const plan of placementEvidence.variantPlacementPlans) {
...
function getPendingConfirmations(
    productionEvidence: MainImageProductionDocumentStructureEvidence
): string[] {
    const pending: string[] = [];
    for (const document of productionEvidence.documents) {
        if (document.evidenceLevel === 'project_preference_pending_confirmation') {
...
function buildMissingPlacementBlockers(input: {
    ...
    for (const document of input.productionEvidence.documents) {
...
function makeOperation(input: Omit<MainImageProductionExecutionOperation, 'executionBoundary'>): MainImageProductionExecutionOperation {
    return {
        ...input,
        executionBoundary: 'planned operation only; live Photoshop execution must be performed by the executor and then read back evidence'
    };
}
...
function buildCreateDocumentOperation(
    ...
    return makeOperation({
        ...
        sourceEvidenceIds: [document.id, document.sizeProfileId]
    });
}
...
function buildParentGroupOperation(input: {
    ...
    return makeOperation({
        ...
        requiredReadback: ['documentInfo'],
        sourceEvidenceIds: [input.document.id, input.groupName]
    });
}
...
function buildChildGroupOperation(input: {
    ...
    return makeOperation({
        ...
        requiredReadback: ['documentInfo'],
        sourceEvidenceIds: [input.document.id, input.childGroupId, input.variantId]
    });
}
...
function buildPlaceImageOperation(input: {
    ...
    return makeOperation({
        ...
        asset: input.asset,
        requiredReadback: ['documentInfo'],
        sourceEvidenceIds: [input.document.id, input.childGroupId, input.variantId, input.placement.id]
    });
}
...
function findExportSpec(input: {
    ...
    return input.productionEvidence.exportSpecs.find((spec) => (
...
function buildWarnings(input: {
    ...
    return Array.from(new Set([
        ...input.productionEvidence.warnings,
        ...input.placementEvidence.warnings,
        ...input.pendingConfirmations,
        '当前只是 Photoshop 操作计划 evidence；执行后必须读回文档、组、bounds、导出文件和截图/像素证据。'
    ].map(cleanString).filter(Boolean)));
}

export function buildMainImageProductionExecutionPlanEvidence(
    input: MainImageProductionExecutionPlanInput
): MainImageProductionExecutionPlanEvidence {
    const productionEvidence = input.productionDocumentStructureEvidence;
    if (!productionEvidence || productionEvidence.status !== 'ready_production_document_structure') {
        return makeBlockedEvidence({
            status: 'blocked_missing_production_structure',
            blocker: 'ready_main_image_production_document_structure_required'
        });
    }

    const placementEvidence = input.variantPlacementStrategyEvidence;
    if (!placementEvidence || placementEvidence.status !== 'ready_variant_placement_plan') {
        return makeBlockedEvidence({
    ...
    const placementLookup = buildPlacementLookup(placementEvidence);
    ...
    const documents = productionEvidence.documents.map((document) => buildDocumentExecutionPlan({
        document,
        productionEvidence,
        placementLookup,
        asset
    ...
    const pendingConfirmations = getPendingConfirmations(productionEvidence);
    ...
    return {
        ...
        status,
        documents,
        exportSpecs: productionEvidence.exportSpecs,
        ...
        verificationPolicy: {
            ...
            qualityClaimBoundary: '执行计划只证明下一步操作顺序合理；只有执行后 evidence 才能进入主图质量验收。'
        },
        canClaimOutputQuality: false,
        canClaimDesignComplete: false,
        noPhotoshopWrites: true,
        ...
        limitations: [
            ...
            '该 evidence 只覆盖主图生产结构，详情页和 SKU 必须接入各自子 skill 后再扩展。'
        ],
        evidence: [{
            source: 'main-image-production-execution-plan',
            summary: `status=${status}; documents=${documents.length}; operations=${plannedOperationCount}; pending=${pendingConfirmations.length}`,
            status: pendingConfirmations.length > 0 ? 'needs_review' : 'ready'
        }]
    };
}
