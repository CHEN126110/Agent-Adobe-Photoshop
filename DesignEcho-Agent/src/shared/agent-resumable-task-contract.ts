...
function extractLifecycle(message: AgentResumableTaskMessageLike): AgentRequestLifecycleEvidence | undefined {
    const candidates = [
    ...
    for (const candidate of candidates) {
        ...
        if (candidate.version === 'agent-request-lifecycle/v0' && isPlainObject(candidate.decision)) {
            return candidate as unknown as AgentRequestLifecycleEvidence;
        }
    }
    return undefined;
}
...
export function buildAgentResumableTaskContract(
    ...
    if (!latestAssistant) {
        return buildContract('ready_for_model_contextual_reply', {
            requested: true,
            previousUserInput,
            warnings: ['只有用户历史，没有助手执行证据；只能交给模型做上下文解释，不能自动恢复执行。']
        });
    }
    ...
    if (!lifecycle) {
        return buildContract('ready_for_model_contextual_reply', {
            requested: true,
            previousUserInput,
            warnings: ['上一轮助手消息没有结构化生命周期证据；不能自动恢复工具执行。']
        });
    }
