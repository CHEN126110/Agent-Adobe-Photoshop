...
function hasReliableRuntimeInspection(
    inspection: SkuTemplateLayoutRuntimeInspection | null
): boolean {
    if (!inspection || inspection.hasReliableInspection === false) return false;
    if (normalizeText(inspection.schema) !== 'sku-template-layout-inspection/v3') return false;
    if (!isRecord(inspection.historyStateRef)) return false;
    const documentId = readNumber(inspection.historyStateRef.documentId);
    const historyStateId = readNumber(inspection.historyStateRef.historyStateId);
    return Boolean(
        documentId !== null
...
export function buildSkuTemplateLayoutPreflightFromRuntimeInspection(input: {
    ...
    if (!hasReliableRuntimeInspection(inspection)) {
        let diagnostic = '未取得带当前 Photoshop 修订标识的可靠模板检查结果。';
        if (inspection?.hasReliableInspection === false) {
            diagnostic = '运行时明确标记本次模板检查结果不可靠。';
        } else if (inspection && normalizeText(inspection.schema) !== 'sku-template-layout-inspection/v3') {
            diagnostic = '模板检查结果缺少受支持的 sku-template-layout-inspection/v3 结构。';
        } else if (inspection && !isRecord(inspection.historyStateRef)) {
            diagnostic = '模板检查结果缺少 Photoshop documentId/historyStateId 修订标识。';
        }
        return {
            schema: 'sku-template-layout-preflight/v0',
            ...
            warnings: [
                ...readStringArray(inspection?.warnings),
                ...readStringArray(inspection?.blockers),
                diagnostic
            ],
            boundaries: {
                writesPhotoshop: false,
...
export type SkuTemplateAggregatePreflightDecision = {
    schema: 'sku-template-aggregate-preflight-decision/v0';
    status:
        | 'ready'
        | 'blocked_unreliable_inspection'
        | 'blocked_terminal_template'
        | 'requires_layout_repair_handoff';
    executableContentRepairTargetKeys: string[];
    boundaries: {
        writesPhotoshop: false;
...
export function buildSkuTemplateAggregatePreflightDecision(input: {
    inspectionStatuses: ReadonlyArray<SkuTemplateLayoutPreflight['skuPlaceholderInspectionStatus']>;
    terminalBlockerCount: number;
    layoutRepairTargetCount: number;
    contentRepairTargetKeys: readonly string[];
}): SkuTemplateAggregatePreflightDecision {
    let status: SkuTemplateAggregatePreflightDecision['status'] = 'ready';
    if (input.inspectionStatuses.some(item => item !== 'inspected')) {
        status = 'blocked_unreliable_inspection';
    } else if (input.terminalBlockerCount > 0) {
        status = 'blocked_terminal_template';
    } else if (input.layoutRepairTargetCount > 0) {
        status = 'requires_layout_repair_handoff';
    }
    const executableContentRepairTargetKeys = status === 'ready'
        ? Array.from(new Set(input.contentRepairTargetKeys.map(normalizeText).filter(Boolean)))
        : [];
    return {
        schema: 'sku-template-aggregate-preflight-decision/v0',
        status,
        executableContentRepairTargetKeys,
        boundaries: {
            writesPhotoshop: false,
            grantsPermission: false
        }
