...
import type { AgentResumeReadonlyContextExecutorResult } from './agent-resume-readonly-context-executor';
...
export interface AgentResumePlanningEvidence {
...
export interface AgentResumePlanningEvidenceInput {
...
export function buildAgentResumePlanningEvidence(
    input: AgentResumePlanningEvidenceInput
): AgentResumePlanningEvidence {
...
export function buildAgentResumePlanningMessages(
    ...
    const readonlyEvidence: AgentResumeReadonlyEvidence | undefined = input.readonlyExecutor?.evidence;
    const payload = {
        previousUserInput: input.contract.previousUserInput,
        previousSkillId: input.contract.previousSkillId,
        contractStatus: input.contract.status,
        ...
        receivedEvidence: input.refreshRun.receivedEvidence,
        readonlyToolResults: input.readonlyExecutor?.readonlyToolResults,
        readonlyEvidence: sanitizeAgentResumePlanningValue(readonlyEvidence)
    };
    ...
    return [
        ...
        },
        {
            ...
            content: [
                ...
                '6. proposedExecutionPlan：如果只读证据足够，给出可审查执行计划对象；必须包含 objective、steps、writeToolWhitelist、readbackTargets、requiresUserApproval=true。',
