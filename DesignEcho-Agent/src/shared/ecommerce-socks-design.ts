...
export interface EcommerceSocksChildDispatchRun {
    ...
    childExecutionPath: 'none' | 'dry_run' | 'unified_executor' | 'test_override' | 'own_registry';
...
export function buildEcommerceSocksChildDispatchRun(params: {
    ...
    // own_registry：父 Skill 在调用方未注入 runSkill 时自持调起子任务（编排是它的本职，
    // 不该因为某条调用路径漏传就静默退化成 plan-only）。
    childExecutionPath?: 'unified_executor' | 'test_override' | 'own_registry';
