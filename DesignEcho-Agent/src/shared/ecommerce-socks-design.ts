...
export interface EcommerceSocksDispatchOrchestrationPlan {
    ...
    failurePolicy: {
        onChildFailure: 'stop_and_report';
        onMissingEvidence: 'block_dispatch';
...
export interface EcommerceSocksChildDispatchRunRecord {
    ...
    skillId: EcommerceSocksChildSkillPlan['skillId'];
    label: string;
    state: 'dry_run_skipped' | 'completed' | 'failed' | 'needs_review' | 'not_run_after_failure';
    expectedReportKey: string;
    reason?: 'dry_run_only_no_child_executor_call' | 'unified_child_skill_runner_call' | 'test_override_child_call' | 'stopped_after_previous_child_failure';
    message?: string;
    error?: string;
    canClaimOutputQuality?: boolean;
}
...
export interface EcommerceSocksDesignEntryEvidence {
        ...
        phases: [
            {
                ...
                status: 'planned',
                owner: 'parent-skill',
                evidence: ['父 skill 负责整理用户需求、项目上下文和交付范围。']
            },
            {
                id: 'child_dispatch',
                label: '子 skill 调度',
                ...
                owner: 'parent-skill',
                evidence: params.dispatchDecision.blockedReasons
            },
            {
                id: 'child_acceptance',
                label: '子 skill 输出验收',
                ...
                owner: 'child-skill',
                evidence: params.childSkills.map((item) => `${item.label} 质量验收归属 ${item.skillId}`)
            },
            {
                id: 'parent_report',
                label: '父 skill 汇总报告',
                status: downstreamStatus,
                owner: 'parent-skill',
                evidence: ['父 skill 只能汇总子 skill 报告，不能替代子 skill 质量验收。']
            }
        ],
        acceptanceResponsibility: {
            parent: 'aggregate_child_reports_only',
...
export function buildEcommerceSocksChildDispatchRun(params: {
    ...
    if (canReportDryRun) {
        ...
        childRuns = params.dispatchOrchestration.childSteps.map((step) => {
            ...
            let state: EcommerceSocksChildDispatchRunRecord['state'] = 'not_run_after_failure';
            let reason: EcommerceSocksChildDispatchRunRecord['reason'] = 'stopped_after_previous_child_failure';
...
export function buildEcommerceSocksDesignEntryEvidence(params: {
    userIntent: string;
    projectPath?: string;
    deliverables: EcommerceSocksDeliverable[];
    executionMode?: 'plan-only' | 'dispatch';
    ...
    childStrategyPacketSet?: EcommerceSocksChildStrategyPacketSet;
    childStrategyReviewGate?: EcommerceSocksChildStrategyReviewGate;
}): EcommerceSocksDesignEntryEvidence {
