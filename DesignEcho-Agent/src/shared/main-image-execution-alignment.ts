...
function getToolEvidence(toolResults: MainImageExecutionAlignmentInput['toolResults']): {
    toolNames: string[];
...
function makeCheck(
    ...
    return { id, label, status, summary, expectedEvidence, actualEvidence };
}
...
function summarizeCopyExecution(
    ...
    if (hasTextWrite) return '执行证据中包含文本写入工具，可支撑文案槽位计划。';
    return '计划存在候选文案，但当前主图 executor 没有文本写入证据；只能视为文案槽位未落地。';
}
...
export function buildMainImageExecutionAlignment(input: MainImageExecutionAlignmentInput): MainImageExecutionAlignment {
    ...
    const { toolNames, successfulToolNames, failedToolNames } = getToolEvidence(input.toolResults);
    ...
    checks.push(makeCheck(
        ...
        hasDocumentInfo
            ? '执行证据中包含 getDocumentInfo，主图计划具备文档上下文。'
            : '主图计划缺少 getDocumentInfo 成功证据，不能证明 Photoshop 文档上下文已读取。',
    ...
    checks.push(makeCheck(
        ...
        hasSubjectBounds
            ? '执行证据中包含主体检测或活动图层 bounds，可支撑缩放落位计划。'
            : '缺少 getSubjectBounds/getLayerBounds 成功证据，不能证明主体大小和位置已被读取。',
    ...
    const layoutEvidenceCount = Math.max(successfulSmartLayout, Math.min(successfulTransform, successfulMove));
    checks.push(makeCheck(
        'layout.execution',
        '主体缩放落位执行',
        resolveCoverageStatus({
            plannedCount: plannedLayoutCount,
            actualCount: layoutEvidenceCount,
            emptyStatus: 'watch'
        }),
        summarizeCoverage({
            plannedCount: plannedLayoutCount,
            ...
            completeSummary: `已找到 ${layoutEvidenceCount}/${plannedLayoutCount} 个尺寸的布局执行证据。`,
            partialSummary: `只找到 ${layoutEvidenceCount}/${plannedLayoutCount} 个尺寸的布局执行证据。`
        }),
    ...
    checks.push(makeCheck(
        ...
        summarizeCoverage({
            ...
            completeSummary: `已找到 ${successfulExportCount}/${plansRequiringExport.length} 个 quickExport 成功证据。`,
            partialSummary: `计划要求导出 ${plansRequiringExport.length} 个尺寸，但只找到 ${successfulExportCount} 个 quickExport 成功证据。`
        }),
    ...
    checks.push(makeCheck(
        ...
        ['result screenshot evidence', 'manual review evidence'],
        [
    ...
    return {
        ...
        warnings,
        limitations: [
            '该对齐报告只比较 Agent 主图计划、DSL 和 executor 工具证据。',
            '该报告不改变 Photoshop 工具参数、执行顺序或成功判定。',
            'aligned 只表示证据类别覆盖，不代表截图相似、审美质量或主图最终可交付。'
        ],
        evidence: [{
            source: 'main-image-execution-alignment',
            summary: `主图计划/执行对齐状态为 ${status}，检查 ${checks.length} 项。`,
            status
        }]
    };
}
