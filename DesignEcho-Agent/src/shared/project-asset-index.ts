...
function selectRoleBalancedVisionCandidates(
    candidates: ProjectAssetIndexVisionCandidate[],
    limit: number
): ProjectAssetIndexVisionCandidate[] {
    if (limit <= 0 || candidates.length === 0) return [];

    const roleOrder: ProjectAssetRole[] = [
        'raw-model-wear',
        'raw-product-still',
        'raw-detail-closeup',
        'color-single',
    ...
    const roleQueues = new Map<ProjectAssetRole, ProjectAssetIndexVisionCandidate[]>();
    for (const candidate of candidates) {
        const queue = roleQueues.get(candidate.role) || [];
        queue.push(candidate);
        roleQueues.set(candidate.role, queue);
    }

    const selected: ProjectAssetIndexVisionCandidate[] = [];
    while (selected.length < limit) {
        let selectedInRound = false;
        for (const role of roleOrder) {
            const candidate = roleQueues.get(role)?.shift();
...
function buildVisionCandidates(assets: ProjectAssetIndexAsset[]): ProjectAssetIndexVisionCandidate[] {
    ...
    return selectRoleBalancedVisionCandidates(rankedCandidates, 12);
}
...
export function buildProjectAssetIndex(input: ProjectAssetIndexInput): ProjectAssetIndex {
    ...
    const assets = input.files.map((file, index) => {
        ...
        const asset: ProjectAssetIndexAsset = {
            ...
            modifiedTimeMs: Number(file.modifiedTimeMs) > 0 ? Number(file.modifiedTimeMs) : undefined,
            width,
            height,
            aspectRatio,
            folderRole,
