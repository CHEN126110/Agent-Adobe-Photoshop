import type { AgentResumePlanningEvidence } from './agent-resume-planning';
...
export interface AgentResumeExecutionGateInput {
    planning?: AgentResumePlanningEvidence;
...
export interface AgentResumeExecutionGate {
    ...
    planningStatus?: AgentResumePlanningEvidence['status'];
    evidenceOnly: true;
...
export function buildAgentResumeExecutionGate(
    ...
    if (readbackTargets.length === 0) {
        return buildGate({
            ...
            proposedWriteTools: effectiveWriteTools,
            executionPlan,
            blockers: ['恢复执行计划缺少 readbackTargets，不能证明写入后如何读回验收。'],
            warnings: planning.warnings
        });
    }
