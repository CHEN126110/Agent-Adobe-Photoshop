...
export const COPYWRITING_PISBEC = [
    ...
    'Evidence 证据：用材质、结构、功能细节、图片可见信息或用户已提供事实支撑。',
...
export const COPYWRITING_LAYER_RULES = [
    {
        ...
        evidence: '使用者、购买者、决策者、影响者、生活状态、审美偏好、消费心理。'
    },
    {
        layer: '为什么在意',
        purpose: '找到目标人群会被什么内容吸引，再决定表达方向。',
        evidence: '审美兴趣、场景兴趣、情绪兴趣、功能兴趣、身份兴趣、社交兴趣、安心兴趣。'
    },
    {
        layer: '看得见',
        purpose: '和图片有关，文案不能脱离画面。',
        evidence: '人物状态、动作、场景、氛围、产品露出、产品细节。'
    },
    {
        layer: '想得到',
        purpose: '和用户场景有关，让用户能代入。',
        evidence: '通勤、约会、旅行、居家、运动、久走、雨天、夏天等真实使用场景。'
    },
    {
        layer: '信得过',
        purpose: '和产品功能有关，把卖点翻译成用户感受。',
        evidence: '轻便=不累，透气=不闷，防滑=安心，好搭=少纠结。'
    },
    {
        layer: '记得住',
        purpose: '和语言表现力有关，让句子短、顺、有画面、有分寸。',
        evidence: '自然表达、轻微转折、生活瞬间，而不是广告口号。'
    }
] as const;
...
export const COPYWRITING_SCORE_CRITERIA: CopywritingScoreCriterion[] = [
    ...
    { id: 'evidence', label: '可信证据', points: 10, question: '是否有功能细节、场景证明或真实体验支撑？' },
...
export function buildCopywritingContextChecklist(
    ...
    return {
        ...
        rules: [
            ...
            '先从图片提取视觉锚点，再写文案。',
            '没有图片证据时，只能写用户已提供或产品事实能支持的内容。',
            '缺少人群、视觉锚点、产品事实或用户场景时不能编造，只能标记证据不足或请求补充信息。',
            '产品卖点必须翻译成用户感受，不直接堆参数。',
            '证据不足时输出需要补充的信息，而不是一本正经地胡说。'
        ]
    };
}
...
export function formatCopywritingFrameworkForPrompt(): string {
    return [
        ...
        'P-I-S-B-E-C：人群是谁 -> 兴趣是什么 -> 场景在哪里 -> 利益是什么 -> 证据是什么 -> 如何轻引导。',
        ...COPYWRITING_PISBEC.map(item => `- ${item}`),
        '',
        '【必须先判断上下文】',
        ...
        '',
        '输出文案前必须自检：人群、兴趣、场景、产品、情绪、证据、图文、风险。低于 70 分应重写。'
    ].join('\n');
}
...
export function formatCopywritingFrameworkForKnowledge(): string {
    return [
        ...
        '边界：没有目标人群、图片证据或产品事实时，不允许编造人群身份、画面、功能或用户痛点。'
    ].join(' ');
}
