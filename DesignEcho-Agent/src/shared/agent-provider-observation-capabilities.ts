...
function buildFinalProviderThinking(
    ...
    if (thinking?.supported === true && thinking.format && thinking.format !== 'none') {
        return {
            status: 'supported',
            basis: 'model-config',
            reason: 'Model config declares provider thinking support; the adapter may retain it for protocol continuity and hidden diagnostics, but it is not user-visible reasoning.',
