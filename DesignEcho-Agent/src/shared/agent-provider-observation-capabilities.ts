...
export type AgentProviderObservationEvidenceSource =
    | 'model-config'
...
export interface AgentProviderObservationSignal {
    ...
    evidenceSource: AgentProviderObservationEvidenceSource;
    reason: string;
...
function buildBooleanSignal(
    ...
    return {
        status: 'unknown',
        evidenceSource: 'conservative-default',
...
function buildToolStreamCapability(
    ...
    if (OPENAI_COMPATIBLE_TOOL_STREAM_PROVIDERS.has(provider) || OPENROUTER_TOOL_STREAM_PROVIDERS.has(provider)) {
        return {
            mode: 'stream',
            status: 'supported',
            evidenceSource: 'runtime-implementation',
...
function buildToolEventsSignal(
    ...
    if (supportsToolUse === true) {
        return {
            status: 'supported',
            evidenceSource: toolStream.evidenceSource,
...
function buildProviderThinkingDelta(
    ...
    if (provider === 'deepseek') {
        return {
            status: 'unsupported',
            evidenceSource: 'runtime-implementation',
            reason: 'DeepSeek tool path disables thinking to avoid unsupported reasoning payload behavior.',
    ...
    if (thinking?.supported === true && thinking.format === 'reasoning_content') {
        return {
            status: 'supported',
            evidenceSource: 'runtime-implementation',
            reason: 'OpenAI-compatible stream parser can observe delta.reasoning_content as provider_thinking_delta.',
            observationSource: 'provider_thinking_delta'
        };
    }
...
function buildFinalProviderThinking(
    ...
    if (provider === 'deepseek') {
        return {
            status: 'unsupported',
            evidenceSource: 'runtime-implementation',
            reason: 'DeepSeek tool path disables thinking; final provider thinking must not be claimed for tool calls.',
...
function buildWarnings(input: {
    ...
    if (input.provider === 'deepseek') {
        warnings.push('DeepSeek tool path disables thinking; do not show provider thinking for tool calls.');
    }
