...
/**
 * 用途的中文名，给界面用。
 * `declared / metadata / inferred / assumed` 这些是判定依据的内部口径，属于诊断信息，
 * 不要出现在用户可见文案里——用户看到「用途判断：metadata」只会以为程序出错了。
 */
export const MODEL_USAGE_KIND_LABELS: Record<ModelUsageKind, string> = {
    'conversation': '对话',
...
function classifyFromMetadata(input: ModelUsageClassificationInput): ModelUsageClassification | undefined {
    ...
    // 判据只看**输出**模态：能出图就是图像模型。
    //
    // 这里曾经额外要求 !canReturnText（"必须不能返回文本"），那个前提对
    // chat completions 形态的图像模型不成立——它们的 output_modalities 是
    // ["image","text"]，出图的同时也回一段文字说明。结果是 OpenRouter 上
    // google/gemini-3-pro-image、openai/gpt-5-image 这类专用出图模型全部
    // 掉进下方 conversation 分支，进而出现在 Agent 主模型候选里。
    //
    // 反向不会误伤：视觉对话模型的 image 在 input_modalities，不在 output，
    // 这里读的是 outputs，够不着它们。
    if (/image-generation|text-to-image|image-to-image/.test(combined)
        || /image/.test(outputsText)) {
        return { kind: 'image-generation', confidence: 'metadata', reason: 'provider exposes image-generation metadata' };
    }
    // 视频/音频保留 !canReturnText：还没观察到"既出视频又出文字"的模型形态，
    // 在没有证据的情况下不跟着放宽，免得把带附件能力的对话模型判飞。
