 ...
 * - 输入栏用 buildAllPrimaryModelOptionGroups()，拿全渠道候选交给 ModelPicker 分页展示；
 *   选中哪个渠道的模型，运行模式就自动跟到哪个渠道（写回在 ChatPanel），
 *   所以列出另一侧模型不会留下「模式与模型冲突」的状态。
 * 模型清单本身是同一个来源，差别只在过不过滤。
 *
 ...
 * - 运行模式过滤：cloud → 只列云端；local → 只列本地。不过滤的全渠道列表走 buildAllPrimaryModelOptionGroups。
 *
...
export function buildPrimaryModelOptionGroups(
    ...
    const all = buildAllPrimaryModelOptionGroups(dynamicModels);
    const wantLocal = mode === 'local';
    return all.filter(group => (group.label === LOCAL_GROUP_LABEL) === wantLocal);
}

/**
 * 全渠道候选（云端分组在前，本地分组在后），不按运行模式过滤。
 * 输入栏的模型选择器用它拿到完整清单，再自己分「云端 / 本地」两页。
 */
export function buildAllPrimaryModelOptionGroups(
    dynamicModels?: ModelConfig[] | null
): PrimaryModelOptionGroup[] {
