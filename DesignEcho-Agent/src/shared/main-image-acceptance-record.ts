...
function buildQualityClaim(
    ...
    return {
        allowed,
        reason: allowed
            ? '主图验收记录具备最终 QA report 与有效人工 approved 验收证据。'
            : '当前验收记录不能声明主图输出质量通过。',
        requiredEvidence,
        blockers,
        boundary: 'acceptanceRecord 只记录和复核已有证据；不能自行执行 Photoshop、不能替代 mainImageQaReport，也不能外推为完整项目完成。'
    };
}
...
function buildNextActions(stage: MainImageAcceptanceRecordStage): string[] {
    ...
    if (stage === 'needs_quality_evidence') return ['补齐结果图、文件探针、reference/pixel probe 或截图 QA 证据。'];
    if (stage === 'needs_manual_review') return ['基于结果图完成人工复核，并记录 approved / needs_review / rejected。'];
    if (stage === 'needs_acceptance_review') return ['最终 QA 已可声明质量证据，但仍需要一次明确的人工验收记录。'];
...
function buildReplay(input: MainImageAcceptanceRecordInput): MainImageAcceptanceRecordReplay {
    return {
        ...
        notes: [
            ...
            'replay 不执行 Photoshop 写操作，只验证证据收敛和脱敏边界。'
        ]
    };
}
...
export function buildMainImageAcceptanceRecord(input: MainImageAcceptanceRecordInput): MainImageAcceptanceRecord {
    ...
    const limitations = [
        'mainImageAcceptanceRecord 不执行 Photoshop、不读取图片、不调用视觉模型，只汇总已有证据。',
        ...
        '记录会脱敏绝对路径和 raw/base64 图片 payload；对外只暴露文件名、计数和证据阶段。'
    ];
    const evidence: EvidenceRef[] = [
        {
            source: 'main-image-acceptance-record',
            summary: `stage=${stage}; qa=${sourceSummary.qaStage}; bridge=${sourceSummary.bridgeStage}; manual=${manualReview.decision}; claim=${qualityClaim.allowed}`,
            status
        },
        ...(input.qaReport?.verificationReport.evidence || []),
        ...(input.controlledProductQaBridge?.verificationReport.evidence || [])
    ].slice(0, 8);
    const verificationReport: VerificationReport = {
        ...
        summary: qualityClaim.allowed
            ? '主图验收记录已具备 QA report 和人工 approved 证据。'
    ...
    return {
        ...
        redaction: {
            rawImagesRedacted: true,
            pathsRedacted: true,
            pathPolicy: 'Only basenames, counts, evidence stages and redacted review notes are exposed.'
        },
        verificationReport
    };
}
