...
import type {
    MainImageScreenshotProbeEvidence,
...
function targetSizeForPath(path: string, sizePlans: MainImageSizePlanEvidence[]): { width: number; height: number } | undefined {
...
export function buildMainImageScreenshotProbeReadiness(
    ...
    const expectedResultPaths = uniqueStrings(screenshotQa.resultImageEvidence.resultPaths || []);
    ...
    const verificationReport: VerificationReport = {
        ...
        evidence: [{
            ...
            summary: `stage=${stage}; resultPaths=${expectedResultPaths.length}; fileProbes=${fileProbes.length}; target=${probeTarget.mode}; pixelProbe=${pixelProbe?.status || 'none'}。`,
            status
        }]
    };
