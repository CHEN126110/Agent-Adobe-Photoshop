...
const BUSINESS_SKILL_READINESS_BOUNDARIES = [
  ...
  'FEX and synthetic fixtures are regression evidence only, not business strategy evidence.'
];
...
export function buildBusinessSkillReadinessContract(
  ...
  const implementationCheckpoint = buildBusinessSkillImplementationCheckpoint({
    ...
    evidence: {
      ...
      visualEvidencePlan: strategyInputs.assetUnderstanding === true
  ...
  return {
    ...
    blockers,
    warnings,
    nextEvidence: buildNextEvidence(missingInputs),
...
function buildWarnings(
  ...
  if (riskFlags.syntheticFixtureOnly === true) {
    warnings.push('synthetic_fixture_is_not_business_strategy_evidence');
  }

  return Array.from(new Set(warnings));
}

function buildNextEvidence(missingInputs: BusinessSkillReadinessInputKey[]): string[] {
  return missingInputs.map((key) => {
    switch (key) {
      ...
      default:
        return 'provide_missing_business_skill_strategy_evidence';
    }
  });
}
