...
function makeContract(input: {
    ...
    const evidenceStatus = input.status === 'ready_for_disposable_photoshop_adapter' ? 'needs_review' : 'failed';
    const evidence: EvidenceRef[] = [{
        source: 'main-image-live-photoshop-adapter-contract',
        summary: `status=${input.status}; mapped=${mappedOperationCount}; blocked=${blockedOperationCount}; missingTools=${missingToolNames.join('/') || 'none'}`,
    ...
    return {
        ...
        verificationReport: {
            ...
            scenario: 'main-image',
            status: evidenceStatus === 'needs_review' ? 'passed' : 'failed',
            scope: 'task',
