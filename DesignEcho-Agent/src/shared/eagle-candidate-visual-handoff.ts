...
export type EagleCandidateVisualEvidenceRequest =
    | 'ocr'
    | 'subject_regions'
    | 'dominant_colors'
    | 'composition'
...
const DEFAULT_REQUESTED_EVIDENCE: EagleCandidateVisualEvidenceRequest[] = [
    'ocr',
    'subject_regions',
    'dominant_colors',
    'composition',
...
export function buildEagleCandidateVisualHandoff(
    ...
    return {
        ...
        visualAnalysisRequest: {
            ...
            requestedEvidence: [...DEFAULT_REQUESTED_EVIDENCE],
        ...
        limitations: uniqueStrings([
            ...
            'Eagle 候选选择只生成视觉分析交接证据，不生成 Photoshop 动作。',
...
function buildBlockedHandoff(input: {
    ...
    return {
        ...
        designDecisionHandoff: {
            ...
            allowedUseLabels: [],
            requiredReview: [],
            boundary: '没有有效候选时，下游不能消费 Eagle 候选作为设计证据。'
        },
        requiredReview: [],
...
function buildCandidateSnapshot(candidate: EagleAssetCandidateCard): EagleCandidateVisualHandoffCandidateSnapshot {
    ...
    return {
        ...
        evidencePreview: normalizeTextList(candidate.evidencePreview).slice(0, 4),
...
function buildMetadataHints(candidate: EagleCandidateVisualHandoffCandidateSnapshot): string[] {
    return uniqueStrings([
        ...
        ...candidate.evidencePreview.map((item) => `evidence: ${item}`)
    ]).slice(0, 12);
}
