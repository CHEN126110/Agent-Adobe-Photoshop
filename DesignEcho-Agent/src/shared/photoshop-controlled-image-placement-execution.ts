...
export interface ControlledPhotoshopImagePlacementDryRunOperation {
    ...
    sourceEvidenceTier: ImagePlacementPlan['evidenceTier'];
...
export interface ControlledPhotoshopImagePlacementPlanEvidence {
...
export interface ControlledPhotoshopImagePlacementToolCallPlan {
    ...
    sourcePlanVersion: ControlledPhotoshopImagePlacementPlanEvidence['version'];
...
function makeEstimate(operationCount: number): ControlledPhotoshopImagePlacementPlanEvidence['efficiencyEstimate'] {
...
function makePlanEvidence(input: {
    status: ControlledPhotoshopImagePlacementPlanStatus;
    targetCount?: number;
    operations?: ControlledPhotoshopImagePlacementDryRunOperation[];
    blockers?: string[];
    warnings?: string[];
}): ControlledPhotoshopImagePlacementPlanEvidence {
    const operations = input.operations || [];
    const evidenceStatus = resolvePlanEvidenceStatus(input.status, operations);
...
function resolvePlanEvidenceStatus(
    status: ControlledPhotoshopImagePlacementPlanStatus,
    operations: ControlledPhotoshopImagePlacementDryRunOperation[]
...
function normalizeOperation(
    ...
    const operation: ControlledPhotoshopImagePlacementDryRunOperation = {
        ...
        sourceEvidenceTier: target.placementPlan.evidenceTier,
...
function makeToolCallPlan(input: {
    plan: ControlledPhotoshopImagePlacementPlanEvidence;
    status: ControlledPhotoshopImagePlacementToolCallPlanStatus;
    toolCalls?: ControlledPhotoshopImagePlacementToolCall[];
...
function makeFallbackPlacementPlan(
    ...
    return {
        ...
        assetRole: 'unknown',
        intent: 'supporting',
        evidenceTier: operation.sourceEvidenceTier,
        source: {
...
function makeBenchmarkReport(input: {
    ...
    return {
        ...
        warnings: [
            'benchmark is estimate/sample evidence only; it must not be used as a design quality claim',
...
export function buildControlledPhotoshopImagePlacementToolCallPlan(
    ...
    return makeToolCallPlan({
        ...
        warnings: [
            ...
            'tool-call plan is still no-write evidence; live execution requires an explicit disposable-document or user-approved run'
        ]
    });
}
