...
import type { MainImageDesignStandardsEvidence } from './main-image-design-standards-evidence';
import type { MainImageProductionExecutorDryRunEvidence } from './main-image-production-executor-dry-run';
...
export interface MainImageDesignReadinessReportInput {
    strategyInputEvidence?: MainImageDesignReadinessStrategyEvidence | null;
    qaReport?: MainImageQaReport | null;
    userCheckpointApproved?: boolean;
}

export interface MainImageDesignReadinessStrategyEvidence {
    status: string;
...
function buildChecks(input: MainImageDesignReadinessReportInput): MainImageDesignReadinessCheck[] {
    const strategy = input.strategyInputEvidence;
    const designStandards = strategy?.designStandardsEvidence;
    const dryRun = strategy?.productionExecutorDryRunEvidence;
    ...
    return [
        ...
        makeCheck(
            ...
            dryRun
                ...
                : '没有 production executor dry-run evidence。'
        ),
        makeCheck(
            'user-checkpoint',
            '用户 checkpoint',
...
function inferStatus(input: MainImageDesignReadinessReportInput): MainImageDesignReadinessStatus {
    const strategy = input.strategyInputEvidence;
    if (!strategy) return 'blocked_missing_strategy_evidence';
    if (strategy.status !== 'ready_for_strategy_contract') return 'blocked_strategy_inputs_not_ready';
    if (strategy.designStandardsEvidence.status !== 'ready_for_design_strategy') {
        return 'blocked_design_standards_not_ready';
    }
    if (strategy.productionExecutorDryRunEvidence.status !== 'completed_dry_run') {
...
function collectBlockers(
    ...
    const blockers = [
        ...(strategy?.blockers || []),
        ...(strategy?.designStandardsEvidence.blockers || []),
        ...(strategy?.productionExecutorDryRunEvidence.blockers || []),
        ...(input.qaReport?.blockers || [])
    ];

    if (status === 'blocked_missing_strategy_evidence') blockers.push('main_image_strategy_input_evidence_required');
...
function collectWarnings(input: MainImageDesignReadinessReportInput): string[] {
    ...
    return cleanStrings([
        ...(strategy?.warnings || []),
        ...(strategy?.designStandardsEvidence.warnings || []),
        ...(strategy?.productionExecutorDryRunEvidence.warnings || []),
        ...(input.qaReport?.warnings || [])
    ]);
}

function buildNextActions(status: MainImageDesignReadinessStatus): string[] {
    if (status === 'blocked_missing_strategy_evidence') {
        return ['先生成主图 strategy input evidence，不能直接进入 executor。'];
    }
    if (status === 'blocked_strategy_inputs_not_ready') {
        return ['补齐素材、主体 bounds、尺寸计划、文案角色、导出和性能预算 evidence。'];
    }
    if (status === 'blocked_design_standards_not_ready') {
        return ['补齐视觉或人工 grounding，让主图设计规范 evidence 能指导点击图/转化图策略。'];
    }
    if (status === 'blocked_executor_dry_run_not_ready') {
        return ['先完成 production executor bridge 与 dry-run operation evidence。'];
    }
    ...
    return ['质量声明 evidence 已具备，但仍不能把它扩大成完整设计项目完成。'];
}
...
export function buildMainImageDesignReadinessReport(
    ...
    const limitations = [
        'mainImageDesignReadinessReport 只汇总已有主图 evidence，不执行 Photoshop、不调用 provider、不搜索网页。',
        'ready_for_live_executor 只表示可以在明确授权后进入真实执行器，不代表已经执行或输出合格。',
        'quality_claim_ready 只来自 QA report 的 evidence，不等于完整电商项目设计完成。',
        '所有 Photoshop actualBounds、截图、导出和人工验收都必须来自后续真实读回，不能由 dry-run 伪造。'
    ];
    const evidence: EvidenceRef[] = [{
        source: 'main-image-design-readiness-report',
        summary: `status=${status}; enterExecutor=${canEnterLiveExecutor}; qualityClaim=${canClaimOutputQuality}`,
    ...
    const verificationReport: VerificationReport = {
        ...
        summary: canClaimOutputQuality
            ? '主图 QA evidence 允许质量声明，但不代表完整设计项目完成。'
