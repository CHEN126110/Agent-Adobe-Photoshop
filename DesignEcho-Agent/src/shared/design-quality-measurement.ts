/**
 * 画面测量提取器（design quality measurement）—— 把"画面结构"换算成评审可引用的真实观察值。
 *
 * 衔接：上一层 [design-quality-assertion.ts] 把 DesignQualityMeasurements 作为证据交给 Brief 相对
 * 的视觉评审；只有真正有唯一答案的事实断言才能直接确定性判定。本模块从一份归一化的
 * "画面快照结构"（DesignSurfaceSnapshot，来自 getLayerHierarchy / getAllTextLayers /
 * getLayerBounds / getDocumentInfo 等现有工具的结构化结果）确定性地算出观察值，不负责定义美学阈值。
 *
 * 关键纪律（与项目一致）：
 * - 纯逻辑：只做几何/结构计算，不调模型、不读像素、不触发 IPC、不依赖运行环境。
 ...
 *   若把缺省层静默剔除只看剩下的层，"居中标题 + 左对齐正文"的有意排版会只剩居中层而误报
 *   overall.above-baseline finding。
