...
import {
    ...
    type MainImageStrategyInputEvidence
...
import {
    ...
    buildMainImageScreenshotEvidenceFromSizePlans,
    type MainImageResultScreenshotEvidence,
    type MainImageManualReviewEvidence,
...
import type { MainImageMemoryEvidence } from './main-image-memory-evidence';
...
export interface MainImageAgentDraftPlan {
    ...
    mainImageStrategyInputEvidence: MainImageStrategyInputEvidence;
...
function buildMainImageDsl(input: {
    ...
    return {
        ...
        evidence: [{
            source: 'main-image-agent-draft-plan',
            summary: `按 ${width}x${height} 主图生成安全区、主视觉槽、标题槽和卖点槽。`,
            status: 'needs_review'
        }]
    };
}
...
function buildExecutionPlan(input: {
    ...
    const steps: ExecutionPlanStep[] = [
        makeStep(
            ...
            ['current document info', 'project assets', 'selected asset evidence']
        ),
        makeStep(
            ...
            ['selected asset name/path', 'asset dimensions or current document evidence']
        ),
        makeStep(
    ...
    for (const plan of input.sizePlans) {
        steps.push(makeStep(
            ...
            ['tool result', 'post-transform layer bounds', 'focus evidence']
        ));
    }
    ...
    steps.push(makeStep(
        ...
        input.copyCandidateCount > 0
            ...
            : '缺少可靠文案证据时不编造商品卖点，只保留可编辑文本槽位或请求补充。',
    ...
    steps.push(makeStep(
        ...
        '主图工具执行成功不等于设计完成，必须回收 bounds、截图或人工复核证据。',
    ...
    return {
        ...
        evidence: [{
            source: 'main-image-agent-draft-plan',
            summary: `Agent-first 主图草案计划包含 ${steps.length} 个步骤。`,
            status: input.readiness === 'blocked' ? 'failed' : 'needs_review'
        }],
        limitations: [
...
function buildVerificationReport(input: {
    ...
    const status: DesignAgentOsStatus = input.blockers.length > 0
        ? 'failed'
        : 'needs_review';
    const toolEvidence = input.toolNames.length > 0
    ...
    return {
        ...
        checks: [
            ...
            },
            {
                ...
                label: '文案上下文',
                status: input.copyStrategy.hasCopyEvidence ? 'needs_review' : 'unknown',
                summary: input.copyStrategy.reason
            },
            {
                id: 'main-image-tool-evidence',
                label: '工具执行证据',
        ...
        evidence: [{
            source: 'main-image-agent-draft-plan',
            summary: `readiness=${input.readiness}; assetMode=${input.selectedAssetStrategy.mode}; toolCount=${input.toolNames.length}`,
            status
        }]
    };
}
...
export function buildMainImageAgentDraftPlan(input: MainImageDraftInput): MainImageAgentDraftPlan {
    ...
    const screenshotEvidence = input.screenshotEvidence
        || buildMainImageScreenshotEvidenceFromSizePlans(sizePlans);
    ...
    const mainImageStrategyInputEvidence = buildMainImageStrategyInputs({
        userText: input.userText,
        ...
        mainImageMemoryEvidence: input.mainImageMemoryEvidence,
        designPlacementIntelligenceEvidence: input.designPlacementIntelligenceEvidence
    });
    const mainImageStrategyContract = buildMainImageStrategyContract({
        ...
        strategyInputs: input.strategyInputs || mainImageStrategyInputEvidence.strategyInputs
    });
    ...
    const readiness: MainImageDraftReadiness = blockers.length > 0
        ...
        : assetSelection.readiness !== 'ready' || selectedAssetStrategy.mode === 'missing' || !subjectBounds || sizePlans.length === 0
            ? 'needs_context'
            : 'ready';
    const intent = buildUserIntentFromText(input.userText || '帮我做主图', {
    ...
    const copyStrategy = {
        ...
        hasCopyEvidence: copyCandidates.length > 0,
    ...
    return {
        ...
        mainImageDesignPlacementIntelligence: input.designPlacementIntelligenceEvidence || null,
