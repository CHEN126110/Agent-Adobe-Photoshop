...
export interface ReferenceVisualQaSnapshotEvidence {
    source: 'getScreenSnapshotsWithOverlay' | 'getCanvasSnapshot' | 'getDocumentSnapshot' | 'bounds-only' | 'manual';
    snapshotCount?: number;
    overlayCount?: number;
    notes?: string[];
    pixelProbe?: ReferenceVisualQaPixelProbeEvidence;
}

export interface ReferenceVisualQaPixelProbeEvidence {
    mode: 'pixel-probe';
    status: ReferenceVisualQaStatus;
    width?: number;
    height?: number;
...
export interface ReferenceReplicationVisualQaReport {
    ...
    evidence: string[];
    limitations: string[];
    snapshotEvidence?: ReferenceVisualQaSnapshotEvidence;
    evidenceReport?: ReferenceReplicationVisualQaEvidenceReport;
}

export interface ReferenceReplicationVisualQaEvidenceReport {
    ...
    snapshot: {
        source: ReferenceVisualQaSnapshotEvidence['source'] | 'none';
        snapshotCount: number;
        overlayCount: number;
...
function formatPixelProbeEvidence(pixelProbe?: ReferenceVisualQaPixelProbeEvidence): string {
...
export function buildReferenceReplicationVisualQaReport(input: {
    ...
    if (counts.total > 0) {
        evidence.push(`bounds 对比 ${counts.total} 项：通过 ${counts.ok}，观察 ${counts.watch}，不匹配 ${counts.mismatch}，未验证 ${counts.unverified}`);
        ...
        if (mismatchDetails) {
            evidence.push(`bounds 不匹配明细: ${mismatchDetails}`);
        }
        if (watchDetails) {
            evidence.push(`bounds 观察项明细: ${watchDetails}`);
        }
    }

    if (input.snapshotEvidence) {
        const snapshotCount = Number(input.snapshotEvidence.snapshotCount || 0);
        const overlayCount = Number(input.snapshotEvidence.overlayCount || 0);
        evidence.push(`截图证据: ${input.snapshotEvidence.source}，截图 ${snapshotCount} 张，overlay ${overlayCount} 项`);
        if (input.snapshotEvidence.source === 'bounds-only' || snapshotCount <= 0) {
            limitations.push('当前只有 Photoshop 图层 bounds 证据，没有截图或 overlay 图像证据。');
        }
        if (Array.isArray(input.snapshotEvidence.notes)) {
            evidence.push(...input.snapshotEvidence.notes.slice(0, 4));
        }
        const pixelProbeLine = formatPixelProbeEvidence(input.snapshotEvidence.pixelProbe);
        if (pixelProbeLine) {
            evidence.push(pixelProbeLine);
            if (input.snapshotEvidence.pixelProbe?.status !== 'ok') {
                limitations.push('截图像素探针未达到 ok，仅能作为诊断信号；不能据此宣称高保真复刻完成。');
            }
            if (input.snapshotEvidence.pixelProbe?.boundary) {
                limitations.push(input.snapshotEvidence.pixelProbe.boundary);
            }
        }
    } else {
        limitations.push('当前没有接入截图或 overlay 图像证据，不能作为截图级视觉相似度结论。');
    }
    ...
    return {
        ...report,
        evidenceReport: buildReferenceReplicationVisualQaEvidenceReport(report)
    };
}

export function buildReferenceReplicationVisualQaEvidenceReport(
    report: Omit<ReferenceReplicationVisualQaReport, 'evidenceReport'>
): ReferenceReplicationVisualQaEvidenceReport {
    const snapshotCount = Number(report.snapshotEvidence?.snapshotCount || 0);
    const overlayCount = Number(report.snapshotEvidence?.overlayCount || 0);
    const source = report.snapshotEvidence?.source || 'none';
    const pixelProbe = report.snapshotEvidence?.pixelProbe;
    const hasImageEvidence = source !== 'none' && source !== 'bounds-only' && snapshotCount > 0;
    ...
    if (source === 'getScreenSnapshotsWithOverlay' && snapshotCount <= 0) {
        blockers.push('已请求 overlay 截图，但没有获得任何截图证据。');
        nextActions.push('确认当前 UXP 插件已重新载入最新版本，并复测 overlay 截图工具。');
    } else if (!hasImageEvidence) {
        warnings.push('当前没有截图级图像证据，只能作为 bounds 级几何验收。');
    ...
    if (hasImageEvidence && overlayCount <= 0) {
        warnings.push('已获得截图，但没有 overlay 标注项，截图只能作为画面证据，不能说明目标框偏差。');
    }
    ...
    if (report.status === 'ok' && hasImageEvidence) {
