 ...
 * - B3 大小：≤2GB（Node readFileSync 硬边界；不再按体积猜解析成本，见下方常量注释）；
 * - B4 P0 不落盘：profile 只作为工具结果返回，学习写入是 P2（必须走 review gate）；
 * - B7 诚实降级：读不到的字段进 warnings，不臆造。
 */

/**
 * 离线解析的文件体积上限 = Node `fs.readFileSync` 的硬边界 2 GiB。
 *
 * 这里曾经是 500MB，按「文件大」猜「解析贵」。真机语料实测推翻了这个前提：readPsd 已经
 * skipLayerImageData / skipCompositeImageData / skipThumbnail，解析成本只跟图层数走，跟
 * 体积无关——2043MB 的详情页.psb（1440×28002、169 图层）解析仅 24ms、堆增长 2MB；507MB
 * 的 800.psb 解析 15ms、堆增长 1MB。真正的成本在读盘（2GB 约 10.9s），是 IO 不是内存。
 *
 * 而这个猜测的代价是实打实的：某店铺 1025 份源文件里有 350 份（34%）落在 500MB~2GB，
 * 其中 129 份 SKU、124 份详情页——恰恰是最需要被学习的两类交付物，全被挡在门外。
 * 超过 2 GiB 的 47 份是 readFileSync 自身拒绝（"greater than 2 GiB"），那才是真做不到。
 */
export const PSD_DESIGN_SOURCE_MAX_BYTES = 2 * 1024 * 1024 * 1024;
...
export function validatePsdDesignSourceFile(input: {
    ...
    if (Number.isFinite(size) && size > PSD_DESIGN_SOURCE_MAX_BYTES) {
        return {
            ok: false,
            reason: `文件 ${Math.round(size / 1024 / 1024 / 1024 * 10) / 10}GB 超过 Node 单次读取文件的 2GB 硬上限，离线解析读不进来（不是解析能力不足）。请在 Photoshop 中打开该文件后用 getLayerHierarchy（includeBounds: true）读取结构。`
        };
    }
