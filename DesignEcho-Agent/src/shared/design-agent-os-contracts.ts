...
export interface DesignAgentOsEvidence {
    intent: UserIntent;
    brief: DesignBrief;
    assetUnderstanding?: AssetUnderstanding;
...
export interface MainImageSizePlanEvidence {
    sizeKey: string;
...
export interface DetailPageScreenPlanEvidence {
    screenId: string;
    role?: string;
...
export interface SkuBatchPlanEvidence {
    size: number;
    comboCount: number;
...
export interface CopywritingCandidateEvidence {
    text?: string;
    charCount?: number;
...
export interface SmartScalingExecutionEvidence {
...
function statusFromToolCallCounts(toolCallCount: number, failedToolCalls: number): DesignAgentOsStatus {
    if (failedToolCalls > 0) return 'needs_review';
    if (toolCallCount > 0) return 'passed';
    return 'not_run';
}
...
export function buildUserIntentFromText(
    rawText: unknown,
    options: {
        scenario?: DesignAgentOsScenario;
        action?: DesignAgentOsAction;
    ...
    return {
        ...
        evidence: [{
            source: 'user-input',
            summary: raw ? '从用户输入构造意图诊断；该判断需要后续证据复核。' : '缺少用户输入，只能记录未知意图。',
            status: raw ? 'needs_review' : 'unknown'
        }]
    };
}

export function buildDesignBriefFromIntent(
    intent: UserIntent,
    options: {
        goal?: string;
        audience?: string;
        styleDirection?: string;
        outputSpec?: DesignBrief['outputSpec'];
        constraints?: string[];
    } = {}
): DesignBrief {
    const constraints = [
    ...
    return {
        ...
        evidence: [{
            source: 'user-intent',
            summary: '由 UserIntent 派生设计简报；缺少字段保持为空，不补写虚构信息。',
            status: 'needs_review'
        }]
    };
}

export function buildVisualUnderstandingFromMinimalRepresentation(
    ...
    return {
        ...
        primaryRoles,
        evidence: [{
            source: 'reference-replication',
            summary: `结构化视觉表示包含 ${elements.length} 个元素。`,
            status: elements.length > 0 ? 'needs_review' : 'failed',
            confidence: elements.length > 0 ? 0.72 : 0.1
        }],
...
export function buildDesignDslFromMinimalRepresentation(
    representation: MinimalDesignRepresentation,
    options: { scenario?: DesignAgentOsScenario; constraints?: string[] } = {}
): DesignDSL {
    const elements = Array.isArray(representation.elements) ? representation.elements : [];
    return {
        dslVersion: 'design-agent-os/v0',
        ...
        constraints: options.constraints || [
            '只读 DSL 证据，不直接改变 Photoshop 执行参数。'
        ],
        evidence: [{
            source: 'minimal-design-representation',
            summary: `生成 DesignDSL 区域 ${elements.length} 个。`,
            status: elements.length > 0 ? 'needs_review' : 'failed',
            confidence: elements.length > 0 ? 0.68 : 0.1
        }]
    };
}
...
export function buildExecutionTraceFromToolResults(
    ...
    const status = statusFromToolCallCounts(toolCalls.length, failedToolCalls);
    return {
        ...
        evidence: [{
            source: 'tool-results',
            summary: `记录工具调用 ${toolCalls.length} 次，失败 ${failedToolCalls} 次。`,
            status
        }]
    };
}
...
export function buildVerificationReportFromQa(input: {
    ...
    return {
        ...
        evidence: [{
            source: 'verification',
            summary: `VerificationReport 状态为 ${status}。`,
            status
        }]
    };
}
...
export function buildMainImageExecutionPlan(input: {
    ...
    for (const plan of input.sizePlans) {
        steps.push({
            ...
            expectedEvidence: ['tool result', 'layer bounds', 'execution summary']
        });
        if (plan.quickExportPlanned) {
            steps.push({
                ...
                expectedEvidence: ['export tool result']
            });
        }
    }
    return {
        ...
        steps,
        evidence: [{
            source: 'main-image-size-plan',
            summary: `主图执行计划包含 ${steps.length} 个步骤。`,
            status: steps.length > 0 ? 'needs_review' : 'unknown'
        }],
        limitations: [
...
export function buildDetailPageExecutionPlan(input: {
    screens?: DetailPageScreenPlanEvidence[];
    screenCount?: number;
    assetCount?: number;
}): ExecutionPlan {
    ...
    const steps: ExecutionPlanStep[] = screens.map((screen, index) => ({
        ...
        expectedEvidence: ['fill result', 'layer bounds', 'copy audit', 'placement audit']
    }));
    return {
        ...
        steps,
        evidence: [{
            source: 'detail-page-fill-plan',
            summary: `详情页计划屏数 ${input.screenCount ?? screens.length}，可用素材 ${input.assetCount ?? 'unknown'}。`,
            status: steps.length > 0 ? 'needs_review' : 'unknown'
        }],
        limitations: [
            '详情页 evidence 记录屏级计划和执行摘要，不代表版式质量或截图级验收通过。'
        ]
    };
}

export function buildSkuExecutionPlan(input: {
    specs?: SkuBatchPlanEvidence[];
    colorCount?: number;
    ...
    const steps: ExecutionPlanStep[] = specs.map((spec) => ({
        ...
        expectedEvidence: ['skuLayout result', 'export result', 'combination summary']
    }));
    return {
        ...
        steps,
        evidence: [{
            source: 'sku-batch-plan',
            summary: `SKU 颜色数 ${input.colorCount ?? 'unknown'}，组合数 ${input.totalCombinations ?? 'unknown'}。`,
            status: steps.length > 0 ? 'needs_review' : 'unknown'
        }],
        limitations: [
            'SKU evidence 记录组合与导出摘要，不替代真实文件检查或颜色图层视觉验收。'
        ]
    };
}

export function buildReferenceReplicationDesignAgentOsEvidence(input: {
    userInput?: string;
    representation: MinimalDesignRepresentation;
    ...
    const intent = buildUserIntentFromText(input.userInput, {
        scenario: 'reference-replication',
        action: 'create',
        constraints: ['参考图复刻只读契约证据，不代表高保真完成。']
    });
    const brief = buildDesignBriefFromIntent(intent, {
    ...
    return {
        intent,
        brief,
        visualUnderstanding: buildVisualUnderstandingFromMinimalRepresentation(input.representation),
        designDsl: buildDesignDslFromMinimalRepresentation(input.representation, { scenario: 'reference-replication' }),
        ...
        verificationReport: buildVerificationReportFromQa({
            ...
            limitations: [
                ...
                '当前 evidence 不等于截图级高保真复刻。'
            ]
        })
    };
}

export function buildMainImageDesignAgentOsEvidence(input: {
    userInput?: string;
    imageType?: string;
    ...
    const intent = buildUserIntentFromText(input.userInput, {
        scenario: 'main-image',
        action: 'create',
        constraints: ['主图契约 evidence 不改变当前主图执行参数。']
    });
    ...
    return {
        ...
        assetUnderstanding: {
            ...
            risks: subject ? [] : ['缺少主体 bounds 证据。'],
            evidence: [{
                source: 'photoshop-document',
                summary: input.docInfo
                    ? `当前文档 ${input.docInfo.name || 'unknown'}，画布 ${input.docInfo.width || '?'}x${input.docInfo.height || '?'}。`
                    : '未记录 Photoshop 文档信息。',
                status: input.docInfo ? 'needs_review' : 'unknown'
            }]
        },
        executionPlan: buildMainImageExecutionPlan({ sizePlans: input.sizePlans }),
        ...
        verificationReport: buildVerificationReportFromQa({
            ...
            limitations: [
                '当前主图 evidence 记录缩放和工具结果，不代表审美质量自动通过。',
                '截图级主图验收和人工评分仍需后续闭环。'
            ]
        })
    };
...
export function buildDetailPageDesignAgentOsEvidence(input: {
    userInput?: string;
    screenCount?: number;
    assetCount?: number;
    screens?: DetailPageScreenPlanEvidence[];
    ...
    const intent = buildUserIntentFromText(input.userInput, {
        scenario: 'detail-page',
        action: 'create',
        constraints: ['详情页契约 evidence 只读，不改变填充计划或 Photoshop 写入顺序。']
    });
    const brief = buildDesignBriefFromIntent(intent, {
    ...
    return {
        ...
        assetUnderstanding: {
            ...
            risks: input.assetCount ? [] : ['缺少可确认的详情页素材数量证据。'],
            evidence: [{
                source: 'detail-page-context',
                summary: `详情页素材数量：${input.assetCount ?? 'unknown'}。`,
                status: input.assetCount ? 'needs_review' : 'unknown'
            }]
        },
        ...
        verificationReport: buildVerificationReportFromQa({
            ...
            limitations: [
                '详情页 evidence 不等于版式质量、截图级 QA 或完整设计验收。',
...
export function buildSkuDesignAgentOsEvidence(input: {
    userInput?: string;
    colorCount?: number;
    totalCombinations?: number;
    specs?: SkuBatchPlanEvidence[];
    ...
    const intent = buildUserIntentFromText(input.userInput, {
        scenario: 'sku',
        action: 'create',
        constraints: ['SKU 契约 evidence 只读，不改变组合生成和导出逻辑。']
    });
    const brief = buildDesignBriefFromIntent(intent, {
    ...
    return {
        ...
        assetUnderstanding: {
            ...
            risks: input.colorCount ? [] : ['缺少颜色图层数量证据。'],
            evidence: [{
                source: 'sku-context',
                summary: `SKU 颜色数量：${input.colorCount ?? 'unknown'}，组合数量：${input.totalCombinations ?? 'unknown'}。`,
                status: input.colorCount ? 'needs_review' : 'unknown'
            }]
        },
        ...
        verificationReport: buildVerificationReportFromQa({
            ...
            limitations: [
                'SKU evidence 不替代导出文件存在性、颜色准确性和版式视觉验收。'
            ]
        })
    };
}
...
function normalizeCopywritingCandidates(value: CopywritingCandidateEvidence[] | string[] | undefined): CopywritingCandidateEvidence[] {
    if (!Array.isArray(value)) return [];
    return value
        .map((item): CopywritingCandidateEvidence => {
...
export function buildCopywritingDesignAgentOsEvidence(input: {
    userInput?: string;
    originalText?: string;
    layerId?: number | null;
    candidates?: CopywritingCandidateEvidence[] | string[];
    success: boolean;
    degraded?: boolean;
    error?: string;
    hasImage?: boolean;
    ...
    const intent = buildUserIntentFromText(input.userInput || '撰写文案', {
        scenario: 'copywriting',
        action: 'create',
        constraints: [
            '文案契约 evidence 只读，不改变文本生成或 Photoshop 写入行为。',
    ...
    return {
        ...
        brief,
        assetUnderstanding: {
            source: 'copywriting-context',
            assetCount: input.hasImage ? 1 : 0,
            risks: missingContext,
            evidence: [{
                source: 'copywriting-input',
                summary: input.hasImage
                    ? '文案请求包含参考图片证据。'
                    : '文案请求没有参考图片证据，不能编造画面、款式、材质或场景。',
                status: input.hasImage ? 'needs_review' : 'unknown'
            }]
        },
        executionPlan: {
            planId: 'copywriting-evidence-plan',
            scenario: 'copywriting',
            ...
            steps: [
                {
                    ...
                    expectedEvidence: ['original character count', 'line count', 'punctuation skeleton']
                },
                {
                    ...
                    reason: '按目标人群、商品事实、图片证据和版式约束生成候选。',
                    expectedEvidence: ['model output', 'candidate list']
                },
                {
                    ...
                    expectedEvidence: ['candidate fit status', 'risk notes']
                }
            ],
            evidence: [{
                source: 'copywriting-handler',
                summary: `生成候选 ${candidates.length} 条，风险候选 ${riskCandidateCount} 条。`,
                status: verificationStatus
            }],
            limitations: [
                '该计划记录文案生成和版式校验证据，不代表已写入 Photoshop。',
        ...
        verificationReport: {
            ...
            limitations: [
                '文案 evidence 不替代人工判断语感、图文匹配和品牌调性。',
                '当前文本内容不得作为新文案语义参考，只能作为版式骨架。'
            ],
            evidence: [{
                source: 'copywriting-candidates',
                summary: `candidateCount=${candidates.length}; status=${verificationStatus}`,
                status: verificationStatus
            }]
        }
    };
...
function normalizePostTransformBounds(bounds: SmartScalingExecutionEvidence['postTransformBounds']): AssetUnderstanding['subjectBounds'] | undefined {
...
export function buildSmartScalingDesignAgentOsEvidence(input: SmartScalingExecutionEvidence & {
    userInput?: string;
    success?: boolean;
    toolResults?: Array<{ toolName?: string; result?: unknown }>;
}): DesignAgentOsEvidence {
    const scenario = scenarioFromSmartScalingDesignType(input.designType);
    const intent = buildUserIntentFromText(input.userInput || '智能缩放与放置', {
        scenario,
        action: 'edit',
        constraints: ['智能缩放契约 evidence 只读；planned destinationBox 不等于 Photoshop 已执行结果。']
    });
    ...
    const warnings = [
        ...
        postBounds ? '' : '缺少 Photoshop 执行后的 layer bounds 证据。',
    ...
    return {
        intent,
        brief: buildDesignBriefFromIntent(intent, {
            goal: '将素材缩放并放入目标视觉区域',
            constraints: [
        ...
        assetUnderstanding: {
            ...
            risks: warnings,
            evidence: [{
                source: 'smart-scaling-policy',
                summary: `scale=${input.decision.scalePercent.toFixed(2)}%; fillRatio=${input.decision.fillRatio.toFixed(2)}; subjectVisibleRatio=${input.decision.subjectVisibleRatio.toFixed(2)}。`,
                status: verificationStatus
            }]
        },
        executionPlan: {
            ...
            steps: [{
                ...
                expectedEvidence: ['Photoshop transform result', 'post-transform layer bounds', 'visual QA']
            }],
            evidence: [{
                source: 'smart-scaling-policy',
                summary: postBounds
                    ? '已记录执行后 bounds，但仍需视觉和审美复核。'
                    : '仅记录缩放计划，尚未证明 Photoshop 已完成变换。',
                status: verificationStatus
            }],
            limitations: [
                '智能缩放只计算几何放置，不保证审美最佳。',
        ...
        verificationReport: {
            ...
            summary: failed
                ...
                : postBounds
                    ...
                    : '智能缩放仅有计划证据，缺少 Photoshop 执行后 bounds。',
            checks: [
                {
                ...
                },
                {
                    ...
                    summary: postBounds ? '存在执行后 bounds 证据。' : '缺少执行后 bounds，不能确认 Photoshop 已完成变换。'
                },
                {
            ...
            evidence: [{
                source: 'smart-scaling-verification',
                summary: `postBounds=${postBounds ? 'yes' : 'no'}; cropRisk=${cropRisk}`,
                status: verificationStatus
            }]
        }
    };
...
export function buildKnowledgeSearchDesignAgentOsEvidence(input: {
    userInput?: string;
    ...
    const intent = buildUserIntentFromText(input.userInput || input.query.query || '检索设计知识', {
        scenario,
        action: 'analyze',
    ...
    return {
        ...
        executionPlan: {
            ...
            steps: [{
                ...
                expectedEvidence: ['knowledge results', 'allowed uses', 'source summaries']
            }],
            evidence: [{
                source: 'design-knowledge-search',
                summary: `检索到 ${results.length} 条知识结果。`,
                status
            }],
            limitations: [
        ...
        verificationReport: {
            ...
            limitations: [
                '知识 evidence 不证明模型已经理解或正确使用这些知识。',
                '外部网页搜索、引用归一化和多模态检索仍需后续接入。'
            ],
            evidence: results.slice(0, 6).map(result => ({
                source: result.sourceType,
                summary: `${result.title}: ${result.summary.slice(0, 160)}; evidenceLevel=${result.evidenceLevel}; sourceRank=${result.sourceRank}`,
                status: 'needs_review' as DesignAgentOsStatus
            }))
        }
    };
}
