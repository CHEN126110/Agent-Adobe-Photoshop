...
function normalizeAssetVersion(value: unknown): ProjectVisualSamplingCacheEntry['assetVersion'] {
    if (!value || typeof value !== 'object') return undefined;
    const record = value as Record<string, unknown>;
    const modifiedTimeMs = Number(record.modifiedTimeMs);
    const sizeBytes = Number(record.sizeBytes);
    const normalized = {
        ...(Number.isFinite(modifiedTimeMs) && modifiedTimeMs > 0 ? { modifiedTimeMs } : {}),
...
export function sanitizeProjectVisualInsightCacheEntries(
    ...
    for (const entry of entries || []) {
        ...
        const sanitized: ProjectVisualSamplingCacheEntry = {
            ...
            path: path || undefined,
            assetVersion: normalizeAssetVersion(entry.assetVersion),
