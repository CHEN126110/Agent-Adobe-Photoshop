...
export type ProjectAssetUnderstandingIntakeStatus =
    | 'ready_with_cached_visual_evidence'
...
export interface ProjectAssetUnderstandingIntake {
    ...
    canUseCachedVisualEvidence: boolean;
...
export function buildProjectAssetUnderstandingIntake(
    ...
    const requiredNextEvidence = buildRequiredNextEvidence(status);
    ...
    return {
        ...
        canUseCachedVisualEvidence: coverage.entriesWithInsight > 0 || coverage.cacheHits > 0,
        ...
        warnings,
        limitations: [
            'ProjectAssetUnderstandingIntake is hidden evidence and must not be shown as model thinking.',
            'It summarizes project asset candidates and visual evidence readiness only.',
            'It does not read image bitmap data, call a vision model, touch Photoshop, or change executor behavior.',
            'It cannot choose the best image or claim design quality without downstream visual and QA evidence.',
            'Cached visual evidence can reduce repeated analysis, but still requires design-specific verification.'
        ],
        evidence: buildEvidence(input.skillId, status, coverage)
    };
}
...
function buildCoverage(
    ...
    return {
        ...
        cacheHits: Number(visualSamplingPlan?.cacheSummary?.hit || 0),
        ...
        entriesWithInsight: Number(visualInsightCache?.summary?.entriesWithInsight || 0)
    };
}
...
function buildStatus(input: {
    ...
    if (input.coverage.entriesWithInsight > 0 || input.coverage.cacheHits > 0) {
        return 'ready_with_cached_visual_evidence';
    }
    return 'needs_bounded_visual_analysis';
}

function buildRequiredNextEvidence(status: ProjectAssetUnderstandingIntakeStatus): string[] {
    switch (status) {
        case 'ready_with_cached_visual_evidence':
            return [];
...
function buildWarnings(
    ...
    return uniqueStrings([
        ...
        status === 'needs_bounded_visual_analysis'
            ...
            : '',
        status === 'ready_with_cached_visual_evidence'
            ? 'cached_visual_evidence_is_not_design_quality_acceptance'
            : ''
    ]);
}
...
function buildEvidence(
    ...
    return [{
        source: 'project-asset-understanding-intake',
        summary: `skill=${skillId}; status=${status}; images=${coverage.totalImages}; selected=${coverage.selectedCandidates}; insights=${coverage.entriesWithInsight}`,
        status: status === 'ready_with_cached_visual_evidence' ? 'needs_review' : 'unknown'
    }];
}
