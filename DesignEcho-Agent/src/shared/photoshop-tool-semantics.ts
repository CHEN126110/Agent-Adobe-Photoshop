...
export const PHOTOSHOP_TOOL_SEMANTICS: PhotoshopToolSemantic[] = [
    {
        ...
        acceptance: {
            evidence: ['after snapshot text.content', 'after snapshot text.style', 'after layer bounds', 'text layer kind', 'optional overlay bounds'],
            pass: ['文本图层存在', '内容完全匹配', '可读的字号/字距/行高参数匹配', '实际 bounds 在阈值内'],
            needsReview: ['bounds 轻微偏差', '字体名称未知或 fallback', '缺少 overlay 证据'],
    ...
    },
    {
        ...
        acceptance: {
            evidence: ['before/after text.content', 'target layer id list', 'diff textChanged count'],
    ...
    },
    {
        ...
        parameters: [
            ...
            },
            {
                name: 'tracking / leading / alignment',
                required: false,
                meaning: '字距、行距和对齐等排版细节；tracking/leading 已有 after 证据，alignment 仍需要结合 bounds 或视觉验收。',
        ...
        acceptance: {
            evidence: ['after text.fontName', 'after text.fontSize', 'after layer bounds', 'font assertion summary'],
    ...
    },
    {
        ...
        acceptance: {
            evidence: ['plannedBox', 'actualBounds', 'IoU / edge delta / center offset', 'optional overlay screenshot'],
            pass: ['实际 bounds 在阈值内', '内容和主要样式已通过'],
            needsReview: ['轻微宽度差异可能来自字体字形', '只有 bounds-only 没有截图证据'],
    ...
    },
    {
        ...
        parameters: [
            {
                name: 'layerId',
                required: false,
                meaning: '目标图层；缺省时通常依赖当前选中图层。',
                failureModes: ['当前选区不明确会移动错图层', '多选时只验证单个图层会产生假成功']
            },
            {
                name: 'x/y',
                unit: 'px',
            ...
            },
            {
                name: 'alignment',
                required: false,
                meaning: '对齐方向；需要知道对齐到画布、选区还是参考图层。',
                failureModes: ['只传 left/center 却没有明确参照物', '对齐后破坏原本的网格或视觉节奏']
            },
            {
                name: 'scaleUniform / percent',
                unit: '%',
                required: false,
                meaning: '基础缩放参数；应来自目标框、主体占比或用户明确要求。',
                failureModes: ['固定百分比导致主体过大或过小', '缩放后没有复核 bounds 和清晰度']
            }
        ],
        commonFailureModes: [
        ...
        acceptance: {
            evidence: ['before/after layer bounds', 'target layer id', 'alignment or transform params', 'optional plannedBox'],
            pass: ['目标图层明确', 'after bounds 符合计划坐标或计划框', '未出现未解释的画布越界'],
            needsReview: ['依赖当前选区', '只有工具成功消息但缺少 plannedBox', '缩放后缺少截图证据'],
            fail: ['显式目标图层不存在', 'after bounds 未变化或偏差过大', '移动到了画布外']
        },
    ...
    },
    {
        ...
        acceptance: {
            evidence: ['before/after layer hierarchy', 'target layer id', 'reorderLayer action', 'targetLayerId for above/below'],
    ...
    },
    {
        ...
        acceptance: {
            evidence: ['before/after layer hierarchy', 'source layerId', 'targetGroupId', 'moveLayerToGroup result'],
    ...
    },
    {
        ...
        parameters: [
            ...
            },
            {
                name: 'includeBounds',
                required: false,
                meaning: '返回真实图层 bounds，作为用户可见聚焦和后续验收证据。',
        ...
        acceptance: {
            evidence: ['focusLayer result.focusedLayer', 'focusLayer result.bounds', 'focusLayer result.viewport.exactPanZoomSupported'],
            pass: ['目标图层被选中', '返回真实图层 ID、名称和 bounds', '明确声明未做精确 pan/zoom'],
            needsReview: ['只提供 layerName 且名称有潜在歧义', '目标图层 bounds 为空或很小', '频繁聚焦影响任务性能'],
            fail: ['目标图层不存在', '工具声称完成 pan/zoom 但没有真实证据', '聚焦到非本轮修改图层']
        },
    ...
    },
    {
        ...
        designSemantics: [
            ...
            '自动选图必须有候选来源和评分证据；不能把“找到一张图片”伪装成理解了用户要的图片。',
        ...
        parameters: [
            ...
            {
                name: 'x/y/center/scale/fitToCanvas',
                required: false,
                meaning: '初始落位参数；应只作为执行候选，最终仍需读取 after bounds。',
                failureModes: ['center=true 掩盖实际需要的视觉重心', 'fitToCanvas 破坏留白或裁切主体']
            }
        ],
        commonFailureModes: [
            '图片成功置入但不是用户需要的素材。',
        ...
        acceptance: {
            evidence: ['result layerId', 'after layer bounds', 'source path or selection evidence', 'optional placementAudit'],
            pass: ['成功创建或替换目标图片层', '来源证据明确', 'actual bounds 与目标框或审计结果一致'],
            needsReview: ['自动选图分数不足', '缺少主体边界', '只有 bounds 证据没有截图证据'],
            fail: ['图片来源不可用', '未生成目标图片层', '替换后原占位图仍可见或 bounds 严重偏移']
        },
    ...
    },
    {
        ...
        acceptance: {
            evidence: ['created layer id', 'after layer kind', 'after layer bounds', 'fill/corner parameters when readable'],
            pass: ['创建形状层', 'bounds 与计划框匹配', '核心样式有执行或明确记录为未验证'],
            needsReview: ['样式读回不足', '层级关系未验证', '只有结构证据没有截图证据'],
    ...
    },
    {
        ...
        commonFailureModes: [
            ...
            '效果加在错误图层或错误层级。',
            '缺少截图证据时难以判断投影半径和透明度是否合理。'
        ],
        acceptance: {
            evidence: ['tool result', 'target layer id', 'style recipe attempt summary', 'optional after bounds/effects snapshot'],
            pass: ['目标图层明确', 'recipe 执行成功', '效果参数在可接受范围内'],
            needsReview: ['Photoshop snapshot 不能读回具体效果参数', '只有工具成功消息没有截图证据'],
...
export function buildPhotoshopToolSemanticsSummary(category?: PhotoshopToolSemanticCategory): string {
    ...
    return items.map((item) => [
        ...
        `验收证据: ${item.acceptance.evidence.join(', ')}`
    ].join('\n')).join('\n\n');
}
