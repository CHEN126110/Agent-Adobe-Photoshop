...
export const PHOTOSHOP_TOOL_SEMANTICS: PhotoshopToolSemantic[] = [
    ...
    },
    {
        ...
        designSemantics: [
            '文本图层的图层面板名称与画面可见文字是两个独立属性；未指明属性的“A 改成 B”必须先读回并把 A 绑定到唯一匹配的当前属性，不能猜测。',
        ...
        commonFailureModes: [
            '工具返回成功但目标图层文本未变化。',
            '只改了部分批量更新项。',
            '普通改文案复用旧样式缓存，把稍后修改的字体或字号恢复成旧值。',
            '换行被压成一行导致版面溢出。'
        ],
        acceptance: {
            checks: ['before/after text.content', 'before/after readable text style', 'target layer id list', 'diff textChanged count'],
            pass: ['显式目标内容全部匹配', '换行归一化后仍符合预期', '调用时的字体、字号、字距、行距和缩放保持'],
            needsReview: ['目标来自当前选中图层且多选不明确', '内容匹配但视觉溢出未验证', '快照缺少可比较样式字段'],
            fail: ['显式目标不存在', '目标不是文本层', 'after 内容不匹配', '改文案时发生未请求的样式漂移']
        },
    ...
    },
    {
        ...
        zhName: '修改字体与文字样式',
        maturity: 'mvp',
        intentSignals: ['改字体', '改字号', '调整字距', '调整行距', '统一字体'],
        ...
        designSemantics: [
            ...
            'setTextStyle 是单图层字段 patch：只修改显式提供的字体、字号、字距或行距，文本内容、混合 style ranges 和省略的样式必须保持不变。',
        ...
        parameters: [
            ...
            {
                name: 'tracking / leading',
                required: false,
                meaning: 'tracking 为千分之一 em；leading 为 pt 固定行高，提供 leading 会关闭 autoLeading。对齐不是本工具能力。',
                failureModes: ['把画布 px 当成 tracking 或 leading', '工具不支持的字段被当作已完成']
            }
        ],
        punctuationAndLineBreakRules: [
            '样式修改不应改变文本内容和换行。',
        ...
        commonFailureModes: [
            ...
            '字号断言通过但实际视觉宽度偏离。',
            '稀疏 textStyleRange 覆盖导致字体或其他未请求属性漂移。',
            '样式工具不支持的字段被当作已完成。'
        ],
        acceptance: {
            checks: ['same-target before/after requested fields', 'before/after text.content', 'unrequested readable style fields', 'textStyleRange structure', 'after layer bounds'],
            pass: ['显式请求字段匹配', '文本内容与未请求样式保持', '混合样式范围未被压平'],
            needsReview: ['字体名只能规范化匹配', '字号匹配但 bounds 或换行明显变化', 'autoLeading 计算值随字号发生歧义'],
            fail: ['显式目标不存在', '请求字段不匹配', '字体、内容或其他未请求属性漂移', '工具返回失败或回滚失败']
        },
        benchmarkNeeds: ['统一中文字体', '缺失字体反馈', '字号调整', '字距/行距规划 case', '中英混排字体'],
        notSolvedByThis: ['不负责安装字体', '不保证字体授权', '不支持颜色、字重、段落对齐或文本框宽高', '不把规范化名称匹配当成字形完全一致']
    },
    {
        ...
        parameters: [
            {
                name: 'action',
                required: true,
                meaning: '子动作：inspectTemplateLayout / getCapabilities / listLayerSets / getProgress 只读；execute / arrangeDynamic 写入并导出。旧版 executeOne / executeBatch 已停用且不在公开能力面。',
                failureModes: ['跳过只读检查直接 execute', '把自选备注 arrangeDynamic 与组合 execute 混用', '调用已停用的 executeOne / executeBatch']
            },
            {
                name: 'combos',
                required: false,
