...
export function isAgentIntentDecisionBoundaryOk(
    ...
    return intake.evidenceOnly === true
...
function analyzeIntentDecision(
    ...
    if (typeof expected.requiresPhotoshop === 'boolean'
        ...
        requiredNextEvidence.push('photoshop_requirement_review_required');
    }

    if (lifecycle.execution.requiresPhotoshop && !lifecycle.execution.canStart) {
        blockers.push('执行前上下文不足，当前路由不能安全开始。');
        requiredNextEvidence.push('photoshop_context_or_document_required');
    }
    ...
    if (expected.shouldUseTools === true && toolCallCount === 0) {
        blockers.push('期望执行工具，但没有工具调用证据。');
        requiredNextEvidence.push('tool_call_evidence_required');
    }
    if (expected.shouldUseTools === false && toolCallCount > 0) {
        warnings.push('当前任务期望不使用工具，但实际存在工具调用证据。');
        requiredNextEvidence.push('unexpected_tool_usage_review');
    }
    if (lifecycle.decision.route === 'autonomous_agent') {
        warnings.push('当前进入 autonomous_agent，需要确认上下文、预算和可验收计划，避免简单任务绕远路。');
        requiredNextEvidence.push('autonomous_route_context_and_budget_review');
    }
    ...
    return {
        ...
        requiredNextEvidence: uniqueStrings(requiredNextEvidence)
    };
}
