...
import type { MainImageVisionPreflightEvidence } from './main-image-vision-preflight';
...
export interface MainImageQaReportInput {
    ...
    visionPreflight?: MainImageVisionPreflightEvidence | null;
...
function buildResultImageSummary(input: MainImageQaReportInput): MainImageQaReportResultImageSummary {
    const resultPaths = input.screenshotQa?.resultImageEvidence?.resultPaths || [];
...
function buildQualityClaim(
    ...
    return {
        allowed,
        reason: allowed
            ? '主图 QA 报告具备结果图、脱敏 pixel probe 和人工 approved 证据。'
            : '当前证据不足以声明主图设计质量通过。',
        requiredEvidence,
        blockers: uniqueBlockers,
        boundary: 'qualityClaim 只表示证据链可进入人工确认后的质量声明，不是模型自动审美评分。'
    };
}
    ...
    return [
        ...
        makeSection(
            ...
            input.candidatePreflight
                ...
                : '没有素材候选预检证据。',
            [
                ...(input.candidatePreflight?.evidence || []).map((item) => item.summary),
        ...
        makeSection(
            ...
            input.executionAlignment
                ...
                : '没有主图计划与工具结果对齐证据。',
            [
                ...(input.executionAlignment?.evidence || []).map((item) => item.summary),
...
export function buildMainImageQaReport(input: MainImageQaReportInput): MainImageQaReport {
    ...
    const limitations = [
        'mainImageQaReport 只汇总已有主图证据，不执行 Photoshop，不调用视觉模型，不修改设计结果。',
        '报告中的路径只保留文件名摘要；不会返回 raw image、base64、原始像素数据或 buffer。',
        '文件探针、尺寸、hash 和 pixel probe 只能证明部分技术证据，不等于审美验收。',
        '只有截图 QA passed、pixelProbe=ok、manualReview=approved 且无脱敏阻断时，qualityClaim 才允许为 true。'
    ];
    const evidence: EvidenceRef[] = [
        {
            source: 'main-image-qa-report',
            summary: `stage=${stage}; resultImages=${resultImageSummary.resultImageCount}; pixelProbe=${resultImageSummary.pixelProbeStatus}; manual=${resultImageSummary.manualReviewDecision}; claim=${qualityClaim.allowed}`,
            status
        },
        ...(input.screenshotQa?.verificationReport?.evidence || []),
        ...(input.screenshotProbeReadiness?.verificationReport?.evidence || [])
    ].slice(0, 8);
