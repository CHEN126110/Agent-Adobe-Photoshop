...
export type ImagePlacementEvidenceTier = 'metadata' | 'subject-bounds' | 'bounds' | 'screenshot';
...
export interface ImagePlacementPlan {
    ...
    assetRole: SmartScalingAssetRole;
    intent: SmartScalingIntent;
    evidenceTier: ImagePlacementEvidenceTier;
    source: ImagePlacementSource;
    target: ImagePlacementTarget;
...
function resolveEvidenceTier(input: ImagePlacementPlanInput): ImagePlacementEvidenceTier {
...
function buildPlanWarnings(input: ImagePlacementPlanInput, decision: SmartScalingDecision, evidenceTier: ImagePlacementEvidenceTier): string[] {
    const warnings = [...decision.warnings];
    if (evidenceTier === 'metadata') {
...
function resolvePlanStatus(input: {
    ...
    if (input.evidenceTier === 'metadata') return 'needs_review';
    return 'ready';
}
...
export function buildImagePlacementPlan(input: ImagePlacementPlanInput): ImagePlacementPlan {
    ...
    const evidenceTier = resolveEvidenceTier(input);
...
function verifyStatus(input: {
    ...
    if (input.plan.evidenceTier === 'metadata') return 'needs_review';
...
function buildVerificationWarnings(input: ImagePlacementVerificationInput, deviation?: ImagePlacementVerification['deviation']): string[] {
    ...
    if (input.plan.execution.requiredReadback.includes('screenshot') && !input.screenshotEvidence?.available) {
        warnings.push('当前计划要求截图证据，但没有提供 screenshotEvidence。');
    }
    return Array.from(new Set(warnings));
}
...
function buildVerificationBlockers(input: ImagePlacementVerificationInput, status: ImagePlacementVerificationStatus): string[] {
    ...
    if (input.screenshotEvidence?.reviewStatus === 'failed') {
        blockers.push(input.screenshotEvidence.reason || '截图复核失败。');
    }
...
export function verifyImagePlacement(input: ImagePlacementVerificationInput): ImagePlacementVerification {
    ...
    const screenshotStatus = input.screenshotEvidence?.reviewStatus;
    ...
    if (input.screenshotEvidence?.reviewStatus === 'passed') {
    ...
    return {
        version: 'image-placement-core/v0',
        status,
        evidenceTier: actualBounds ? 'bounds' : input.plan.evidenceTier,
        deviation,
        ...
        limitations: [
            ...
            '没有 screenshotEvidence 时，不能声明画面重心、裁切观感或详情页整体质量通过。',
...
export function formatImagePlacementCorePolicyForPlanner(): string[] {
    return [
        ...
        'Never treat planned destinationBox as Photoshop execution evidence; read actualBounds after UXP execution.',
