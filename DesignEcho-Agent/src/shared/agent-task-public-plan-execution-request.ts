...
function issueAgentTaskPublicPlanRequestId(): string {
    if (typeof globalThis.crypto?.randomUUID !== 'function') {
        throw new Error('当前运行环境无法安全签发公开计划 requestId；已拒绝创建可确认请求。');
    }
    return `agent-task-public-plan-${globalThis.crypto.randomUUID()}`;
}
...
export function buildAgentTaskPublicPlanExecutionRequest(
    ...
    const requestId = String(input.requestId || '').trim() || issueAgentTaskPublicPlanRequestId();
