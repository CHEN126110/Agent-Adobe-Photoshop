...
export interface MainImageVariantPlacementStrategyEvidence {
...
function resolveStatus(input: {
    ...
    if (input.styleEvidence.status !== 'ready_visual_grounded') return 'blocked_missing_visual_grounding';
...
function buildBlockers(status: MainImageVariantPlacementStrategyStatus): string[] {
    switch (status) {
        ...
        case 'blocked_missing_visual_grounding':
            return ['main_image_visual_grounding_required'];
...
function buildVerificationPolicy(plans: MainImageVariantPlacementPlan[]): MainImageVariantPlacementStrategyEvidence['verificationPolicy'] {
...
function getEvidenceStatus(status: MainImageVariantPlacementStrategyStatus): 'ready' | 'needs_review' | 'unknown' | 'failed' {
    if (status === 'ready_variant_placement_plan') return 'ready';
    if (status === 'blocked_missing_visual_grounding' || status === 'blocked_missing_subject_bounds') return 'needs_review';
    return 'failed';
}

export function buildMainImageVariantPlacementStrategyEvidence(
    ...
    const variants = getStyleVariants(input.projectStyleStrategyEvidence);
    const variantPlacementPlans = status === 'ready_variant_placement_plan' && asset && subjectBounds
        ...
        : [];
    const projectStyle = input.projectStyleStrategyEvidence;
    ...
    return {
        ...
        limitations: [
            ...
            '款式判断必须来自 projectStyleStrategyEvidence 的视觉或人工 grounding，不能从文件名猜测。',
            'destinationBox 和 subjectDestinationBox 是计划值，不是 Photoshop actualBounds。',
            '点击图/转化图只是同一素材下的设计方向与置入策略，最终质量仍依赖执行后验收。'
        ],
        evidence: [{
            source: 'main-image-variant-placement-strategy',
            summary: `status=${status}; variants=${variants.length}; sizes=${sizePlans.length}; placements=${variantPlacementPlans.length}`,
