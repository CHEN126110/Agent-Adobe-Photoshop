...
/** 栅格结构参数与间距档位；非法值交给布局引擎回落默认档并告警，预检不该比执行更严格。 */
function normalizeGridNumber(value: unknown): number | undefined {
...
function collectResolvedRenderLayoutVisibleBlocks(
    ...
    operationRequests.forEach((operation, operationIndex) => {
        ...
        // 预检必须与真实执行吃同一套栅格：结构参数与间距档位共同决定版心与间距，
        // 少传任何一个都会让这里算出的坐标与实际写入不一致，文案可见性预检就失去意义。
        const resolved = solveLayout({
            canvas,
            columns: normalizeGridNumber(operation.params.columns),
            marginScale: normalizeGridNumber(operation.params.marginScale),
            gutterScale: normalizeGridNumber(operation.params.gutterScale),
            gapScale: normalizeGridNumber(operation.params.gapScale),
            blocks
        });
