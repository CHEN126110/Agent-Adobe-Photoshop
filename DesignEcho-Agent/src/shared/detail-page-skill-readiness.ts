...
export interface DetailPageSkillProjectContext {
    ...
    existingDocumentAvailable?: boolean;
    targetScopeAvailable?: boolean;
    requestedChangeAvailable?: boolean;
    editContentModeAvailable?: boolean;
}
...
function assessProjectSection(
    ...
    const attachedImageCount = toNumber(project?.attachedImageCount);
    const contentSourceAvailable = project?.contentSourceAvailable === true
        || attachedImageCount > 0;
    const existingDocumentAvailable = project?.existingDocumentAvailable === true;
    const targetScopeAvailable = project?.targetScopeAvailable === true;
    const requestedChangeAvailable = project?.requestedChangeAvailable === true;
    const editContentModeAvailable = project?.editContentModeAvailable === true;

    if (workMode === 'edit_existing') {
        const blockers: string[] = [];
        const requiredNextChecks: string[] = [];
        if (!existingDocumentAvailable) {
            blockers.push('局部编辑需要当前 Photoshop 详情页文档。');
            requiredNextChecks.push('existing_document');
        }
        if (!targetScopeAvailable) {
            blockers.push('局部编辑需要显式 targetScope，不能默认改写整页。');
            requiredNextChecks.push('target_scope');
        }
        if (!requestedChangeAvailable) {
            blockers.push('局部编辑需要明确 requestedChange。');
            requiredNextChecks.push('requested_change');
        }
        if (!editContentModeAvailable) {
            blockers.push('局部编辑需要结构化 editContentMode（image_only、copy_only 或 both），不能根据自然语言关键词猜测写入类型。');
            requiredNextChecks.push('edit_content_mode');
        }
        if (blockers.length > 0) {
            return buildSection('blocked', '局部编辑输入不完整。', {
                blockers,
                requiredNextChecks
            });
        }
        return buildSection(
            'ready',
            '当前文档、局部目标和修改要求已明确；局部编辑不依赖项目素材索引。'
        );
    }

    if (workMode === 'template_fill') {
        if (!contentSourceAvailable) {
            return buildSection(
                'blocked',
                '模板套版缺少可用内容来源。',
                {
                    blockers: ['需要 contentSource、用户附件或项目素材中的至少一种内容来源。'],
                    requiredNextChecks: ['content_source']
                }
            );
        }
        return buildSection(
            'ready',
            attachedImageCount > 0
                ? `用户附件已可作为模板内容来源：${attachedImageCount} 张。`
                : '模板内容来源已明确；无需额外要求 projectPath 或 ProjectAssetIndex。'
        );
    }

    if (!projectPathKnown && assetImageCount <= 0 && attachedImageCount <= 0) {
        return buildSection(
            'blocked',
            '详情页执行缺少项目路径或可用图片素材。',
            {
    ...
    if (
        attachedImageCount <= 0
        && (visualInsightCount <= 0 || shouldAnalyzeCount > 0)
    ) {
        return buildSection(
            'needs_context',
            '已有候选图片，但素材理解还不完整。',
