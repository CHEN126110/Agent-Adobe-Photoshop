...
export interface DetailPageSkillTemplateEvidence {
...
export interface DetailPageSkillProjectEvidence {
...
export interface DetailPageSkillReadiness {
    ...
    sections: {
        ...
        projectVisualEvidence: DetailPageSkillReadinessSection;
...
export interface BuildDetailPageSkillReadinessInput {
    mode?: DetailPageSkillReadinessMode;
    template?: DetailPageSkillTemplateEvidence | null;
    project?: DetailPageSkillProjectEvidence | null;
...
function buildSection(
    ...
    return {
        ...
        requiredNextEvidence: unique(options.requiredNextEvidence || [])
    };
}

function assessTemplateSection(template: DetailPageSkillTemplateEvidence | null): DetailPageSkillReadinessSection {
    ...
    if (!parseSuccess || screenCount <= 0) {
        return buildSection(
            ...
            '当前没有可靠的详情页模板解析结果。',
            {
                ...
                requiredNextEvidence: ['parseDetailPageTemplate success result', 'parsed detail-page screens']
            }
        );
    }
    ...
    return buildSection(
        ...
        {
            warnings,
            requiredNextEvidence: warnings.length > 0 ? ['template structure review'] : []
        }
    );
}
    ...
    if (!projectPathKnown && assetImageCount <= 0) {
        return buildSection(
            ...
            '详情页执行缺少项目路径或可用图片素材。',
            {
                ...
                requiredNextEvidence: ['ContextSnapshot', 'ProjectAssetIndex or selected images']
            }
        );
    }
    ...
    if (visualCandidateCount <= 0 && selectedCandidateCount <= 0) {
        return buildSection(
            ...
            '项目里还没有适合详情页设计的视觉候选。',
            {
                ...
                requiredNextEvidence: ['detail-page visual candidates']
            }
        );
    }
    ...
    if (visualInsightCount <= 0 || shouldAnalyzeCount > 0) {
        return buildSection(
            ...
            '已有候选图片，但素材理解还不完整。',
            {
                ...
                requiredNextEvidence: ['VisualInsightCache or visual model analysis']
            }
        );
    }
    ...
    if (imagePlacementCoreAvailable !== true) {
        return buildSection(
            ...
            '图片置入能力还需要执行前确认。',
            {
                ...
                requiredNextEvidence: ['ImagePlacementPlan', 'placement verification evidence']
            }
        );
    }
    ...
    if (verificationToolsAvailable !== true) {
        return buildSection(
            ...
            '详情页执行结果还需要完整验收。',
            {
                ...
                requiredNextEvidence: ['parseDetailPageTemplate live audit', 'auditDetailPagePlacement', 'copy layout audit']
            }
        );
    }
...
function resolveStatus(input: {
    ...
    const sections = [input.projectVisualEvidence, input.imagePlacement, input.verification];
...
function buildEvidence(status: DetailPageSkillReadinessStatus, mode: DetailPageSkillReadinessMode): EvidenceRef[] {
    return [{
        source: 'detail-page-skill-readiness',
        summary: `mode=${mode}; readiness=${status}.`,
        status: status === 'blocked' ? 'failed' : 'needs_review'
    }];
}
...
export function buildDetailPageSkillReadiness(
    ...
    const projectVisualEvidence = assessProjectSection(mode, input.project || null);
    ...
    const status = resolveStatus({ mode, template, projectVisualEvidence, imagePlacement, verification });
    const sections = { template, projectVisualEvidence, imagePlacement, verification };
    ...
    const requiredNextEvidence = unique(allSections.flatMap((section) => section.requiredNextEvidence));
    ...
    return {
        ...
        evidence: buildEvidence(status, mode),
        limitations: [
