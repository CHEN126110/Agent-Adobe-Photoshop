...
function buildRun(input: {
    ...
    return {
        ...
        evidenceOnly: typeof input.evidenceOnly === 'boolean'
            ? input.evidenceOnly
            : input.shouldRunPhotoshop !== true,
        rawPayloadRedacted: true,
        shouldRunPhotoshop: input.shouldRunPhotoshop === true,
        mustNotRunWriteTools: input.mustNotRunWriteTools !== false,
        mustNotClaimTaskCompletion: true,
        plannedWriteTools,
        executedWriteTools: normalizeStringList(operationResults.map((result) => result.toolName)),
...
export function runAgentResumeControlledExecutionRunner(
    ...
    return buildRun({
        ...
        warnings: [
            ...request.warnings,
            'runner 默认只生成 dry-run 证据，不调用 Photoshop 写工具。'
        ]
    });
}
