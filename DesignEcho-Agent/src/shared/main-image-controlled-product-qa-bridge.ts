...
import type { MainImageManualReviewEvidence } from './main-image-visual-loop';
...
export interface MainImageControlledProductQaEvidence {
    evidenceVersion: 'main-image-controlled-product-qa-evidence/v0';
...
function buildQualityClaim(stage: MainImageControlledProductQaBridgeStage, gate: MainImageControlledProductQaGate): MainImageControlledProductQualityClaim {
    return {
        ...
        allowed: false,
        reason: stage === 'passed'
            ? 'controlled product 证据已进入截图 QA/readiness 并通过，但 bridge 仍不能替代最终 mainImageQaReport。'
            : 'controlled product 证据尚未满足截图 QA/readiness 完整闭环，不能声明主图设计质量通过。',
        boundary: 'bridge 只把 product-disposable-live 结果接入现有 screenshot QA 和 probe readiness；最终质量声明必须由 mainImageQaReport 在真实上下文下决定。',
        ...
        },
        {
            ...
            status: input.screenshotQa.stage === 'blocked'
                ...
                : input.screenshotQa.stage === 'passed'
                    ? 'passed'
                    : 'needs_review',
            summary: `mainImageScreenshotQa=${input.screenshotQa.stage}; resultImages=${input.screenshotQa.resultImageEvidence.resultPaths.length}; pixelProbe=${input.screenshotQa.pixelProbe?.status || 'none'}`
        },
        {
...
function buildBridge(input: {
    ...
    const limitations = [
        'bridge 只做 product-disposable-live 到现有截图 QA/readiness 的证据映射，不执行 Photoshop 写操作。',
        'bridge 摘要会脱敏路径和原始图片 payload；canonical screenshot QA/readiness 对象仍按现有内部证据契约保留路径。',
        'bridge 不生成 mainImageQaReport，也不伪造 agentDraft、candidate 或 vision 上下文。',
        'bridge 通过只代表截图 QA/readiness 证据闭环，不代表最终设计质量声明。'
    ];
    const qualityClaim = buildQualityClaim(stage, input.gate);
    const evidence: EvidenceRef[] = [
        {
            source: 'main-image-controlled-product-qa-bridge',
            summary: `stage=${stage}; gate=${input.gate.stage}; screenshotQa=${input.screenshotQa.stage}; readiness=${input.screenshotProbeReadiness.stage}。`,
            status
        },
        {
            source: 'main-image-screenshot-qa',
            summary: `canonicalStage=${input.screenshotQa.stage}; resultImages=${input.screenshotQa.resultImageEvidence.resultPaths.length}; pixelProbe=${input.screenshotQa.pixelProbe?.status || 'none'}。`,
            status: input.screenshotQa.status
        },
        {
            source: 'main-image-screenshot-probe-readiness',
            summary: `canonicalStage=${input.screenshotProbeReadiness.stage}; fileProbes=${input.screenshotProbeReadiness.resultFileProbes.length}; target=${input.screenshotProbeReadiness.probeTarget.mode}。`,
            status: input.screenshotProbeReadiness.status
        }
    ];
    ...
    return {
        ...
        redaction: {
            rawImagesRedacted: true,
            pathsRedacted: true,
            pathPolicy: 'bridge 摘要只保留 basename 和计数；canonical screenshot QA/readiness 对象保持现有内部证据路径契约。'
        },
        verificationReport
    };
}

export function buildMainImageControlledProductQaEvidence(
    input: MainImageControlledProductQaBridgeInput
): MainImageControlledProductQaEvidence {
...
export function buildMainImageControlledProductQaBridge(
    ...
    return buildMainImageControlledProductQaEvidence(input).bridge;
}
