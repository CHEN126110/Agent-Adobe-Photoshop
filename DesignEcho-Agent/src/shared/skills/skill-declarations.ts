...
export const SKUSkill: SkillDeclaration = {
    ...
    routing: {
        ...
        intentSignals: [
            ...
            'regex:(SKU|sku).{0,16}(做|制作|设计|生成|完成|优化|调整|修复|色卡|颜色卡|模板|组合图|双装|备注图|自选备注|批量|导出)',
        ...
        negativeSignals: ['regex:(你|agent|模型).{0,6}(会|能|可以).{0,6}(做|处理).{0,4}sku', 'regex:(怎么|如何).{0,8}(做|制作|生成).{0,4}sku', 'regex:sku.{0,6}(能力|支持哪些|是什么|什么意思)', 'regex:^(帮我|请|给我)?\\s*(做|做个|做一下|进行)?\\s*(个)?\\s*(SKU|sku).{0,16}(分析|素材盘点|模板检查|可行性评估|规划|说明).{0,16}$', '说明理解', '不执行工具', '不要执行', '只讨论', '只说明', '仅查看', '只查看', '仅检查', '只检查', 'SKU 素材做主图', 'SKU 素材做详情页', 'SKU 素材导出白底图', '文档关闭'],
