...
const SKU_EXECUTION_ACTION_PATTERN = /(?:再做|再生成|补|补一下|补充|做|创建|新建|建立|整理|生成|制作|设计|修复|调整|修改|优化|处理|跑|出图|导出|批量生成|批量出图|开始|执行)/i;
...
export function isSkuNoteOnlyText(input: string): boolean {
    const text = String(input || '');
    if (/(?:不要|别|不能|不应|不再|并非).{0,4}(?:只|仅|单独)(?:做|生成|要)?(?:.{0,20})?(?:自选备注|备注图|备注)/.test(text)) {
        return false;
    }
...
export function inferSkuIntentParamsFromText(input: string): SkuIntentParams {
    ...
    return {
        ...
        generateNotes: sourceOnly ? false : !noteDisabled && noteRequested,
        onlyNotes,
