...
export const RUNTIME_SESSION_IDENTITY_ISSUANCE_REQUEST_VERSION =
    'runtime-session-identity-issuance-request/v0' as const;
export const RUNTIME_SESSION_IDENTITY_ISSUANCE_GRANT_VERSION =
    'runtime-session-identity-issuance-grant/v0' as const;
...
export interface RuntimeArtifactAuthorizationRequest {
    ...
    /** Same-generation plan-neutral identity to bind; mutually exclusive with previousRunId. */
...
export interface RuntimeSessionIdentityIssuanceRequest {
    version: typeof RUNTIME_SESSION_IDENTITY_ISSUANCE_REQUEST_VERSION;
    requestId: string;
}

export interface RuntimeSessionIdentityIssuanceGrant {
    version: typeof RUNTIME_SESSION_IDENTITY_ISSUANCE_GRANT_VERSION;
    projectId: string;
    runtimeIdentity: RuntimeSessionIdentity;
    expiresAt: string;
    boundaries: {
...
export function readRuntimeSessionIdentityIssuanceRequest(
    value: unknown
): RuntimeSessionIdentityIssuanceRequest | undefined {
    if (!isRecord(value)
        || !hasOnlyKeys(value, ['version', 'requestId'])
        || value.version !== RUNTIME_SESSION_IDENTITY_ISSUANCE_REQUEST_VERSION
...
export function readRuntimeSessionIdentityIssuanceGrant(
    value: unknown
): RuntimeSessionIdentityIssuanceGrant | undefined {
    if (!isRecord(value)
        || !hasOnlyKeys(value, [
            'version',
            'projectId',
        ...
        || value.version !== RUNTIME_SESSION_IDENTITY_ISSUANCE_GRANT_VERSION
...
export function readRuntimeArtifactAuthorizationRequest(
    ...
    if (!isRecord(value)
        ...
        || (value.identityRunId !== undefined && !isBoundedText(value.identityRunId, 160))
        || (value.previousRunId !== undefined && value.identityRunId !== undefined)) {
        return undefined;
    }
    return {
        ...
        ...(value.previousRunId !== undefined
            ...
            : {}),
        ...(value.identityRunId !== undefined
            ? { identityRunId: value.identityRunId.trim() }
            : {})
    };
}
