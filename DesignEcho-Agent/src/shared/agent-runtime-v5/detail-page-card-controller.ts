 ...
 *
 * 卡片动作**不再经 ChatPanel 发送管线重发消息**（那会重复插入用户
 ...
 * 3. SELECT_PRODUCT_IMAGES / RUN_PROJECT_VISUAL_ANALYSIS → 能力未就绪，rejected
 */
export function submitVisualObservationCardAction(
    ...
    if (actionId === 'SELECT_PRODUCT_IMAGES') {
        return rejected('SELECT_IMAGES_NOT_READY', '选择代表图片功能正在接入；不会用固定结构草案代替真实素材理解。');
    }
