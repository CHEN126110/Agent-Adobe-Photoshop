...
function projectRuntimeManifestSelection(
    ...
    return {
        ...artifactManifest,
        // 内部 Task Profile 必须保留统一的用户级 Skill 入口；这不会改变 artifact owner。
        workflow_entry_skill_ids: unique(selectedManifests.flatMap(
            (manifest) => manifest.workflow_entry_skill_ids || []
        )),
...
export function buildRuntimeContractBundleForAgentTask(
    ...
    if (evaluationProfile && !isDesignEvaluationProfileApplicableToTask(evaluationProfile, {
        skillId: evaluationOwner.skill_id,
        taskType: evaluationOwner.task_type
    })) return undefined;
    const methodOverlay = selection.artifactManifest
        ? buildRuntimeMethodManifestOverlay(selection.methodManifests)
        : undefined;
    const stagePlan = buildRuntimeStagePlan(manifest, expectedWorkMode);
    ...
    return {
        ...
        manifestBoundary,
        stagePlan,
        runtimeLoopContract: buildReActReflexionLoopContract(
            manifest,
            stagePlan.steps.map((step) => step.stage)
        ),
