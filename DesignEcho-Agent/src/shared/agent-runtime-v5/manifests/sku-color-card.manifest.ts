...
export const SKU_COLOR_CARD_MANIFEST: SkillRuntimeManifest = {
    ...
    optional_inputs: ['output_path', 'canvas_size', 'card_size', 'layout_columns', 'show_index_numbers', 'retouch_mode', 'source_mode', 'shape_reference'],
    input_sources: {
        ...
        retouch_mode: ['structured_input', 'project_context'],
        source_mode: ['structured_input', 'project_asset'],
        shape_reference: ['structured_input', 'project_asset']
    },
    performance_profile: {
    ...
    exit_criteria: [
        ...
        '适用的纯底素材已生成保持真实版型的透明主体，并通过等比缩放统一到同批尺度；场景图明确不进入该链',
        '逐卡统一尺度资产已置入并完成来源身份、真实边界与可编辑性读回；card 模式还需读回商品图剪切关系，flat 模式按无剪切结构验收，不补造剪切关系',
        '逐卡主体尺度、完整裁切和重心已经真实快照复核；必要调整后再次观察',
        '阴影、投影分离与光影修正不属于当前阶段退出判据，不得因缺少这些层判定失败或宣称已完成',
