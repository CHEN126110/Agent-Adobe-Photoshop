...
export const RUNTIME_DESIGN_WORK_MODES = Object.freeze([
    'create_new',
    'redesign',
    'template_fill',
    'edit_existing',
...
export type RuntimeDesignWorkMode = typeof RUNTIME_DESIGN_WORK_MODES[number];
...
export interface SkillRuntimeWorkModeContract {
    ...
    /** 由 Skill 声明的真实生产义务；通用 Runtime 只执行并验证，不从任务文本猜测。 */
    production_obligation?: SkillRuntimeProductionObligation;
    /**
     * E2 对原子交付动作的显式结果绑定。未声明时仍只接受复合 Skill 返回的
    ...
    /**
     * 当前模式要求的请求级执行范围。exact_text_replacement 表示只有 Harness 已从
     * 用户请求和当前文档唯一目标读回签发 ExactProperty Scope 后，才允许进入写入。
     */
    execution_scope_kind?: 'exact_text_replacement';
    /**
     * 当前工作模式首轮真正需要展示的最小 Capability seed。它只控制 schema 可见性，
     * 必须是 capability_ceiling 的子集；未声明时沿用 Manifest available_tools。
     */
    initial_capabilities?: string[];
    /**
     * 当前工作模式允许装载的 Capability 硬上限。它不是首轮种子，也不授予执行权限；
     * Resolver 必须同时用它裁剪 Manifest seed、Workflow bridge 与 on-demand catalog。
     * 未声明时沿用 Manifest 的开放式按需能力语义。
     */
    capability_ceiling?: string[];
    /**
     * 可选的模式级阶段短链；用于把有唯一答案的局部修改从完整创意流程中拆出。
     * 仍必须是 canonical Runtime 阶段的有序子序列，并至少保留 R0 / E1 / R5。
     */
    runtime_stages?: RuntimeStage[];
}

export type RuntimeDeliveryProofKind =
    | 'saved_editable_document'
    | 'exported_image_slices';

export type SkillRuntimeProductionObligation =
    | 'photoshop_mutation_with_readback'
    | 'none';

export interface SkillRuntimeDeliveryOutputBinding {
    capability_refs: string[];
    proof_kind: RuntimeDeliveryProofKind;
}
...
export interface SkillRuntimeManifest {
    ...
    /**
     * 当前可调用的用户级 Skill 入口。多个内部 Task Profile 可以共享同一个入口，
     * 但各自继续拥有独立的输入、阶段、交付物与评价契约。
     */
    workflow_entry_skill_ids?: string[];
    /**
     * 同一个用户级 Skill 可拥有多个交付物 Task Profile。这里声明由该 Skill 入口承接、
     * 但应由结构化 task_type Manifest 取得 artifact-owner 权威的任务类型。
     */
    task_type_variants?: string[];
    required_inputs: string[];
    optional_inputs?: string[];
