...
export type SkillPackageContractIssueCode =
    ...
    | 'initial_capability_empty'
    | 'initial_capability_duplicate'
    | 'initial_capability_outside_ceiling'
    | 'initial_capability_unavailable'
    | 'capability_ceiling_empty'
    | 'capability_ceiling_duplicate'
    | 'capability_ceiling_unavailable'
    | 'tool_boundary_empty'
    ...
    | 'delivery_output_binding_unknown_output'
    | 'delivery_output_binding_invalid_capability'
    ...
    | 'workflow_entry_declaration_missing'
    | 'workflow_entry_declaration_not_business_workflow'
    | 'workflow_entry_declaration_direct_execution_allowed'
    | 'workflow_entry_declaration_not_autonomous_entry'
...
export function validateSkillPackageContracts(input: {
    ...
    const results = input.manifests.map((manifest): SkillPackageContractResult => {
        ...
        const packageResolution = input.resolutions?.get(manifest.skill_id);
        const availableCapabilityIds = packageResolution
            ? new Set([
                ...packageResolution.selectedCapabilityIds,
                ...packageResolution.onDemandCapabilityIds
            ])
            : undefined;
        ...
        for (const [workMode, contract] of Object.entries(manifest.work_mode_contracts || {})) {
            if (contract?.initial_capabilities) {
                const rawInitialCapabilities = contract.initial_capabilities;
                const initialCapabilities = unique(rawInitialCapabilities);
                const initialPath = `work_mode_contracts.${workMode}.initial_capabilities`;
                if (initialCapabilities.length === 0) {
                    add('initial_capability_empty', initialPath, 'at least one capability is required');
                }
                if (initialCapabilities.length !== rawInitialCapabilities.length) {
                    add('initial_capability_duplicate', initialPath, rawInitialCapabilities.join(','));
                }
                const ceilingSet = contract.capability_ceiling
                    ? new Set(contract.capability_ceiling)
                    : undefined;
                for (const capabilityId of initialCapabilities) {
                    if (ceilingSet && !ceilingSet.has(capabilityId)) {
                        add('initial_capability_outside_ceiling', initialPath, capabilityId);
                    }
                    const belongsToDeclaredNamespace = capabilityId.startsWith('skill.')
                        || namespaces.some((namespace) => capabilityId.startsWith(namespace));
                    if (!belongsToDeclaredNamespace) {
                        add('tool_namespace_mismatch', initialPath, capabilityId);
                    }
                    if (availableCapabilityIds && !availableCapabilityIds.has(capabilityId)) {
                        add('initial_capability_unavailable', initialPath, capabilityId);
                    }
                }
            }
            if (contract?.capability_ceiling) {
                const rawCeiling = contract.capability_ceiling;
                const capabilityCeiling = unique(rawCeiling);
                const ceilingPath = `work_mode_contracts.${workMode}.capability_ceiling`;
                if (capabilityCeiling.length === 0) {
                    add('capability_ceiling_empty', ceilingPath, 'at least one capability is required');
                }
                if (capabilityCeiling.length !== rawCeiling.length) {
                    add('capability_ceiling_duplicate', ceilingPath, rawCeiling.join(','));
                }
                const forbiddenOverlap = capabilityCeiling.filter(
                    (capabilityId) => forbiddenTools.includes(capabilityId)
                );
                if (forbiddenOverlap.length > 0) {
                    add('tool_allow_deny_overlap', ceilingPath, forbiddenOverlap.join(','));
                }
                for (const capabilityId of capabilityCeiling) {
                    const belongsToDeclaredNamespace = capabilityId.startsWith('skill.')
                        || namespaces.some((namespace) => capabilityId.startsWith(namespace));
                    if (!belongsToDeclaredNamespace) {
                        add('tool_namespace_mismatch', ceilingPath, capabilityId);
                    }
                    if (availableCapabilityIds && !availableCapabilityIds.has(capabilityId)) {
                        add('capability_ceiling_unavailable', ceilingPath, capabilityId);
                    }
                }
            }
            if (contract?.runtime_stages) {
                const runtimeStages = contract.runtime_stages;
                const missingRequiredStages = MINIMUM_REQUIRED_STAGES.filter(
                    (stage) => !runtimeStages.includes(stage)
                );
                if (missingRequiredStages.length > 0) {
                    add(
                        'runtime_stage_missing',
                        `work_mode_contracts.${workMode}.runtime_stages`,
                        missingRequiredStages.join(',')
                    );
                }
                if (!isCanonicalOrderedSubset(runtimeStages, REQUIRED_STAGE_ORDER)) {
                    add(
                        'runtime_stage_order_invalid',
                        `work_mode_contracts.${workMode}.runtime_stages`,
                        runtimeStages.join('>')
            ...
            const deliveryOutputs = unique(contract?.delivery_outputs);
            for (const [outputRef, binding] of Object.entries(
                contract?.delivery_output_bindings || {}
            )) {
                if (!deliveryOutputs.includes(outputRef)) {
                    add(
                        'delivery_output_binding_unknown_output',
                        `work_mode_contracts.${workMode}.delivery_output_bindings.${outputRef}`,
                        outputRef
                    );
                }
                const capabilityRefs = unique(binding.capability_refs);
                if (capabilityRefs.length === 0
                    || capabilityRefs.some((capabilityRef) => (
                        !availableTools.includes(capabilityRef)
                        || !capabilityRef.startsWith('delivery.')
                    ))) {
                    add(
                        'delivery_output_binding_invalid_capability',
                        `work_mode_contracts.${workMode}.delivery_output_bindings.${outputRef}.capability_refs`,
                        capabilityRefs.join(',') || 'missing'
                    );
                }
            }
        }
        const manifestDeliveryOutputs = unique(manifest.delivery_outputs);
        for (const [outputRef, binding] of Object.entries(
            manifest.delivery_output_bindings || {}
        )) {
            if (!manifestDeliveryOutputs.includes(outputRef)) {
                add(
                    'delivery_output_binding_unknown_output',
                    `delivery_output_bindings.${outputRef}`,
                    outputRef
                );
            }
            const capabilityRefs = unique(binding.capability_refs);
            if (capabilityRefs.length === 0
                ...
                add(
                    'delivery_output_binding_invalid_capability',
                    `delivery_output_bindings.${outputRef}.capability_refs`,
                    capabilityRefs.join(',') || 'missing'
                );
            }
        }
        ...
        const legacyDeclarationIds = unique(manifest.legacy_skill_ids);
        const workflowEntryDeclarationIds = unique(manifest.workflow_entry_skill_ids);
        const declarationIds = unique([...legacyDeclarationIds, ...workflowEntryDeclarationIds]);
        for (const alias of legacyDeclarationIds) {
        ...
        for (const entrySkillId of workflowEntryDeclarationIds) {
            const declaration = declarationById.get(entrySkillId);
            if (!declaration) {
                add('workflow_entry_declaration_missing', 'workflow_entry_skill_ids', entrySkillId);
                continue;
            }
            if (declaration.routeClass !== 'business-workflow') {
                add(
                    'workflow_entry_declaration_not_business_workflow',
                    `declaration:${entrySkillId}.routeClass`,
                    String(declaration.routeClass)
                );
            }
            if (declaration.modelDirectExecution !== 'forbidden') {
                add(
                    'workflow_entry_declaration_direct_execution_allowed',
                    `declaration:${entrySkillId}.modelDirectExecution`,
                    String(declaration.modelDirectExecution)
                );
            }
            if (declaration.controlledRouteEntry !== 'autonomous-react-loop') {
                add(
                    'workflow_entry_declaration_not_autonomous_entry',
                    `declaration:${entrySkillId}.controlledRouteEntry`,
                    String(declaration.controlledRouteEntry)
                );
            }
        }
