...
export const COMPLETED_AESTHETIC_IMPROVEMENT_TRIGGER = 'completed_aesthetic_improvement' as const;

export interface ReflexionIssueConstraint {
    issueId: string;
    description: string;
    expectedFix: string;
...
export interface ReflexionHandoff {
    ...
    /**
     * R5 负责质量返工；E2 负责交付补齐；Runtime 只在有界运行预算到点但当前
     * 生产阶段尚未闭合时签发同阶段承接。三者都只提供下一 generation 的约束，
     * 不直接执行 Tool，也不把未完成状态改判为成功。
     */
    sourceOwner: 'R5' | 'E2' | 'Runtime';
    targetStage: RuntimeComponentId;
    reenterLoop: 'react';
    /**
     * Harness-owned generation provenance. It only identifies why this handoff was
     * issued so the outer bounded loop can apply the correct retry ceiling; it does
     * not grant Tool permission and does not change completion or quality verdicts.
     */
    trigger?: typeof COMPLETED_AESTHETIC_IMPROVEMENT_TRIGGER;
    /**
     * Preserve each diagnosed problem together with its targeted repair across generations.
     * The legacy parallel arrays below remain for backward compatibility only; consumers
...
/**
 * Read only the Harness-issued provenance tuple. This marker is not an execution
 * authorization and cannot turn an ordinary quality failure into a completed task.
 */
export function isCompletedAestheticImprovementReflexionHandoff(
    handoff?: {
        status?: unknown;
        sourceOwner?: unknown;
        targetStage?: unknown;
...
export function buildReActReflexionLoopContract(
    manifest: SkillRuntimeManifest,
    runtimeStages: readonly string[] = manifest.runtime_stages
...
export function buildReflexionHandoffFromReviewReport(reviewReport: any): ReflexionHandoff {
    ...
    const issueConstraints = issues
        .map((issue: any, index: number): ReflexionIssueConstraint | undefined => {
            const description = compactText(issue?.description);
            const expectedFix = compactText(issue?.expectedFix);
            if (!description || !expectedFix) return undefined;
            return {
