...
export interface RuntimeActionPlanExecutionObservationInput {
    ...
    /** Provider call identity，仅用于把 E1 计划归属绑定到同一条交付结果；不含 Tool 名或参数。 */
    executionRef?: string;
    /** Photoshop 写动作只有取得 Host mutation proof 时才为 true。 */
...
export interface RuntimeActionPlanExecutionJournal {
    ...
    boundaries: {
        ...
        containsOpaqueExecutionRefs: true;
        containsOpaqueTargetIdentity: true;
        bindsReadbackToMutation: true;
        requiresObservedStateChangeForMutation: true;
...
const EXECUTION_REF_PATTERN = /^[A-Za-z0-9][A-Za-z0-9_.:-]{0,159}$/;
...
export function appendRuntimeActionPlanExecutionObservation(input: {
    ...
    const executionRef = String(input.observation.executionRef || '').trim();
    return {
        ...input.journal,
        observations: [
            ...input.journal.observations,
            {
                ...
                ...(EXECUTION_REF_PATTERN.test(executionRef) ? { executionRef } : {}),
                ...(input.observation.stateChangeObserved === true
