...
const COMMON_ART_DIRECTION: DesignMethodKnowledgeDefinition = {
    ...
    applicableSkillIds: [],
    applicableStages: ['R3'],
    sourceRevision: 'design-method-art-direction-v3',
    ...
    method: [
        ...
        '参考只用于比较关系、机制和取舍，不照抄成品；一旦相关差异已经足以支撑当前方向即可停止，Eagle 离线或没有合适结果不阻断普通设计。',
        '结构正确、干净、可编辑只是制作底线，不等于视觉方向已经成熟。面向正式使用的成品若只有“安全素材 + 常规标题 + 留白”而缺少与目标相关的识别度、张力、情绪或说服机制，应把它视为尚未解决的设计问题。',
        '优先质量时不要因为首稿可执行就停止：由 Agent 自行选择相关参考、替代素材、缩略图比较或独立视觉批评中的一种或多种证据来挑战当前方案，再决定保留、局部修订或换方向；这是完成标准，不是固定工具顺序。',
