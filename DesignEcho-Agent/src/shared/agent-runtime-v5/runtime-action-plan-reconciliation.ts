...
import {
    ...
    type RuntimeActionPlanResultKind,
    type RuntimeActionPlanStep,
    type RuntimeActionPlanStepKind
...
export interface RuntimeActionPlanReconciliation {
    ...
    metrics: {
        ...
        operationKindMismatchObservationCount: number;
...
export interface RuntimeActionPlanExecutionClosure {
    version: 'runtime-action-plan-execution-closure/v0';
    complete: boolean;
    incompleteDeliverableRefs: string[];
    pendingStepIds: string[];
    boundaries: {
        e1ExecutionOnly: true;
        defersQualityReportToR5: true;
        requiresDeclaredDeliveryInE1: true;
        leavesUndeclaredDeliveryRequirementsToE2: true;
        observationOnly: true;
        executesTools: false;
        schedulesSteps: false;
        grantsPermission: false;
        countsAsQualityPass: false;
        countsAsTaskCompletion: false;
    };
}
...
function isRuntimeActionPlanStepClosedForE1(input: {
    step: RuntimeActionPlanStep;
    state: RuntimeActionPlanStepReconciliation | undefined;
    reconciliation: RuntimeActionPlanReconciliation;
}): boolean {
    const { step, state, reconciliation } = input;
    if (!state) return false;
    if (state.status === 'blocked_by_dependency'
        || state.status === 'ready'
        || state.status === 'failed') {
        return false;
    }
    const missingE1Outcomes = state.missingExpectedOutcomes.filter((outcome) => (
        outcome !== 'quality_report'
    ));
    if (missingE1Outcomes.length > 0) return false;
    if (step.kind === 'mutate') {
        return reconciliation.attributions.some((attribution) => (
            attribution.outcome === 'attributed'
            && attribution.stepId === step.stepId
            && attribution.executionOutcome === 'succeeded'
            && attribution.observedOutcomes.includes('document_change')
        ));
    }
    if (step.kind === 'verify' && step.expectedOutcomes.includes('readback')) {
        return reconciliation.verificationBindings.some((binding) => (
            binding.readbackStepId === step.stepId
        ));
    }
    return state.status === 'completed'
        || state.declarationOutcomeUsed
        || state.attempts > 0;
}

/**
 * 把现有 R4 declaration + execution reconciliation 投影成 E1 执行闭合事实。
 * 它不解释自然语言 completionCriteria，也不拥有调度、质量或最终交付权。
 * quality_report 由 R5 产生；R4 已声明的 deliver 节点必须在 E1 取得 delivery_record。
 * Manifest 中未写入 R4 的额外交付要求仍由 E2 结算。
 */
export function projectRuntimeActionPlanExecutionClosure(input: {
    declaration: RuntimeActionPlanDeclaration;
    reconciliation: RuntimeActionPlanReconciliation;
}): RuntimeActionPlanExecutionClosure {
    const declarationFingerprint = buildRuntimeActionPlanDeclarationFingerprint(input.declaration);
    const fingerprintMatches = declarationFingerprint === input.reconciliation.declarationFingerprint;
    const declarationStepById = new Map(
        input.declaration.payload.steps.map((step) => [step.stepId, step])
    );
    const reconciliationStepById = new Map(
        input.reconciliation.steps.map((step) => [step.stepId, step])
    );
    const closedStepIds = new Set<string>();
    if (fingerprintMatches && input.declaration.readiness === 'ready') {
        input.declaration.payload.steps.forEach((step) => {
            if (isRuntimeActionPlanStepClosedForE1({
                step,
                state: reconciliationStepById.get(step.stepId),
                reconciliation: input.reconciliation
            })) {
                closedStepIds.add(step.stepId);
            }
        });
    }
    const pendingStepIds = input.declaration.payload.steps
        .filter((step) => !closedStepIds.has(step.stepId))
        .map((step) => step.stepId);
    const incompleteDeliverableRefs = input.declaration.payload.deliverableCoverage
        .filter((coverage) => {
            return coverage.stepIds.some((stepId) => !closedStepIds.has(stepId));
        })
        .map((coverage) => coverage.deliverableRef);
    const reconciliationSafeForClosure = (
        input.reconciliation.status === 'completed'
        || input.reconciliation.status === 'in_progress'
    )
        && input.reconciliation.issues.length === 0
        && input.reconciliation.droppedObservationCount === 0
        && input.reconciliation.metrics.failedStepCount === 0
        && input.reconciliation.metrics.ambiguousObservationCount === 0
        && input.reconciliation.metrics.dependencyBlockedObservationCount === 0
        && input.reconciliation.metrics.operationKindMismatchObservationCount === 0
        && input.reconciliation.metrics.unmatchedObservationCount === 0
        && input.reconciliation.metrics.repeatAfterCompletionCount === 0
        && input.reconciliation.metrics.unboundStateChangeCount === 0
        && input.reconciliation.metrics.unboundReadbackCount === 0;
    return {
        version: 'runtime-action-plan-execution-closure/v0',
        complete: fingerprintMatches
            && input.declaration.readiness === 'ready'
            && reconciliationSafeForClosure
            && pendingStepIds.length === 0
            && incompleteDeliverableRefs.length === 0,
        incompleteDeliverableRefs,
        pendingStepIds,
        boundaries: {
            e1ExecutionOnly: true,
            defersQualityReportToR5: true,
...
function outcomesForObservation(
    ...
    switch (observation.operationKind) {
        ...
        case 'photoshop_write':
            return observation.stateChangeObserved === true && observation.target
                ? ['document_change']
                : [];
        case 'save_export':
...
function buildMetrics(input: {
    ...
    return {
        ...
        operationKindMismatchObservationCount: count('operation_kind_mismatch'),
...
function buildStatus(input: {
    ...
    if (input.metrics.ambiguousObservationCount > 0
        ...
        || input.metrics.operationKindMismatchObservationCount > 0
...
export function reconcileRuntimeActionPlanExecution(input: {
    ...
    if (input.declaration.readiness === 'ready') {
        for (const observation of input.journal.observations) {
            ...
            const capabilityMatchingSteps = steps.filter((step) => capabilityMatches(step, observation));
            const matchingSteps = capabilityMatchingSteps.filter((step) => (
                isRuntimeActionPlanStepOperationCompatible(step.kind, observation.operationKind)
            ));
            ...
            if (eligibleSteps.length === 0) {
                ...
                if (matchingSteps.length === 0 && capabilityMatchingSteps.length > 0) {
                    outcome = 'operation_kind_mismatch';
                    candidateStepIds = capabilityMatchingSteps.map((step) => step.stepId);
                    appendIssue(issues, `operation_kind_mismatch:${observation.sequence}`);
                } else if (unresolvedMatchingSteps.length > 0) {
...
export function buildRuntimeActionPlanReconciliationDigest(
    ...
    return {
        ...
        operationKindMismatchObservationCount: reconciliation.metrics.operationKindMismatchObservationCount,
