...
export interface RuntimeTaskSnapshotTaskRun {
    taskRunId: string;
...
export interface RuntimeTaskSnapshot {
    ...
    taskRun: RuntimeTaskSnapshotTaskRun;
...
function buildTaskRunProjection(session: RuntimeSession): RuntimeTaskSnapshotTaskRun {
    const taskRun = session.taskRun;
    return {
        taskRunId: taskRun.taskRunId,
        status: taskRun.status,
        planRevision: taskRun.planRevision,
        cursor: taskRun.cursor,
        ...(taskRun.currentNodeId ? { currentNodeId: taskRun.currentNodeId } : {}),
        nodes: taskRun.nodes.slice(0, MAX_LIST).map((node) => ({
            nodeId: cleanText(node.nodeId, 120),
            kind: node.kind,
            status: node.status,
            operationResultCount: node.operationResultIds.length
        })),
        ...(taskRun.pendingInteraction ? {
            pendingInteraction: {
                interactionId: taskRun.pendingInteraction.interactionId,
                planRevision: taskRun.pendingInteraction.planRevision,
                ...(taskRun.pendingInteraction.nodeId
                    ? { nodeId: taskRun.pendingInteraction.nodeId }
                    : {}),
                ...(taskRun.pendingInteraction.expectedRevision ? {
                    expectedRevision: {
                        documentId: taskRun.pendingInteraction.expectedRevision.documentId,
                        historyStateId: taskRun.pendingInteraction.expectedRevision.historyStateId
                    }
                } : {})
            }
        } : {}),
        ...(taskRun.documentBinding ? {
            document: {
                documentId: taskRun.documentBinding.documentId,
                expectedHistoryStateId: taskRun.documentBinding.expectedRevision.historyStateId,
                status: taskRun.documentBinding.status,
                ...(taskRun.documentBinding.writer
                    ? { writerTaskRunId: taskRun.documentBinding.writer.taskRunId }
                    : {}),
                ...(taskRun.documentBinding.conflict
                    ? { conflictKind: taskRun.documentBinding.conflict.kind }
                    : {})
            }
        } : {}),
...
export function buildRuntimeTaskSnapshot(
    ...
    const waitingForRuntime = session.stageState.status === 'awaiting_confirmation'
        || session.taskRun.status === 'waiting_user'
    ...
    const snapshot: Omit<RuntimeTaskSnapshot, 'projectionFingerprint'> = {
        ...
        taskRun: buildTaskRunProjection(session),
...
const TASK_RUN_NODE_STATUS_VALUES: readonly RuntimeTaskRunNodeStatus[] = [
    ...
    'verification_failed', 'failed', 'unknown', 'completed'
];
...
function hasPositiveInteger(value: unknown): value is number {
    return hasNonNegativeInteger(value) && value > 0;
}

function hasTaskRunRevision(value: unknown): boolean {
    if (!isRecord(value)) return false;
    return hasOnlyKeys(value, ['documentId', 'historyStateId'])
        && hasPositiveInteger(value.documentId)
        && hasPositiveInteger(value.historyStateId);
}

function hasTaskRunNode(value: unknown): boolean {
    if (!isRecord(value)) return false;
    return hasOnlyKeys(value, ['nodeId', 'kind', 'status', 'operationResultCount'])
        && hasBoundedString(value.nodeId, 120)
        && hasEnumValue(value.kind, ACTION_STEP_KIND_VALUES)
        && hasEnumValue(value.status, TASK_RUN_NODE_STATUS_VALUES)
        && hasNonNegativeInteger(value.operationResultCount);
}

function hasTaskRunPendingInteraction(value: unknown): boolean {
    if (!isRecord(value)) return false;
    return hasOnlyKeys(value, [
        'interactionId', 'planRevision', 'nodeId', 'expectedRevision'
    ])
        && hasBoundedString(value.interactionId, 160)
        && hasNonNegativeInteger(value.planRevision)
        && (!hasOwn(value, 'nodeId') || hasBoundedString(value.nodeId, 120))
        && (!hasOwn(value, 'expectedRevision') || hasTaskRunRevision(value.expectedRevision));
}

function hasTaskRunDocument(value: unknown): boolean {
    if (!isRecord(value)) return false;
    return hasOnlyKeys(value, [
        'documentId', 'expectedHistoryStateId', 'status', 'writerTaskRunId', 'conflictKind'
    ])
        && hasPositiveInteger(value.documentId)
        && hasPositiveInteger(value.expectedHistoryStateId)
        && hasEnumValue(value.status, ['observed', 'owned', 'needs_reobserve', 'conflict'] as const)
        && (!hasOwn(value, 'writerTaskRunId') || hasBoundedString(value.writerTaskRunId, 160))
        && (!hasOwn(value, 'conflictKind') || hasBoundedString(value.conflictKind, 120));
}

function hasTaskRunOperationResult(value: unknown): boolean {
    if (!isRecord(value)) return false;
    return hasOnlyKeys(value, [
        'operationId', 'toolName', 'planRevision', 'nodeId', 'status',
        'applicationStatus', 'transactionState'
    ])
        && hasBoundedString(value.operationId, 160)
        && hasBoundedString(value.toolName, 120)
        && hasNonNegativeInteger(value.planRevision)
        && (!hasOwn(value, 'nodeId') || hasBoundedString(value.nodeId, 120))
        && hasEnumValue(value.status, [
            'applied', 'verified', 'verification_failed', 'failed', 'unknown'
        ] as const)
        && hasEnumValue(value.applicationStatus, ['not_applied', 'applied', 'unknown'] as const)
        && hasEnumValue(value.transactionState, [
            'not_started', 'suspended', 'rolled_back', 'committed', 'commit_unknown',
            'rollback_unknown', 'transport_unknown', 'transport_reconciled', 'readback_reconciled'
        ] as const);
}
...
function hasSnapshotTaskRun(value: unknown): boolean {
    if (!isRecord(value)
        ...
        || !hasNonNegativeInteger(value.planRevision)
        || !hasNonNegativeInteger(value.cursor)
        || (hasOwn(value, 'currentNodeId') && !hasBoundedString(value.currentNodeId, 120))
        || !Array.isArray(value.nodes)
        || value.nodes.length > MAX_LIST
        || !value.nodes.every(hasTaskRunNode)
    ...
    if (value.cursor > value.nodes.length) return false;
    if (value.status === 'waiting_user') return hasOwn(value, 'pendingInteraction');
    return true;
}
...
export function readRuntimeTaskSnapshot(value: unknown): ReadableRuntimeTaskSnapshot | undefined {
    ...
    if (!isRecord(value)
        || !hasOnlyKeys(value, [
            'version', 'projectionFingerprint', 'identity', 'goal', 'runtime', 'taskRun', 'actionPlan', 'execution', 'recovery',
        ...
        || !hasSnapshotTaskRun(value.taskRun)
    ...
    const taskRun = value.taskRun as Record<string, unknown>;
    const interaction = value.interaction as Record<string, unknown>;
    const hasReconciliation = hasOwn(sources, 'runtimeActionPlanReconciliation');
    if (taskRun.taskRunId !== identity.sessionId
        || (taskRun.status === 'waiting_user') !== (interaction.waitingForUser === true)
        || hasOwn(sources, 'runtimeActionPlan') !== Boolean(actionPlan)
