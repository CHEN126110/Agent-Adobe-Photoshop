...
export type DetailScreenCopyStrategy = 'headline' | 'benefit' | 'evidence' | 'parameter' | 'emotional';
...
export interface DetailScreenStructuralEvidence {
...
export interface DetailScreenPlan {
    ...
    structuralEvidence: DetailScreenStructuralEvidence;
    confidence: number;
    risks: string[];
...
function isCopyStrategy(value: unknown): value is DetailScreenCopyStrategy {
    return ['headline', 'benefit', 'evidence', 'parameter', 'emotional'].includes(String(value || ''));
}
...
function inferRoleFromKeywords(screen: DetailScreenPlanInput): DetailScreenRole | null {
    ...
    if (includesAny(haystack, ['材质', '面料', '纤维', '桑蚕丝', '棉'])) return 'material-proof';
    if (includesAny(haystack, ['细节', '特写', '工艺', '做工', '纹理'])) return 'detail-proof';
...
function inferRoleFromStructure(
    ...
    if (imageCount >= 1 && copyCount >= 1 && imageAreaRatio < 0.18) return 'material-proof';
...
function getPendingMainMessage(): string {
    return '待模型 Agent 结合产品证据、用户意图和模板结构决定这一屏的主信息。';
}

function getPendingSupportingPoints(): string[] {
    return [
        '结构识别只提供候选职责，不直接决定卖点或文案角度',
        '正式执行前需要模型 Agent 明确该屏讲什么、用什么图片证明'
    ];
}
...
export function inferDetailScreenPlans(
    ...
    return (screens || []).map((screen) => {
        ...
        if (!keywordRole && structuralRole === 'unknown') {
            risks.push(visualRoleHint ? '主要依赖视觉分块推断屏职责候选' : '屏职责结构证据不足');
        }
        if (keywordRole) {
