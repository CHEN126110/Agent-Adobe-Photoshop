...
export interface MainImageLiveExecutorRequestInput {
    ...
    productionExecutorDryRunEvidence?: MainImageProductionExecutorDryRunEvidence | null;
    requestLabel?: string;
}
...
const EMPTY_ACCEPTANCE_PLAN: MainImageLiveExecutorAcceptancePlan = {
    ...
    requiresQaReport: true,
    requiresManualReviewBeforeQualityClaim: true,
    boundary: 'No live executor operation can run until readiness and dry-run evidence are both ready.'
};
...
function inferStatus(input: MainImageLiveExecutorRequestInput): MainImageLiveExecutorRequestStatus {
    ...
    if (!input.productionExecutorDryRunEvidence) return 'blocked_missing_dry_run';
    if (input.productionExecutorDryRunEvidence.status !== 'completed_dry_run') {
...
function collectBlockers(
    ...
    const blockers = [
        ...
        ...(input.productionExecutorDryRunEvidence?.blockers || [])
    ];
...
function collectWarnings(input: MainImageLiveExecutorRequestInput): string[] {
    return cleanStrings([
        ...
        ...(input.productionExecutorDryRunEvidence?.warnings || [])
    ]);
}
...
function toOperationRequest(
    ...
    return {
        ...
        sourceEvidenceIds: cleanStrings(operation.sourceEvidenceIds),
...
function buildOperationRequests(
    ...
    return (input.productionExecutorDryRunEvidence?.operationResults || []).map(toOperationRequest);
}

function buildAcceptancePlan(
    dryRun: MainImageProductionExecutorDryRunEvidence | null | undefined,
    operationRequests: MainImageLiveExecutorOperationRequest[]
    ...
    return {
        ...
        requiresQaReport: true,
        requiresManualReviewBeforeQualityClaim: true,
        boundary: 'After the future live executor runs, actual bounds, document state, export evidence, screenshot/pixel QA and manual review must be read back before any quality claim.'
    };
}
...
export function buildMainImageLiveExecutorRequestPackage(
    ...
    const acceptancePlan = buildAcceptancePlan(input.productionExecutorDryRunEvidence, operationRequests);
    ...
    const limitations = [
        ...
        'operationRequests 来自 dry-run evidence，actualResult 固定为 null，不能伪造 layerId、bounds、截图或导出结果。',
        'canDispatchLiveExecutor 只表示请求包可交给后续独立 executor，不代表已经执行。',
        '执行后必须读回 Photoshop 状态、截图 / pixel probe 和人工验收，才能进入质量声明。'
    ];
    const evidence: EvidenceRef[] = [{
        source: 'main-image-live-executor-request',
        summary: `status=${status}; operations=${operationRequests.length}; canDispatch=${status === 'ready_for_executor_dispatch'}`,
