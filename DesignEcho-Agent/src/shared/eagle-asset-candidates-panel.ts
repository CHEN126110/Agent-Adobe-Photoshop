...
export function buildEagleAssetCandidatesPanel(
    ...
    const limitations = uniqueStrings([
        ...
        'Eagle 素材候选入口只展示已经进入 DesignEcho 的只读候选证据。',
...
function candidateFromVisualCase(item: EagleVisualCaseIndexItem): EagleAssetCandidateCard {
    ...
    return {
        ...
        allowedUseLabels,
        evidencePreview: normalizeTextList(item.evidence).slice(0, 4),
...
function candidateFromKnowledgeResult(item: DesignKnowledgeResult): EagleAssetCandidateCard {
    ...
    const dimensions = parseDimensionsFromEvidence(item.evidence);
    const sourceUrl = sanitizeSourceUrl(item.sourceUrl);
    return {
        candidateId: sanitizeText(item.id) || `eagle:${title}`,
        title,
        sourceLabel: extractEvidenceValue(item.evidence, 'Eagle item id') || 'Eagle item',
        ...(sourceUrl ? { sourceUrl } : {}),
        ...
        folderPreview: splitCsvEvidence(extractEvidenceValue(item.evidence, 'Folders')).slice(0, 4),
...
function derivePanelStatus(
    ...
    if (!input.readonlyKnowledge && !input.visualCaseIndex) return 'needs_evidence';
...
function buildSummary(status: EagleAssetCandidatesPanelStatus, candidateCount: number): string {
    ...
    return '等待 Eagle 只读知识或视觉案例索引证据；当前面板不会主动搜索 Eagle。';
}
...
function parseDimensionsFromEvidence(evidence: string[] | undefined): { width: number; height: number } | undefined {
    const value = extractEvidenceValue(evidence, 'Image dimensions');
...
function extractEvidenceValue(evidence: string[] | undefined, label: string): string {
    const prefix = `${label}:`;
    const line = normalizeTextList(evidence).find((item) => item.startsWith(prefix));
    if (!line) return '';
