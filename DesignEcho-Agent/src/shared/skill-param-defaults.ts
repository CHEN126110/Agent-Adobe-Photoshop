...
import {
    inferSkuConfigurationActionFromText,
...
function buildIntentBoundParams(input: SkillParamDefaultsInput): Record<string, any> {
    ...
    if (input.skillId === 'sku-batch') {
        ...
        if (paramNames.has('stage') && (!hasOwnParam(params, 'stage') || inferred.stage !== 'full')) {
            bound.stage = inferred.stage || 'full';
        }

        if (paramNames.has('configAction') && !hasOwnParam(params, 'configAction')) {
            const configAction = inferSkuConfigurationActionFromText(userInput);
            if (configAction) bound.configAction = configAction;
        }
