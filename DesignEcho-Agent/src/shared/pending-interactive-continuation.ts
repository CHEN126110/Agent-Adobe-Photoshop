...
export interface PendingInteractiveContinuation {
    ...
    /** 由活动 RuntimeSession 在暂停边界签发；普通 Skill continuation 可暂时没有。 */
...
/**
 * 交互卡真正冻结的是“进入等待前最后一次 Host 观察”，不是 TaskRun 开工时的旧文档。
 * Skill 可能在生成卡片前切换到另一份模板或源文档；只有暂停边界的 document/revision
 * 同时自洽，才可以作为恢复时的写前版本守卫。
 */
export function resolvePendingInteractiveContinuationPauseRevision(
    continuation: PendingInteractiveContinuation | undefined
): PhotoshopHistoryStateRef | undefined {
    const observation = continuation?.scopeObservation;
    if (!continuation
        || observation?.version !== 'pending-interactive-continuation-scope-observation/v0'
        || observation.source !== 'pause_boundary_get_document_info'
        || observation.photoshopDocumentState !== 'present') {
        return undefined;
    }
    const scopeDocumentId = Number(continuation.scope.photoshopDocumentId || 0);
    const observedDocumentId = Number(observation.photoshopDocumentId || 0);
    const revision = readPhotoshopHistoryStateRef({
        historyStateRef: observation.photoshopHistoryStateRef
    });
    if (!revision
        || !Number.isSafeInteger(scopeDocumentId)
        || scopeDocumentId < 1
        || observedDocumentId !== scopeDocumentId
        || revision.documentId !== scopeDocumentId) {
        return undefined;
    }
    return { ...revision };
}
...
export function buildPendingInteractiveContinuation(input: {
    ...
    return {
        ...
        scope: {
            ...
            ...(cleanIdentity(input.conversationBranchId)
                ? { conversationBranchId: cleanIdentity(input.conversationBranchId) }
                : {}),
...
export function attachRuntimeTaskRunBindingToPendingContinuation(input: {
    continuation: PendingInteractiveContinuation;
    binding: RuntimeTaskRunInteractionBinding;
}): PendingInteractiveContinuation {
    if (!validateRuntimeTaskRunInteractionBinding(input.binding)) {
        throw new Error('runtime_task_run_interaction_binding_invalid');
    }
    if (input.binding.interactionId !== input.continuation.id) {
...
function resolveInteractiveContinuationBinding(input: {
    ...
    const expectedConversationBranchId = cleanIdentity(continuation.scope.conversationBranchId);
    const actualConversationBranchId = cleanIdentity(input.conversationBranchId);
    if (expectedConversationBranchId && expectedConversationBranchId !== actualConversationBranchId) {
        return {
            status: 'rejected',
            code: 'interactive_continuation_conversation_branch_mismatch',
            message: '这张确认卡来自已被编辑撤回的消息分支，不能在当前分支执行。请按新消息重新发起任务。'
        };
    }
    ...
    const taskRunBinding = continuation.taskRunBinding
        ? {
            ...continuation.taskRunBinding,
            ...(pauseRevision ? { expectedRevision: pauseRevision } : {})
        }
        : undefined;
    return {
        status: 'accepted',
...
function resolveInteractiveContinuation(input: {
    ...
    if (!runtimeOwnsPhotoshopState && expectedRevision) {
        ...
        if (!actualRevision
            ...
            || actualRevision.historyStateId !== expectedRevision.historyStateId) {
            return {
                status: 'rejected',
                code: 'interactive_continuation_photoshop_revision_mismatch',
                message: 'Photoshop 文档版本在等待期间发生了变化；本轮不会重放旧写入。请重新观察当前文档后再决定。'
            };
        }
    }
    return resolution;
