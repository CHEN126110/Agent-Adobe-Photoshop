...
function resolveInteractiveContinuation(input: {
    ...
    const taskRunBinding = resolution.continuation.taskRunBinding;
    ...
    if (taskRunBinding?.expectedRevision) {
        const actualRevision = input.photoshopHistoryStateRef;
        if (!actualRevision
            || actualRevision.documentId !== taskRunBinding.expectedRevision.documentId
            || actualRevision.historyStateId !== taskRunBinding.expectedRevision.historyStateId) {
            return {
                status: 'rejected',
