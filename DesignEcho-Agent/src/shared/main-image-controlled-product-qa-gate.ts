...
import type { MainImageScreenshotProbeEvidence } from './main-image-screenshot-qa';
...
function sanitizePixelProbe(
    value: MainImageScreenshotProbeEvidence | null | undefined
): MainImageScreenshotProbeEvidence | undefined {
...
function sanitizeManualReview(
    value: MainImageManualReviewEvidence | null | undefined
): MainImageManualReviewEvidence | undefined {
    if (!value) return undefined;
...
function buildQualityClaim(input: {
    ...
    if (input.stage === 'needs_runner') blockers.push('缺少 live runner 执行证据。');
    if (input.resultImageSummary.resultImageCount === 0) blockers.push('缺少可复核的结果图导出证据。');
    if (input.resultFileProbeSummary.fileProbeCount === 0) blockers.push('缺少结果文件探针证据。');
    ...
    if (!input.pixelProbe || input.pixelProbe.status !== 'ok') blockers.push('缺少 pixel probe=ok 的截图级证据。');
    ...
    return {
        ...
        reason: input.stage === 'passed'
            ? '受控商品执行证据已具备结果图、pixel probe 和人工复核，但仍需由最终主图 QA 报告承接质量声明。'
            : '当前受控商品执行证据不足以声明主图设计质量通过。',
        requiredEvidence: [
        ...
        boundary: '该 gate 只汇总受控 product-disposable-live 执行后的证据缺口；最终质量声明必须由截图 QA、pixel probe、人工复核和 mainImageQaReport 共同决定。'
    };
}
    ...
    return [
        ...
        },
        {
            ...
            summary: hasResultImage
                ? `发现 ${input.resultImageSummary.resultImageCount} 个结果图文件名证据。`
                : '尚未发现 exportGroup/readback/final snapshot 中的结果图路径证据。'
        },
        {
...
function buildNextActions(stage: MainImageControlledProductQaGateStage): string[] {
    ...
    if (stage === 'needs_manual_review') {
        return ['基于结果图和 pixel probe 证据完成人工复核，记录 approved/needs_review/rejected。'];
    }
    return ['把该 gate 交给最终 mainImageQaReport 合并，避免绕过最终质量边界。'];
}
...
export function buildMainImageControlledProductQaGate(
    ...
    if (runner?.status === 'completed_requires_review' && !resultImageSummary.finalSnapshotCaptured) {
        warnings.push('runner 已完成但缺少 finalAcceptanceSnapshot 成功证据。');
    }
    ...
    const evidence: EvidenceRef[] = [{
        source: 'main-image-controlled-product-qa-gate',
        summary: `stage=${stage}; runner=${cleanString(runner?.status) || 'missing'}; resultImages=${resultImageSummary.resultImageCount}; pixelProbe=${cleanString(pixelProbe?.status) || 'none'}; manual=${cleanString(manualReview?.decision) || 'none'}`,
        status
    }];
    ...
    const limitations = [
        '该 gate 只汇总 product-disposable-live 受控执行后的证据缺口，不直接评价审美质量。',
    ...
    const summary = stage === 'passed'
        ? '受控商品执行 QA gate 已具备结果图、pixel probe 和人工复核证据，但仍需最终 QA 报告承接质量声明。'
