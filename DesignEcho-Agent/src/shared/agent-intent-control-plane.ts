...
const EXPLICIT_ALL_TOOL_FORBID_PATTERNS = [
    /(?:^|[，,。！？!?；;\n]|但是|但|并且|而且)\s*(?:我\s*)?(?:本轮|这次|当前)?\s*(?:(?:不希望|不想让)\s*(?:你|模型|agent)?|不要|别|先别|不需要|无需|禁止|不使用|不用|不调用|不执行)\s*(?:调用|使用|执行|运行|跑)?\s*(?:任何|全部|所有)?\s*(?:工具|tools?)\s*(?=$|[，,。！？!?；;\n]|然后|接着|但|但是|并且|而且)/i
];
const EXPLICIT_SKILL_BRIDGE_FORBID_PATTERNS = [
    /(?:^|[，,。！？!?；;\n]|但是|但|并且|而且)\s*(?:我\s*)?(?:本轮|这次|当前)?\s*(?:(?:不希望|不想让)\s*(?:你|模型|agent)?|不要|别|禁止|不使用|不用|不调用|不执行|无需使用|无需调用)\s*(?:再)?\s*(?:使用|调用|执行|运行)?\s*(?:任何|全部|所有|业务|工作流|workflow)?\s*(?:skill|技能)\s*(?=$|[，,。！？!?；;\n]|然后|接着|但|但是|并且|而且)/i
];
...
const READ_ONLY_NEGATIVE_PATTERNS = [
    /(保存|另存|导出|关闭|新建|创建|制作|生成|加入|添加|放入|拖入|导入|删除|重命名|改名|置顶|置底|上移|下移|移到|移动到|挪到|排序|调整|整理|归组|编组|修改|替换|换成|改成)|(?:组合.{0,8}(?:图层|编组)|(?:图层|编组).{0,8}组合)/i,
...
const SKILL_ROUTING_AUTHORIZATION_ACTION_PATTERN = /(帮我|请|麻烦你|开始|执行|做|制作|生成|导出|保存|创建|新建|处理|分析|查看|检查|识别|理解|设计|优化|调整|整理|归组|编组|修改|重写|撰写|重新写|改成|改为|改一下|改下|换成|换一张|换一下|换个|替换|更换|更新|搜索|搜一下|查找|检索|找一些|找一下|帮我找|帮我搜|search|find)/i;
...
const BASIC_PHOTOSHOP_WRITE_ACTION_PATTERN =
    /(真实执行|实际执行|执行|创建|新建|建立|添加|新增|放入|写入|生成|修改|调整|优化|编辑|整理|归组|编组|重写|撰写|重新写|改成|改为|改一下|改下|改|替换|换成|更换|更新|应用|采用|移动|重命名|改名|排序|对齐|缩放|删除|复制|清除|移除|去除|create|draw|add|insert|write|generate|edit|modify|adjust|optimize|rewrite|replace|apply|update|move|rename|align|scale|delete|copy|clear|remove|export)/i;
...
function isCapabilityRestrictionMetaDiscussion(input: string): boolean {
    return includesAny(input, [CAPABILITY_RESTRICTION_META_DISCUSSION_PATTERN])
        && includesAny(input, [/[?？]/, /(?:讨论|假设|假如|如果|比如|例如|这句话)/i]);
}
...
export function extractExplicitUserCapabilityConstraint(
    ...
    if (hasExplicitAllToolForbidDirective(constraintText)) {
    ...
    const deniedProviderToolNames = extractExplicitDeniedProviderToolNames(constraintText);
...
function isStandaloneCopyDeliverableRequest(
    input: string,
    options?: { hasOpenDocument?: boolean }
): boolean {
    const text = normalizeText(input);
    if (!text) return false;
    if (!COPY_DELIVERABLE_OBJECT_PATTERN.test(text)) return false;
    if (COPY_FORMATTING_MUTATION_PATTERN.test(text)) return false;
    // 已经打开文档时，「把标题改成春季新品」指的就是画面上那行标题，不是要一段独立文案。
    // 系统握着真实的文档状态却不用它，改去猜用户说的「标题」是哪个，结果把明确的画面修改
    // 判成纯文字交付、连带 toolScope=none——模型一个工具都拿不到（真机实测过这一条）。
    // 有事实可依时就用事实，不靠关键词补差；「写几个标题候选」这类生成语义不受影响。
    if (options?.hasOpenDocument && COPY_EXISTING_TEXT_MUTATION_PATTERN.test(text)) return false;
...
export function buildAgentIntentControlPlaneDecision(
    ...
    if (isStandaloneCopyDeliverableRequest(text, { hasOpenDocument: input.hasDocument === true })) {
        return makeDecision('chat_only', {
