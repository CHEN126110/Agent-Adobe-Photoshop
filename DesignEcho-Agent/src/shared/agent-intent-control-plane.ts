...
import {
    buildBusinessCapabilityTargetPattern,
    buildExplicitCreativeDesignIntentPatterns
} from './design-category-terms';
import {
    hasExplicitAlternateDocumentTarget,
    hasExplicitCurrentDocumentProtection,
    hasExplicitCurrentDocumentReuse
...
/**
 * 当前语义分类是否已经确认用户在本轮要求真实行动。
 *
 * 它只为 TaskPlan 提供请求身份，不选择 Skill、不授予 Tool，也不替代执行点授权。
 * generic autonomous 包络必须保持 unknown，避免“工具可用”被误写成“必须执行”。
 */
export type AgentIntentExecutionDisposition =
    | 'explicit_execution'
    | 'non_execution'
    | 'unknown';
...
export interface AgentIntentControlPlaneDecision {
    ...
    executionDisposition: AgentIntentExecutionDisposition;
    reason: string;
    userVisibleSummary: string;
    matchedSignals: string[];
}

export type AgentCapabilityToolScopeCeiling = 'none' | 'read_only';

/**
 * 当前用户消息给 Runtime 的 deny-only 能力约束。
 *
 * 它不判断用户想做什么，也不选择 Skill / Tool；只记录用户明确说出的能力上限。
 * 允许能力仍由 Engine 的 autonomous 包络、Capability Resolver 和执行预检共同决定。
 */
export interface AgentCapabilityConstraint {
    version: 'agent-capability-constraint/v0';
    source: 'explicit_user_instruction';
    skillBridgePolicy: 'allow' | 'forbid';
    deniedCapabilityKinds: Array<'skill'>;
    /** 精确禁止的 provider Tool / Skill bridge 名称；不扩大成全局禁用。 */
    deniedProviderToolNames: string[];
    /** 禁止某一工具域，但保留项目、知识、外部生成等其它能力。 */
    deniedToolDomains: Array<'photoshop'>;
    toolScopeCeiling?: AgentCapabilityToolScopeCeiling;
    matchedSignals: string[];
}

const CASUAL_SUFFIX_PATTERN = '[\\s!！?？,，.。~～]*$';
/** 无模型兼容路径中的多智能体提示；不得进入有模型热路径或签发团队写入门禁。 */
...
// 已收敛为本文件单一来源（导出供 routing 复用）。
// 注意：ACK 两份定义存在真实语义分歧（本文件把「明白」当确认；routing 把「开始」当确认），
// 各有调用方依赖差异，未合并；取舍需随协议 owner 收口一起处理。
 ...
 */
const TOOL_DOMAIN_INSTANCE_PATTERN = /(当前|这个|这份|打开的|我的|文档里|画布上?|画面上?)[^，,。！？!?；;\n]{0,8}(文档|图层|画布|画面|版式|详情页|主图|长图|白底图|设计稿|模板|素材|项目|图片|psd|psb)|项目(?:内|里|中)?(?:都)?(?:有什么|有哪些|包含什么|包括什么)|文档(结构|里|中)|图层(结构|列表)|有几屏|每屏|第几屏|分了几屏|(看看|看一下|查看|帮我看|检查一下?)[^。！!？?]{0,12}(文档|图层|画面|画布|版式|详情页|模板|素材|psd|psb)|项目状态|设计方向|设计方案|团队产出|设计状态/i;
...
const BUSINESS_SKILL_CAPABILITY_TARGET_PATTERN = buildBusinessCapabilityTargetPattern();
...
// 只有独立分句里明确要求本轮停止执行，才形成请求级只读上限。不要在这里解析
// 业务对象、条件任务或原稿保护；这些语义由 Agent 和执行点约束处理。
const EXPLICIT_CURRENT_TURN_NO_EXECUTION_PATTERN =
    /(?:^|[，,。！？!?；;\n])\s*(?:(?:本轮|这次|当前|现在|先|暂时)\s*){0,2}(?:不要|别|不用|无需|禁止|不)\s*(?:再\s*)?(?:执行|动手|操作|开始\s*(?:做|制作|生成|导出|处理))\s*(?=$|[，,。！？!?；;\n])/i;
...
const NO_WRITE_DIRECTIVE_PATTERNS = [
    /(?:不要|别|先别|不需要|无需|禁止)[^，,。！？!?；;\n]{0,24}(?:写入|改动|修改|操作)(?:[^，,。！？!?；;\n]{0,16}(?:photoshop|ps|工具|画面|画布|文档))?|(?:不要|别|先别|不需要|无需|禁止)[^，,。！？!?；;\n]{0,24}(?:执行|调用|使用|跑)[^，,。！？!?；;\n]{0,16}(?:photoshop|ps|工具|画面|画布|文档)/i,
    /(不要|别|先别|不需要|无需|禁止)[^，,。！？!?；;\n]{0,16}(创建|新建|生成|保存|导出)[^，,。！？!?；;\n]{0,16}(photoshop|ps|画面|画布|文档)/i
];

// 下面三组只提取用户明确给出的能力边界，不参与 chat / plan / execute 意图分类。
// 因而它们保持窄匹配：宁可把无法确定的表达交给语义 Router，也不靠扩大关键词表猜意图。
const EXPLICIT_ALL_TOOL_FORBID_PATTERNS = [
    /(?:^|[，,。！？!?；;\n]|但是|但|并且|而且)\s*(?:我\s*)?(?:本轮|这次|当前)?\s*(?:(?:不希望|不想让)\s*(?:你|模型|agent)?|不要|别|先别|不需要|无需|禁止|不使用|不用|不调用|不执行)\s*(?:调用|使用|用|执行|运行|跑)?\s*(?:任何|全部|所有)?\s*(?:工具|tools?)\s*(?=$|[，,。！？!?；;\n]|然后|接着|但|但是|并且|而且)/i
];
const EXPLICIT_SKILL_BRIDGE_FORBID_PATTERNS = [
    /(?:^|[，,。！？!?；;\n]|但是|但|并且|而且)\s*(?:我\s*)?(?:本轮|这次|当前)?\s*(?:(?:不希望|不想让)\s*(?:你|模型|agent)?|不要|别|禁止|不使用|不用|不调用|不执行|无需使用|无需调用)\s*(?:再)?\s*(?:使用|用|调用|执行|运行)?\s*(?:任何|全部|所有|业务|工作流|workflow)?\s*(?:skill|技能)\s*(?=$|[，,。！？!?；;\n]|然后|接着|但|但是|并且|而且|来|去)/i
];

const EXPLICIT_GLOBAL_NO_WRITE_CAPABILITY_PATTERNS = [
    /(?:^|[，,。！？!?；;\n]|但是|但|并且|而且)\s*(?:我\s*)?(?:本轮|这次|当前)?\s*(?:(?:不希望|不想让)\s*(?:你|模型|agent)?|不要|别|先别|不需要|无需|禁止)\s*(?:再)?\s*(?:在|对)?\s*(?:photoshop|ps|当前画面|画面|画布|当前文档|文档|当前图层|图层)\s*(?:中|里|上)?\s*(?:进行)?\s*(?:任何|全部|所有)\s*(?:写入|改动|修改|操作)/i,
    /(?:^|[，,。！？!?；;\n]|但是|但|并且|而且)\s*(?:我\s*)?(?:本轮|这次|当前)?\s*(?:(?:不希望|不想让)\s*(?:你|模型|agent)?|不要|别|先别|不需要|无需|禁止)\s*(?:再)?\s*(?:任何|全部|所有)\s*(?:写入|改动|修改|操作)\s*(?:photoshop|ps|当前画面|画面|画布|当前文档|文档|当前图层|图层)/i,
    /(?:^|[，,。！？!?；;\n]|但是|但|并且|而且)\s*(?:我\s*)?(?:本轮|这次|当前)?\s*(?:(?:不希望|不想让)\s*(?:你|模型|agent)?|不要|别|先别|不需要|无需|禁止)\s*(?:再)?\s*(?:写入|改动|修改|操作)\s*(?:任何|全部|所有)?\s*(?:photoshop|ps|当前画面|画面|画布|当前文档|文档|当前图层|图层)(?!\s*的)/i,
    /(?:^|[，,。！？!?；;\n])\s*(?:本轮|这次|当前)\s*(?:保持)?\s*只读(?:查看|检查|分析)?(?=$|[\s，,。！？!?；;\n])/i
];

const EXPLICIT_PHOTOSHOP_TOOL_FORBID_PATTERNS = [
    /(?:^|[，,。！？!?；;\n]|但是|但|并且|而且)\s*(?:我\s*)?(?:本轮|这次|当前)?\s*(?:(?:不希望|不想让)\s*(?:你|模型|agent)?|不要|别|先别|不需要|无需|禁止|不使用|不用|不调用|不执行)\s*(?:再)?\s*(?:调用|使用|用|执行|运行|跑)?\s*(?:任何|全部|所有)?\s*(?:photoshop|ps)\s*(?:的)?\s*(?:工具|tools?)/i,
    /(?:^|[，,。！？!?；;\n]|但是|但|并且|而且)\s*(?:我\s*)?(?:本轮|这次|当前)?\s*(?:(?:不希望|不想让)\s*(?:你|模型|agent)?|不要|别|先别|不需要|无需|禁止|不使用|不用|不调用|不执行)\s*(?:再)?\s*(?:调用|使用|用|执行|运行|跑|操作|打开)?\s*(?:任何|全部|所有)?\s*(?:photoshop|ps)/i
];

const EXPLICIT_SKU_SKILL_GROUP_FORBID_PATTERNS = [
    /(?:^|[，,。！？!?；;\n]|但是|但|并且|而且)\s*(?:我\s*)?(?:本轮|这次|当前)?\s*(?:(?:不希望|不想让)\s*(?:你|模型|agent)?|不要|别|先别|禁止|不使用|不用|不调用|不执行)\s*(?:再)?\s*(?:使用|用|调用|执行|运行)?\s*sku\s*(?:skill|技能)/i
];

type ExplicitCapabilityDenyTarget = 'all_tools' | 'skill' | 'photoshop';

const CAPABILITY_DENY_RELATION_PATTERNS = [
    /(?:不要|别|禁止|不准|不允许|不可以|不许|不用|无需|不可|不能)\s*(?:再)?\s*(?:(?:给我|让你|让模型|让\s*agent)\s*)?(?:调用|使用|用|动用|执行|运行|打开|碰|走)?\s*(?:任何|全部|所有|一切)?\s*(?:业务工作流\s*)?((?:photoshop|ps)\s*(?:工具|tools?)?|skill|技能|工具|tools?)(?:\s*(?:调用|执行|运行|使用))?/gi,
    /(?:所有|全部|任何|一切)?\s*((?:photoshop|ps)\s*(?:工具|tools?)?|skill|技能|工具|tools?)\s*(?:调用|执行|使用|运行)?\s*(?:一个也|一律|全都|都)?\s*(?:不要|别|不准|不允许|不可以|不许|禁止|不用|不可|不能)\s*(?:调用|使用|用|动用|执行|运行)?/gi
    ,/(?:no|do\s+not\s+(?:use|call|run)|don['’]?t\s+(?:use|call|run))\s*((?:(?:any|all)\s+)?(?:(?:photoshop|ps)\s*(?:tools?)?|skills?|tools?))/gi
];

const EXPLICIT_SAFETY_OPERATION_DENY_RULES: Array<{
    toolName: string;
...
}> = [
    {
        ...
        actionPattern: /(?:删除|删掉|删了|删|去掉|移除|清除)/gi,
        objectPattern: /(?:图层|背景|文字层|文本层)/gi
    },
    {
        toolName: 'saveDocument',
        actionPattern: /(?:保存|另存|覆盖|存一下)/gi,
        objectPattern: /(?:文档|文件|PSD|PSB)/gi
    },
    {
        toolName: 'closeDocument',
        actionPattern: /(?:关闭|关掉)/gi,
        objectPattern: /(?:文档|文件|PSD|PSB)/gi
    },
    {
        toolName: 'createDocument',
        actionPattern: /(?:创建|新建|另建)/gi,
        objectPattern: /(?:文档|文件|画布)/gi
    }
];

const CAPABILITY_RESTRICTION_META_DISCUSSION_PATTERN =
    /(?:为什么|为何|是否|是不是|会不会|能否|可不可以|合不合理|合理吗|过于保守|是不是太|怎么理解|什么意思|讨论|假设|假如|如果|比如|例如|这句话)/i;
const REQUEST_QUOTED_PAYLOAD_PATTERN = /["“'‘「『][^"”'’」』\r\n]{0,240}["”'’」』]/g;
const DENIED_TASK_DELEGATION_FRAME_PATTERN = /^(?:我|我们)?\s*(?:(?:从来|一直)?(?:没有|没)|从未|可没|并未|并没有|不是)\s*(?:曾经)?\s*(?:让|叫|要求|请|委托|想让|希望|说要)\s*(?:你|agent|智能体|模型)?/i;
const DENIED_TASK_DELEGATION_DISCOURSE_PREFIX_PATTERN = /^(?:别误会|不要误会|澄清一下|说明一下|需要澄清)\s*[，,：:]?\s*/i;
const HYPOTHETICAL_META_QUESTION_PATTERN = /^(?:如果|假如|假设|比如|例如|比方说|要是)[^。！!；;]{0,220}(?:怎么(?:做|处理|理解)?|如何(?:做|处理|理解)?|会怎样|会发生什么|什么(?:门禁|权限|影响|后果|风险|类型)|哪些(?:门禁|权限|影响|后果|风险)|权限|门禁|需要确认什么)[^。！!；;]{0,24}[?？呢]$/i;
const QUOTED_EXECUTION_DENIAL_FRAME_PATTERN = /^(?:我\s*)?(?:不要|别|禁止|不准|不许|不可以|不允许|无需|不用)\s*(?:真的|实际)?\s*(?:执行|操作|照做|采纳|处理)?\s*引用内容\s*[。！!？?]*$/i;
const EXPLICIT_META_DISCUSSION_FRAME_PATTERNS = [
    /(?:^|[，,。！？!?；;：:]|但是|但)\s*(?:我\s*)?(?:只(?:是)?|仅仅)(?:想)?\s*(?:问|讨论|了解|知道|举例|假设)/i,
    /^(?:我们)?\s*(?:讨论|聊聊)|^(?:请)?\s*(?:解释|说明)[^。！？!?；;]{0,48}(?:含义|意思|类型|权限|流程|命令)/i,
    /^(?:如果|假如|假设|比如|例如|比方说|要是)\s*(?:用户|有人|别人|他人|我|我们)?[^。！？!?；;]{0,96}(?:说|输入|写|要求|请|让|叫|要你|让\s*agent)[^。！？!?；;]{0,96}(?:会怎样|会怎么|怎么处理|如何处理|直接做吗|执行吗|属于什么|是什么类型|触发什么|有什么影响|可以吗|呢|[?？])/i,
    /^(?:请)?\s*(?:解释|说明)[^。！？!?；;]{0,72}(?:会产生|有什么|有哪些|是否有)[^。！？!?；;]{0,24}(?:影响|风险|后果|限制|门禁|权限)/i,
    /^(?:不要|别)\s*(?:真的|实际)\s*(?:执行|动手|操作)/i
];
const RESERVED_PROVIDER_NAMES = new Set([
    'tool', 'tools', 'skill', 'photoshop', 'ps', 'agent', 'any', 'all', 'sku'
]);
...
const READ_ONLY_INSPECT_PATTERNS = [
    ...
    // 对外部链接/网页的「看看/评审」是分析请求，不是创作委托：真机 run#242
    // （2026-08-16 13:05「帮我看看这个淘宝链接的设计」）曾被判 write_photoshop，
    // 触发完成契约推回空转。句尾锚定只认纯评审问句——「看看链接的设计然后照着做」
    // 这类复合委托不命中、仍走创作授权；尾部容忍跟贴的 URL；不含品类词，不涨债务棘轮。
    /(看看|看一下|查看|帮我看|分析|评审|评价|评估).{0,24}(链接|网址|网页|页面|网站|url|URL).{0,20}(的设计|设计怎么样|设计如何|怎么样|如何|什么水平|好不好|行不行|值不值得|可不可以)[\s!！?？,，.。~～]*(?:https?:\/\/\S+|www\.\S+)?[\s!！?？,，.。~～]*$/i,
    /(链接|网址|网页|页面|网站|url|URL).{0,20}(的设计|设计怎么样|设计如何|怎么样|如何|什么水平)[\s\S]{0,80}(看看|看一下|查看|帮我看|分析|评审|评价|评估)[\s!！?？,，.。~～]*$/i,
...
// 从零创作的品类子集与动态正则由 design-category-terms Provider 统一编译；
// 此处只消费具名兼容语义，不再让 legacy control-plane 持有品类正则构造细节。
const EXPLICIT_CREATIVE_DESIGN_PATTERNS = buildExplicitCreativeDesignIntentPatterns();
...
const CREATIVE_DESIGN_INQUIRY_NEGATIVE_PATTERN = /(是否|能否|能不能|可不可以|可以不可以|行不行|要不要|需不需要|是不是|有没有|还剩|还差|还缺|缺哪些|哪些准备|准备哪些|准备什么|需要哪些(?:准备|能力|条件|输入|工具)|还需要.{0,12}准备|前置条件|系统架构|架构来说|准备好|可以开始|能开始|开始了吗|做了吗)/i;

const READ_ONLY_NEGATIVE_PATTERNS = [
    /(保存|另存|导出|关闭|新建|创建|制作|生成|加入|添加|放入|拖入|导入|删除|重命名|改名|置顶|置底|上移|下移|移到|移动到|挪到|排序|调整|整理|归组|编组|修改|修复|改|替换|换成|改成)|(?:组合.{0,8}(?:图层|编组)|(?:图层|编组).{0,8}组合)/i,
...
const EXPLICIT_SKILL_EXECUTION_PATTERNS = [
    ...
    /(图层).{0,20}(顺序|层级|排序|置顶|置底|上移|下移|重命名|改名|删除|复制|拷贝|整理|归组|组合|编组|解除编组|选中|选择)/i,
    ...
    /(选中|选择|重命名|改名|删除|复制|拷贝|整理|归组|组合|编组|解除编组|置顶|置底|上移|下移).{0,18}(当前|选中|目标)?.{0,10}(图层|组|层)/i,
    /从浅到深|从深到浅/i,
...
const SKILL_ROUTING_AUTHORIZATION_ACTION_PATTERN = /(帮我|请|麻烦你|开始|执行|完成|交付|产出|做|制作|生成|导出|保存|创建|新建|处理|分析|查看|检查|识别|理解|设计|优化|调整|整理|归组|编组|修改|重写|撰写|重新写|改成|改为|改一下|改下|换成|换一张|换一下|换个|替换|更换|更新|搜索|搜一下|查找|检索|找一些|找一下|帮我找|帮我搜|search|find)/i;
const SKILL_ROUTING_DISCUSSION_PATTERN = /(要不要|要不需要|需不需要|是否|能否|能不能|可不可以|可以不可以|怎么|如何|为什么|是什么|有哪些|聊聊|说明|解释|总结|复盘|学习|了解|评价|评估|讨论|说的是|喜欢|打算|计划|别人|他人)/i;
// “怎么设计 / 哪个参考更好”可以是执行任务里的推理要求，不能仅凭疑问词把整条请求降级为只规划。
// 这里只识别用户把任务明确委托给 Agent 的句法，不绑定主图、详情页、SKU 等任何业务品类。
const EXPLICIT_TASK_DELEGATION_PATTERN = /(?:(?:(?:帮我|给我|替我|为我|请你|麻烦(?:你)?(?:帮我|给我)?|我想(?:让|请)你|我想你|需要你|我要你|希望你|要求你|交给你)\s*|请\s*(?:为[^，。！？!?；;\n]{1,16})?\s*)(?:直接|现在|马上|立即|开始|继续)?\s*(?:完成|交付|产出|执行|创建|制作|生成|修改|调整|重做|替换|落地|设计|做|出)|^(?:(?:现在|马上|立即|直接|开始|继续)\s*)?(?:(?:从零|从0|从头)\s*)?(?:设计|做|创作|制作|生成|完成|交付|产出|新建|创建|搭建|建立)|^(?:把|将)[^。！？!?；;\n]{1,40}(?:完成|交付|产出|设计|做出来|制作|生成|重做|优化|调整)|(?:^|[，,。；;\n])\s*(?:然后|随后|接着|之后|接下来|再)?\s*(?:无论[^，。！？!?；;\n]{0,16}如何都?)?\s*(?:直接)?\s*(?:完成|交付|产出|执行|创建|制作|生成|修改|调整|重做|替换|落地|设计|做|出|改))/i;
...
const BASIC_PHOTOSHOP_WRITE_OBJECT_PATTERN =
    /(photoshop|ps|文档|画布|画面|页面|设计工作|交付稿|当前版本|这版方案|图层组|图层|文字层|文本层|矩形|形状|文字图层|文本图层|文字|文本|文案|标题|副标题|排版|版式|字体|字号|字距|行距|间距|颜色|背景|图片|图标|icon|投影|描边|图层效果|group|layer|rectangle|shape|text layer|copy|headline|subtitle|layout|typography|image|background|drop shadow|shadow|stroke|layer effect|layer style)/i;
// 一份中央 Photoshop 变更谓词，既覆盖正式动作（删除/关闭/保存），也覆盖中文
// 结果补语（删掉/关掉/存一下/换掉）。它只参与“这是不是明确命令”的 provenance，
// 不选择业务 Skill、不决定具体 Tool；执行能力仍由 Engine 与 ToolDecision 授予。
const PHOTOSHOP_MUTATION_ACTION_PATTERN =
    /真实执行|实际执行|执行|完成|交付(?!稿|物)|产出|落地|做出来|出(?=\s*(?:一|个|张|版|套|组|份))|重做|创建|新建|另建|建立|绘制|画(?!面|布|稿)|添加|新增|放入|写入|生成|修改|调整|优化|编辑|整理|归组|编组|重写|撰写|重新写|改成|改为|改一下|改下|改|替换|换成|换掉|更换|更新|应用|采用|移动|移走|移开|重命名|改名|排序|对齐|缩放|放大|缩小|删除|删掉|复制|清除|清空|移除|去除|去掉|擦除|擦掉|关闭|关掉|保存|存一下|另存(?:为|一份)?|导出(?:来|一下)?|加粗|调大|调小|调成|create|draw|add|insert|write|generate|edit|modify|adjust|optimize|rewrite|replace|apply|update|move|rename|align|scale|delete|copy|clear|remove|export/gi;
const PHOTOSHOP_MUTATION_NEGATION_PREFIX_PATTERN =
    /(?:不要|别|勿|禁止|无需|不用|未|没有|不(?:要|再)?)\s*(?:再\s*)?$/i;
const DIRECT_MUTATION_DISCUSSION_LEAD_PATTERN =
    /^(?:我(?:正在|在|喜欢|打算|想知道|想了解|想学习|觉得|认为|感觉)|别人|他人|我们(?:讨论|聊聊|学习)|(?:请)?(?:评价|评估|说明|解释|讨论))/i;
const DIRECT_MUTATION_INFORMATION_QUESTION_PATTERN =
    /^(?:(?:请问)?(?:你|agent|智能体|模型|系统)?\s*(?:会不会|会|能不能|能否|可不可以|可以不可以|可以|能|支持)|怎么|如何|为什么|为何|是否|要不要|需不需要)|(?:有什么|有哪些|会不会|是否)(?:风险|影响|后果|问题|注意)|(?:风险|影响|后果|副作用|注意事项|是否安全|是否可行)\s*[?？吗嘛么呢]*$/i;
const DIRECT_MUTATION_POLITE_QUESTION_PATTERN =
    /(?:可以吗|行吗|好吗|好不好|行不行|[?？])\s*$/i;
const DIRECT_MUTATION_REQUEST_CUE_PATTERN =
    /(?:帮我|给我|替我|为我|请你|麻烦(?:你)?|直接|马上|立即)/i;
const DIRECT_MUTATION_OBJECT_CUE_PATTERN =
    /(?:^|[，,。；;\n])\s*(?:请|请你|帮我|麻烦(?:你)?|直接|现在|马上|立即)?\s*(?:把|将|给)[^，,。！？!?；;\n]{1,64}$/i;
const DIRECT_MUTATION_LEADING_WRAPPER_PATTERN =
    /^(?:请|请你|帮我|麻烦(?:你)?|直接|现在|马上|立即|先|再)\s*/i;
const IMPLICIT_CURRENT_DOCUMENT_MUTATION_PATTERN =
    /^(?:请|请你|帮我|麻烦(?:你)?|直接|现在|马上|立即|先|再)?\s*(?:另存(?:为|一份)?|导出(?:来|一下)?)\s*[。！!]*$/i;
const DIRECT_MUTATION_NOMINALIZED_SUFFIX_PATTERN =
    /(?:状态|记录|历史|风险|影响|建议|流程|步骤|方法|教程|说明|结果|情况|功能|能力|要求|注意事项)\s*[?？。！!]*$/i;
const BASIC_PHOTOSHOP_WRITE_INQUIRY_NEGATIVE_PATTERN =
    /(能不能|能否|是否|可不可以|可以不可以|可以吗|行不行|怎么|如何|为什么|是什么).{0,24}(执行|完成|交付|产出|落地|重做|创建|新建|建立|绘制|画|添加|新增|放入|写入|生成|修改|调整|优化|编辑|重写|撰写|重新写|改成|改为|改一下|改下|改|替换|换成|更换|更新|应用|采用|移动|重命名|改名|排序|对齐|缩放|删除|复制|清除|移除|去除|create|draw|add|insert|write|generate|edit|modify|adjust|optimize|rewrite|replace|apply|update|move|rename|align|scale|delete|copy|clear|remove|export)|(?:执行|完成|交付|产出|落地|重做|创建|新建|建立|绘制|画|添加|新增|放入|写入|生成|修改|调整|优化|编辑|重写|撰写|重新写|改成|改为|改一下|改下|改|替换|换成|更换|更新|应用|采用|移动|重命名|改名|排序|对齐|缩放|删除|复制|清除|移除|去除|create|draw|add|insert|write|generate|edit|modify|adjust|optimize|rewrite|replace|apply|update|move|rename|align|scale|delete|copy|clear|remove|export).{0,24}(能不能|能否|是否|可不可以|可以不可以|可以吗|行不行|怎么|如何|为什么|是什么)/i;
...
const COPY_CANDIDATE_DELIVERABLE_PATTERN = /(?:(?:[一二三四五六七八九十\d]+)\s*(?:个|条|版|组|套)?\s*)?(?:文案|标题|副标题|slogan|标语|广告语|卖点表达|产品描述)[^。！？!?；;\n]{0,16}(?:候选|备选|版本)|(?:候选|备选)[^。！？!?；;\n]{0,16}(?:文案|标题|副标题|slogan|标语|广告语|卖点表达|产品描述)/i;
...
const AGENT_CAPABILITY_OR_WILLINGNESS_QUESTION_PATTERNS = [
    /^(?:(?:请问|我想确认|想确认)\s*)?(?:你|agent|智能体|模型|系统)\s*(?:是否\s*)?(?:愿不愿意|有没有办法|有无办法|会不会|能不能|能否|可不可以|可以不可以|支不支持|支持不支持|是否愿意|是否可以|是否方便|是否支持)/i,
 ...
 *
 * 只有显式 Agent 主体开头才成立：`你可以帮我做吗？` 是能力问句；
 * `请帮我做，可以吗？` 已先建立委托，末尾只是人际包装；`能麻烦你做吗？`
 * 同样不在本判据里。Semantic、Capability ceiling 与 TaskProgress 共用这一真相源。
 */
...
function resolveExplicitCapabilityDenyTargets(input: string): Set<ExplicitCapabilityDenyTarget> {
    const targets = new Set<ExplicitCapabilityDenyTarget>();
    for (const pattern of CAPABILITY_DENY_RELATION_PATTERNS) {
        for (const match of input.matchAll(pattern)) {
            const target = String(match[1] || '').toLowerCase();
            if (includesAny(target, [/photoshop|^ps\b/i])) {
                targets.add('photoshop');
            } else if (includesAny(target, [/skill|技能/i])) {
                targets.add('skill');
            } else {
                targets.add('all_tools');
            }
        }
    }
    return targets;
...
function extractExplicitSafetyOperationDenyNames(input: string): string[] {
    const names: string[] = [];
    const clauses = input
        .split(/[，,。！？!?；;\n]+|但是|但|而是|然后|接着/)
    ...
    for (const clause of clauses) {
        if (isCapabilityRestrictionMetaDiscussion(clause)) {
            continue;
        }
        const hasExplicitDeny = includesAny(clause, [
            /(?:不要|别|禁止|不准|不允许|不可以|不许|不用|无需|千万(?:不要|别)|(?:(?:从来|一直)?(?:没有|没)|从未|并未|并没有|不是)\s*(?:让|叫|要求|请|委托)(?:你)?)/i
        ]);
        if (!hasExplicitDeny) continue;
        for (const rule of EXPLICIT_SAFETY_OPERATION_DENY_RULES) {
            rule.actionPattern.lastIndex = 0;
            rule.objectPattern.lastIndex = 0;
            const hasDeniedAction = includesAny(clause, [rule.actionPattern]);
            const hasExplicitObject = includesAny(clause, [rule.objectPattern]);
            const hasUnqualifiedDeleteObject = rule.toolName === 'deleteLayer'
                && clause.replace(/[\s，,。！？!?；;]+/g, '').length <= 12;
            if (!hasDeniedAction || (!hasExplicitObject && !hasUnqualifiedDeleteObject)) continue;
...
function maskRequestQuotedPayloads(input: string): string {
    return input.replace(REQUEST_QUOTED_PAYLOAD_PATTERN, ' 引用内容 ');
}

function hasIndependentDirectRequestTail(input: string): boolean {
    const connectorBoundaryPattern = /(?:[，,。；;]\s*)?(而是|然后|随后|接着|之后|接下来|再|另外|不过|但是|但)\s*([^，,。！？!?；;]{1,160})/gi;
    const punctuationBoundaryPattern = /[，,。；;]\s*((?:请|请你|帮我|麻烦(?:你)?|直接|现在|马上|立即|继续|开始|先|只|仅|把|将)[^，,。！？!?；;]{1,160})/gi;
    const tails = [
        ...Array.from(input.matchAll(connectorBoundaryPattern), (match) => String(match[2] || '').trim()),
        ...Array.from(input.matchAll(punctuationBoundaryPattern), (match) => String(match[1] || '').trim())
    ];
    for (const tail of tails) {
        if (!tail) continue;
        if (includesAny(tail, [EXPLICIT_TASK_DELEGATION_PATTERN])
            || isDirectPhotoshopMutationImperative(tail)
            || (
                includesAny(tail, [CONTEXTUAL_INSPECTION_ACTION_PATTERN])
                && isToolDomainContextInput(tail)
            )) {
            return true;
        }
    }
...
/**
 * 识别“正在谈论一条指令”与“当前直接下达指令”的语用边界。
 *
 * 这里不识别任何业务品类，只处理引号、转述、假设和显式否认框架。命中后既不
 * 选择 Skill，也不授予 Tool；如果后续有“而是/然后/直接……”引出的独立肯定请求，
 * 则保留后续请求交给正常控制面处理。
 */
function isPureExecutionMetaOrDeniedFrame(input: string): boolean {
    const text = normalizeText(input);
    if (!text) return false;
    const masked = maskRequestQuotedPayloads(text);
    const hasQuotedPayload = masked !== text;
    const hypotheticalMetaQuestion = includesAny(masked, [HYPOTHETICAL_META_QUESTION_PATTERN]);
    const quotedMetaFrame = hasQuotedPayload && includesAny(masked, [
        /(?:如果|假如|假设|比如|例如)[^。！？!?；;]{0,36}(?:说|输入|写|要求|提到|我要|我想)?[^。！？!?；;]{0,16}引用内容/i,
        /引用内容[^。！？!?；;]{0,48}(?:这句话)?(?:是什么意思|代表什么|怎么理解|如何理解|怎么处理|如何处理|会怎样|属于什么|什么类型|权限|含义)/i,
        /(?:请)?(?:解释|说明)[^。！？!?；;]{0,24}引用内容[^。！？!?；;]{0,32}(?:含义|意思|类型|权限|流程|命令)?/i,
        /(?:他说|她说|他们说|用户说|别人说|有人说|转述|引用)[^。！？!?；;]{0,24}引用内容/i,
        QUOTED_EXECUTION_DENIAL_FRAME_PATTERN
    ]);
    const deniedFrameText = masked.replace(DENIED_TASK_DELEGATION_DISCOURSE_PREFIX_PATTERN, '');
    const deniedDelegation = includesAny(deniedFrameText, [DENIED_TASK_DELEGATION_FRAME_PATTERN]);
    const explicitMetaDiscussion = includesAny(masked, EXPLICIT_META_DISCUSSION_FRAME_PATTERNS);
    if (hypotheticalMetaQuestion) return true;
    if (!quotedMetaFrame && !deniedDelegation && !explicitMetaDiscussion) return false;
    return !hasIndependentDirectRequestTail(text);
}

function isPureSafetyOperationDenial(input: string): boolean {
    const text = normalizeText(input);
    if (!text || extractExplicitSafetyOperationDenyNames(text).length === 0) return false;
    if (hasExplicitCurrentDocumentReuse(text)
        || (hasExplicitCurrentDocumentProtection(text) && hasExplicitAlternateDocumentTarget(text))) {
        return false;
    }
    return !hasIndependentDirectRequestTail(text);
}
...
function isExplicitCreativeDesignRequest(input: string): boolean {
    ...
    if (findControlledSkillRoutingIntent(input)?.skillId === 'layer-management') return false;
    ...
    return hasExplicitTaskDelegation(input)
        && includesAny(input, EXPLICIT_CREATIVE_DESIGN_PATTERNS)
        && !hasSpecNegative
...
function hasExplicitConversationOnlyDirective(input: string): boolean {
    if (includesAny(input, [EXPLICIT_CURRENT_TURN_NO_EXECUTION_PATTERN])) return true;
...
function hasExplicitReadOnlyNoWriteDirective(input: string): boolean {
    if (!EXPLICIT_READ_ONLY_SCOPE_PATTERN.test(input)) return false;
    return includesAny(input, NO_WRITE_DIRECTIVE_PATTERNS)
        || hasOnlyNegatedPhotoshopMutations(input);
}

function hasExplicitAllToolForbidDirective(input: string): boolean {
    return includesAny(input, EXPLICIT_ALL_TOOL_FORBID_PATTERNS)
        || resolveExplicitCapabilityDenyTargets(input).has('all_tools');
}

function isCapabilityRestrictionMetaDiscussion(input: string): boolean {
    const hasQuestionOrMetaFrame = includesAny(input, [
        /[?？]/,
        /(?:讨论|假设|假如|如果|比如|例如|这句话|了解|含义|限制|利弊|优缺点|哪些|有什么|会怎样|会带来)/i
    ]);
    return hasQuestionOrMetaFrame
        && includesAny(input, [
            CAPABILITY_RESTRICTION_META_DISCUSSION_PATTERN,
            /(?:为什么|为何|是否|是不是|会不会|能否|可不可以|有什么|哪些|限制|利弊|优缺点|更好吗|怎么理解|什么意思)/i
        ]);
}

function buildOperationalCapabilityConstraintText(input: string): string {
    return input
        .split(/(?:[，,。！？!?；;\n]|但是|但|并且|而且|然后|接着)/i)
        .map((clause) => clause.trim())
        .filter(Boolean)
        .filter((clause) => !isCapabilityRestrictionMetaDiscussion(clause))
        .join('；');
}

function extractExplicitDeniedProviderToolNames(input: string): string[] {
    const names: string[] = [];
    const patterns = [
        /(?:^|[，,。！？!?；;\n]|但是|但|并且|而且)\s*(?:我\s*)?(?:本轮|这次|当前)?\s*(?:(?:不希望|不想让)\s*(?:你|模型|agent)?|不要|别|先别|禁止|不使用|不用|不调用|不执行)\s*(?:再)?\s*(?:使用|调用|执行|运行)?\s*([a-z][a-z0-9_.-]{1,79})\s*(?:工具|tools?|skill|技能)/gi,
        /(?:^|[，,。！？!?；;\n]|但是|但|并且|而且)\s*(?:我\s*)?(?:本轮|这次|当前)?\s*(?:(?:不希望|不想让)\s*(?:你|模型|agent)?|不要|别|先别|禁止|不使用|不用|不调用|不执行)\s*(?:再)?\s*(?:使用|调用|执行|运行)?\s*(?:工具|tools?|skill|技能)\s*[:：]?\s*([a-z][a-z0-9_.-]{1,79})/gi
    ];
    for (const pattern of patterns) {
        for (const match of input.matchAll(pattern)) {
            const name = String(match[1] || '').trim();
            if (!name || RESERVED_PROVIDER_NAMES.has(name.toLowerCase())) continue;
            if (!names.some((item) => item.toLowerCase() === name.toLowerCase())) names.push(name);
        }
    }
    if (includesAny(input, EXPLICIT_SKU_SKILL_GROUP_FORBID_PATTERNS)) {
        for (const skill of SKILL_REGISTRY) {
            if (!skill.id.toLowerCase().startsWith('sku-')) continue;
            if (!names.some((item) => item.toLowerCase() === skill.id.toLowerCase())) {
                names.push(skill.id);
            }
        }
    }
...
/**
 * 提取当前用户消息中的显式 deny-only 能力约束。
 *
 * 这不是关键词意图路由：结果不能把 chat 变成 execute、不能选业务 Skill，也不能授予写权限；
 * 它只允许把模型选择的能力面收窄。未明确说出的边界保持开放，交给语义 Router 与 Runtime。
 */
export function extractExplicitUserCapabilityConstraint(
    userInput: unknown
): AgentCapabilityConstraint {
    const text = normalizeText(userInput).toLowerCase();
    const constraintText = buildOperationalCapabilityConstraintText(text);
    const isNonExecutionFrame = isPureExecutionMetaOrDeniedFrame(text)
        || isAgentCapabilityOrWillingnessQuestion(text)
        || isPureSafetyOperationDenial(text);
    const explicitCapabilityDenyTargets = resolveExplicitCapabilityDenyTargets(constraintText);
    const protectsCurrentDocumentForAlternateTarget = hasExplicitCurrentDocumentProtection(text)
        && hasExplicitAlternateDocumentTarget(text);
    const matchedSignals: string[] = [];
    let toolScopeCeiling: AgentCapabilityToolScopeCeiling | undefined;

    const explicitlyForbidsCurrentTurnExecution = includesAny(
        constraintText,
        [EXPLICIT_CURRENT_TURN_NO_EXECUTION_PATTERN]
    );
    if (isNonExecutionFrame) {
        // 用户只是在引用、假设、解释或明确否认一条动作指令。默认自主 Agent 的宽能力
        // 包络不能把这种元语境重新变成真实 Tool 权限；纯文字回答已经足够。
        toolScopeCeiling = 'none';
        matchedSignals.push('explicit_non_execution_frame');
    } else if (hasExplicitAllToolForbidDirective(constraintText)) {
        toolScopeCeiling = 'none';
        matchedSignals.push('explicit_all_tools_forbidden');
    } else if (explicitlyForbidsCurrentTurnExecution) {
        // “先不要动手/本轮不要开始做”只撤回有状态执行，不等于“不要使用任何工具”。
        // 保留只读观察后，Agent 仍能按用户要求检查素材、画面或文档；最终写入边界继续
        // 由同一个 capability ceiling 与 ToolDecision 执行，不能因未知语义重新取得写权限。
        toolScopeCeiling = 'read_only';
        matchedSignals.push('explicit_current_turn_no_execution');
    }
    const photoshopToolsForbidden = includesAny(
        constraintText,
        EXPLICIT_PHOTOSHOP_TOOL_FORBID_PATTERNS
    ) || explicitCapabilityDenyTargets.has('photoshop');
    if (photoshopToolsForbidden) {
        matchedSignals.push('explicit_photoshop_tools_forbidden');
    }
    if (!toolScopeCeiling
        && !protectsCurrentDocumentForAlternateTarget
        && (includesAny(constraintText, EXPLICIT_GLOBAL_NO_WRITE_CAPABILITY_PATTERNS)
            || hasExplicitReadOnlyNoWriteDirective(constraintText))) {
        toolScopeCeiling = 'read_only';
        matchedSignals.push('explicit_no_write');
    }

    const skillBridgePolicy = includesAny(constraintText, EXPLICIT_SKILL_BRIDGE_FORBID_PATTERNS)
        || explicitCapabilityDenyTargets.has('skill')
        ? 'forbid'
        : 'allow';
    if (skillBridgePolicy === 'forbid') {
        matchedSignals.push('explicit_skill_bridge_forbidden');
    }
    const safetyOperationDenyNames = protectsCurrentDocumentForAlternateTarget
        ? extractExplicitSafetyOperationDenyNames(constraintText)
            .filter((toolName) => toolName !== 'createDocument')
        : extractExplicitSafetyOperationDenyNames(constraintText);
    const deniedProviderToolNames = Array.from(new Set([
        ...extractExplicitDeniedProviderToolNames(constraintText),
        ...safetyOperationDenyNames
    ]));
    if (deniedProviderToolNames.length > 0) {
        matchedSignals.push('explicit_named_provider_forbidden');
    }
    ...
    return {
        ...
        source: 'explicit_user_instruction',
        skillBridgePolicy,
        deniedCapabilityKinds: skillBridgePolicy === 'forbid' ? ['skill'] : [],
        deniedProviderToolNames,
        deniedToolDomains: photoshopToolsForbidden ? ['photoshop'] : [],
        ...(toolScopeCeiling ? { toolScopeCeiling } : {}),
        matchedSignals
    };
}
...
function hasExplicitTaskDelegation(input: string): boolean {
    ...
    if (isStandaloneCopyDeliverableRequest(input)) return false;
    return EXPLICIT_TASK_DELEGATION_PATTERN.test(input)
        || isDirectPhotoshopMutationImperative(input);
}
...
export function isBasicPhotoshopWriteTask(input: string): boolean {
    ...
    // 文案本身可以是最终交付物。“撰写 / 突出”只说明内容目标，不能在没有
    ...
    if (BASIC_PHOTOSHOP_WRITE_INQUIRY_NEGATIVE_PATTERN.test(text)
        && !isExistingTextMutationRequest(text)
        && !/(请|帮我|麻烦|直接|现在|马上|真实执行|实际执行|不要只解释|不要只说明)/i.test(text)) {
        return false;
    }
    return isDirectPhotoshopMutationImperative(text);
}

function findUnnegatedPhotoshopMutation(
    input: string
): RegExpMatchArray | undefined {
    return Array.from(input.matchAll(PHOTOSHOP_MUTATION_ACTION_PATTERN)).find((match) => {
        const actionIndex = match.index ?? 0;
        const prefix = input.slice(Math.max(0, actionIndex - 8), actionIndex);
        return !includesAny(prefix, [PHOTOSHOP_MUTATION_NEGATION_PREFIX_PATTERN]);
    });
}

function hasOnlyNegatedPhotoshopMutations(input: string): boolean {
    const matches = Array.from(input.matchAll(PHOTOSHOP_MUTATION_ACTION_PATTERN));
...
/**
 * 识别一个已经写成命令形态的 Photoshop 原子变更。
 *
 * 这不是对象/品类路由器：动作在句首时允许模型理解任意目标；对象在句首时只复用既有
 * Photoshop 域对象；“把/将/给 + 对象 + 动作”则由中文句法提供目标关系。问法、讨论、
 * 否定动作和显式只读边界先退出，避免把能力咨询或“只检查，不删除”签成制作义务。
 */
function isDirectPhotoshopMutationImperative(input: string): boolean {
    const text = normalizeText(input);
    if (!text) return false;
    if (hasExplicitConversationOnlyDirective(text.toLowerCase())) return false;
    if (hasExplicitPlanOnlyNoWriteDirective(text.toLowerCase())) return false;
    if (hasExplicitReadOnlyNoWriteDirective(text.toLowerCase())) return false;
    if (includesAny(text, [DIRECT_MUTATION_DISCUSSION_LEAD_PATTERN])) return false;
    if (includesAny(text, [DIRECT_MUTATION_INFORMATION_QUESTION_PATTERN])) return false;

    const mutation = findUnnegatedPhotoshopMutation(text);
    if (!mutation) return false;
    const actionIndex = mutation.index ?? 0;
    const beforeAction = text.slice(0, actionIndex);
    const afterAction = text.slice(actionIndex + mutation[0].length).replace(/[\s。！!]+$/g, '');
    const strippedLead = beforeAction.replace(DIRECT_MUTATION_LEADING_WRAPPER_PATTERN, '').trim();
    const objectIndex = text.search(BASIC_PHOTOSHOP_WRITE_OBJECT_PATTERN);
    const actionStartsCommand = strippedLead.length === 0 && afterAction.length > 0;
    const directedObjectCommand = includesAny(beforeAction, [DIRECT_MUTATION_OBJECT_CUE_PATTERN]);
    const knownObjectCommand = objectIndex >= 0 && objectIndex <= 12;
    const implicitDocumentCommand = includesAny(text, [IMPLICIT_CURRENT_DOCUMENT_MUTATION_PATTERN]);
    const hasCommandShape = actionStartsCommand
        || directedObjectCommand
        || knownObjectCommand
        || implicitDocumentCommand;
    if (!hasCommandShape) return false;
    if (!actionStartsCommand
        && !directedObjectCommand
        && !includesAny(text, [DIRECT_MUTATION_REQUEST_CUE_PATTERN])
        && includesAny(text, [DIRECT_MUTATION_NOMINALIZED_SUFFIX_PATTERN])) {
        return false;
    }

    if (includesAny(text, [DIRECT_MUTATION_POLITE_QUESTION_PATTERN])) {
        return directedObjectCommand
            || includesAny(text, [DIRECT_MUTATION_REQUEST_CUE_PATTERN]);
    }
    return true;
}

const CONTEXTUAL_INSPECTION_ACTION_PATTERN = /^(?:(?:请|请你|帮我|麻烦(?:你)?|我想(?:让|请)?你)\s*)?(?:(?:先|只|仅|先只)\s*)?(?:分析|评估|评价|判断|确认|检查|查看|审核|审查|验收|复核|诊断|梳理|盘点|比较)/i;

function isContextualInspectionRequest(input: string): boolean {
    return includesAny(input, [CONTEXTUAL_INSPECTION_ACTION_PATTERN])
        && isToolDomainContextInput(input)
        && !findUnnegatedPhotoshopMutation(input);
}

/**
 * 指向「已经存在的那行字」的替换语义（改成 / 换成 / 替换成…），区别于「写几个候选」的生成语义。
 * 前者必然有一个画面上的对象要改，后者才可能是纯文字交付。
 */
const COPY_EXISTING_TEXT_MUTATION_PATTERN =
    /(?:把|将)?\s*(?:这段|这句|下面|以下|原文|现有|那个|这个)?\s*(?:文案|标题|副标题|slogan|标语|广告语|产品描述)\s*.{0,20}(?:改成|改为|换成|替换成|改一下|换一下)/i;

function isExistingTextMutationRequest(input: string): boolean {
    return COPY_EXISTING_TEXT_MUTATION_PATTERN.test(input);
}
...
function isStandaloneCopyDeliverableRequest(input: string): boolean {
    ...
    // 「把标题改成春季新品」明确指向既有文字对象，不是要一段独立文案候选。
    // 这一语义由同一 mutation pattern 所有，TaskPlan/Completion 不再复制动作词表。
    if (isExistingTextMutationRequest(text)) return false;
    return (COPY_AUTHORING_REQUEST_PATTERNS.some((pattern) => pattern.test(text))
        || includesAny(text, [COPY_CANDIDATE_DELIVERABLE_PATTERN]))
    ...
    const executionAuthorization = input.executionAuthorization
        || (requestKind === 'read_only_inspect' || requestKind === 'autonomous_execution'
            ...
            : requestKind === 'execute_skill'
                ? 'candidate_only'
                : 'none');
    const executionDisposition = input.executionDisposition
...
export function buildAgentIntentControlPlaneDecision(
    ...
    if (isPureExecutionMetaOrDeniedFrame(text)
        || isAgentCapabilityOrWillingnessQuestion(text)) {
        return makeDecision('chat_only', {
            reason: '用户在询问 Agent 能力/意愿，或引用、假设、解释、明确否认一条动作指令；这不是当前回合的直接执行授权。',
            userVisibleSummary: '这是能力或指令语义讨论，直接回答，不执行其中提到的动作。',
            matchedSignals: ['explicit_non_execution_frame'],
            executionDisposition: 'non_execution'
        });
    }

    if (isPureSafetyOperationDenial(text)) {
        return makeDecision('chat_only', {
            reason: '用户只明确禁止了一个危险原子动作，没有同时下达独立的肯定执行请求。',
            userVisibleSummary: '已记录这项操作禁令，本轮不把否定句反向解释为执行任务。',
            matchedSignals: ['explicit_safety_operation_denial'],
            executionDisposition: 'non_execution'
        });
    }
    ...
    if (hasExplicitCurrentDocumentProtection(text)
        && hasExplicitAlternateDocumentTarget(text)) {
        return makeDecision('autonomous_execution', {
            reason: '用户保护的是当前源文档，同时明确指定了另一个目标文档；保护关系只约束源稿，不能撤销新目标上的执行能力。',
            userVisibleSummary: '这是保护当前源稿、在独立目标中继续处理的请求。',
            matchedSignals: ['protected_source_with_alternate_target'],
            executionAuthorization: 'confirmed_tool_required',
            executionDisposition: 'explicit_execution'
        });
    }

    if (hasExplicitCurrentDocumentReuse(text)) {
        return makeDecision('autonomous_execution', {
            reason: '用户明确把当前文档、画布或稿件指定为继续修改的目标；这是文档动作关系，不应被前一分句中针对副本或新文档的否定误降成只读。',
            userVisibleSummary: '这是继续修改当前文档的明确请求。',
            matchedSignals: ['explicit_current_document_reuse'],
            executionAuthorization: 'confirmed_tool_required',
            executionDisposition: 'explicit_execution'
        });
    }
    ...
    if (isContextualInspectionRequest(text)) {
        return makeDecision('read_only_inspect', {
            reason: '用户要求分析、判断或比较一个明确的当前对象；必须先读取真实上下文，不能把评估动作升级成写入。',
            userVisibleSummary: '这是当前对象的只读检查请求，需要先取得真实观察结果。',
            matchedSignals: ['contextual_inspection']
        });
    }
    ...
    // 治理切片 2（GATE-SIMPLIFY-002）：纯「看看这个链接的设计/怎么样」评审问句不是创作委托，
    // 不得命中写入授权（真机 run#242 误伤）——复合委托（「看看链接…然后照着做」）不受影响。
    if (REFERENCE_LINK_PATTERN.test(text)
        && DESIGN_OR_REPLICATION_INTENT_PATTERN.test(normalized)
        && !isReadOnlyInspectRequest(normalized)
    ...
    if (skillRoutingIntent) {
        if (skillRoutingIntent.mode === 'inspect'
            && !includesAny(normalized, READ_ONLY_NEGATIVE_PATTERNS)) {
            return makeDecision('read_only_inspect', {
                reason: `用户只要求检查业务对象；声明路由 ${skillRoutingIntent.skillId} 的 inspect 模式不能签发写入授权。`,
                userVisibleSummary: '这是业务对象的只读检查请求，只读取真实上下文，不修改画面。',
                matchedSignals: [
                    `shared_skill_routing:${skillRoutingIntent.skillId}`,
                    'declared_skill_inspection'
                ]
            });
        }
        ...
        if (
            ...
            return makeDecision('autonomous_execution', {
                ...
                executionDisposition: confirmed ? 'explicit_execution' : 'unknown'
            });
        }
        ...
        return makeDecision('execute_skill', {
            ...
            executionAuthorization,
            executionDisposition: executionAuthorization === 'confirmed_tool_required'
                ? 'explicit_execution'
                : 'unknown'
        });
    }
    ...
    // 任务意图判定。注意：这不是「没匹配上就默认执行」的兜底，而是按动作特征做的判定——
    // 命中才判执行，命中不了继续往下走。动词表的不完整是已知缺口（真机实测「删掉那个多余的
    // 白色方块」因为「删掉」不在表里而漏判，连带 toolScope=none，模型一个工具都拿不到）。
    //
    // 这套动词判断只服务无模型兼容路径；模型可用时普通自然语言直接进入通用 Agent，
    // 不能再靠扩充这里的词表决定回复、澄清、Skill 或写权限。
    if (/(帮我|请|需要|想让你|麻烦你|做|处理|生成|执行|修改|调整|优化|整理|删掉|删除|去掉|移除|换成|改成|替换|移动|挪到|对齐|调大|调小|放大|缩小)/i.test(normalized)) {
        return makeDecision('autonomous_execution', {
            reason: '输入带有明确的动作意图，但未命中受控业务技能；交给模型先理解目标并决定是否需要工具。',
            userVisibleSummary: '这是需要模型先理解目标的任务请求。',
            matchedSignals: ['unrouted_task_like_input'],
            // 动作特征只说明「像任务」，不是用户确认的执行授权。若吃 makeDecision 的
