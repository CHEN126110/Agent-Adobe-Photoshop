...
export interface DesignResultReviewDiagnosticEvidenceLike {
...
export interface DesignResultReviewMessageLike {
    ...
    agentRequestLifecycle?: Partial<AgentRequestLifecycleEvidence>;
    agentDiagnosticEvidence?: unknown;
}
...
function buildScenario(input: {
    ...
    const evidenceKeys = uniqueStrings([
        ...relevantMessages.flatMap((message) => getDiagnosticEvidenceKeys(message.agentDiagnosticEvidence)),
    ...
    const businessResult = buildBusinessResultSummary(
        relevantMessages.map((message) => message.agentDiagnosticEvidence)
    );
    const deliverables = {
    ...
    const status = deriveScenarioStatus({ evidenceKeys, qa, businessResult, blockers, humanReview });
...
function messageMatchesScenario(
    ...
    return inferScenarioFromDiagnosticEvidence(message.agentDiagnosticEvidence) === scenario;
}

function inferScenarioFromDiagnosticEvidence(
    evidence: unknown
): DesignResultReviewDerivedScenario {
    if (!isRecord(evidence)) return 'general-design';
    const keys = getDiagnosticEvidenceKeys(evidence);
    ...
    const scenario = sanitizeText((evidence.businessSkillExecutionPlanIntake as { scenario?: unknown } | undefined)?.scenario)
        || sanitizeText((evidence.businessSkillImagePlacementVerificationIntake as { scenario?: unknown } | undefined)?.scenario);
    return deriveDesignResultReviewScenario(scenario);
}

function getDiagnosticEvidenceKeys(
    evidence: unknown
): string[] {
    if (!isRecord(evidence)) return [];
    const keys = Array.isArray(evidence.evidenceKeys)
        ? evidence.evidenceKeys.map(sanitizeText)
        : [];
...
function buildBusinessResultSummary(
    ...
    for (const evidence of evidenceItems) {
        ...
        mergeMainImageQaEvidence(summary, evidence.mainImageQaReport);
        mergeMainImageScreenshotEvidence(summary, evidence.mainImageScreenshotQa);
        mergeMainImageScreenshotEvidence(summary, evidence.mainImageScreenshotProbeReadiness);
        mergeDetailPageReadinessEvidence(summary, evidence.detailPageSkillReadiness);
        mergeBusinessSkillIntakeEvidence(summary, evidence.businessSkillExecutionPlanIntake);
        mergeBusinessSkillIntakeEvidence(summary, evidence.businessSkillImagePlacementVerificationIntake);
        mergeBusinessVisualControlEvidence(summary, evidence.businessSkillVisualEvidenceControlDecision);
        mergeDesignAgentOsEvidence(summary, evidence.designAgentOs);
    }

    summary.hasEvidence = Boolean(
        ...
        || summary.requiredNextEvidence.length
    ...
    summary.requiredNextEvidence = uniqueStrings(summary.requiredNextEvidence).slice(0, 8);
...
function mergeMainImageQaEvidence(
    ...
    summary.requiredNextEvidence.push(...normalizeTextList(qualityClaim?.requiredEvidence));
    summary.requiredNextEvidence.push(...normalizeTextList(qualityClaim?.blockers));
    summary.blockers.push(...normalizeTextList(value.blockers));
    summary.warnings.push(...normalizeTextList(value.warnings));
    summary.requiredNextEvidence.push(...normalizeTextList(value.nextActions));
}

function mergeMainImageScreenshotEvidence(
    summary: DesignResultReviewBusinessResultSummary,
    value: unknown
): void {
    if (!isRecord(value)) return;
    summary.qaStages.push(sanitizeText(value.stage));
    const resultImageEvidence = readRecord(value.resultImageEvidence);
    summary.resultImageCount = Math.max(summary.resultImageCount, toCount(resultImageEvidence?.resultImageCount));
    summary.requiredNextEvidence.push(...normalizeTextList(value.requiredNextEvidence));
    summary.blockers.push(...normalizeTextList(value.blockers));
    summary.warnings.push(...normalizeTextList(value.warnings));
}

function mergeDetailPageReadinessEvidence(
    summary: DesignResultReviewBusinessResultSummary,
    value: unknown
): void {
    if (!isRecord(value)) return;
...
function mergeBusinessSkillIntakeEvidence(
    summary: DesignResultReviewBusinessResultSummary,
    value: unknown
): void {
    if (!isRecord(value)) return;
...
function mergeBusinessVisualControlEvidence(
    summary: DesignResultReviewBusinessResultSummary,
    value: unknown
): void {
    if (!isRecord(value)) return;
    summary.readinessStatuses.push(sanitizeText(value.decision));
    summary.requiredNextEvidence.push(...normalizeTextList(value.requiredNextEvidence));
}

function mergeDesignAgentOsEvidence(
    summary: DesignResultReviewBusinessResultSummary,
    value: unknown
): void {
    if (!isRecord(value)) return;
    ...
    summary.readinessStatuses.push(sanitizeText(executionTrace?.status));
...
function deriveScenarioStatus(input: {
    ...
    if (
        input.evidenceKeys.length > 0
        || input.businessResult.hasEvidence
        || input.qa.verified > 0
