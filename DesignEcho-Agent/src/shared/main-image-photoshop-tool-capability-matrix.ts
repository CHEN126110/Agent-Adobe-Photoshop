...
function makeCapability(input: {
    ...
    return {
        ...
        boundary: 'Capability matrix is static no-write evidence; live Photoshop execution still requires a separate runner and readback.'
    };
}
...
export function buildMainImagePhotoshopToolCapabilityMatrix(
    ...
    const evidence: EvidenceRef[] = [{
        source: 'main-image-photoshop-tool-capability-matrix',
        summary: `supported=${supportedCapabilityIds.join('/') || 'none'}; blocked=${blockedCapabilityIds.join('/') || 'none'}`,
        status: blockedCapabilityIds.length === 0 ? 'needs_review' : 'failed'
    }];
    ...
    return {
        ...
        verificationReport: {
            ...
            limitations: [
                ...
                'blocked 能力必须先补真实 UXP 工具语义或显式降级策略，不能用 prompt 兜底。'
            ],
            evidence
        }
    };
