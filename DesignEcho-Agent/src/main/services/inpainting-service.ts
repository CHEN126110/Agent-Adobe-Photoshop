...
export interface InpaintingRequest {
    ...
    /**
     * 一次生成的候选变体数（1-4，默认 1）。对齐官方 Generative Fill 的"一次出 3 个变体任挑"：
     * 同一选区/蒙版/提示词并行请求 N 次，模型自身的随机性让每个候选不同。
     * 每个候选都是一次完整的付费生成，所以上限压到 4。
     */
    count?: number;
}
...
export interface InpaintingResult {
    ...
    rawImages?: string[];
    imageBuffer?: Buffer;
    /** 多变体时的全部候选（含第一张）。单变体时与 imageBuffer 同内容，调用方按数组统一处理 */
    imageBuffers?: Buffer[];
...
/** 不指定模型时的默认重绘通道 */
const DEFAULT_INPAINTING_MODEL: InpaintingModel = 'google/gemini-3-pro-image-preview';
...
/**
 * 单次请求的候选变体上限。每个变体都是一次完整的付费生成，
 * 且即梦对突发并发敏感（整图重生买到的教训），不做更大的敞口。
 */
const MAX_VARIANT_COUNT = 4;

/** 多变体并发的单批上限，与整图重生对即梦限流的处理保持一致 */
const VARIANT_BATCH_MAX_PARALLEL = 3;
...
export class InpaintingService {
    async inpaint(request: InpaintingRequest, onProgress?: InpaintingProgressCallback): Promise<InpaintingResult> {
        ...
        try {
            ...
            // 多变体：同一选区/蒙版/提示词并行生成 N 次，靠模型自身的随机性拉开候选差异。
            // 进度只转发第一个变体——N 个变体的阶段序列完全一致，都转发会互相打架。
            const variantCount = this.clamp(Math.floor(Number(request.count) || 1), 1, MAX_VARIANT_COUNT);
            const runOne = (forwardProgress: boolean): Promise<Buffer> => provider === 'jimeng'
                ? this.runJimengOfficialFlow(request, promptPlan, model, fullImage, fullMask, forwardProgress ? onProgress : undefined)
                : this.runCroppedProviderFlow(
                    request,
                    promptPlan,
                    model,
                    provider,
                    ...
                    fullMask,
                    references,
                    forwardProgress ? onProgress : undefined
                );
            const outputRgbaVariants = variantCount === 1
                ? [await runOne(true)]
                : await this.runVariantBatches(runOne, variantCount, warnings);
            ...
            const outputPngs: Buffer[] = [];
            for (const outputRgba of outputRgbaVariants) {
                outputPngs.push(await sharp(outputRgba, {
            ...
            return {
                ...
                images: [],
                rawImages: [],
                imageBuffer: outputPngs[0],
                imageBuffers: outputPngs,
                warnings,
                meta: {
                    provider,
                    model,
    ...
    /**
     * 并行生成 N 个变体，单批并发不超过 VARIANT_BATCH_MAX_PARALLEL。
     * 部分失败不拖垮整批：有成功就返回成功项并把失败原因写进 warnings（如实告知），
     * 全部失败才把首个错误抛出去——和整图重生的 partialFailures 是同一处置哲学。
     */
    private async runVariantBatches(
        runOne: (forwardProgress: boolean) => Promise<Buffer>,
        variantCount: number,
        warnings: string[]
    ): Promise<Buffer[]> {
        const succeeded: Buffer[] = [];
        const failures: string[] = [];
        for (let start = 0; start < variantCount; start += VARIANT_BATCH_MAX_PARALLEL) {
            const batchSize = Math.min(VARIANT_BATCH_MAX_PARALLEL, variantCount - start);
            const settled = await Promise.allSettled(
                Array.from({ length: batchSize }, (_, index) => runOne(start === 0 && index === 0))
            );
            settled.forEach((outcome, index) => {
                if (outcome.status === 'fulfilled') {
                    succeeded.push(outcome.value);
                } else {
                    const reason = outcome.reason?.message || String(outcome.reason || '未知错误');
                    failures.push(`第 ${start + index + 1} 个候选：${reason}`);
                }
            });
        }
        if (succeeded.length === 0) {
            throw new Error(`局部重绘 ${variantCount} 个候选全部生成失败。${failures[0] || ''}`.trim());
        }
        if (failures.length > 0) {
            warnings.push(`${variantCount} 个候选中 ${failures.length} 个生成失败，已返回 ${succeeded.length} 个可用结果。${failures[0]}`);
        }
        return succeeded;
    }
