...
// 默认走"钉死日期的快照 ID"而不是 alias：GA alias 已漂到 20260528 快照丢了 4K，
// 而 OpenRouter 的 400 报文自己把 20251120 快照列入 4K 白名单——钉死它就不会再漂。
const DEFAULT_MODEL: OpenRouterGeminiImageModel = 'google/gemini-3-pro-image-preview-20251120';
 ...
 */
const MODELS_WITH_IMAGE_CONFIG: OpenRouterGeminiImageModel[] = [
    'google/gemini-3-pro-image-preview-20251120',
    'google/gemini-3-pro-image-preview',
    'google/gemini-3-pro-image'
];
 ...
 *
 * 旗舰是 Nano Banana Pro，OpenRouter 上有三种写法：
 * - google/gemini-3-pro-image-preview-20251120（钉死日期的快照 ID）：OpenRouter 400
 *   报文自己把它列入 4K 白名单，是默认集成——alias 漂移救不了，钉死 ID 才防得住。
 * - google/gemini-3-pro-image-preview（alias，当前解析到 20251120 快照）：支持 4K，
 *   仅作旧持久化选择兜底；哪天漂走，把 4K 从这行删掉即可。
 * - google/gemini-3-pro-image（GA alias → 20260528 快照）：同价但 4K 被拒，
 *   如实钳到 2K——把注定被拒的请求发出去，就是一次付费的无效调用。
 */
const MODEL_IMAGE_SIZE_CAPS: Partial<Record<OpenRouterGeminiImageModel, Array<'1K' | '2K' | '4K'>>> = {
    'google/gemini-3-pro-image-preview-20251120': ['1K', '2K', '4K'],
...
export class OpenRouterGeminiImageService {
    ...
    async generateFromImage(
        ...
        onProgress?.({
            progress: 28,
            stage: 'provider-validate',
            message: 'Preparing OpenRouter image-to-image request'
        });
        ...
        onProgress?.({
            progress: 46,
            stage: 'provider-submit',
            message: 'Submitting image-to-image request to OpenRouter'
        });
    ...
    private resolveRequestedAspectRatio(requested: string | undefined, width: number, height: number): string {
        const normalized = String(requested || '').trim();
        if (!normalized) {
            return this.resolveAspectRatio(width, height);
        }
        if ((SUPPORTED_ASPECT_RATIOS as readonly string[]).includes(normalized)) {
            return normalized;
        }
        throw this.createStageError(
            `OpenRouter 图像比例「${normalized}」不在支持清单内。当前可用：${SUPPORTED_ASPECT_RATIOS.join('、')}`,
            'provider-validate'
        );
    }
