    ...
    formatMessages(
        ...
        for (const msg of messages) {
            ...
            if (msg.role === 'user') {
                ...
                // AdapterMessage.reasoningContent 没有 Anthropic thinking block 必需的
                // signature，不能伪造为可回放的原生思考。长度截断若只留下 reasoning，
                // 就跳过这一空 assistant 回合，由后续 Harness continuation user 消息续跑；
                // 否则会向 Anthropic 发送 content: [] 的非法消息。
