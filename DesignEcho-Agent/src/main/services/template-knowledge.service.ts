 ...
 * 素材元信息（尺寸 / 体积 / 修改时间）缓存
 *
 * 目录扫描会对每个素材读一次文件头，几百个素材时按 mtime+size 命中缓存，避免重复小 IO。
 */
type AssetMetaCacheEntry = {
    mtimeMs: number;
    size: number;
    width: number;
    height: number;
...
const LIBRARY_ASSET_META_CACHE_MAX = 2000;
const libraryAssetMetaCache = new Map<string, AssetMetaCacheEntry>();
 ...
 * 读取素材的尺寸 + 体积 + 修改时间
 *
 * 普通文件（用户直接拷进设计库目录的 PSD/JPG/PNG…）此前完全没有这些元信息，
 * 面板拿不到宽高就无法按真实比例占位，只能等图片解码后再重排。
 */
function readLibraryAssetFileMeta(filePath: string): {
    width: number;
    height: number;
    sizeBytes: number;
    updatedAt: number;
    ...
    const cached = libraryAssetMetaCache.get(cacheKey);
    ...
    const dimensions = readLibraryAssetDimensions(filePath);
    const entry: AssetMetaCacheEntry = {
    ...
    if (libraryAssetMetaCache.has(cacheKey)) {
        libraryAssetMetaCache.delete(cacheKey);
    }
    libraryAssetMetaCache.set(cacheKey, entry);
    if (libraryAssetMetaCache.size > LIBRARY_ASSET_META_CACHE_MAX) {
        const oldestKey = libraryAssetMetaCache.keys().next().value as string | undefined;
        if (oldestKey) {
            libraryAssetMetaCache.delete(oldestKey);
        }
    }
...
function formatLibraryAssetSize(sizeBytes: number): string | undefined {
...
function formatLibraryAssetUpdatedLabel(updatedAt: number): string | undefined {
...
function buildLibraryAssetEntry(
    ...
    return {
        ...
        tags,
        ...buildLibraryAssetFileMetaFields(fullPath, packageInfo),
 ...
 * 组装素材的尺寸 / 体积 / 时间字段
 *
 * 包目录素材优先用 info.json 里已记录的宽高；普通文件回落到文件头解析。
 * 面板用 width/height 预留卡片高度（消除瀑布流跳动），用 fileSizeBytes/updatedAt 排序。
 */
function buildLibraryAssetFileMetaFields(
    fullPath: string,
    packageInfo?: LibraryAssetPackageInfo
): Pick<LibraryAssetEntry, 'width' | 'height' | 'fileSize' | 'fileSizeBytes' | 'updatedAt' | 'updatedLabel'> {
    const fileMeta = readLibraryAssetFileMeta(fullPath);
    const packageWidth = Number(packageInfo?.width || 0);
    const packageHeight = Number(packageInfo?.height || 0);
    const width = packageWidth > 0 ? packageWidth : (fileMeta?.width || 0);
    const height = packageHeight > 0 ? packageHeight : (fileMeta?.height || 0);
    const sizeBytes = fileMeta?.sizeBytes || 0;
    const updatedAt = Number(packageInfo?.updatedAt || 0) || fileMeta?.updatedAt || 0;

    return {
        width,
        height,
        fileSize: formatLibraryAssetSize(sizeBytes),
        fileSizeBytes: sizeBytes || undefined,
        updatedAt: updatedAt || undefined,
        updatedLabel: formatLibraryAssetUpdatedLabel(updatedAt)
    };
}
...
export class TemplateKnowledgeService {
    ...
    static findTemplateForSKU(params: FindSKUTemplateParams): SKUTemplateCandidate | null {
        ...
        return pickBestSkuTemplateCandidate(this.getSKUTemplateCandidates(), {
            comboSize,
            keyword: params.keyword,
            noteMode: params.noteMode === true,
    ...
    static getAvailableSKUSpecs(params?: GetAvailableSKUSpecsParams): number[] {
        return collectSkuTemplateSizes(this.getSKUTemplateCandidates(), params?.sources);
    }
