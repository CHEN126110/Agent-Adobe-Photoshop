...
export class MCPHostService {
    ...
    private shouldAwaitFinalResultAfterHttpAbort(request: JsonRpcRequest): boolean {
        if (request.method !== 'tools/call') return false;
        const params = asRecord(request.params);
        const hostToolName = String(params.name || '').trim();
        const hostToolArgs = asRecord(params.arguments);
        const isMutation = (name: unknown, args: unknown): boolean => {
            const kind = classifyPhotoshopToolSkillExecution(name, asRecord(args));
            return kind === 'photoshop_write'
                || kind === 'save_export'
                || kind === 'unknown';
        };
        if (hostToolName === 'photoshop.tools.call') {
            return isMutation(hostToolArgs.name, hostToolArgs.arguments);
        }
        if (hostToolName === 'photoshop.tools.batch_call') {
            const calls = Array.isArray(hostToolArgs.calls) ? hostToolArgs.calls : [];
    ...
    private async handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
        ...
        if (method === 'POST' && url.pathname === '/mcp') {
            ...
            const awaitFinalResultAfterAbort =
                this.shouldAwaitFinalResultAfterHttpAbort(request);
        ...
        switch (name) {
            ...
            case 'photoshop.tools.call': {
                ...
                return this.unwrapPhotoshopMcpPayload(
                    await this.wsServer.callMCPTool(toolName, toolArgs, {
    ...
    private inferCategory(toolName: string): string {
        ...
        if (toolName.includes('Selection') || toolName.includes('Matting') || toolName.includes('Image') || toolName.includes('Background') || toolName.includes('Subject')) return 'image';
