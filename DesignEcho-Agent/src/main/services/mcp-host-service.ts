...
export class MCPHostService {
    ...
    private getToolDefinitions(): MCPToolDefinition[] {
        return [
            ...
            },
            {
                ...
                description: 'Capture a lightweight Photoshop acceptance snapshot for task verification and debug evidence.',
                inputSchema: {
                    type: 'object',
                    properties: {
    ...
    private async callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
        switch (name) {
            ...
            case 'photoshop.tools.call': {
                ...
                return this.unwrapPhotoshopMcpPayload(await this.wsServer.callMCPTool(toolName, toolArgs));
            }
    ...
    private inferToolKind(toolName: string): 'read' | 'write' | 'export' {
        ...
        if (/^export/.test(toolName)) return 'export';
        return 'write';
    }
    ...
    private async runPhotoshopBatch(
        ...
        options: { continueOnError: boolean; allowWrites: boolean; allowRisky: boolean; delayMs: number }
    ): Promise<unknown> {
        await this.ensurePhotoshopConnected();
        const results: any[] = [];
