...
export class EagleLibraryService {
    ...
    async getPreview(request: EagleLibraryPreviewRequest): Promise<EagleLibraryPreviewResponse> {
        ...
        if (request?.purpose === 'composer_ui') {
            const libraryId = cleanText(request.libraryId, 160);
            const resolvedLibraryPath = this.libraryPathById.get(libraryId);
            if (!libraryId || !resolvedLibraryPath) {
                return {
                    success: false,
                    status: 'not_found',
                    itemId,
                    error: '这个 Eagle 素材库尚未在本次会话打开，请先在素材库页面打开对应库后再重试预览。',
                    boundaries
                };
            }
            libraryPath = resolvedLibraryPath;
        } else if (request?.purpose === 'eagle_library_ui') {
            ...
            return {
                ...
                status: 'unavailable',
                itemId,
                error: 'Eagle 素材预览只允许由素材库或对话框界面按需请求。',
                boundaries
            };
        }

        try {
            const index = await this.loadIndex(libraryPath, false);
            const item = index.itemById.get(itemId);
            if (!item) {
                return {
                    success: false,
