...
export function registerInpaintingHandlers(context: UXPContext): void {
    ...
    const inpaintingGenerateHandler = async (params: {
        ...
        logService?.logAgent(
            'info',
            `[Inpainting] Start request, model=${params.model || 'google/gemini-3-pro-image'}, imageFromBinary=${params.imageFromBinary === true}, maskFromBinary=${params.maskFromBinary === true}`
        );
