...
export function registerInpaintingHandlers(context: UXPContext): void {
    ...
    const inpaintingGenerateHandler = async (params: {
        ...
        /** 候选变体数（1-4）。对齐官方 Generative Fill 一次出多个变体任挑的交互 */
        count?: number;
        ...
        logService?.logAgent(
            'info',
            `[Inpainting] Start request, model=${params.model || 'google/gemini-3-pro-image-preview'}, imageFromBinary=${params.imageFromBinary === true}, maskFromBinary=${params.maskFromBinary === true}`
        );
        ...
        try {
            ...
            // 多变体：service 返回 imageBuffers 数组；老路径只有 imageBuffer 单张，归一成数组统一处理
            const outputBuffers = (Array.isArray(result.imageBuffers) ? result.imageBuffers : [])
                .filter((item): item is Buffer => item instanceof Buffer && item.length > 0);
            if (outputBuffers.length === 0 && result.imageBuffer instanceof Buffer && result.imageBuffer.length > 0) {
                outputBuffers.push(result.imageBuffer);
            }
            const imageFilePaths: string[] = [];
            const previewBase64List: string[] = [];
            for (const outputBuffer of outputBuffers) {
                ...
                if (previewBase64) {
                    previewBase64List.push(previewBase64);
                }
                imageFilePaths.push(await persistInpaintingImageToTempFile(outputBuffer, 'png'));
            }
            if (previewBase64List.length > 0) {
                images.length = 0;
                images.push(...previewBase64List);
            }
            // 单数字段继续回传第一张：UXP 插件与 Agent 可能跨版本共存，老插件只认 imageFilePath
            const imageFilePath = imageFilePaths[0] || '';
            const warnings = Array.isArray(result.warnings) ? result.warnings : [];
            if (warnings.length > 0) {
                logService?.logAgent('warn', `[Inpainting] Degraded: ${warnings.join(' | ')}`);
            }
