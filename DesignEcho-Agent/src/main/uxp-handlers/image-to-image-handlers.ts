...
function parseImageToImageError(error: any): {
    ...
    if (/does not support size preset/i.test(rawMessage)) {
        return {
            message: rawMessage,
            stage: 'validate-size-preset',
            code,
            detail: 'Selected size preset is not supported by the current model'
        };
    }
...
export function registerImageToImageHandlers(context: UXPContext): void {
    ...
    const imageToImageGenerateHandler = async (params: {
        ...
        try {
            ...
            const previewBuffer = await sharp(result.image)
                .resize({ width: 640, height: 640, fit: 'inside', withoutEnlargement: true })
                .png()
                .toBuffer();
            const previewBase64 = previewBuffer.toString('base64');
            const outputExtension = typeof metadata.format === 'string' && metadata.format.trim().length > 0
                ? metadata.format.trim().toLowerCase()
                : 'png';
            const imageFilePath = await persistImageToTempFile(result.image, outputExtension);
