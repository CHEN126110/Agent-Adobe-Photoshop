...
export function registerArtifactRepositoryHandlers(context?: ArtifactRepositoryHandlerContext): void {
    ipcMain.handle('artifactRepository:issueSessionIdentity', async (
        event: IpcMainInvokeEvent,
        projectPath: unknown,
        request: unknown
    ) => {
        try {
            ...
            const issuanceRequest = readRuntimeSessionIdentityIssuanceRequest(request);
            if (!issuanceRequest) {
                throw new ArtifactRepositoryError(
                    'invalid_identity_issuance_request',
                    'Runtime 身份签发请求非法或含未知字段。'
                );
            }
            const grant = runtimeArtifactAuthorizationService.issueSessionIdentity({
                senderId,
