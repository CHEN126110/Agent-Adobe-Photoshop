...
export function registerConfigHandlers(context: IPCContext): void {
    ...
    const writeStateStoreSync = (entries: Record<string, string>): boolean => {
        try {
            ...
            logService?.logAgent('error', `[Config] State store write (sync) failed: ${error?.message || String(error)}`);
            return false;
        }
    };
    ...
    ipcMain.on('state:setPersistedValueSync', (event, key: string, value: string) => {
        try {
            ...
            event.returnValue = written ? { success: true } : { success: false, error: 'file write failed' };
        } catch (error: any) {
            logService?.logAgent('error', `[Config] state:setPersistedValueSync failed: ${error?.message || String(error)}`);
