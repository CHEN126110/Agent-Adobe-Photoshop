...
function buildChatTestReferenceActionPlan(tools: unknown[]): Record<string, unknown> | undefined {
    ...
    const deliverableRefs = readChatTestSchemaStringEnum(
        tools,
        toolName,
        ['properties', 'deliverableCoverage', 'items', 'properties', 'deliverableRef', 'enum']
    );
    ...
    const verificationStepIds = (deliverableRefs.length > 0 ? deliverableRefs : [''])
        .map((_, index) => `readback-reference-layout-${index + 1}`);
    return {
        ...
        steps: [
            ...
            ...verificationStepIds.map((stepId, index) => ({
                stepId,
                kind: 'verify',
                goal: `独立读取第 ${index + 1} 项交付对应的画布结果。`,
        ...
        ...(deliverableRefs.length > 0 ? {
            deliverableCoverage: deliverableRefs.map((deliverableRef, index) => ({
                deliverableRef,
                stepIds: ['execute-reference-layout', verificationStepIds[index]]
            }))
        } : {}),
        missingInputs: []
    };
