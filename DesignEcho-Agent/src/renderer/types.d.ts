...
export interface DesignEchoAPI {
    bfl: {
        ...
        image2image: (
            ...
            prompt: string,
            inputImage: string,
            options?: { width?: number; height?: number; additionalImages?: string[] }
        ) => Promise<any>;
        inpaint: (
            prompt: string,
            inputImage: string,
    ...
    testOllamaCloud?: (apiKey: string, modelId?: string) => Promise<{
        success: boolean;
        kind: 'ok' | 'auth' | 'billing' | 'model_access' | 'rate_limit' | 'timeout' | 'network' | 'protocol' | 'service_unavailable' | 'unknown';
        message?: string;
        error?: string;
        status?: number;
        model?: string;
    ...
    webSearch?: (request: { query: string; limit?: number }) => Promise<{
    ...
    recommendAssets: (params: {
        ...
        recommendations?: Array<{
            ...
            visualScore?: number;
            visualRole: 'hero_product' | 'hero_scene' | 'model_context' | 'detail_evidence' | 'material_evidence' | 'background' | 'decorative' | 'reference' | 'unknown';
            ...
            directUseSuitability: 'suitable' | 'conditional' | 'unsuitable';
