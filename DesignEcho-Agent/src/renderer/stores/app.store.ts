...
interface Conversation {
    id: string;
    /**
     * 当前消息树的稳定分支身份。编辑已发送的用户消息会生成新分支，旧 Run Record
     * 仍保留用于审计，但不得自动恢复到新分支。
     */
    branchId: string;
    title: string;
...
function sanitizeConversationForPersistence(conversation: Conversation): Conversation {
    const persistedBranchId = typeof conversation.branchId === 'string'
        ? conversation.branchId.trim().slice(0, 160)
        : '';
    return {
        ...conversation,
        // 历史对话没有 branchId：加载时一次性补齐，并在下一次消息保存时落盘。
        branchId: persistedBranchId || crypto.randomUUID(),
