...
import {
    ...
    type EagleKnowledgeConnectionStatus,
...
export function KnowledgeLibraryPage({
    ...
    const [eagleConnection, setEagleConnection] = useState<EagleKnowledgeConnectionStatus | null>(null);
    const [checkingEagleConnection, setCheckingEagleConnection] = useState(false);
    ...
    const recentlyUsedItems = useMemo(() => managedItems
        .filter((item) => item.status === 'active' && Number(item.usageCount || 0) > 0)
        .sort((left, right) => Date.parse(String(right.lastUsedAt || '')) - Date.parse(String(left.lastUsedAt || '')))
        .slice(0, 4), [managedItems]);
    ...
    async function refreshEagleConnection(): Promise<void> {
        setCheckingEagleConnection(true);
        const status = await getKnowledgeLibraryService().probeEagleConnection();
        setEagleConnection(status);
        setCheckingEagleConnection(false);
    }
    ...
    function confirmRoleSelection(useRole: KnowledgeReferenceUseRole): void {
        ...
        setMessage(`已作为「${KNOWLEDGE_REFERENCE_USE_ROLES[useRole].label}」插入对话：${result.title}`);
        setRolePickerTarget(null);
    }
                    ...
                    {SECTION_ITEMS.map((item) => {
                        ...
                        return (
                            <button key={item.id} type="button" className={section === item.id ? 'is-active' : ''} aria-current={section === item.id ? 'page' : undefined} onClick={() => setSection(item.id)}>
                                ...
                                {item.id === 'review' && counts.review > 0 && <em aria-label={`${counts.review} 条待审核`}>{counts.review}</em>}
                            </button>
                        );
                    })}
                </nav>
                <div className="knowledge-library-nav__status">
                    <span><Database size={14} aria-hidden="true" />Eagle 管素材文件</span>
                    <span><ShieldCheck size={14} aria-hidden="true" />这里管可复用判断</span>
                </div>
            </aside>
                ...
                {section === 'overview' && (
                    <section className="knowledge-overview" aria-labelledby="knowledge-overview-title">
                        <header className="knowledge-overview__hero">
                            <div>
                                <span className="knowledge-eyebrow">Design decision memory</span>
                                <h1 id="knowledge-overview-title">把收藏，变成下一次设计能用的判断</h1>
                                <p>Eagle 继续保存原图、文件夹和标签；这里保存你从参考中确认过的构图、表达方法、适用场景与反例。Agent 只在任务需要时取用。</p>
                            </div>
                            <div className="knowledge-overview__actions">
                                <button type="button" className="knowledge-button knowledge-button--primary" onClick={() => setSection('library')}>
                                    <Search size={15} aria-hidden="true" />查找可用知识
                                </button>
                                <button type="button" className="knowledge-button knowledge-button--secondary" onClick={onOpenEagle}>
                                    <Database size={15} aria-hidden="true" />打开 Eagle 素材
                                </button>
                            </div>
                        </header>

                        <ol className="knowledge-lifecycle" aria-label="知识形成流程">
                            <li><span>1</span><div><strong>收集素材</strong><small>原图仍在 Eagle</small></div></li>
                            <li><span>2</span><div><strong>看图与提炼</strong><small>形成有来源的候选</small></div></li>
                            <li><span>3</span><div><strong>人工审核</strong><small>你决定是否可复用</small></div></li>
                            <li><span>4</span><div><strong>加入任务</strong><small>Agent 按用途引用</small></div></li>
                        </ol>

                        <div className="knowledge-overview__grid">
                            <article className="knowledge-overview-card knowledge-overview-card--metrics">
                                <div className="knowledge-overview-card__heading">
                                    <div><span className="knowledge-eyebrow">现在有什么</span><h2>知识状态</h2></div>
                                    <button type="button" className="knowledge-link-button" onClick={() => setSection('library')}>查看全部<ArrowRight size={14} aria-hidden="true" /></button>
                                </div>
                                <div className="knowledge-metric-grid">
                                    <button type="button" onClick={() => { setStatusFilter('active'); setSection('library'); }}><CheckCircle2 size={18} aria-hidden="true" /><strong>{counts.active}</strong><span>已验证知识</span></button>
                                    <button type="button" onClick={() => setSection('review')}><Clock3 size={18} aria-hidden="true" /><strong>{counts.review}</strong><span>等你审核</span></button>
                                    <div><Library size={18} aria-hidden="true" /><strong>{selectedReferences.length}</strong><span>当前消息引用</span></div>
                                </div>
                                <p className="knowledge-plain-note">只有“已验证知识”会进入默认知识检索。Eagle 搜索结果、网页内容和视觉分析都先是候选，不会自动升级。</p>
                            </article>

                            <article className="knowledge-overview-card knowledge-source-health">
                                <div className="knowledge-overview-card__heading">
                                    <div><span className="knowledge-eyebrow">Eagle 两条通道</span><h2>素材来源</h2></div>
                                    <button type="button" className="knowledge-icon-button" aria-label="重新检查 Eagle 实时连接" onClick={() => void refreshEagleConnection()} disabled={checkingEagleConnection}>
                                        <RefreshCw className={checkingEagleConnection ? 'is-spinning' : ''} size={15} aria-hidden="true" />
                                    </button>
                                </div>
                                <div className="knowledge-source-health__row">
                                    <span className="knowledge-source-health__icon"><Database size={16} aria-hidden="true" /></span>
                                    <div><strong>本地素材浏览</strong><p>在 Eagle 页面直接打开已选的 .library，不要求 Agent 实时检索连接。</p></div>
                                    <button type="button" className="knowledge-link-button" onClick={onOpenEagle}>去浏览<ArrowRight size={14} aria-hidden="true" /></button>
                                </div>
                                <div className={`knowledge-source-health__row is-${eagleConnection?.state || 'checking'}`}>
                                    <span className="knowledge-source-health__icon"><ScanSearch size={16} aria-hidden="true" /></span>
                                    <div><strong>{checkingEagleConnection ? '正在检查 Agent 全库检索…' : (eagleConnection?.title || '尚未检查 Agent 全库检索')}</strong><p>{eagleConnection?.detail || '此通道需要运行中的 Eagle 4.0+ MCP Server；不可用时不会阻断普通设计任务。'}</p></div>
                                </div>
                            </article>

                            <article className="knowledge-overview-card knowledge-current-task">
                                <div className="knowledge-overview-card__heading">
                                    <div><span className="knowledge-eyebrow">Message context</span><h2>当前消息会带什么</h2></div>
                                    {selectedReferences.length > 0 && <span className="knowledge-soft-badge">{selectedReferences.length} / 5</span>}
                                </div>
                                {selectedReferences.length === 0 ? (
                                    <div className="knowledge-overview-empty">
                                        <Library size={20} aria-hidden="true" />
                                        <div><strong>当前消息还没有知识引用</strong><p>只有你主动插入的内容才会随这条消息发送。Agent 自己检索的候选会另外标明来源，不会冒充正式知识。</p></div>
                                        <button type="button" className="knowledge-link-button" onClick={() => setSection('library')}>去选择<ArrowRight size={14} aria-hidden="true" /></button>
                                    </div>
                                ) : (
                                    <div className="knowledge-current-task__list">
                                        {selectedReferences.map((reference) => (
                                            <div key={reference.bindingRef}>
                                                <span className="knowledge-reference-role">{KNOWLEDGE_REFERENCE_USE_ROLES[reference.useRole || 'general'].label}</span>
                                                <strong>{reference.title}</strong>
                                                <button type="button" aria-label={`移除知识引用：${reference.title}`} onClick={() => onRemoveReference(reference.bindingRef)}><X size={14} aria-hidden="true" /></button>
                                            </div>
                                        ))}
                                    </div>
                                )}
                            ...
                            <article className="knowledge-overview-card knowledge-recent-use">
                                <div className="knowledge-overview-card__heading">
                                    <div><span className="knowledge-eyebrow">Recently used</span><h2>最近用过</h2></div>
                                </div>
                                {recentlyUsedItems.length === 0 ? (
                                    <div className="knowledge-overview-empty is-compact"><History size={19} aria-hidden="true" /><div><strong>还没有使用记录</strong><p>把知识加入任务后，这里会显示最近使用的判断。</p></div></div>
                                ) : (
                                    <ul className="knowledge-recent-use__list">
                                        {recentlyUsedItems.map((item) => <li key={item.id}><div><strong>{item.title}</strong><span>{formatMemoryKind(item.kind)} · {item.usageCount || 0} 次</span></div><time>{formatDate(item.lastUsedAt)}</time></li>)}
                                    </ul>
                                )}
                            </article>
                        </div>
                ...
                {section === 'library' && (
                    <section className="knowledge-assets" aria-labelledby="knowledge-assets-title">
                        <header className="knowledge-assets__header">
                            <div><span className="knowledge-eyebrow">Verified knowledge</span><h1 id="knowledge-assets-title">可用知识</h1><p>查找已经验证的设计判断，或按需搜索 Eagle / Web 候选。选择用途后，引用会插入右侧对话的当前光标位置。</p></div>
                        ...
                        {selectedReferences.length > 0 && (
                            <div className="knowledge-reference-tray" aria-label="当前消息知识引用">
                                <div><strong>准备插入当前消息</strong><span>{selectedReferences.length} / 5 条；这些知识会带着明确用途交给 Agent。</span></div>
                        ...
                        <div className="knowledge-assets__body">
                            <aside className="knowledge-source-rail" aria-label="知识来源筛选">
                                ...
                                <div className="knowledge-source-rail__note"><SlidersHorizontal size={14} aria-hidden="true" /><span>Eagle 和 Web 结果只是候选。只有你明确插入当前消息时才会成为对话引用。</span></div>
                            </aside>
                            ...
                            <div className="knowledge-result-area" aria-busy={searching}>
                                ...
                                {(visibleManagedItems.length > 0 || visibleSearchResults.length > 0) ? (
                                    ...
                                    <div className="knowledge-empty-state knowledge-empty-state--large"><BookOpen size={24} aria-hidden="true" /><strong>{query ? '没有符合条件的知识' : '还没有当前可用的长期知识'}</strong><span>{query ? '调整关键词或来源；如果只想看本地素材，请去 Eagle 页面。' : '可以先从 Eagle 找参考并执行“视觉理解”，候选经你审核后才会出现在这里。'}</span></div>
                                )}
...
function ManagedKnowledgeCard({ item, selectedReferences, onAddReference, onOpenLearning, onRevise, onRetire, onRestore }: {
    ...
    return (
        <article className={`knowledge-card knowledge-card--${item.status}`}>
            ...
            <dl className="knowledge-card__meta"><div><dt>适用范围</dt><dd>{formatScope(item.scope)}</dd></div><div><dt>最近使用</dt><dd>{item.lastUsedAt ? formatDate(item.lastUsedAt) : '尚未使用'}</dd></div><div><dt>累计使用</dt><dd>{item.usageCount || 0} 次</dd></div><div><dt>当前版本</dt><dd>第 {item.revision || 1} 版</dd></div></dl>
            {humanizeMemorySources(item).length > 0 && <details className="knowledge-card__details"><summary>来源与审核记录</summary><ul>{humanizeMemorySources(item).map((note) => <li key={note}>{note}</li>)}</ul></details>}
            ...
            <div className="knowledge-card__actions">
                {item.status === 'active' && result && <button type="button" className="is-primary" disabled={selected} onClick={() => onAddReference(result, item.learnedInsights)}><Plus size={14} aria-hidden="true" />{selected ? '已插入对话' : '插入对话'}</button>}
                {item.status === 'active' && <button type="button" onClick={onRevise}><PencilLine size={14} aria-hidden="true" />发布修订</button>}
                {item.status === 'active' && <button type="button" className="is-danger" onClick={onRetire}><Trash2 size={14} aria-hidden="true" />停止使用</button>}
...
function SearchKnowledgeCard({ result, selectedReferences, analysisState, previewState, onAddReference, onAnalyze, onPreview, onClearPreview, onOpenLearning, onRetire }: {
    ...
    const isBuiltIn = result.sourceType === 'local_recipe' || result.sourceType === 'manual_rule';
    const canPinToTask = result.allowedUses.includes('user_reference');
    const visibleTags = getVisibleKnowledgeTags(result.tags);
    return (
        <article className={`knowledge-card knowledge-card--search knowledge-card--${freshness}`}>
            ...
            <p className="knowledge-card__summary">{summarizeKnowledgeResult(result)}</p>
            {visibleTags.length > 0 && <div className="knowledge-card__tags">{visibleTags.map((tag) => <span key={tag}>{tag}</span>)}</div>}
            <dl className="knowledge-card__meta"><div><dt>使用方式</dt><dd>{canPinToTask ? '可固定到任务' : (isBuiltIn ? 'Agent 按需取用' : '审核后可用')}</dd></div><div><dt>来源类型</dt><dd>{sourceLevelLabel(result.sourceLevel)}</dd></div><div><dt>更新时间</dt><dd>{formatDate(result.updatedAt)}</dd></div></dl>
            {result.sourceNotes.length > 0 && <details className="knowledge-card__details"><summary>查看来源说明</summary><ul>{result.sourceNotes.slice(0, 4).map((note) => <li key={note}>{humanizeResultSourceNote(note)}</li>)}</ul></details>}
            {isEagle && <div className="knowledge-card__reason"><ShieldAlert size={14} aria-hidden="true" />现在只有标题、标签等元数据；点击“看图并提炼”后才会形成视觉候选。</div>}
            ...
            <div className="knowledge-card__actions">
                {canPinToTask && <button type="button" className="is-primary" disabled={selected || freshness !== 'current'} onClick={() => onAddReference(result)}><Plus size={14} aria-hidden="true" />{selected ? '已插入对话' : '插入对话'}</button>}
                {!canPinToTask && isBuiltIn && <span className="knowledge-card__muted"><ShieldCheck size={14} aria-hidden="true" />系统会在相关设计任务中按需提供</span>}
                {!canPinToTask && !isBuiltIn && <span className="knowledge-card__muted"><Clock3 size={14} aria-hidden="true" />先审核，再用于正式任务</span>}
                ...
                {isEagle && analysisState.status !== 'queued' && <button type="button" disabled={analysisState.status === 'running'} onClick={onAnalyze}><ScanSearch className={analysisState.status === 'running' ? 'is-spinning' : ''} size={14} aria-hidden="true" />{analysisState.status === 'running' ? '正在看图' : '看图并提炼'}</button>}
                ...
                <button type="button" className="is-danger" onClick={onRetire}><Trash2 size={14} aria-hidden="true" />不再显示</button>
            </div>
        </article>
    );
}
...
function summarizeKnowledgeResult(result: DesignKnowledgeResult): string {
    const normalized = String(result.summary || '')
        .replace(/[\r\n\t]+/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
    if (!normalized) return '这条知识暂时没有可显示的说明。';
    if (result.sourceType === 'eagle_library') {
        return truncatePlainText(normalized
            .replace(/^Tags:\s*/i, '标签：')
            .replace(/\s*\|\s*Size:\s*/i, '；尺寸：')
            .replace(/\s*\|\s*Format:\s*/i, '；格式：')
            .replace(/(\d+)x(\d+)/i, '$1 × $2'), 260);
    }

    const preferredSections = [
        { start: '视觉意图：', ends: ['选择边界：', '适用条件：', '不适用条件：'] },
        { start: '核心判断：', ends: ['适用条件：', '不适用条件：', '来源：'] },
        { start: '设计目标：', ends: ['适用条件：', '不适用条件：', '选择边界：'] },
        { start: '原则：', ends: ['适用条件：', '不适用条件：', '来源：'] }
    ];
    for (const section of preferredSections) {
        const startIndex = normalized.indexOf(section.start);
        if (startIndex < 0) continue;
        const contentStart = startIndex + section.start.length;
        const endIndexes = section.ends
            .map((marker) => normalized.indexOf(marker, contentStart))
            .filter((index) => index >= 0);
        const contentEnd = endIndexes.length > 0 ? Math.min(...endIndexes) : normalized.length;
        const excerpt = normalized.slice(contentStart, contentEnd).trim();
        if (excerpt) return truncatePlainText(excerpt, 260);
    }

    const withoutTechnicalPrefix = normalized
        .replace(/^【[^】]+】\s*/, '')
        .replace(/\b(?:recipeId|version|sourceRevision|lifecycle|evaluationProfileId)=[^；;]+[；;]?\s*/gi, '')
        .trim();
    return truncatePlainText(withoutTechnicalPrefix || normalized, 260);
}

function getVisibleKnowledgeTags(tags: readonly string[]): string[] {
    return Array.from(new Set(tags
        .map((tag) => String(tag || '').trim())
        .filter((tag) => tag.length > 0 && tag.length <= 24)
        .filter((tag) => !/\.v\d+(?:\.|$)/i.test(tag))
        .filter((tag) => !/^(?:photoshop|design|ecommerce|runtime|knowledge|recipe|skill)[-._]/i.test(tag))
        .filter((tag) => !/[a-z][A-Z]/.test(tag))
        .filter((tag) => !/^(?:eagle|eagle_library|readonly|local-case|jpg|jpeg|png|web|external)$/i.test(tag))
        .filter((tag) => !/[=/\\]/.test(tag))))
        .slice(0, 5);
}
...
function humanizeMemorySources(item: DesignMemoryItem): string[] {
    return Array.from(new Set(item.sourceNotes.flatMap((entry) => {
        if (entry.source === 'interactive-card-confirmation') return ['由你在任务中明确确认'];
        if (entry.source === 'knowledge-library-revision') return ['由你在知识库发布修订'];
        if (entry.source === 'knowledge-library-lifecycle') return [];
        if (entry.source.includes('learning') || entry.source.includes('review')) return ['由学习候选经人工审核'];
        if (entry.source.includes('eagle')) return ['来自 Eagle 视觉参考'];
        if (entry.source.includes('benchmark')) return ['来自已审核基准案例'];
        if (entry.source.includes('import')) return ['来自人工导入案例'];
        return entry.summary ? [entry.summary] : [];
    }).filter(Boolean))).slice(0, 5);
}

function humanizeResultSourceNote(value: string): string {
    const normalized = String(value || '')
        .replace(/\b(?:revision|sourceRevision|contentFingerprint|retrievedAt|publishedAt|expiresAt)=[^;]+;?/gi, '')
        .replace(/[_-]+/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
    if (!normalized) return '来源信息已记录';
    if (/^来源修订[:：]/i.test(normalized)) return '知识来源版本已记录';
    if (/^生命周期[:：]\s*active$/i.test(normalized)) return '当前版本正在使用';
    if (/^生命周期[:：]/i.test(normalized)) return '版本状态已记录';
    return normalized;
}
