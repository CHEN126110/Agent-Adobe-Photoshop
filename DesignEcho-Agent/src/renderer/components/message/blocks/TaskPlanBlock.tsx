...
function resolveStepPresentation(status: AgentTaskPlanPresentationStepStatus): StepPresentation {
    switch (status) {
        ...
        case 'running':
            return { Icon: LoaderCircle, label: '正在制作' };
        case 'failed':
            return { Icon: CircleX, label: '这一步还没完成' };
        case 'blocked':
            return { Icon: CircleMinus, label: '等待前一步完成' };
        case 'pending':
        default:
            return { Icon: Circle, label: '稍后进行' };
    }
}
...
export function TaskPlanBlock({ block }: TaskPlanBlockProps): React.ReactElement {
    ...
    return (
        <section
            ...
            aria-label={`设计进度：${presentation.goal}`}
            aria-live="polite"
        >
            <div className="task-plan-heading">
                <span className="task-plan-heading-label">设计进度</span>
