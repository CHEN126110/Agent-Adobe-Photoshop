...
interface EagleLibraryPageProps {
    isActive: boolean;
    /** 只有显式添加动作才会进入对话；普通浏览选择始终留在 Eagle 页面内部。 */
    onAddToConversation: (assetRefs: EagleAssetRef[]) => void;
}
...
interface EagleContextMenuState {
...
function AssetCard({
    ...
    return (
        <button
            ...
            type="button"
            draggable
            className={`eagle-item-card ${selected ? 'selected' : ''} ${primary ? 'primary' : ''} ${dragging ? 'dragging' : ''}`}
...
export const EagleLibraryPage: React.FC<EagleLibraryPageProps> = ({
    ...
    const [contextMenu, setContextMenu] = useState<EagleContextMenuState | null>(null);
    ...
    const contextMenuFirstActionRef = useRef<HTMLButtonElement>(null);
    ...
    useLayoutEffect(() => {
        ...
        const nextX = Math.max(8, Math.min(contextMenu.x, pageRect.width - menuRect.width - 8));
        const nextY = Math.max(8, Math.min(contextMenu.y, pageRect.height - menuRect.height - 8));
        if (nextX !== contextMenu.x || nextY !== contextMenu.y) {
            setContextMenu((current) => current ? { ...current, x: nextX, y: nextY } : current);
            return;
        }
        contextMenuFirstActionRef.current?.focus({ preventScroll: true });
    }, [contextMenu]);
    ...
    }, [activeFilterPanel, contextMenu, isActive, items, itemsPerRow, previewItem, selectedIds.size, selectedItem]);
    ...
    function buildConversationAssetRefs(sourceItems: EagleLibraryItem[]): EagleAssetRef[] {
        if (!library) return [];
        return sourceItems
    ...
    function resolveInteractionItems(item: EagleLibraryItem): EagleLibraryItem[] {
        return selectedIds.has(item.id) && selectedItems.length > 0 ? selectedItems : [item];
    }

    function addAssetRefsToConversation(assetRefs: EagleAssetRef[]): void {
        if (assetRefs.length === 0) return;
        onAddToConversation(assetRefs);
        setContextMenu(null);
    }

    function addCurrentSelectionToConversation(): void {
        addAssetRefsToConversation(buildConversationAssetRefs(selectedItems));
    }
    ...
    function handleAssetDragStart(
        ...
        const assetRefs = buildConversationAssetRefs(resolveInteractionItems(item));
        if (assetRefs.length === 0) {
            event.preventDefault();
            return;
        }
        const serializedPayload = serializeEagleComposerDragPayload(assetRefs);
        if (!serializedPayload) {
            event.preventDefault();
            return;
        }
        event.dataTransfer.effectAllowed = 'copy';
        event.dataTransfer.setData(EAGLE_COMPOSER_DRAG_MIME, serializedPayload);
        setDraggingIds(new Set(assetRefs.map((assetRef) => assetRef.itemId)));
        setContextMenu(null);
    }

    function handleAssetDragEnd(): void {
        setDraggingIds(new Set());
    }

    function openAssetContextMenu(item: EagleLibraryItem, clientX: number, clientY: number): void {
        const pageRect = pageRef.current?.getBoundingClientRect();
        if (!pageRect) return;
        setContextMenu({
            item,
            assetRefs: buildConversationAssetRefs(resolveInteractionItems(item)),
    ...
    function handleAssetContextMenu(
        ...
        openAssetContextMenu(item, event.clientX, event.clientY);
    }
    ...
    function handleAssetCardKeyDown(
        ...
        openAssetContextMenu(item, rect.left + Math.min(28, rect.width / 2), rect.top + Math.min(28, rect.height / 2));
    }

    function closeContextMenu(restoreCardFocus = false): void {
        const itemId = contextMenu?.item.id;
        setContextMenu(null);
        if (!restoreCardFocus || !itemId) return;
        window.requestAnimationFrame(() => itemRefs.current.get(itemId)?.focus({ preventScroll: true }));
    }

    function handleContextMenuKeyDown(event: React.KeyboardEvent<HTMLDivElement>): void {
        if (event.key === 'Escape') {
            event.preventDefault();
            event.stopPropagation();
            closeContextMenu(true);
        ...
        else if (event.key === 'ArrowDown') nextIndex = currentIndex < 0 ? 0 : (currentIndex + 1) % actions.length;
        else nextIndex = currentIndex <= 0 ? actions.length - 1 : currentIndex - 1;
    ...
    return (
        <section ref={pageRef} className={pageClasses} aria-label="Eagle 素材库">
                                        ...
                                        <div className="eagle-justified-row" key={`${row.entries[0]?.item.id || rowIndex}:${row.entries.length}`} style={{ gap: `${JUSTIFIED_GAP}px` }}>
                                            {row.entries.map(({ item, width }) => (
                                                <AssetCard
                                                    ...
                                                    contextMenuOpen={contextMenu?.item.id === item.id}
                                                    cardRef={(node) => {
                                                    ...
                                                    onDragStart={(event) => handleAssetDragStart(item, event)}
                                                    onDragEnd={handleAssetDragEnd}
                                                    onContextMenu={(event) => handleAssetContextMenu(item, event)}
                                                    onKeyDown={(event) => handleAssetCardKeyDown(item, event)}
                                                />
                                            ))}
                                        </div>
                                    ))}
            ...
            <aside className="eagle-inspector" aria-label="素材信息">
                {selectedIds.size > 1 ? (
                    <div className="eagle-multi-selection">
                        ...
                        <span>
                            当前只是素材库浏览选择，不会自动进入对话。
                            可拖到右侧输入框，或使用右键菜单添加
                            {selectedIds.size > EAGLE_ASSET_GROUP_LIMIT ? `；一次最多添加前 ${EAGLE_ASSET_GROUP_LIMIT} 项` : ''}。
                        </span>
                        <div className="eagle-multi-selection-actions">
                            <button type="button" className="is-primary" onClick={addCurrentSelectionToConversation}><Plus size={13} />添加到对话</button>
                    ...
                    <>
                        ...
                        <div className="eagle-inspector-scroll">
                            ...
                            <button type="button" className="eagle-inspector-use" onClick={addCurrentSelectionToConversation}><Plus size={14} />添加到对话</button>
                            <div className="eagle-source-note">
                                <span>当前仅在素材库中选中</span>
                                <small>单击只更新这里的素材信息；拖拽、右键或上方按钮才会添加到对话。</small>
                            </div>
                        </div>
                    </>
                ) : (
                    <div className="eagle-inspector-empty"><FileImage size={24} /><span>选择一个素材查看信息</span><small>单击只查看；拖到右侧输入框或右键添加到对话。Ctrl 或 Shift 可多选。</small></div>
                )}
            </aside>
            ...
            {contextMenu && (
                <div
                    ref={contextMenuRef}
                    className="eagle-asset-context-menu"
                    role="menu"
                ...
                >
                    <button
                        ...
                        type="button"
                        role="menuitem"
                        onClick={() => addAssetRefsToConversation(contextMenu.assetRefs)}
                    >
                        <Plus size={14} aria-hidden="true" />
                        {contextMenu.assetRefs.length > 1 ? `添加 ${contextMenu.assetRefs.length} 项到对话` : '添加到对话'}
                    </button>
                    <button
                        type="button"
                        role="menuitem"
                    ...
                    >
                        <Maximize2 size={14} aria-hidden="true" />打开大图预览
                    </button>
                </div>
            )}
