...
export function KnowledgeSourceManagementPanel(): React.ReactElement {
    ...
    // 判断原生联网搜索能不能用，要看真正执行任务的主模型；
    // orchestrator.primaryModel 不参与执行（详见 SettingsModal 同名变量的说明）。
    const selectedModel = getModelById(
        modelPreferences.primaryModel || modelPreferences.preferredCloudModels.layoutAnalysis
    );
    ...
    return (
        <section className="knowledge-panel" data-testid="knowledge-source-management-panel">
            ...
            <div className="knowledge-source-overview">
                ...
                {/* 「当前主模型 / 工具流」卡已撤：与来源治理无关，模型信息在输入栏选择器里已有。
                    与本页真正相关的「原生联网搜索支不支持」下沉到了 Web Search 那张卡的标题旁。 */}
            </div>
            ...
            <article className="knowledge-source-card">
                <div className="knowledge-source-card__heading">
                    ...
                    <div>
                        <h3>
                            小米官方 Web Search
                            {/* 支持与否由当前主模型的 provider 决定，标在标题旁而不是单独一张卡 */}
                            <span className={`knowledge-inline-badge ${runtimeCapability.providerNativeWebSearch.status === 'ready' ? 'is-ready' : 'is-idle'}`}>
                                {runtimeCapability.providerNativeWebSearch.status === 'ready' ? '当前主模型支持' : '当前主模型不支持'}
                            </span>
                        </h3>
                        <p>模型原生联网搜索，只在支持的模型与提供方上进入请求计划。</p>
                    </div>
