...
import { buildSkillRoutingRecommendation } from '../../../shared/skill-routing';
import {
    getSkillById,
...
export function buildAutonomousSkillParams(
    ...
    // 普通自然语言不经过前置 Router。Harness 这里只采信当前用户消息中 deny-only
    // 的显式边界，并在 Capability/Tool 执行点再次强制；不会根据任务措辞补造 Skill 身份。
...
async function executeSkillWithLifecycle(
    ...
    options: {
        ...
        warnings?: string[];
        observations?: Array<{ source: string; summary: string }>;
        /** 仅由结构化 continuation / 受控 owner 显式签发；普通自然语言保持 Agent 自主裁量。 */
