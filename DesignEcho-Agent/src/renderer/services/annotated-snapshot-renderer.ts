...
export async function renderAnnotatedSnapshot(input: {
    ...
    /**
     * UXP 截图相对文档的缩放，仅作诊断记录。
     * 注意：UXP 端返回的 layer.bounds **已经**是截图坐标系
     * （get-annotated-snapshot.ts 里按 (bounds.left - viewX) * scale 换算过），
     * 因此绘制时 **不能再乘一次 scale** —— 那会把所有框缩到左上角、与画面完全对不上
     * （真机表现：标注框看起来全都不准）。
     */
    scale: number;
    ...
    for (const layer of input.layers) {
        ...
        // bounds 已是截图坐标系，直接用（再乘 scale 会造成双重缩放）。
