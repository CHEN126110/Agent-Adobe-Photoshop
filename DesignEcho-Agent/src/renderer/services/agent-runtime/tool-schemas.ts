    ...
    },
    {
        ...
        inputSchema: objectSchema({
            ...
            marginScale: {
                type: 'integer',
                enum: [0, 1, 2, 3, 4, 5],
                description: '版心边距的**档位下标**，0 最窄、5 最宽，省略取 4。留白多少是设计判断：紧凑信息版面可选低档，需要呼吸感的画面选高档。'
            },
            gutterScale: {
                type: 'integer',
                enum: [0, 1, 2, 3, 4, 5],
                description: '列与列之间的间距档位下标，省略取 2。仅在声明了 columns 时有意义。'
            },
            gapScale: {
                type: 'integer',
                enum: [0, 1, 2, 3, 4, 5],
                description: '仅 blocks 模式生效：模块之间垂直间距的**档位下标**，0 最紧凑、5 最松，省略取 3。间距只能选档位不能给像素——同一版面反复使用同几档间距，是版式看起来成体系的直接原因。返回结果的 grid.spacingScale 会给出本次各档位的实际像素值。'
            },
            blocks: {
                type: 'array',
                items: {
