...
function collectVisualReviewCounts(
    ...
    for (const item of effectiveCaptured) {
        ...
        const legacyCapturedCount = Math.max(1, observations.length);
        const legacyReviewed = observations.length > 0
            ? observations.filter((observation) => observation.reviewed === true).length
            : (readAgentVisualObservation(item.result)?.reviewed === true ? 1 : 0);
        expectedCount += legacyCapturedCount;
        capturedCount += legacyCapturedCount;
        reviewedCount += legacyReviewed;
        passedCount += legacyReviewed;
    }
...
function collectLatestRenderLayoutQualityState(
    ...
    if (layoutLogIndex < 0 || !layoutResult) return undefined;
    ...
    for (const finding of findings) {
        ...
        if (closureKind === 'visual') {
            const visual = collectVisualReviewCounts(toolCallLog, DESIGN_REVIEW_TOOLS, layoutLogIndex);
            reviewedObservationCount += visual.reviewedCount;
            if (visual.reviewedCount > 0) verifiedClosureCount += 1;
        ...
        const visual = collectVisualReviewCounts(toolCallLog, DESIGN_REVIEW_TOOLS, actionIndex);
        reviewedObservationCount += visual.reviewedCount;
        if (visual.reviewedCount > 0) verifiedClosureCount += 1;
        else unresolvedFindingCount += 1;
    }
    ...
    if (findings.length === 0 && qualityState === 'needs_review') {
        const visual = collectVisualReviewCounts(toolCallLog, DESIGN_REVIEW_TOOLS, layoutLogIndex);
        reviewedObservationCount += visual.reviewedCount;
        if (visual.reviewedCount === 0) unresolvedFindingCount += 1;
    }
