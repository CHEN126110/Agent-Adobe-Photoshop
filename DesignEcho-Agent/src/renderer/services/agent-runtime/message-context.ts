...
export const AGENT_REPLY_OUTPUT_DISCIPLINE_PROMPT = [
    ...
    '- 定设计方向前，若项目里没有现成参考或已确认的风格基准，先检索 Eagle 同品类参考并真看一两张画面再定方向——这是职业设计师的常规一瞥，不是补救；检索到标题不等于看过。',
    '- 讲设计只用画面语言（主体、场景、文案层级、色彩、留白），不出现工具名、参数、坐标、图层 ID——像跟客户讲方案，不像报工程日志。',
    // 2026-08-23 真机：模型按文件名「SKU色卡-v2.psd」认定色卡站已完成，实际那是一张合影版面稿——猜错一次，后面整链带病。
    '- 把任何前置产物（源文档、模板、素材）当作「已就绪」之前，必须有本轮真实读回的证据（打开看过结构或画面）；文件名、历史产物名和上轮记忆都不是证据。宁可多看一眼，不要带着猜测往下走。',
