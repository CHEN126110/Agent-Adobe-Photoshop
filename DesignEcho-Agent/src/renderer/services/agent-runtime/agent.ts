...
// 终局审美 Judge 只允许一次，并从普通模型/视觉/候选预算中事前保留；不是额外无限额度。
const MAX_FINAL_QUALITY_JUDGE_CALLS = 1;
...
export class Agent {
    ...
    /** 从普通 performance budget 预留的一次性终局 Judge 槽；不建立第二预算 owner。 */
    private finalQualityJudgeCallCount = 0;
    private performanceVisionCandidateCount = 0;
    /** 同一 observationKey 在主循环与 R5 终局 Judge 间复用时只计一个视觉候选。 */
    private performanceVisionCandidateKeys = new Set<string>();
    private performanceVisualAnalysisCount = 0;
    /** 已完成 generation 的累计活跃执行时长；不含 generation 之间的等待。 */
    private performanceActiveElapsedBeforeRunMs = 0;
    /** 当前 generation 的 Agent.run 起点。 */
    ...
    private resolveFinalQualityJudgeAssertions(): DesignAssertion[] {
        const evaluationProfile = this.resolveRuntimeEvaluationProfile();
        // Reserve 是对已发布 Runtime 评价义务的资源承诺，不是从自然语言/旧完成契约猜出的
        // 通用创意税。plan-neutral 启动阶段没有 Profile，也没有可执行的 ReviewSet 义务，
        // 此时提前扣 1 次模型、1 个视觉候选和 90 秒会把身份声明本身饿死。
        if (!evaluationProfile || !this.runtimeSession || !this.config.runtimeStagePlan) return [];
        return getDesignEvaluationProfileVlmAssertions(evaluationProfile);
    }

    private shouldReserveFinalQualityJudgeBudget(): boolean {
        if (this.finalQualityJudgeCallCount >= MAX_FINAL_QUALITY_JUDGE_CALLS) return false;
        const budget = this.config.performanceBudget;
        if (!budget
            || budget.maxModelCalls < 1
            || budget.maxVisualAnalyses < 1
            || budget.maxVisionCandidates < 1
            || budget.softTimeBudgetMs <= AGENT_FINALIZATION_TIME_RESERVE_MS) {
            return false;
        }
        ...
        if (!canAttemptModelVision(primaryModel) && !canAttemptModelVision(expertModel)) return false;
        return this.resolveFinalQualityJudgeAssertions().length > 0;
    }
    ...
    private buildPerformanceBudgetExhaustionMessage(
        ...
        const outcome = this.hasObservedTaskMutation()
    ...
    private readPerformanceBudgetExhaustion(
        ...
        const elapsedMs = this.readPerformanceActiveElapsedMs();
        const reservesFinalQualityJudge = budgetClass === 'task'
            && this.shouldReserveFinalQualityJudgeBudget();
        const effectiveSoftTimeBudgetMs = reservesFinalQualityJudge
            ? Math.max(0, budget.softTimeBudgetMs - AGENT_FINALIZATION_TIME_RESERVE_MS)
            : budget.softTimeBudgetMs;
        if (effectiveSoftTimeBudgetMs >= 0
            && this.performanceRunStartedAtMs > 0
            && elapsedMs >= effectiveSoftTimeBudgetMs) {
            return {
                dimension: 'soft_time',
                code: 'agent_soft_time_budget_exhausted',
                message: this.buildPerformanceBudgetExhaustionMessage(
                    'soft_time',
        ...
        const effectiveModelCallLimit = reservesFinalQualityJudge
            ? Math.max(0, budget.maxModelCalls - 1)
            : budget.maxModelCalls;
        if (scope !== 'tool'
            && effectiveModelCallLimit >= 0
            && this.performanceModelCallCount >= effectiveModelCallLimit) {
            return {
                ...
                message: this.buildPerformanceBudgetExhaustionMessage(
                    ...
                    this.performanceModelCallCount
                ),
    ...
    private getPerformanceVisionCandidateLimit(): number {
        ...
        if (!this.shouldReserveFinalQualityJudgeBudget()) return hardLimit;
        ...
        return resolveVisionCandidateLimitForFinalQuality({
            hardLimit,
    ...
    private hasPerformanceVisualAnalysisCapacity(
        ...
        const hardLimit = Math.max(0, Math.floor(configured));
        const effectiveLimit = budgetClass === 'task' && this.shouldReserveFinalQualityJudgeBudget()
            ? Math.max(0, hardLimit - 1)
            : hardLimit;
        return this.performanceVisualAnalysisCount < effectiveLimit;
    }
    ...
    private synchronizeRuntimePerformanceUsage(): void {
        ...
        this.runtimeSession = recordRuntimeSessionPerformanceUsage({
            ...
            usage: {
                ...
                visionCandidates: this.performanceVisionCandidateCount,
                visualAnalyses: this.performanceVisualAnalysisCount,
                activeElapsedMs: this.readPerformanceActiveElapsedMs(),
                observationKeys: Array.from(this.performanceVisionCandidateKeys)
            }
        });
    }

    private readPerformanceActiveElapsedMs(nowMs = Date.now()): number {
        const currentGenerationElapsedMs = this.performanceRunStartedAtMs > 0
            ? Math.max(0, Math.floor(nowMs - this.performanceRunStartedAtMs))
            : 0;
        return Math.max(
            0,
            Math.floor(this.performanceActiveElapsedBeforeRunMs + currentGenerationElapsedMs)
        );
    }
    ...
    private consumePerformanceVisionCandidate(observationKey?: string): boolean {
        ...
        if (normalizedKey) this.performanceVisionCandidateKeys.add(normalizedKey);
        this.synchronizeRuntimePerformanceUsage();
        return true;
    }
    ...
    private beginPerformanceModelCall(
        ...
        if (budgetClass === 'final_quality_judge'
            && this.finalQualityJudgeCallCount >= MAX_FINAL_QUALITY_JUDGE_CALLS) {
        ...
        const directVisionCharge = resolveDirectVisionCandidateCharge({
            ...
            billedObservationKeys: this.performanceVisionCandidateKeys,
        ...
        if (billedDirectVisionCandidateCount > 0) {
            ...
            if (this.performanceVisionCandidateCount + billedDirectVisionCandidateCount > candidateLimit) {
        ...
        if (billedDirectVisionCandidateCount > 0) {
            this.performanceVisionCandidateCount += billedDirectVisionCandidateCount;
        }
        for (const key of directVisionCharge.normalizedObservationKeys) {
            this.performanceVisionCandidateKeys.add(key);
        }
        if (budgetClass === 'final_quality_judge') this.finalQualityJudgeCallCount += 1;
        this.synchronizeRuntimePerformanceUsage();
    ...
    activateRuntimeContractFromDeclaration(input: {
        ...
        for (let index = 0; index < this.performanceModelCallCount; index += 1) {
    ...
    private commitDesignTeamChildAllowance(
        ...
        this.performanceModelCallCount = Math.max(
            recorded.modelCalls,
            this.performanceModelCallCount
        ) + allowance.maxModelCalls;
        this.performanceToolCallCount = Math.max(
            recorded.toolCalls,
            this.performanceToolCallCount
        ) + allowance.maxToolCalls;
        this.iteration = Math.max(recorded.iterations, this.iteration + 1)
            + allowance.maxModelCalls;
        this.performanceVisionCandidateCount = Math.max(
            recorded.visionCandidates,
            this.performanceVisionCandidateCount
        ) + allowance.maxVisionCandidates;
        this.performanceVisualAnalysisCount = Math.max(
            recorded.visualAnalyses,
            this.performanceVisualAnalysisCount
    ...
    async run(task: string, images?: ImageAttachment[]): Promise<AgentRunResult> {
        ...
        this.finalQualityJudgeCallCount = 0;
        this.performanceVisionCandidateCount = 0;
        this.performanceVisionCandidateKeys.clear();
        this.performanceVisualAnalysisCount = 0;
        this.performanceActiveElapsedBeforeRunMs = 0;
        ...
        if (this.config.runtimeSessionSeed && this.runtimeSession) {
            ...
            this.performanceActiveElapsedBeforeRunMs = Math.max(
                this.performanceActiveElapsedBeforeRunMs,
                performanceUsage.activeElapsedMs
            );
            this.performanceVisionCandidateKeys = new Set(performanceUsage.observationKeys);
        }
                    ...
                    if (pendingConfirmationCards.length > 0) {
                        ...
                        if (pendingInteractiveContinuation && this.runtimeSession) {
                            const suspension = suspendRuntimeSessionForInteraction({
                                ...
                                expectedRevision: this.runtimeSession.taskRun.documentBinding?.expectedRevision
                            });
        ...
        for (let i = 0; i < toolResults.length; i++) {
            ...
            if (trustedArtifact) {
                ...
                for (const observationKey of trustedArtifact.reviewedObservationKeys) {
                    this.performanceVisionCandidateKeys.add(observationKey);
                }
                this.synchronizeRuntimePerformanceUsage();
                this.messages.push(createRuntimeObservationMessage(
                    ...
                    contentBlocks: [
                        {
                            type: 'text',
                            text: [
                                `（${toolName} 返回的画布图像，供你核对实际状态；本次运行视觉候选 ${this.performanceVisionCandidateCount}/${visionCandidateLimit}）`,
    ...
    private resolveTaskPlanObligationGap(): 'task_progress_missing' | 'delivery_action_missing' | undefined {
        ...
        const hasAttemptedDeliveryAction = this.toolCallLog.some((entry) => {
    ...
    private async applyLoopGuardLivenessRecovery(input: {
        ...
        if (hasNextIteration && pendingRecoveryToolNames.length === 0) {
            ...
            const stageNeedsInput = this.resolveRuntimeStageNeedsInputRecovery();
            const baseProgressToolNames = this.selectRuntimeStageProgressToolNames(visibleTools);
        ...
        if (livenessDecision.reason === 'alternative_capability') {
            this.queueRecovery({
                ...
                reason: '当前路径没有推进任务，下一轮改用同一 Runtime 阶段尚未尝试的兼容能力。'
            });
            this.messages.push(createHarnessControlMessage([
    ...
    private resolveRequiredToolRecovery(toolResults: ToolResult[]): { toolNames: string[]; isHandoff: boolean; reason: string; failedTools: string[] } | null {
        ...
        for (const result of [...toolResults].reverse()) {
            ...
            const toolNames = this.readRequiredToolNames(result.output)
    ...
    private buildToolResultFallbackMessage(): string {
        const stateSummary = this.buildSummaryFromStatefulWrites()
            || buildObservedDesignDraftSummary(this.toolCallLog);
        const outputPaths = Array.from(new Set(this.toolCallLog
            .map((entry) => this.readOutputPathFromToolResult(entry.result))
            .filter(Boolean)))
            .slice(0, 3);
        const hasViewableResult = this.hasObservedTaskMutation()
            || this.hasSuccessfulToolActivity('save_export');
    ...
    private async buildRunResult(input: {
        ...
        const hasNothingToAccept = executionSummary.status === 'completed'
            && !this.hasTaskProgressToolCalls();
        ...
        let rawVisibleMessage = userResultProjection.message;
    ...
    private async evaluateDesignQualityVlmAssertions(
        ...
        const remainingVisionCandidates = Math.max(
            0,
            this.getPerformanceVisionCandidateHardLimit() - this.performanceVisionCandidateCount
        );
