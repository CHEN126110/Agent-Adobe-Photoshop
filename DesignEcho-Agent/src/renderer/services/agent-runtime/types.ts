...
export interface AgentConfig {
    ...
    /**
     * 开场 Photoshop 观察由结构化 owner 决定，不从用户措辞猜：
    ...
    performanceBudget?: {
        ...
        /** cost_profile 派生的单次主模型输出硬上限。 */
        maxPrimaryOutputTokens?: number;
        /** false 表示该轻量任务不启用 provider thinking。 */
    ...
    /**
     * 用户已批准计划的请求级写工具白名单。字段存在时（包括空数组），
     * Photoshop 写入与导出只能命中该集合；只读工具不受影响。
     */
    runtimeWriteToolAllowlist?: string[];
    /** Engine 已签发的精确属性替换范围；只用于唯一目标/CAS 规范化和终局验收绑定。 */
     ...
     */
    runtimeStageContextItems?: readonly RuntimeContextItem[];
     ...
     */
    getDynamicOperatingContext?: () => string;
...
export interface AgentCallbacks {
    /** Model-authored public reasoning summary. Source metadata is mandatory; Provider thinking never enters this callback. */
...
export type AgentStopReason =
    ...
    // 卡在「只有用户能给的信息」上，已把问题说清楚并等待回答。这是正常暂停不是失败：
    // 用户回答后由普通对话进入下一轮，不需要额外的续跑机制。
    | 'awaiting_user_input'
    | 'cancelled'
    | 'error';
...
export interface AgentToolCallLogEntry {
    ...
    result: any;
    /**
     * 调用归属只用于审计和执行语义分流：Harness 确定性工作流入口、开工观察与质量版本复核
     * 都不能伪装成模型主动选择的工具；后两者也不能冒充业务写入或质量通过。
     */
    ...
    /**
     * 距本轮 run 起点（performanceLedger.runStartedAtMs）的毫秒数；
     * 只在时序账本已启动时写入，缺失代表旧记录或未开启时序——不作任何推断。
     * 用途：诊断口径「首次成功写入延迟」（收敛指标之一）。
     */
    elapsedMs?: number;
}
...
export interface TaskCompletionRequirement {
    ...
    actual?: unknown;
    /**
     * Completion 只在确定性事实缺口成立时携带硬失败资格。
     * 审美 finding、可选构成与缺少主观判断不得填写这些字段。
     */
    method?: 'deterministic';
    blockerKind?: DesignQualityBlockerKind;
    /** 指向当前 TaskRun 已校验事实的稳定 token，不保存路径、Prompt 或结果载荷。 */
    proofRef?: string;
}
...
export interface TaskCompletionContract {
    ...
    blockers: string[];
    warnings: string[];
    /** Profile 任务的双轴完成状态；发布审核不能反向改写产物完成状态。 */
...
export interface AgentExecutionSummary {
    ...
    /**
     * 完成观察门禁口径：最后一次写入之后、模型自己完成的文档/画面/图层读回次数。
     * 这是"改后有没有复核"的安全判据，Harness 自身的质量复核不计入；不代表质量通过。
     * 展示"这次看了多少"请用 observedToolCallCount。
     */
    successfulObservationCalls?: number;
    /**
     * 用户可见口径：本次运行成功的观察类工具总数（素材总览、参考分析、项目资源检索、
     * 画面读取…），与运行档案 activityClass==='observation' 同源。
     */
     ...
     * 已把"看似完成"降级为 needs_review。没有 Manifest 必需检查缺口时它仍是终态信号；
     * 若 Profile 明确缺少写后结构或全图复核，只允许回到对应核验阶段补读，不能重放原任务或重复 mutation。
    ...
    /** 用户可见结果摘要；只描述实际产出与当前是否有可看版本，不承载内部完成判定。 */
    userVisibleSummary?: string;
    /** 用户可见下一动作；只描述设计推进或用户选择，不承载 Runtime 恢复指令。 */
    userVisibleNextStep?: string;
    summaryText: string;
}
