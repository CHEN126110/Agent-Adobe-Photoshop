...
import { blocksDeterministicRouteByNegation } from '../../../shared/agent-negation-intent';
import {
    getSkillById,
...
const AGENT_MATTING_ATOMIC_TOOL_NAMES = new Set(['removeBackground', 'applyMattingResult']);
const SKU_PATTERNS = [
    /sku/i,
    /\u6279\u91cf\u914d\u8272/i,
    /\u6279\u91cf\u51fa\u56fe/i,
...
/** 抠图暂停只在能力/执行点约束这些原子动作，不再靠用户文本主题词短路整轮对话。 */
export function isAgentMattingAtomicTool(toolName: unknown): boolean {
    return AGENT_MATTING_ATOMIC_TOOL_NAMES.has(String(toolName || '').trim());
}
...
function extractLayerManagementRoutingParams(input: string): Record<string, any> {
    ...
    const declaredMode = resolveSkillRoutingMode('layer-management', input);

    // 语义整理是“先看画面和层树，再由模型声明成员”的自主动作。
    // 它必须先于“查看图层状态”解析，否则同一句里的“理解/位置/关系”会把明确
    // 的整理请求降级成 inspect，最终只观察不写入。模式信号归 Skill 声明维护，
    // 这里不再复制一套详情页/屏数关键词。
    if (declaredMode === 'organize') {
        return {
            ...params,
            action: 'organize',
    ...
    if (declaredMode === 'reorder' && normalized.includes('从浅到深')) {
        return { ...params, action: 'reorder', sortBy: 'lightness', sortDirection: 'light-to-dark' };
    }
    if (declaredMode === 'reorder' && normalized.includes('从深到浅')) {
...
function matchDeterministicIntent(
    ...
    // 叫停类指令（「先不要做sku」「取消」…）一律不由关键词选技能：正则匹配的是名词，
    // 读不出否定——真机实测「先不要做sku」照样命中 isSkuIntent 并路由到 sku-batch，
    // 又弹一次 SKU 组合卡。而「取消该取消什么」需要挂起操作与上轮任务的上下文，是模型的活。
    // 单点拦在这里，不在每个 isXxxIntent 里散落补丁。
    if (blocksDeterministicRouteByNegation(input)) return null;
...
/**
 * 恒 false 是**有意的纪律**，不是待实现的空壳：寒暄/致谢/确认这类对话一律不走本地固定模板，
 * 必须交给模型回复（避免"你好"被一句写死的问候顶掉，让 Agent 显得像自动回复机）。
 * 由 smoke-agent-intent-engine 的 conversational-persona-and-capability-do-not-use-local-fixed-templates
 * 用例守护。**不要当死代码清理掉**——删了这条纪律就静默失效。
 */
