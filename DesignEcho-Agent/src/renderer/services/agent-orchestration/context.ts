    ...
    const visitFolder = (folder: any) => {
        ...
        for (const image of images) {
            ...
            collected.push({
                ...
                modifiedTimeMs: typeof image?.modifiedTimeMs === 'number' ? image.modifiedTimeMs : undefined,
...
export function normalizePhotoshopDocumentInfo(
    ...
    const historyStateRef = readPhotoshopHistoryStateRef(docInfo);
    ...
    return {
        hasDocument: true,
        documentId,
        ...(historyStateRef && historyStateRef.documentId === documentId
            ? { historyStateRef }
            : {}),
