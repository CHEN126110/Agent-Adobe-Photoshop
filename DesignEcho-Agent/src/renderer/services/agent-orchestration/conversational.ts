...
function getSkillCapabilityFocusDomain(skill: SkillDeclaration): CapabilityFocusDomain | 'multi' | null {
    ...
    if (normalizedId === 'sku-batch') return 'sku';
...
function buildConversationSkillSemanticScope(skill: SkillDeclaration): string {
    const normalizedId = String(skill.id || '').trim().toLowerCase();
    if (normalizedId === 'sku-batch') return 'SKU 色卡、组合模板、占位符、颜色配置、组合图、自选备注与批量导出';
...
export type ConversationalModelFailureAttempt = {
    ...
    reason?: string;
    /** 本次失败对应的模型标识。缺了它，用户只会看到笼统的「当前模型」，无从判断该查哪一个。 */
    modelLabel?: string;
};
...
export type ConversationalModelFailure = {
    ...
    /** 实际失败的模型标识；路由/分类等内部角色可能用的并不是用户在设置里选中的那个模型。 */
...
function classifyConversationalModelError(error: unknown): ConversationalModelFailureKind {
    return classifyModelProviderFailure(error).kind;
}
...
function pickConversationalFailureKind(attempts: ConversationalModelFailureAttempt[]): ConversationalModelFailureKind {
    ...
    for (const kind of [
        ...
        'service_unavailable',
        'protocol'
    ] as ConversationalModelFailureKind[]) {
...
function buildConversationalModelFailure(
    ...
    const failedModelLabel = attempts.map((attempt) => attempt.modelLabel).find(Boolean) || '';
    return {
        version: 'conversational-model-failure/v0',
        kind: pickConversationalFailureKind(attempts),
        attempts: attempts.slice(0, 4),
        ...(failedModelLabel ? { failedModelLabel } : {})
    };
}

/**
 * 从 provider 抛出的错误里取出模型标识。适配器统一抛 `{modelId}: {原因}`
 * （真机样例：`deepseek-v4-flash: ...`），这里只认这一种前缀，取不到就返回空字符串，
 * 绝不猜测——宁可不显示，也不能报一个错的模型名把用户引去改好的那份配置。
 */
function readFailedModelLabelFromError(error: unknown): string {
    const message = String(error instanceof Error ? error.message : error || '').trim();
    const match = message.match(/^([A-Za-z0-9][\w.\-/]{2,60}):\s/);
...
function buildConversationalModelErrorAttempt(
    ...
    const modelLabel = readFailedModelLabelFromError(error);
    return {
        purpose,
        status: 'error',
