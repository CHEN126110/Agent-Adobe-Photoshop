    ...
    async execute({
        ...
        for (const [sizeStr, combos] of Object.entries(combosBySize)) {
            ...
            if (!onlyNotes) {
                if (!templateDoc) {
                    recordSkuDiagnostic(
                        resolvedAssets?.comboTemplateError || `${size}双: 模板不可用`,
                        `${size}双缺少可用的组合模板，本次未生成该规格。`
                    );
                        ...
                        if (noteResult?.success) {
                            ...
                            for (const fileInfo of noteFiles) {
                                ...
                                if (resolvedFile.success && resolvedFile.record) {
                                    ...
                                    recordSkuDiagnostic(resolvedFile.error, `${size}双自选备注有一个导出文件无法确认，已跳过该文件。`);
                                }
                            }
        ...
        const requiredCompletedComboSizes = onlyNotes
            ? []
            : Object.entries(combosBySize)
                .filter(([, combos]) => Array.isArray(combos) && combos.length > 0)
                .map(([size]) => Number(size))
                .filter(Number.isFinite);
        const allRequestedComboSizesComplete = requiredCompletedComboSizes.every((size) => completedComboSizes.has(size));
        const deliveryOutcome = resolveSkuBatchDeliveryOutcome({
            hasAnyProcessedOutput,
            allRequestedComboSizesComplete,
        ...
        const draftReviewNotice = requiresDraftReview
            ? '\n\n发布说明：本轮规格或组合是 Agent 根据当前色卡生成的可撤回草稿，并非用户/项目已确认的业务事实；文件已制作完成，但正式发布前仍需在结果复核中确认规格与组合。'
            : '';
        const successMessage = processedSizes.length > 0
            ? `${skuDeliverySummary.compactText}${draftReviewNotice}`
            : [
                'SKU 没有生成可交付文件。',
                ...skuUserWarnings.map((warning) => `- ${warning}`)
            ].join('\n');
