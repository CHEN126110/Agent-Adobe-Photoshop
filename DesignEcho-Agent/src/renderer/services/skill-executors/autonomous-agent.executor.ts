...
function createExecuteToolWrapper(
    ...
    return async (toolName, params, runtimeContext) => {
        ...
        // 用户保护只随活动文档变化而解除；切回用户明确保护的开局文档会重新上锁。
 ...
 * 写保护的解除点在 createExecuteToolWrapper 内（createDocument / openProjectFile 置 false，
 * switchDocument 按新活动文档名重算）；listDocuments 等只读工具不在其中，指向它们会让模型
...
export function resolveAutonomousCapabilityRuntime(
    ...
    const skillRoutingRecommendation = isSkillRoutingRecommendation(params.skillRoutingRecommendation)
        ? params.skillRoutingRecommendation
        : undefined;
    const exposeSkillRoutingRecommendation = Boolean(skillRoutingRecommendation);
    ...
    const capabilitySession = createAgentCapabilitySession({
        ...
        workMode: runtimeContractBundle?.stagePlan.expectedWorkMode,
        baselineCapabilityIds: useRecommendedSkillFastPath && skillRoutingRecommendation
            ? buildRecommendedSkillFastPathBaseline(skillRoutingRecommendation.capabilityId)
            : [
                ...buildAgentCapabilityBaseline(
