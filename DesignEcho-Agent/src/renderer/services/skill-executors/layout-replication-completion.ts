...
function buildCompletionContract(params: {
    ...
    const visualMode = params.visualQa?.snapshotEvidence?.source === 'getScreenSnapshotsWithOverlay'
        ? 'overlay'
        : Number(params.visualQa?.snapshotEvidence?.snapshotCount || 0) > 0
            ? 'screenshot'
            : 'none';
    ...
    return {
        ...
        evidence: {
            ...
            visual: {
                mode: visualMode,
                snapshotCount: Number(params.visualQa?.snapshotEvidence?.snapshotCount || 0),
                overlayCount: Number(params.visualQa?.snapshotEvidence?.overlayCount || 0),
                pixelProbe: params.visualQa?.snapshotEvidence?.pixelProbe ? {
                    status: params.visualQa.snapshotEvidence.pixelProbe.status,
                    mae: params.visualQa.snapshotEvidence.pixelProbe.mae,
                    rmse: params.visualQa.snapshotEvidence.pixelProbe.rmse,
                    highDeltaRatio: params.visualQa.snapshotEvidence.pixelProbe.highDeltaRatio,
                    darkJaccard: params.visualQa.snapshotEvidence.pixelProbe.darkJaccard,
                    softDarkJaccard: params.visualQa.snapshotEvidence.pixelProbe.softDarkJaccard,
                    rawImagesRedacted: true
                } : undefined
            }
        },
...
function buildLayoutReplicationUserReport(
    ...
    const visual = contract.evidence.visual;
    const pixelProbe = visual?.pixelProbe;
    const coverage = contract.evidence.coverage;
    const evidenceLines = [
    ...
    const limitations = [
        '当前报告只整合结构、bounds、截图/overlay 或 pixel-probe 检查结果，不证明整体审美质量。',
        pixelProbe && pixelProbe.status && pixelProbe.status !== 'ok'
            ? `pixel-probe 为 ${pixelProbe.status}，只能作为观察项，不能作为高保真通过结论。`
            : '',
        visual?.mode === 'none'
    ...
    const nextActions = contract.blockers.length > 0
        ? [
            ...
            '回到 Photoshop 检查失败图层、缺失元素、bounds 或截图结果。'
        ]
        : contract.warnings.length > 0
            ? [
                '人工复核观察项后再决定是否接受结果。',
                '需要更严格验收时，补充截图级 QA 或更多 benchmark case。'
            ]
            : [
...
export function formatLayoutReplicationUserReport(
    ...
    const lines = [
        ...
        `结论: ${report.verdict}`,
        '',
        report.evidenceLines.length > 0 ? '检查结果:' : '',
        ...report.evidenceLines.map(line => `- ${line}`),
...
export function summarizeTemplateApplyCompletion(params: {
    ...
    const autoFillFailed = params.autoFill
        ...
        : false;
    const visualBlocker = Array.isArray(params.visualQa?.evidenceReport?.blockers)
        ? params.visualQa!.evidenceReport!.blockers![0]
        : '';
    const visualWarning = !visualBlocker && Array.isArray(params.visualQa?.evidenceReport?.warnings)
        ? params.visualQa!.evidenceReport!.warnings![0]
        : '';
    ...
    const messageLines = [
        ...
        params.autoFill ? `自动填充: ${params.autoFill.filledScreens} 屏成功, ${params.autoFill.filledImages} 张图片` : '',
        params.autoFill && params.autoFill.guardedScreens > 0 ? `保护策略: ${params.autoFill.guardedScreens} 屏` : '',
        params.autoFill && params.autoFill.failedScreens > 0 ? `自动填充失败: ${params.autoFill.failedScreens} 屏` : '',
        params.visualQa?.snapshotEvidence?.source === 'getScreenSnapshotsWithOverlay'
            ? `Overlay 检查: ${Number(params.visualQa.snapshotEvidence.snapshotCount || 0)} 张截图，${Number(params.visualQa.snapshotEvidence.overlayCount || 0)} 项标注`
            : '',
        params.visualQa?.snapshotEvidence?.pixelProbe
            ? `截图探针: ${params.visualQa.snapshotEvidence.pixelProbe.status || 'unknown'}`
                + (typeof params.visualQa.snapshotEvidence.pixelProbe.mae === 'number' ? `，MAE ${params.visualQa.snapshotEvidence.pixelProbe.mae}` : '')
                + (typeof params.visualQa.snapshotEvidence.pixelProbe.darkJaccard === 'number' ? `，暗部 Jaccard ${params.visualQa.snapshotEvidence.pixelProbe.darkJaccard}` : '')
                + (typeof params.visualQa.snapshotEvidence.pixelProbe.softDarkJaccard === 'number' ? `，软暗部 Jaccard ${params.visualQa.snapshotEvidence.pixelProbe.softDarkJaccard}` : '')
            : '',
