...
import { buildContextSnapshot } from '../../../shared/project-asset-index';
import {
...
export async function prepareBusinessSkillProjectContextForScenario(
    ...
    // 项目文件、配置与 AssetIndex 属于同一次提交冻结的项目事实；Skill 场景只改变
    // “从既有候选中抽哪些图”，不应为了换 sampling scenario 再递归扫描整个项目。
    // 提交边界已经校验项目身份，场景派生在同一 AssetIndex / visual cache 上完成，
    // 避免 general-design → business Skill 造成第二次 loadConfig / scanProject / CSV 解析。
    if (projectContext.assetIndex) {
        ...
        const previousSnapshot = projectContext.contextSnapshot;
        const contextSnapshot = buildContextSnapshot({
            projectPath: projectContext.projectPath || previousSnapshot?.project?.path,
            projectName: projectContext.projectName || previousSnapshot?.project?.name,
            currentDocument: previousSnapshot?.currentDocument,
            selectedAssetPaths: readSelectedAssetPaths(projectContext),
            userConstraints: previousSnapshot?.userConstraints,
            taskHistory: previousSnapshot?.taskHistory,
            unverifiedItems: previousSnapshot?.unverifiedItems,
            assetIndex: projectContext.assetIndex,
            visualSamplingPlan,
            visualInsightCache
        });
        ...
        return {
            ...
            context: {
                ...
                projectContext: {
                    ...
                    visualInsightCache,
                    contextSnapshot,
                    contextSnapshotSource: projectContext.contextSnapshotSource || 'runtime-project-service',
