...
import {
    buildLayoutMatchCoverageEvidence,
    buildTemplateApplyCoverageEvidence
...
import {
    ...
    type ReferenceReplicationVisualQaEvidenceReport
...
type RedactedOverlaySnapshotResult = {
    ...
    overlayCount: number;
    redacted: true;
    evidenceReport?: ReferenceReplicationVisualQaEvidenceReport;
    snapshots: Array<{
        screenId?: number;
        screenName?: string;
        screenIndex?: number;
...
async function captureReferenceOverlaySnapshotsIfRequested(
    ...
    try {
        ...
        callbacks?.onToolComplete?.('getScreenSnapshotsWithOverlay', {
            ...redactedResult,
            evidenceReport: visualQa.evidenceReport
        });
        return {
            result: {
                ...redactedResult,
...
export const layoutReplicationExecutor: SkillExecutor = {
    ...
    async execute({ params, callbacks, signal, context }: SkillExecuteParams): Promise<AgentResult> {
        ...
        try {
            ...
            const plannerPreflight = buildReferenceReplicationPlannerEvidence({
            ...
            const templateBlueprint = buildDetailTemplateBlueprint(designRepresentation);
            ...
            emitStep(
                'verification',
                '模板蓝图已生成',
                `拆分 ${templateBlueprint.screens.length} 屏；QA：${blueprintQa.summary}`,
                'success'
            );
            ...
            if (templateBlueprintOnly && !templateApplyMode) {
                ...
                const designAgentOs = buildReferenceReplicationDesignAgentOsEvidence({
                    userInput: userInputForOs,
                    representation: designRepresentation,
                    qaReport: blueprintQa,
                    toolResults,
                ...
                const designPlanner = buildReferenceReplicationPlannerEvidence({
                    userInput: userInputForOs,
                    params,
                    context,
                    representation: designRepresentation,
                ...
                return {
                    ...
                    data: { referenceParse, designRepresentation, layoutAnalysis: referenceParse, templateBlueprint, qaReport: blueprintQa, designAgentOs, designPlanner, designPlannerPreflight: plannerPreflight, designPlannerPreflightGate, designPlannerExecutionAlignment },
                    toolResults
                };
            }
            ...
            if (templateApplyMode) {
                ...
                if (!docInfo?.success && params.autoCreateDocument !== false) {
                    ...
                    callbacks?.onMessage?.(`未检测到文档，自动创建模板画布 ${canvasSize.width}x${canvasSize.height}`);
                ...
                const canvas = {
                    width: Math.max(1, Math.round(layoutReplicationToNumber(docInfo.width, 1242))),
                    height: Math.max(1, Math.round(layoutReplicationToNumber(docInfo.height, 3600)))
                };
                ...
                const coverage = buildTemplateApplyCoverageEvidence({
                    blueprintScreens: templateBlueprint.screens,
                ...
                return {
                    ...
                    data: { referenceParse, designRepresentation, layoutAnalysis: referenceParse, templateBlueprint, applyResult, autoFillResult, overlaySnapshotResult: overlayCapture.result, projectPathUsed: projectPath || undefined, qaReport: applyQa, completionContract: applySummary.completionContract, referenceReplicationReport: applySummary.userReport, designAgentOs, designPlanner, designPlannerPreflight: plannerPreflight, designPlannerPreflightGate, designPlannerExecutionAlignment },
                    toolResults
                };
            }
            ...
            if (!docCheckResult?.success) {
                if (params.autoCreateDocument !== false) {
                    ...
                    const canvasSize = resolveLayoutReplicationAutoCanvasSize({
                        params,
                        referenceCanvas: designRepresentation.canvas,
                        fallback: { width: 1242, height: 1600 },
                    ...
                    const autoCreateSummary = summarizeTemplateApplyCompletion({
                        headingSuccess: '已根据参考图创建可编辑复刻骨架',
                        headingReview: '参考图复刻骨架部分创建，需复核',
                        baseSuccess: applyResult.success,
                        screenCount: applyResult.screenCount,
                        createdLayers: applyResult.createdLayers,
                        rootGroupName: applyResult.rootGroupName,
                    ...
                    return {
                        ...
                        data: { referenceParse, designRepresentation, layoutAnalysis: referenceParse, templateBlueprint, applyResult, overlaySnapshotResult: overlayCapture.result, qaReport: applyQa, completionContract: autoCreateSummary.completionContract, referenceReplicationReport: autoCreateSummary.userReport, designAgentOs, designPlanner, designPlannerPreflight: plannerPreflight, designPlannerPreflightGate, designPlannerExecutionAlignment },
                        toolResults
                    };
                }
                return {
            ...
            if (!matchResult || !Array.isArray(matchResult.matches) || matchResult.matches.length === 0) {
                ...
                return {
                    ...
                    data: { referenceParse, designRepresentation, layoutAnalysis: referenceParse, currentElements, completionContract: noMatchSummary.completionContract, referenceReplicationReport: noMatchSummary.userReport, designAgentOs, designPlanner, designPlannerPreflight: plannerPreflight, designPlannerPreflightGate, designPlannerExecutionAlignment }
                };
            }
            emitStep(
                'verification',
            ...
            const coverage = buildLayoutMatchCoverageEvidence({
                representation: designRepresentation,
                matchResult,
            ...
            const toolResults = [{ toolName: 'layout-replication', result: { successCount, failCount, qaReport: matchQa, completionContract: matchSummary.completionContract } }];
            const designAgentOs = buildReferenceReplicationDesignAgentOsEvidence({
                userInput: userInputForOs,
                representation: designRepresentation,
                qaReport: matchQa,
            ...
            return {
                ...
                toolResults,
                data: { referenceParse, designRepresentation, layoutAnalysis: referenceParse, matchResult, qaReport: matchQa, completionContract: matchSummary.completionContract, referenceReplicationReport: matchSummary.userReport, designAgentOs, designPlanner, designPlannerPreflight: plannerPreflight, designPlannerPreflightGate, designPlannerExecutionAlignment }
            };
