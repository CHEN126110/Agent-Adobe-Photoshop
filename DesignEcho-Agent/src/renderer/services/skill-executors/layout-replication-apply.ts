...
async function createSupplementalPlaceholderByRole(
    ...
    if (role === '文案') {
        const content = `[文案占位] 第${screenIndex}屏`;
        const box: PixelBox = {
            left: 24,
            top: 24,
    ...
    for (const screen of blueprint.screens) {
        ...
        for (const requiredRole of requiredGroups) {
            if (roleLayerMap[requiredRole].length === 0) {
                const supplemental = await createSupplementalPlaceholderByRole(requiredRole, screen.index, canvas.width, canvas.height, callbacks);
