...
const SCREEN_ROLE_ASSET_MAP: Record<DetailScreenRole, AssetType[]> = {
    ...
    'material-proof': ['detail', 'product'],
    'detail-proof': ['detail', 'product'],
...
function estimateCharsPerLine(copyStrategy: DetailScreenPlan['copyStrategy'] | undefined, role: string | undefined): number {
    ...
    if (copyStrategy === 'evidence') return 12;
...
function buildScreenPlanDecisionLines(screenPlan: DetailScreenPlan): string[] {
    ...
    const structuralReasons = screenPlan.structuralEvidence?.reasons || [];
    if (structuralReasons.length > 0) {
        lines.push(`Structural candidates: ${structuralReasons.join(' / ')}`);
    }
    if (screenPlan.requiresModelDecision) {
        lines.push('Decision boundary: template and filename rules are candidate evidence only; decide the actual content from product evidence and user intent.');
    }
    return lines;
}
    ...
    const draftPlan = {
        ...
        decisionBoundary: {
            ...
            note: requiresModelDecision
                ? '素材排序只代表结构和文件证据候选，仍需模型 Agent 决定最终视觉叙事。'
