...
export interface DetailImageExecutionDeferral {
    layerId: number;
    layerName: string;
    code: DetailImageExecutionDeferralCode;
    reason: string;
    recoverable: true;
}
...
export function resolveDetailImageExecutionDeferral(
    ...
    if (
        ...
        || image.expectedRelation?.mattingApplied === true
    ) {
        return {
            ...base,
            code: 'matting_required',
            reason: '素材需要真实抠图/蒙版处理；FillPlan 内联 receipt 不是可信执行证据。'
        };
    }
    if (image.executionDeferred === true) {
        return {
            ...base,
            code: 'plan_marked_deferred',
            reason: '图片计划已明确标记为当前不可自动执行。'
        };
    }
    if (
        image.container?.mode === 'clip_to_base'
        && Number(image.container.baseLayerId || image.expectedRelation?.clippingBaseId || 0) <= 0
    ) {
        return {
            ...base,
            code: 'clip_base_missing',
            reason: 'clip_to_base 缺少真实 baseLayerId，不能降级后冒充已建立剪切蒙版。'
        };
    }
    return null;
}
