 ...
 * 用户可见的能力名称。
 *
 * SkillDeclaration.name 是英文内部标识（'E-commerce Socks Design'、'SKU Batch'…），
 * displayName 才是给人看的中文名。此前这里回退到 name，导致 17 个 user-facing 技能
 * 在对话里全部以英文露出（真机：用户发出需求后第一行就是 "E-commerce Socks Design"），
 * 违反「用户可见字符串一律简体中文」。displayName 缺失时宁可用通用措辞也不露内部标识。
 */
function getSafeSkillLabel(skillId: string): string {
    ...
    return String(skill.displayName || '').trim() || '该能力';
}
...
async function capturePendingContinuationScopeObservation(
    ...
    return {
        ...
        ...(photoshopContext.historyStateRef
            ? { photoshopHistoryStateRef: photoshopContext.historyStateRef }
            : {})
    };
}
...
export async function executeSkillWithExecutor(
    ...
    executeParams.callbacks?.onProgress?.(
        userVisibleSkill ? `执行能力：${skillLabel}` : '正在处理请求',
        0
    );
    executeParams.callbacks?.onMessage?.(
        userVisibleSkill ? `正在执行「${skillLabel}」。` : '正在处理请求。'
    );
    ...
    try {
        ...
        if (!pendingInteractiveContinuation && userVisibleSkill) {
            ...
            pendingInteractiveContinuation = buildPendingInteractiveContinuation({
                ...
                conversationBranchId: executeParamsForBusiness.context?.conversationBranchId,
