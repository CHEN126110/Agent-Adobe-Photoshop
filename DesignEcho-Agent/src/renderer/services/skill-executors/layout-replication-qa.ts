...
import type { ReferenceElementCoverageEvidence } from './layout-replication-coverage';
...
function scoreStructure(input: {
    ...
    const normalizedScreenScore = clamp01(screenCount > 0 ? screenCount / Math.max(1, Math.ceil(elementCount / 2)) : 0);
    const score = clamp01(
        0.45
    ...
    const reason = screenCount > 0
        ? `已解析 ${elementCount} 个元素，并拆分为 ${screenCount} 个结构屏。`
        : `已解析 ${elementCount} 个元素，但尚未形成屏级结构。`;
...
function scoreEditability(input: {
    ...
    if (createdLayers > 0 || generatedScreens.length > 0) {
        ...
        return createDimension(
            ...
            '可编辑性',
            score,
            `模板已落地 ${generatedScreens.length} 屏，创建 ${createdLayers} 个图层，失败/跳过 ${failedOps} 次。`
        );
    }
...
export function buildReferenceReplicationQaReport(input: {
    ...
    coverage?: ReferenceElementCoverageEvidence;
    visualQa?: ReferenceReplicationVisualQaReport;
    ...
    const visualQa = input.visualQa || (
        input.stage === 'template-applied'
            ? buildReferenceReplicationVisualQaReport({
                ...
                snapshotEvidence: { source: 'bounds-only', snapshotCount: 0, overlayCount: 0 }
            })
            : undefined
    );
    ...
    const screenCount = Array.isArray(input.blueprintScreens) ? input.blueprintScreens.length : 0;
    if (elementCount > 0) evidence.push(`已解析元素 ${elementCount} 个`);
    if (styleStats.styledElementCount > 0) {
        evidence.push(`已解析样式元素 ${styleStats.styledElementCount} 个，其中颜色 ${styleStats.colorElementCount} 个、效果 ${styleStats.effectElementCount} 个`);
        evidence.push(...formatReferenceStyleRecipeAnalysisForQa(styleRecipeAnalysis));
    ...
    if (screenCount > 0) evidence.push(`已生成结构屏 ${screenCount} 个`);
    if (input.coverage && input.coverage.expected > 0) {
        evidence.push(`参考元素覆盖：${input.coverage.applied}/${input.coverage.expected}，失败 ${input.coverage.failed}，跳过 ${input.coverage.skipped}`);
    ...
    if (generatedImageCount > 0 || generatedCopyCount > 0) {
        evidence.push(`文档占位：图片 ${generatedImageCount} 个，文案 ${generatedCopyCount} 个`);
    }
    const styleRecipeStats = input.applyStats?.styleRecipeStats;
    if (styleRecipeStats && styleRecipeStats.attempted > 0) {
        evidence.push(`样式 recipe 执行：尝试 ${styleRecipeStats.attempted} 项，成功 ${styleRecipeStats.applied} 项，失败 ${styleRecipeStats.failed} 项，跳过 ${styleRecipeStats.skipped} 项`);
    ...
    if (visualQa && visualQa.counts.total > 0) {
        evidence.push(visualQa.summary);
        evidence.push(...visualQa.evidence.slice(0, 4));
    ...
    if (input.autoFillStats) {
        evidence.push(
            `自动填充：成功 ${Number(input.autoFillStats.filledScreens || 0)} 屏，失败 ${Number(input.autoFillStats.failedScreens || 0)} 屏，保护 ${Number(input.autoFillStats.guardedScreens || 0)} 屏`
        );
    }
