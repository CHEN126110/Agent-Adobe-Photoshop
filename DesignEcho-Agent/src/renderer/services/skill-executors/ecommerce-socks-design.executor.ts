...
import { buildRuntimeSelectedSkillHandoff } from '../../../shared/agent-runtime-v5/runtime-selected-skill-handoff';
import {
...
import { isControlledRouteAutonomousEntrySkill } from '../../../shared/skills/skill-declarations';
...
function toChildOutputQuality(result: AgentResult, expectedReportKey: string): boolean {
    ...
    // 状态“completed”只表示子运行结束，不等于子 Skill 已签发质量结论。
    // 父级只能消费显式 canClaimOutputQuality 或子 QA owner 的允许结果，不能自行升级。
    return false;
}
...
function resolveManifestOwnedChildWorkMode(
    step: EcommerceSocksDispatchOrchestrationChildStep,
    childParams: SkillExecuteParams,
    userIntent: string
): RuntimeDesignWorkMode | undefined {
    const params = (childParams.params || {}) as Record<string, any>;
    const explicitWorkMode = normalizeRuntimeDesignWorkMode(
        params.declaredWorkMode || params.workMode
    );
    if (explicitWorkMode) return explicitWorkMode;
    if (step.skillId === 'main-image-design') return 'create_new';
    if (step.skillId === 'detail-page-design') {
        return wantsExistingDetailPageTemplate(userIntent, params)
            ? 'template_fill'
            : 'create_new';
    }
    return undefined;
...
function buildManifestOwnedAutonomousChildParams(
    step: EcommerceSocksDispatchOrchestrationChildStep,
    childParams: SkillExecuteParams,
    userIntent: string,
    projectPath: string | undefined,
    manifest: NonNullable<ReturnType<typeof resolveSkillRuntimeManifestSelection>['artifactManifest']>
): SkillExecuteParams | undefined {
    const runtimeSelectedSkillHandoff = buildRuntimeSelectedSkillHandoff({
        skillId: step.skillId,
        source: 'controlled_route_react_handoff',
        routeClass: 'business-workflow',
        directExecution: 'forbidden'
    });
    if (!runtimeSelectedSkillHandoff) return undefined;
    const declaredWorkMode = resolveManifestOwnedChildWorkMode(step, childParams, userIntent);
    const inheritedContext = childParams.context as any;
    const userTask = [
        `请完成父级电商袜品设计任务中的「${step.label}」子交付。`,
        `该子交付已经绑定「${manifest.display_name}」Runtime；请按其 Brief、参考、策略、真实 Photoshop 制作、写后观察、质量复核与交付阶段推进。`,
        '父级只负责调度和汇总；子交付是否完成必须以当前 Runtime 的真实执行结果和质量记录为准。',
        `原始用户目标：${userIntent}`
    ].join('\n');
    ...
    return {
        ...
        params: {
            ...
            task: userTask,
            skillId: step.skillId,
            declaredSkillId: step.skillId,
            declaredTaskType: manifest.task_type,
            ...(declaredWorkMode ? { declaredWorkMode } : {}),
            runtimeSelectedSkillHandoff,
            intentMode: 'manifest-owned-child-delivery',
            parentSkillId: 'ecommerce-socks-design',
            parentDeliverable: step.deliverable,
            expectedReportKey: step.expectedReportKey,
            projectPath,
            ...
            agentIntentControlPlane: {
                ...
                userVisibleSummary: `这是父级已经授权下发的「${step.label}」子交付，需要真实执行并由子 Runtime 自己验收。`,
                reason: '父级电商袜品设计按照用户明确的整套交付请求，下发 Manifest-owned 子任务。'
            }
        },
        context: {
            ...inheritedContext,
...
async function runManifestOwnedAutonomousChild(
    step: EcommerceSocksDispatchOrchestrationChildStep,
    childParams: SkillExecuteParams,
    executeParams: SkillExecuteParams,
    userIntent: string,
    ...
    if (selection.status !== 'resolved' || !selection.artifactManifest) {
        return buildFailedChildResultFromError(
            step,
            `controlled_child_manifest_missing:${step.skillId}:${selection.status}`
        );
    }
    const autonomousParams = buildManifestOwnedAutonomousChildParams(
        step,
        childParams,
        userIntent,
        projectPath,
        selection.artifactManifest
    );
    if (!autonomousParams) {
        return buildFailedChildResultFromError(step, `controlled_child_handoff_invalid:${step.skillId}`);
    }

    executeParams.callbacks?.onStep?.({
        kind: 'observation',
        title: `${step.label}进入声明式设计 Runtime`,
        detail: `已绑定 ${selection.artifactManifest.task_type}；父级不直调旧版 ${step.skillId} executor。`,
        status: 'running',
        toolName: 'autonomous-agent',
        toolCallId: `ecommerce-socks-child-runtime-${step.skillId}`,
        percent: step.progressRange.start + 1
    });

    const result = executeParams.runSkill
        ? await executeParams.runSkill('autonomous-agent', autonomousParams)
        : await runChildSkillViaRegistry('autonomous-agent', autonomousParams);
    return summarizeChildResult(step, result);
}
...
async function runChildDispatch(
    ...
    for (const step of steps) {
        ...
        try {
            ...
            if (childExecutionPath === 'test_override' && overrideExecutor) {
                childResult = summarizeChildResult(step, await overrideExecutor.execute(childParams));
            } else if (isControlledRouteAutonomousEntrySkill(step.skillId)) {
                childResult = await runManifestOwnedAutonomousChild(
                    step,
                    childParams,
                    executeParams,
                    userIntent,
...
export const ecommerceSocksDesignExecutor: SkillExecutor = {
    ...
    async execute(executeParams: SkillExecuteParams): Promise<AgentResult> {
        ...
        const childQualityIncomplete = executedChildDispatch
            && childReportAggregation.canClaimDesignComplete !== true;
        const resultSuccess = !realDispatchFailed && !childQualityIncomplete;
        ...
        if (realDispatchFailed) {
            issue = `child_dispatch_${childDispatchRun.status}`;
            resultError = `Child dispatch ${childDispatchRun.status}`;
        } else if (childQualityIncomplete) {
            issue = `child_report_${childReportAggregation.status}`;
            resultError = `Child quality ${childReportAggregation.status}`;
        }
