...
3. 新增巡检命令
   ...
   - `npm run maintenance:validate`（核心验证）
   ...
   - `validate` 执行核心构建、类型、审计、规划和仓库卫生检查；失败必须暴露真实原因。
   - `preflight:core` 是同一核心验证入口的显式别名，不创建临时测试脚本，也不通过假绿掩盖失败。
...
7. 构建或诊断命令可能重新生成 ignored 临时目录；需要保持磁盘简洁时，在验证后再次运行 `maintenance:repo-hygiene:clean-ignored-temp`。
