...
3. business prompts, DSL semantics, planner evidence requirements and QA thresholds
...
This skill is responsible for deterministic SKU batch generation from confirmed combination specifications, reusable templates and real color-card sources. SKU Template design and SKU Color Card creation are separate task semantics and must not be silently absorbed into `sku-batch`.
...
8. User Feedback UX: show concise model-authored public design judgments, real tool summaries and evidence cards; Provider private reasoning stays in the hidden diagnostic channel and must not be persisted or rendered as user-visible thinking.
...
2. static audits, reusable core tests and validation wiring that do not change business output
3. reports, project memory and diagnostics
4. bug fixes that preserve business strategy, user-visible defaults and Photoshop write order
...
6. Do not change these three skills' business strategy, user-visible defaults or acceptance thresholds without the user checkpoint described above.
...
5. `maintenance:planning-check` and `audit:agent-business-boundaries` pass
