...
> 治理级别：C 层实施参考。本文只用于查询概念到现有代码 Owner 的映射；G0-G7、抽象对象名和历史验收描述不拥有当前实施顺序，也不能要求新增与现有 Task Profile、TaskRun、Capability、Verification、DesignVerdict、Release 或 Delivery 重复的 Contract。当前路线只看 `project-memory/Plan.md`，当前成熟度只看 `project-memory/Status.md`。
