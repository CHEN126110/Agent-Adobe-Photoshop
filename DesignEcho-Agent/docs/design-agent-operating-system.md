...
DesignEcho 的生产 Agent 不是通用聊天助手、任意电脑控制 Agent 或 Photoshop 命令行外壳。它是专业视觉设计与 Photoshop 生产 Agent，职责是把设计目标和有来源的内容 /素材转成可编辑、可复核、可交付的视觉结果。

这条边界属于产品定义，不建立 `standard-design-agent-role-contract`、任务族枚举、角色级 Tool 白名单或新的 R0 分类器。Model 依据完整目标和上下文理解请求；现有 Task Profile 表达交付物语义，Capability / Policy 决定能力与权限，TaskRun 记录活动任务，Release 投影结果。关键词、正则、文件名和 Tool 名不得替模型抢先接受、拒绝或选择 Skill。

设计知识解释、Photoshop 工艺说明和必要对话可以自然回答而不创建生产任务；与视觉设计无关的通用代办不进入生产 Capability / Tool 执行。混合请求只承接其中明确的设计部分。是否需要 mutation 由用户目标和现有任务语义决定，不能由“设计 Agent”身份自动升级。

“从零创作”属于 Design Kernel 在空白目标状态下的通用设计能力，不是独立 Task Type、Skill、Executor、Workflow、WorkMode 路由或代码子系统。现有业务字段如 `workMode=create_new` 只能作为兼容输入表达目标状态与保护关系，不能上升为 Agent 身份或新增通用分支。
...
Harness 不负责定义“详情页必须几屏”“主图必须先做哪一步”或“所有设计必须先调用 renderLayout”这类专业能力内容。跨品类的构图、层级、排版、色彩、空间关系和 Photoshop 工艺归 Design Kernel；品类、渠道与交付物特有的方法、输入输出和质量标准归 Skill overlay；事实、规则、方法论及其来源记录归 Knowledge；可执行动作归 Tool。
...
Design Kernel 是所有开放式设计任务常驻的通用专业底座，不是 Skill、Tool 或固定 Workflow。它负责把 ProductTruth、AssetHandle 与视觉观察组织为 SceneGraph / DesignIR，运用构图、层级、排版、色彩、空间关系和 Photoshop 图层 / 效果工艺知识形成策略、可执行计划与多尺度复核。

没有业务 Skill 时，Agent 仍可依靠 Design Kernel 完成通用设计；Skill 只叠加品类、渠道、交付物特有的知识、约束、计划片段和评价规则，不拥有任务状态、调度或 Photoshop 事务。生成式 Provider 只提供候选素材，不拥有设计目标或完成裁决。

### 2.2.2 Task Semantic Binding

Task Semantic Binding 解决“用户要的交付物是什么”，不解决“由哪个 Skill 执行”。每个 Task Profile 至少描述交付物语义、基本目标、典型质量维度、可由项目 /素材 /PSD 观察的输入、只能由用户决定的取舍，以及最低完成条件。

1. 第一批稳定语义覆盖开放式单画布、主图、详情页、SKU 批量、SKU 色卡与 SKU 模板；“SKU”不得继续作为一个同时代表三类任务的模糊身份。未登记的新设计类型绑定通用设计 profile，不因缺少业务 Skill 失去基础设计能力。
2. Model 声明 Task Profile；validator 只校验已登记语义，TaskRun 保存同 revision 的紧凑 binding 引用。Task Semantic Binding 本身不装配上下文、不激活 Manifest；Harness 不从关键词、文档名、文件名、旧路由 hint 或 Tool 参数补造任务类型。
...
Tool semantics 回答“一个 Photoshop 动作有哪些参数、副作用和读回”；Photoshop Craft Recipe 回答“为了某个视觉意图，应在什么条件下选择、组合并验证这些动作”。每条 Recipe 至少声明：

1. `recipeId / version`、目标视觉意图、适用条件与反例。
2. 必要观察、稳定目标对象、参数来源、允许范围和禁止猜测字段。
3. 可逆优先的技术选项、保持项、预期结构与预期像素变化。
...
当前问题不是缺少更多 Contract，而是同一语义、状态和结果被重复表达。目标架构只保留一条生产链：
...
| 关注点 | canonical owner | 其它现有表示的边界 | 当前成熟度 |
|---|---|---|---|
| 产品身份与职业边界 | `Prompt.md` + 本 OS | 只指导行为，不生成 Runtime 对象 | 治理已定义，无需代码化 |
...
| 处置 | 当前对象 | 规则 |
|---|---|---|
| 保留并原地演进 | `design-task-types.ts`、`runtime-context-compiler.ts`、`runtime-session`、Capability resolver/session、`PhotoshopTransactionRunner`、`DesignVerdict`、现有 learning runtime | 不复制 owner，只补齐当前纵切需要的字段和生产消费者 |
| 收为 provider / projection | artifact knowledge、Manifest / Skill、`DesignDocumentRole`、Project State、`DesignTaskRunRecord`、Completion、Run Record、Snapshot、UI、Scorecard、Critic | 只能向 canonical owner 提供方法、观察、finding 或只读视图，不反向拥有身份、状态、权限、调度或发布 |
| 随纵切合并 | `workMode=create_new / edit_existing`、Task Completion 重推断、continuation / resume、Recovery Queue、public plan、shadow reconciliation | `workMode` 逐步归一为目标状态 /保护关系；其余责任并入 TaskRun 节点、interaction、operation result 与 Release，不做全仓机械删除 |
| 随 capability pack 退役 | legacy direct-modal mutation、重复 retry / rollback / no-redo、片内旧 completion / recovery consumer | 只有替代 Owner、调用图和正反回归齐全时逐项退役；未迁移项保持 legacy 且不得进入可执行 R4 |
| 禁止新增 | Role Contract、六任务族 Registry、第二 Task / Outcome Contract、第二 Context Compiler、第二 DAG、第二 Verdict、第二 Release Gate、从零创作子系统 | 产品说明或统一展示不足以成为新生产对象的理由 |
...
`runtime-session/v0` 原地演进为唯一可暂停 TaskRun 聚合，不新建第三 Runtime 或第二 Task Store。TaskRun 唯一持有任务身份、plan revision、节点状态与 cursor、目标引用、pending interaction、结构化 operation result、复核与终态；UI、RuntimeTaskSnapshot、TaskCompletion 和完成摘要只读投影它。
...
#### 2.4.2 TaskRun Liveness Owner（planned）

X1 先让 TaskRun reducer 具备 `continue / waiting_user / checkpoint / finish / abort` 的规范状态；X2 按 capability pack 退役旧 Recovery Queue、no-progress guard、Completion 重推断和关键词恢复提示后，它才对该切片成为唯一 liveness owner。迁移期旧逻辑不得新增责任或与 TaskRun 同时推进同一节点终态。

1. progress key 至少包含 TaskRun / plan revision、stage / node cursor、未解决输入、观察事实指纹、operation result、同目标 readback、ArtifactRef 与未关闭 finding /质量变化；F1 binding 接线后加入可选 `taskSemanticRevision`，但 F 车道不反向取得 liveness 或执行权。finding 使用 `targetRef + principleId + normalizedRegion + evaluationProfileVersion` 形成稳定身份；只有关闭证据或超过 profile `minDelta` 的测量变化算进展，新 findingId、改写措辞或更换理由不算。成功的 Harness 控制调用、重复 Knowledge 读取、相同快照和 Tool 调用次数同样不改变进展。
2. Recovery 只有在对应真实 blocker、候选能力当前可见且可达、不扩大权限、预计能改变 progress key 时才允许发生。同一 `progress key + blocker` 默认最多两次结构化恢复；第三次必须进入 `waiting_user / checkpoint / abort`，禁止继续生成“换个说法再试”的恢复指令。
3. `checkpoint` 是绑定 TaskRun revision、原因和下一恢复条件的可恢复非终态，不得等同 completed。可自行读取的环境事实由 Agent 主动观察；只有用户独占且会改变结果的取舍进入 `waiting_user`。明确安全边界失败进入 `abort`，完成条件与未结义务全部闭合后才允许 `finish`。
4. dispatched mutation 的 ambiguous 结果只允许一次 operation-specific reconciliation，禁止重放写入。仅对“已获授权且交付要求 Photoshop mutation”的任务，预算分配才必须在执行前原子保留至少一次目标绑定写入后的同目标结构 /像素读回和最终 Evaluation；只读、建议和无写入交付任务不得因此被强迫 mutation。
...
3. Global System Prompt 只保留稳定、品类中立的不变量；通用设计纪律进入 Design Kernel，品类方法进入 Manifest 激活的 Skill / Knowledge / Evaluation overlay，Tool semantics 保持独立执行契约。
4. Prompt 不得定义固定生产序列或创建独立 Runtime State。外部 P-01～P-20 只映射到现有 R0-R5/E1-E2，不新建第二 Router 或第三 DAG Runtime。
5. Design Kernel 的通用专业方法常驻可达；只有品类、渠道与交付物特有方法才在对应 Skill scope 和阶段按需装载。新增第四个 Skill 不修改 Global Prompt、Agent loop 或 Completion authority。
...
2. **模型可见能力面**归 Capability Session + Runtime Stage。Skill overlay 可以收窄或补充当前能力，但无 Skill 的开放任务仍由 Design Kernel 与 Agent 自主组合 Capability；Skill 不取得调度权。
3. **真实执行**的目标 owner 是 TaskRun node、Tool execution point 与唯一 PhotoshopTransactionRunner。Design Kernel / Skill 提供设计方法和计划片段；Tool 只负责原子操作；Policy 只裁决安全、权限、读后写与复核纪律，不替 Agent 重新规划。当前 `renameLayer` 与 `groupLayersSafely` 两个纵向切片已完成实机验证；`setTextStyle` 已完成 Runner 代码迁移但仍待 disposable Photoshop 实机验证；其余 legacy mutation 仍待逐项迁移，不能把目标架构写成全量实现事实。
4. **执行事实**归 TaskRun、append-only Tool log 和结构化 operation result。是否 applied、是否 verified、是否观察、是否保存和是否完成只能由节点状态、工具结果与读回得出；单个 `blocked_*`、异常字符串、assistant prose、TaskCompletion 摘要或 UI 卡片不得覆盖已经发生的 mutation。
...
生产 Runtime 不得用单一 `success` 表示设计任务完成。以下事实分别由其现有 owner 产生，任何上层状态都不能补造下层状态：

1. `executionApplied`：Photoshop mutation 已由 TransactionRunner 提交。
2. `executionVerified`：同 target / revision 的写后读回证明预期变化存在且结果不歧义。
3. `designVerdict`：Evaluation 基于最后一次写入后的结构和视觉观察给出 `passed / needs_review / failed / passed_unverified / not_applicable`。
4. `deliveryReady`：声明交付物、保存 /导出结果和 Delivery Receipt 齐全。
5. `userAccepted`：只由用户明确动作产生，不得由 Tool success、质量分、文件存在或 assistant 回复推断。

TaskRun / Release Profile 必须声明 `executionRequirement=mutation_required | mutation_not_required`。前者要求 `executionApplied + executionVerified`；后者要求与交付类型匹配的只读 Observation / Analysis Artifact 验证，不能用 `false / undefined` 混淆“未执行”“未验证”和“不适用”。

Release Gate 的输出固定为 `release_ready / review_required / release_rejected`，避免与 Photoshop transaction 的 `commit` 混淆：

1. `release_ready` 要求当前 `executionRequirement` 已满足、`designVerdict=passed`、`deliveryReady=true` 且没有未关闭的合资格 blocker。设计 mutation 任务必须 `executionVerified=true`；合法只读 /建议 /分析任务不被强迫产生 Photoshop 写入。
2. `needs_review`、`passed_unverified`，以及只有审美 /VLM finding 支撑的 `failed`，映射为 `review_required`，不能直接形成写前阻断或 `release_rejected`。
3. `not_applicable` 只允许明确非设计任务使用；它不能为设计任务提供质量通过信用。
4. `release_rejected` 只接受带 `blockerKind + proofRef` 的确定性 blocker：目标身份 / revision 错误、权限不足、不可逆动作未批准、可证明的商品 /文案事实错误、必需交付物缺失，或可证明的文档结构损坏。Gate 不信任裸 `severity=blocker`。
5. 构图、色彩、排版、对比、工艺、总体观感、VLM brief coverage 与 `above-baseline` 都是质量 finding；除非另有确定性 OCR /结构 /事实 proof 转成上述 blocker kind，否则只能驱动有界修订或 `review_required`。
6. `userAccepted` 始终与 Release 正交；只有当前 Policy 明确要求人工批准时，缺少接受才使结果保持 `review_required`，不能反推执行或质量失败。
7. `designVerdict.blockers`、`scorecard.blockers`、`designQualityHardBlocked`、`summary.blockers` 和同形字段都只是迁移输入；任何 Runtime、Reflexion、Completion、UI 或 Run Record 消费者不得在 Release Gate 前据此硬终止。Reflexion 只消费结构化 finding 并受修订预算约束；能形成 `release_rejected` 或质量硬终止的 owner 只有唯一 Gate。
...
候选按项目事实 /用户偏好、通用 Recipe、Skill patch、代码 /契约缺陷和 Evaluation 缺口分流。Task Semantic Binding 只作为语义身份一致性校验输入，不激活 Skill；Capability / Skill 激活继续由结构化 R0 选择、当前 stage、Capability / Policy owner 与模型在合法候选内的判断共同决定，不能只让模型看一段短描述后独自决定。Hermes 只作为渐进加载、Patch 优先、来源 /生命周期与回滚机制的外部参考，不拥有 DesignEcho 的 TaskRun、Design Kernel、Photoshop 执行、Release 或质量结论。
...
5. PhotoshopTransactionRunner 是 mutation 的目标唯一 owner，负责同一 modal 批次、当前文档 / 目标 / revision、取消、提交、未知写状态读回和回滚；新迁移 Tool 与 Skill 不再各自维护事务或重试状态。当前 `renameLayer` 与 `groupLayersSafely` 已完成实机纵切；`setTextStyle` 已完成代码纵切、待实机验证；其余 legacy mutation 的旧 owner 只作为待退役实现事实保留，不能继续扩张。
6. Operation result 必须区分 `applied`、`verified`、`verification_failed`、`failed` 与 `unknown`。已经 applied 但验收失败的写入不能被重写成“未执行”后重复调用。
...
| `TaskSemanticBinding` | 模型声明的交付物语义、Kernel profile、输入类型与来源引用；不选择 Skill、不授予权限 | 目标契约，待映射现有 task type / artifact knowledge / Manifest |
| `ProductTruth` | 商品 / 对象事实、受众、卖点、约束、来源等级与待确认项 | 业务侧分散存在，待收为 Design Kernel 输入 |
| `AssetHandle` | 稳定素材身份、版本、用途、可用性与生成来源 | 缺统一运行时契约 |
...
| `SceneGraph / DesignIR` | 区域、元素、视觉层级、图层语义、关系、保持项与设计意图 | 多处局部存在，待收为通用底座 |
| `TaskRun` | 唯一任务身份、计划 revision、节点、cursor、交互等待、操作结果、复核与终态 | 目标契约，implementation_not_started |
| `DesignDSL` | 网格、区域、文本块、图片槽、样式、recipe、约束 | 分散在 Grid、reference、smart-scaling |
| `PhotoshopCraftRecipe` | 视觉意图、适用条件、必要观察、参数来源、非破坏性工艺、保持项与结构 / 像素验收 | 首条单画布记录已接入 Knowledge / Design Kernel provider；R4/R5 可达，真实执行与质量收益待 V1 验证 |
| `ExecutionPlan` | 工具调用计划、依赖、降级策略、预期结果与读回 | 分散在 skill executor |
| `PhotoshopTransaction` | 目标 / revision、原子写入、取消、提交、未知状态读回与回滚 | 部分落地：`renameLayer`、`groupLayersSafely` 已完成实机纵切；`setTextStyle` 代码纵切完成、待实机验证；其余 legacy mutation 待迁移 |
...
3. bridge 的退出条件必须可描述：当 v5 runtime 能直接调度同等能力，并通过对应类型检查、静态审计、可复用功能测试和真实 ChatPanel / Photoshop 验收后，旧桥接才可以收口。
...
26. 普通可执行 Tool call 不充当 R4 计划声明；只有合法 `action_plan_declaration` 可以形成 R4 节点。声明本身不授予权限、不算 E1、任务完成或质量通过；完成 M3-C 后，ready 节点由 TaskRun scheduler 经过 execution preflight 和 PhotoshopTransactionRunner dispatch。
27. 完整 R4 声明在活动 TaskRun 中有界保留，并可作为 Runtime Action Plan 正文发布到唯一 Artifact Repository；execution summary、Run Record、Resume 与 RuntimeTaskSnapshot 只保存 digest 或严格 Repository ref，不复制声明正文，也不从 Artifact 反向取得调度权。重型 schema / validator 在 R3 ready 后按需加载。
28. `runtime-action-plan-reconciliation/v0` 当前使用 Capability Session inventory 在事后把 Tool result 映射到计划节点；它是 shadow 迁移债务。M3-C 后 operation result 必须由运行节点直接登记，该 reconciliation 不得继续作为生产执行 owner。
...
2. Design Kernel 负责通用设计理解、结构、策略、工艺映射与复核；Skill 只定义业务特有输入输出、方法、约束、计划片段和验收 overlay，不拥有默认调度器，也不等于 Tool。
...
9. `docs/design-agent-professional-capability-system-plan.md`：专业设计能力来源、基础 Design Capability、视觉观察与判断输入、Photoshop Craft / Effect-to-Technique、生成式素材、语义动作、审美评价、花瓣级目标可行性、真实质量阶梯和经验学习专项方案；只在 `CurrentTask.md` 激活对应切片时参与实施。
10. `docs/agent-governance-implementation-objective.md`：C 层概念到代码 Owner 映射；历史工作包不拥有当前排期。
11. `docs/design-agent-os-implementation-tree.md`：C 层子系统 inventory；旧 smoke、成熟度与 UI 表述不代表当前状态。
...
2. 复用唯一 PhotoshopTransactionRunner，按当前纵切所需 capability pack 迁移 Photoshop 操作的真实执行和真实读回；不以全仓 direct-modal 文件数量作为 Design Foundation 或 TaskRun 的总前置条件。
3. TaskRun 同步拥有 document / revision 与单文档写者身份；Runner 负责单次事务，两者共同而非先后形成安全执行闭环。
4. 让取消、错误、弹窗、无响应、未知写状态、读回和 rollback 都进入结构化控制。
