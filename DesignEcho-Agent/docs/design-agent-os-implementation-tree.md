...
> 治理级别：C 层历史实施 inventory。本文保留子系统和旧证据的检索价值，但不拥有当前里程碑、成熟度或验证命令；其中 `smoke:*`、旧完成比例和旧 UI / Provider thinking 表述只代表历史状态。当前顺序以 `project-memory/Plan.md` 为准，当前事实以 `project-memory/Status.md` 为准，现行验证只使用 `maintenance:validate`、构建、静态审计和真实 E2E。
...
职责：不替模型判断开放语义；只承接显式 deny-only 能力边界、结构化 continuation owner、不可逆确认和无模型兼容。

当前证据：

1. 普通自然语言由 `DesignAgentEngine` 直接送入 `autonomous-agent`，不再先调用独立 `task-classifier`。
2. `routing.ts` 与 `agent-intent-control-plane.ts` 只保留无模型 fallback、明确禁令和协议边界，不拥有模型路径的回复、澄清、Skill 或写权限。
3. `ToolDecision`、`Preflight`、事务目标守卫和 HITL 在真实执行点强制确定性不变量。
4. 当前验证使用 `maintenance:validate`、生产构建、审计和真实 E2E，不再创建、恢复或运行 smoke。

下一实施 gate：

1. 用真实用户语料验证主 Agent 能在同一循环中稳定决定直接回答、观察、追问或行动。
2. 用真实 Provider → Electron → Photoshop E2E 验证 no-Skill、no-Photoshop、精确 Tool deny 和不可逆确认。
3. 把剩余 legacy 关键词兼容债务继续从模型热路径移出，禁止重新建立前置 Router。
