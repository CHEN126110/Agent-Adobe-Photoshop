...
- 状态：triaged
- 下一步：2026-08-30 纯逻辑插桩已完成两级收敛：① UXP saveDocument JSX 光栅路径的 sourceHistoryStateRef 收据存在（save-document.ts:534-550）；② `projectAgenticFinalDeliveryEvidence` 对 r38 形状（双 saveDocument、anchor 匹配、同 history）**能正确产出 refs=[PSD,JPG]**（探针 scratchpad/repro-intake-083.cjs 口径）。活运行断点锁死在 `findReviewedPreview`（agent.ts:5150s）的输入侧，且已静态排除对象同一性怀疑（toolResults 与 toolCallLog **有意共享结果对象**、压缩就地变更，agent.ts:7377-7381 注释明示；`binding.sourceOutput === entry.result` 按设计成立）。剩两个活体候选：(a) 全画布条目的 `readAgentVisualObservation().reviewed===true` 未置位（快照落在末轮、模型直接 final_response 时观察从未被消费标记——可解释 r35 过/r38 不过的时序差）；(b) 绑定前提 `judgeStatus==='completed' && evidenceScope.finalArtifactObserved`（agent.ts:12321）中 finalArtifactObserved 为假。下一步：dev-only 投影 trace（打印 binding 是否建立、finalArtifactObserved、各全画布条目的 reviewed 标记与 history 对齐）随一次真机复跑确证，再定修复归属。
