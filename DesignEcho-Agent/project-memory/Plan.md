...
## 2026-08-28 唯一当前顺序：`DEVELOPMENT-LOOP-RESET-001`

本节是当前唯一实施优先级；下方旧日期段只保留历史上下文，不再拥有“当前主线”权力。

1. `[已完成] Development Loop`：过程价值复盘、GMR 操作规约、权威文档与机器投影、重型验证前依赖完整性 preflight 已闭合；该切片没有增加产品 Runtime 状态或 Gate。
2. `[已完成·推送待网络] 语义抠图固定案例`：隔离 Photoshop 真机已验证实际 source bounds、document /history /layer、两实例分割、二进制 mask 写入、`user-mask-enabled` 读回与真实视觉结果；E2E 暴露的 Provider 坐标空间和 synchronous batchPlay 根因已修，完整核心闸门 58/58 通过并独立本地提交。GitHub 443 当前不可达，远端分支尚未取得成功回执。
3. `[进行中·Provider 完成，面板实机点击待验] 姿态统一`：纯离线算法与机械质量契约、精确 RGBA 二进制捕获、单文档单事务 apply 和专用写后收据已完成三个隔离真机 E2E、完整核心 59/59 与独立提交。面板已移除参考形状和未进入 Provider 的假参数，整批绑定加载时 documentId、逐层读取最新 revision，未知写状态会停止后续项；契约、构建、脚本解析、280×620 视觉检查和完整核心 59/59 通过，随后补入的 documentId 守卫又通过 UXP 全测试、TypeScript 与 production build。新版 UXP 已在 Photoshop 中真实显示，但 Computer Use 因 WebView 子进程目标边界在点击前拒绝注入，隔离文档零写入且用户状态已恢复；下一项只通过可真正触达按钮的路径补齐隔离 E2E，不为测试工具限制另造产品旁路。随后以真实弯曲商品独立验收纹理与商业视觉质量。
4. `[待独立收口] 依赖迁移与 Smile Provider`：分别审计、验证、提交和回滚，不与姿态或业务 Skill 混合。
5. `[待建立基线] 固定真实设计评测`：为主图、详情页、SKU 与参考复刻使用自然一句请求、固定模型 /构建 /素材 /环境、零人工纠偏和多次重复，分开统计技术成功、人工介入、耗时与盲化视觉质量，并按 owner 归因失败。
6. `[条件后置] 通用设计与性能`：固定案例质量稳定后扩展无 Skill 能力；只有质量不退化时才优化速度。
