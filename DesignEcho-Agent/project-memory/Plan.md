...
25. `[D-101 已完成 / 独立 worktree]`：OpenAI-compatible 成功流的请求 JSON /图像 data URL 字节、adapter /测量税、stream open、首块、首语义和 Main 完成时间已沿现有 physical attempt → Runtime Accounting → RunRecord / `debug:runs` 接线。专项攻击、编译产物假 DeepSeek 流、Main /Renderer 类型检查、相邻审计、Agent /UXP production build、唯一一次完整核心闸门 58/58、最终差异审查与独立提交均已完成。真实 DeepSeek 阶段覆盖待固定 Case 采集；该切片不进入 D-097 的 r33 单变量基线。
26. `[待完成 r32 reconciliation]`：r32 fixture 设计文档已经关闭，但用户启动的普通 DesignEcho 当前占用默认端口且未绑定 r32。只读刷新确认 Photoshop 当前有 6 个打开文档且 5 个 dirty；不保存、关闭或丢弃这些文档。待默认端口自然释放后用 clean Debug Runtime 完成唯一 reconciliation，不移动账本或跳过 `unreconciled_live_attempt_exists`。
