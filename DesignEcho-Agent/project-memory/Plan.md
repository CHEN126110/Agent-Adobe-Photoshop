...
19. `[已完成·归因待验证]` D-134 正常程序复测（0888b25f、全新一次性项目、干净新对话）已完成：Agent 全程未绑定主图 Profile，走通用工具完成建档 / 置入 / 文字 / 保存导出，终态 failed / needs_review。注入链各环经静态重放与旁证可用（候选产出、引导块逐字节复现、上下文零裁剪、通道透传逻辑无截断、declareDesignIntent 在 allowedTools），但组合后的实际系统提示未被运行实拍，归因保持待验证诊断；关键混杂是主模型已从 deepseek+thinking 切换为 claude-subscription-opus（无思考、无采纳率基线）。单图直接观察与写类收据链按设计工作；同轮发现 Reflexion 容量缺口（预算耗尽仍诞生注定停机的下一代并覆盖终态文案）。
19a. `[已完成]` Reflexion 容量门按续作类型分流：completed 审美返工沿用完整下限，targetStage=E2（含 E2 签发缺目标阶段的补交付）使用不要求视觉候选的交付闭合下限 `AGENT_DELIVERY_CLOSURE_REENTRY_MINIMUM`，Runtime 按结构化目标阶段取对应下限（E2→交付、执行 / 复核阶段→完整）；不提供容量证明的调用方保持旧行为。E2/Runtime 边界断言已入 audit:agent-business-boundaries（视觉耗尽仍够交付必放行、工具耗尽必拒绝、Runtime→R4 视觉耗尽必拒绝、Runtime→E2 视觉耗尽必放行），五态直测全符合。
19b. `[已完成·实机采纳仍未发生]` 主图 Skill 两段式入口治理：唯一 advisory 候选从静态基座抽出为 `buildRuntimeWorkflowCandidatePromptSection`，经 `getDynamicOperatingContext` 逐轮渲染（每轮 `refreshPrimarySystemMessage` 重组）；declareDesignIntent 本身即结构化选择手柄（taskTypeId/workMode 均为 enum），声明成功时 `runtimeParams.declaredTaskType`、owner 绑定与 Skill 可见性在同一 Tool result 边界提交，下一轮候选提示立即消失。audit:skill-package-contract 断言已迁移到新架构。D-135 最小复现按首偏差纪律在通用 createDocument 出现即停（第 6 工具、4 分 31 秒），随后用主进程 inspector 实拍第一次模型调用的真实出站 customSystemPrompt（10,079 字符）：候选段逐字在场——投递链 Harness 侧实拍闭合；opus 自然句采纳 0/2，属模型采纳行为（thinking 恒 disabled 为已知混杂）。绑定后候选移除因至今无实机声明样本仍只有断言级验证。
19b-next. `[待开始]` 采纳行为专项：在候选投递已实拍无缺的前提下，评估 thinking 接通（19c）与候选呈现位置（system 尾部 vs 任务消息邻接）对 opus 采纳率的影响；任何调整先在开发探针上 A/B，不做单样本生产修补。
19c. `[待开始]` 订阅通道 Final Judge 结构化输出根修（D-134 score_batch_invalid：opus 返回评审散文而非机读评分批次）；订阅通道 requestedThinking 恒为 disabled 与用户偏好 thinking.enabled=true 的矛盾按「只用用户设置的模型、禁静默降级」原则处置（接通或诚实标注）。
