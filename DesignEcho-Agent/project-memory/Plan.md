...
3. `[代码切片完成·真实适用性待样本] 姿态统一`：离线算法、版本化单事务 Provider、三次隔离真机 E2E、面板迁移、documentId 守卫、构建与核心闸门均已取得各自证据。官方 UXP Debug 只暴露插件壳和单向 `postMessage`，WebView 子窗口也不作为 Computer Use 独立目标，真实按钮点击仍待可达路径；不得为测试限制新增产品旁路。指定商品 8 张、跨商品 36 + 48 张和 C-1024 六张代表图均未找到适用的自然中等弯曲单袜，因此停止盲搜或制造合成商业案例，真实适用性与视觉质量待代表样本。
4. `[进行中] 依赖迁移与 Smile Provider`：分别审计、验证、提交和回滚，不与姿态或业务 Skill 混合。
