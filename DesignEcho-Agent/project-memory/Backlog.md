...
1. **INTAKE-083 / 084**：代码根因与核心验证已闭合；等待当前提交的 fresh sidecar Attempt 和人工校准证据，不把自动绿测写成实机完成。
2. **INTAKE-090 运行效率归因**：历史证据已定位到模型高轮次 /长输出侧；先补现有账本的 call kind、Tool origin、视觉 revision 链和上下文来源桶，再用同一可靠性 Case 做单变量实验。
