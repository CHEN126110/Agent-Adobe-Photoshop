...
function buildClassifierPrompt(): string {
    return [
        ...
        ...listDesignTaskTypeCatalog().map((taskType) => (
            `- ${taskType.id} [${taskType.label}]${taskType.declarationGuidance
                ? `: ${taskType.declarationGuidance}`
                : ''}`
        )),
        '',
        'Routing guidance:',
        ...
        '- When route=autonomous_agent and the request is an open-ended visual design task with no matching installed legacy Skill, select the most specific declarable taskTypeId that owns the primary deliverable. Use design.generic.v1 only when no more specific declarable design task type fits.',
