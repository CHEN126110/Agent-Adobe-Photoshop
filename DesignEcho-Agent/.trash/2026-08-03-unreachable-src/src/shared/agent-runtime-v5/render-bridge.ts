...
export function buildRegionRenderSpecFromDetailPageScreen(
    ...
    for (const region of layoutRegions) {
        ...
        let imagePlacement: ImagePlacementSpec | undefined;
        if (region.role === 'primary_visual' || region.role === 'secondary_visual') {
            const queue = slotQueueByRegionRole.get(region.role) || [];
            const slot = queue.shift();
            if (slot) {
                imagePlacement = {
                    fit: slot.placement.fit,
                    anchor: String(slot.placement.anchor || 'center'),
                    scale: Number.isFinite(Number(slot.placement.scale))
                        ? Number(slot.placement.scale)
                        : 1,
                    rotation: Number.isFinite(Number(slot.placement.rotation))
                        ? Number(slot.placement.rotation)
                        : 0,
                    focalPoint: slot.placement.focalPoint
                        ? {
                            x: Number(slot.placement.focalPoint.x),
                            y: Number(slot.placement.focalPoint.y)
                        }
                        : undefined,
                    mask: slot.mask,
