...
function run() {
  const missingHandoff = buildMainImageProductionExecutorBridgeEvidence({});
  ...
  const blockedCapabilityHandoff = buildMainImageProductionExecutorHandoffEvidence({
    productionExecutionPlanEvidence: buildExecutionPlan(),
    availableToolNames: photoshopWriteTools.filter((tool) => tool !== 'exportGroup'),
    outputDir: 'C:/Exports'
  });
  const blockedHandoff = buildMainImageProductionExecutorBridgeEvidence({
    productionExecutorHandoffEvidence: blockedCapabilityHandoff,
    availableToolNames: allBridgeTools
  });
  assert(blockedHandoff.status === 'blocked_handoff_not_ready', 'bridge should block non-ready handoff evidence', blockedHandoff);
  ...
  const missingReadback = buildMainImageProductionExecutorBridgeEvidence({
    productionExecutorHandoffEvidence: readyDryRunHandoff,
    availableToolNames: photoshopWriteTools
  });
  ...
  const dryRunBridge = buildMainImageProductionExecutorBridgeEvidence({
    productionExecutorHandoffEvidence: readyDryRunHandoff,
    availableToolNames: allBridgeTools,
    mode: 'dry-run-bridge'
  });
  ...
  const liveWithoutApproval = buildMainImageProductionExecutorBridgeEvidence({
  ...
  const liveDisconnected = buildMainImageProductionExecutorBridgeEvidence({
    productionExecutorHandoffEvidence: buildReadyHandoff('executor-handoff'),
    availableToolNames: allBridgeTools,
    mode: 'live-executor-bridge',
  ...
  assert(strategyInputs.productionExecutorBridgeEvidence, 'strategy input builder should expose executor bridge evidence', strategyInputs);
  assert(strategyInputs.productionExecutorBridgeEvidence.status === 'blocked_missing_executor_capability', 'strategy builder default should require readback tools before bridge readiness', strategyInputs.productionExecutorBridgeEvidence);
  assert(strategyInputs.productionExecutorBridgeEvidence.executorQueue.length === 0, 'strategy builder must not emit executor queue by default', strategyInputs.productionExecutorBridgeEvidence);
  ...
  console.log(JSON.stringify({
    ...
    checks: [
      ...
      'strategy input builder exposes bridge evidence without changing execution'
    ]
  }, null, 2));
}
