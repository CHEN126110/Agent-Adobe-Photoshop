...
function buildFixture() {
  const assetIndex = buildProjectAssetIndex({
    ...
    files: [
      {
        path: 'D:/A1 neveralone flagship/C-1160/素材/白色.jpg',
      ...
      {
        path: 'D:/A1 neveralone flagship/C-1160/素材/黑色.jpg',
      ...
      {
        path: 'D:/A1 neveralone flagship/C-1160/SKU/2双装/白色+黑色.jpg',
...
function run() {
  ...
  const missingTargetPlanner = buildSkuBatchPlannerEvidence(buildBaseInput(fixture, {
  ...
  const planner = buildSkuBatchPlannerEvidence(buildBaseInput(fixture, {
  ...
  assert(dpi.candidates[0].visualEvidence.status === 'cached_insight', 'SKU DPI should use cached visual insight', dpi.candidates[0]);
  assert(dpi.candidates[0].caseEvidence.matchedCaseIds.includes('eagle-case:eagle-sku-1'), 'SKU DPI should attach Eagle case metadata when tags match', dpi.candidates[0].caseEvidence);
