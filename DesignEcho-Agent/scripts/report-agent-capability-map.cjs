...
function layerHasOperationalFields(body) {
  return {
    currentStatus: body.includes('当前状态'),
    evidenceOrScope: body.includes('已存在证据') || body.includes('当前业务场景') || body.includes('当前样本'),
...
function buildReport() {
  ...
  const layerReports = REQUIRED_LAYERS.map((required) => {
    ...
    const fields = layer ? layerHasOperationalFields(layer.body) : {};
