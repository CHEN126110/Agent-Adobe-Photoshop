...
async function writePreflightReport() {
  ...
  const result = {
    ...
    boundaries: [
      ...
      'currentBridgeReady only means the currently running Agent/MCP/UXP bridge is reachable; it does not prove any design task or aesthetic quality.',
  ...
  try {
    ...
    const result = {
      ...
      cases: [{
        ...
        evidence: documentExecution.report.evidence
      }],
      liveAssertions,
    ...
    if (INCLUDE_LAYER_ORDER_CASE && layerExecution) {
      ...
      result.cases.push({
        ...
        evidence: layerExecution.report.evidence,
