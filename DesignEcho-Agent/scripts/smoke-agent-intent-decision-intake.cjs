...
function assertBoundary(intake) {
  assert(intake.evidenceOnly === true, 'intake must be evidence-only', intake);
...
assert(missingLifecycle.requiredNextEvidence.includes('agent_request_lifecycle_required'), 'missing lifecycle should require lifecycle evidence', missingLifecycle);
...
assert(
  autonomous.requiredNextEvidence.includes('autonomous_route_context_and_budget_review'),
...
assert(
  routeMismatch.requiredNextEvidence.includes('skill_decision_review_required'),
...
assert(toolMismatch.requiredNextEvidence.includes('tool_call_evidence_required'), 'tool mismatch should require tool evidence', toolMismatch);
...
assert(report.evidence.agentIntentDecisionIntake?.status === 'deterministic_skill_ready', 'report should expose intent decision intake', report.evidence);
assert(
...
const serialized = JSON.stringify({
  ...
  routeMismatch,
  toolMismatch,
  report: report.evidence.agentIntentDecisionIntake
});
...
console.log(JSON.stringify({
  ...
  checks: [
    ...
    'autonomous route requires review evidence',
    'route and tool evidence mismatches are visible',
