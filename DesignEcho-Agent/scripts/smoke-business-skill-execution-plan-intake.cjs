...
function buildExecutionPlan(stepCount = 2) {
  return {
    ...
    steps: Array.from({ length: stepCount }, (_, index) => ({
      ...
      expectedEvidence: ['tool result', 'bounds']
    })),
    evidence: [{ source: 'fixture-plan', summary: 'fixture execution plan', status: 'needs_review' }],
...
function buildExecutionTrace(options = {}) {
  ...
  return {
    ...
    evidence: [{ source: 'fixture-trace', summary: 'fixture execution trace', status: failed ? 'failed' : 'passed' }]
  };
}
...
function buildVerificationReport(status = 'needs_review') {
  return {
    ...
    evidence: [{ source: 'fixture-verification', summary: 'fixture verification evidence', status }]
  };
}
...
function buildPlacementIntake(status = 'verified_by_bounds') {
  return {
    ...
    sourceEvidence: ['image_placement_plan', 'photoshop_actual_bounds'],
    requiredNextEvidence: status === 'verified_by_bounds' ? [] : ['photoshop_actual_bounds_required'],
    warnings: [],
...
function runSharedHelperChecks() {
  ...
  assert(noPlan.status === 'no_execution_plan_evidence', 'missing designAgentOs executionPlan should be explicit', noPlan);
  ...
  assert(noPlan.requiredNextEvidence.includes('design_agent_os_execution_plan_required'), 'missing plan should require Design Agent OS execution plan');
  ...
  assert(planOnly.executionPlanEvidence.stepCount === 2, 'plan-only should count execution plan steps');
  assert(planOnly.requiredNextEvidence.includes('execution_trace_required'), 'plan-only should require execution trace');
  ...
  assert(executed.executionPlanEvidence.hasExecutionTrace === true, 'executed intake should detect execution trace');
  assert(executed.executionPlanEvidence.toolCallCount === 2, 'executed intake should count tool calls');
  assert(executed.executionPlanEvidence.placementVerificationStatus === 'verified_by_bounds', 'executed intake should include placement status');
  ...
  assert(failed.status === 'failed_execution_evidence', 'failed trace or verification should fail execution plan intake', failed);
...
function runSourceChecks() {
  ...
  assert(indexSource.includes('attachBusinessSkillExecutionPlanIntakeToResult'), 'unified executor should attach execution plan intake');
  ...
  assert(boundarySource.includes('business-skill-execution-plan-intake'), 'change boundaries should classify execution plan intake');
}
...
async function run() {
  ...
  console.log(JSON.stringify({
    ...
    checks: [
      ...
      'plan-only evidence requires execution trace before execution can be treated as evidenced',
      'execution trace evidence is summarized without claiming design quality',
      'failed trace or verification blocks execution evidence',
