...
assert(xiaomi.providerThinkingDelta.status === 'supported', 'xiaomi reasoning_content can emit provider thinking delta on tool stream', xiaomi);
assert(xiaomi.finalProviderThinking.status === 'supported', 'xiaomi reasoning_content can emit final provider thinking', xiaomi);
...
assert(deepseek.providerThinkingDelta.status === 'unsupported', 'deepseek tool path must not claim provider thinking delta', deepseek);
assert(deepseek.finalProviderThinking.status === 'unsupported', 'deepseek tool path must not claim final provider thinking', deepseek);
assert(deepseek.warnings.some((warning) => warning.includes('tool path disables thinking')), 'deepseek matrix should explain tool-path thinking boundary', deepseek);
...
assert(source.includes('tool path disables thinking'), 'shared module should document deepseek tool-path thinking boundary');
...
console.log(JSON.stringify({
  ...
  checks: [
    ...
    'DeepSeek tool path does not claim provider thinking',
