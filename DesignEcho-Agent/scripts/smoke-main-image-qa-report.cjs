...
const { buildMainImageVisionPreflightEvidence, buildMainImageVisionPreflightPlan } = require(path.join(repoRoot, 'src/shared/main-image-vision-preflight.ts'));
...
function toReportInput(evidence) {
  return {
    agentDraft: evidence.draft,
    candidatePreflight: evidence.candidatePreflight,
    visionPreflight: evidence.visionPreflight,
    executionAlignment: evidence.executionAlignment,
    screenshotQa: evidence.screenshotQa,
    screenshotProbeReadiness: evidence.screenshotProbeReadiness
  };
}
...
record('needs-context-when-agent-draft-is-not-ready', () => {
  const evidence = buildEvidence({ noSubject: true, noSizePlan: true, noScreenshotQa: true });
  const report = buildMainImageQaReport({
    ...toReportInput(evidence),
...
record('needs-result-image-when-no-screenshot-evidence-exists', () => {
  const evidence = buildEvidence({ noScreenshotQa: true });
  const report = buildMainImageQaReport({
    ...toReportInput(evidence),
    screenshotQa: null,
    screenshotProbeReadiness: null
...
record('needs-probe-target-when-result-files-exist-without-reference', () => {
  const evidence = buildEvidence({ pixelProbe: null, referenceImagePath: '' });
  const report = buildMainImageQaReport(toReportInput(evidence));
...
record('source-and-output-have-no-mojibake-or-raw-image-leak', () => {
  ...
  const report = buildMainImageQaReport(toReportInput(buildEvidence({ referenceImagePath: '', pixelProbe: null })));
