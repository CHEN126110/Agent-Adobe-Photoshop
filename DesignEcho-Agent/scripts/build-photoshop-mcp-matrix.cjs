...
function renderMarkdown(report) {
  ...
  lines.push('');
  lines.push('| Tool | Category | Kind | Auto Smoke | Execution Lane | Validation Mode | Popup Risk | Preconditions | Notes |');
  lines.push('|---|---|---|---|---|---|---|---|---|');
  for (const item of report.matrix) {
    lines.push(
      `| ${item.toolName} | ${item.category} | ${item.kind} | ${item.autoSmoke} | ${item.executionLane.lane} (${item.executionLane.recommendedBatchSize}/${item.executionLane.recommendedDelayMs}ms) | ${item.manualValidationMode} | ${item.popupRisk} | ${item.preconditions.replace(/\|/g, '\\|')} | ${item.notes.replace(/\|/g, '\\|')} |`
    );
  }
  lines.push('');
  return lines.join('\n');
...
function main() {
  ...
  const summary = matrix.reduce((acc, item) => {
    ...
    return acc;
  }, { runtimeTools: 0, safe: 0, conditional: 0, manual: 0, manualRisky: 0, blocked: 0 });
