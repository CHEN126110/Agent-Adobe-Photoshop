...
function run() {
  ...
  assert(readyGate?.status === 'ready_for_existing_execution', 'ready evidence should allow existing execution', readyGate);
  ...
  assert(missingGate?.status === 'needs_context', 'missing project context should be needs_context evidence only', missingGate);
  const missingWrapped = attachBusinessSkillExecutionPreflightGateToResult(
    { success: true, message: 'ok' },
    missingGate
  );
  assert(missingWrapped.success === true, 'needs_context evidence must not change executor result success');
  ...
  assert(indexSource.includes('buildBusinessSkillExecutionPreflightGateForSkill'), 'executor entrypoint should build execution preflight');
  assert(indexSource.includes('attachBusinessSkillExecutionPreflightGateToResult'), 'executor entrypoint should attach execution preflight');
  ...
  console.log(JSON.stringify({
    ...
    checks: [
      ...
      'missing context stays evidence-only and does not block executor result',
