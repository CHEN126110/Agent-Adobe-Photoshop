    ...
    documents: [
      {
        ...
        evidenceLevel: 'platform_developer_doc',
        parentGroups: [
          {
            name: '点击图',
            role: 'click-images',
            childGroups: [
              {
                ...
                requiredEvidence: ['actualBounds', 'screenshot', 'exportFile']
              }
            ]
          },
          {
...
function run() {
  ...
  assert(readyCurrent.documents.length === 1, 'contract should preserve document grouping evidence', readyCurrent);
  ...
  const invalidProduction = buildMainImageGroupHierarchyContract({
    productionDocumentStructureEvidence: buildProductionFixture({
      documents: [
        {
  ...
  assert(ready.verificationReport.status === 'passed', 'ready contract verification should pass as no-write readiness evidence', ready.verificationReport);
