...
const BOUNDARIES = [
  ...
  },
  {
    ...
    match: (entry) => entry.filePath === 'DesignEcho-Agent/docs/model-settings-configuration.md'
      ...
      || /agent-(completion-message-consistency|action-disposition|final-outcome-signals)|quality-history-closure|design-task-policy|skill-execution-effect|provider-model-(listing-service|merge)|model-listing-handlers|test-model-usage-classification|verify-(agent-final-outcome-signals|run-fact-ledger)/.test(entry.filePath)
  },
  {
