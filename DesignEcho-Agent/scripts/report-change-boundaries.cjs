...
const BOUNDARIES = [
  ...
  },
  {
    ...
    match: (entry) => entry.filePath === 'DesignEcho-Agent/docs/model-settings-configuration.md'
      ...
      || /diagnose-runs|agent-(completion-message-consistency|action-disposition|final-outcome-signals|response-interruption)|quality-history-closure|design-task-policy|skill-execution-effect|task-run-document-creation-evidence|provider-model-(listing-service|merge)|model-(listing|download)-handlers|test-model-usage-classification|verify-(agent-final-outcome-signals|run-fact-ledger)/.test(entry.filePath)
  },
  {
