...
const BOUNDARIES = [
  ...
  },
  {
    ...
    validation: [
      ...
      'npm run maintenance:preflight:core',
  ...
  },
  {
    ...
    match: (entry) => /semantic-(target|region|matting-guidance)|matting-source-export-geometry|photoshop-(workflow-dispatch|remove-background-workflow)|grounding-(dino|candidate-filter)|target-phrase|mask-regions|sam-postprocessing|guided-filter|bert-wordpiece-tokenizer|src\/main\/services\/sam-service\.ts|src\/main\/uxp-handlers\/visual-handlers\.ts/.test(entry.filePath)
  },
  {
  ...
  },
  {
    ...
    match: (entry) => /delivery|runtime-final-artifact-paths|staged-file|staging-transaction|file-promotion|export-transaction|agent-skill-atomic-tool-execution|agent-workflow-continuation-scope|slice-export|export-group|tools\/canvas\/save-document|test-image-target-fit/.test(entry.filePath)
  },
  {
  ...
  {
    id: 'design-experience-evaluation',
    title: '设计经验候选、发布与生产评审边界',
    validation: [
      'npm run test:design-learning-candidates',
      'npm run test:design-evaluator',
      'npm run test:design-authorship-boundary',
      'npm run audit:tools',
      'npm run build:typecheck:renderer'
    ],
    match: (entry) => /design-learning-candidates|design-quality-assertion|design-workshop\/(design-evaluator|evaluate-design\.executor)|verify-design-(learning-candidates|evaluator)/.test(entry.filePath)
  },
  {
  ...
  },
  {
    ...
    match: (entry) => /mcp-host-(service|client)|mcp-host\.client|check-photoshop-bridge-health|photoshop-controlled-(script|text-style|export|image-placement)-execution|smoke-photoshop-controlled-(script|text-style|export|image-placement)-execution|smoke-photoshop-rasterize-popup-guard-live|src\/main\/websocket\/server\.ts|smoke-uxp-agent-connection-recovery|smoke-photoshop-mcp|smoke-uxp-group-export-tool|uxp-handlers\/(index|webview-handlers)\.ts|uxp-handlers\/types|uxp-handlers\/template-library-handlers|binary-protocol|websocket-client|DesignEcho-UXP\/scripts\/smoke-(create-shape-execution-guard|image-generation-error-helpers|image-generation-options|image-to-image-selection|template-library-core|template-library-state-coordinator|webview-panel-layout|webview-message-core|image-safety|inpainting-image-safety|layer-property-effect-execution-guard|layer-selection-layout-guard|modal-behavior-execution-guard|place-event-image-safety-guard|place-image-execution-guard|reorder-layer-execution-guard|save-export-execution-guard|smart-object-content-execution-guard|tool-error-normalizer|transform-layer-execution-guard|write-tool-failure-normalizer|sock-layout-config|sku-layout-combos-validation|sku-auto-layout-plan|sku-auto-layout-post-qa|sku-layout-auto-planner-integration|main-image-white-bg-from-sku-tool)\.cjs|DesignEcho-UXP\/src\/index\.ts|DesignEcho-UXP\/src\/core\/(base64|canvas-refresh|friendly-progress|image-generation-errors|image-generation-options|image-to-image-selection|image-generation-stage-labels|template-library-core|template-library-state-coordinator|webview-panel-layout|webview-message-core|jsx-bridge|message-handler|mcp-protocol|image-safety|tool-error-normalizer|photoshop-document-state|photoshop-bit-depth)\.ts|DesignEcho-UXP\/src\/types\/photoshop\.d\.ts|DesignEcho-UXP\/src\/tools\/registry\.ts|DesignEcho-UXP\/src\/tools\/types\.ts|DesignEcho-UXP\/src\/tools\/sku\/|DesignEcho-UXP\/src\/tools\/text\/|DesignEcho-UXP\/src\/tools\/layer\/(layer-properties|layer-effects|replace-content|smart-object-tools|transform-layer)|DesignEcho-UXP\/src\/tools\/canvas\/(close-document|create-document|create-shape|get-document-info|list-documents|save-document)|DesignEcho-UXP\/src\/tools\/layout\/(align-layers|align-to-reference|distribute-layers|get-layer-hierarchy|move-layer|focus-layer|select-layer|smart-layout-engine)|DesignEcho-UXP\/src\/tools\/image\/(export-group|export-main-image-docs|inpainting|place-image|white-bg-from-sku-material)|screen-snapshot|optimized-image-transfer|export-layer|get-subject-bounds|remove-background|clipping-mask|rename-layer|reorder-layer|sku-layout-tool|visual-analysis/.test(entry.filePath)
  },
  {
  ...
  },
  {
    ...
    match: (entry) => /compose-design|compound-photoshop-write-settlement|resolved-image-placement-preflight|verify-compose-design-spec|ecommerce-socks-design|ecommerce-socks-strategy-checkpoint|ecommerce-socks-child-strategy-packets|ecommerce-socks-child-strategy-review-gate|ecommerce-socks-child-strategy-handoff|ecommerce-socks-child-strategy-consumer|detail-page\.executor|detail-page-parser|detail-page-screen-plan|smoke-detail-page-(template-(parser-policy|live-case|export-policy)|agent-decision-boundary)|detail-page-design\.skill|main-image-design\.skill|main-image-(agentic-(finalize-runtime|production|workspace)|strategy-contract|asset-hero-strategy|project-style-strategy|design-standards(?:-evidence)?|design-readiness-report|execution-alignment|state-consumption|live-executor-request|live-executor-(checkpoint|runner)|controlled-product-qa-gate|controlled-product-qa-bridge|live-photoshop-adapter-contract|live-adapter-handoff|live-photoshop-tool-adapter|photoshop-tool-capability-matrix|group-hierarchy-contract|variant-placement-strategy|production-spec|production-document-structure|production-execution-plan|production-executor-handoff|production-executor-bridge|production-executor-dry-run|copy-strategy|design-concept-plan|design-core|white-background-export-contract)|main-image\.executor|sku-batch\.executor|sku-delivery-summary|sku-export-readback|sku-configured-execution-plan|sku-layout-execution-batches|sku-auto-layout-executor-policy|sku-visual-review-intake|sku-color-card-retouch-strategy|sku-color-card-image-probes|text-font-replace\.executor|matte-product\.executor|template-save\.executor|project-image-analysis(\.executor|-intent)|agent-panel-bridge\.executor|design-reference-search\.executor|visual-analysis\.executor|find-edit-element\.executor|smart-layout\.executor|skill-step-events|sku-self-select-note-policy|sku-intent-params|sku-template-design-loop|verify-sku-template-handoff|knowledge\/socks-categories|agent-system-prompt|smoke-sku-(self-select-note-policy|contrast-pair-policy|intent-params|result-status|project-source-policy|execution-manifest|configured-execution-plan|auto-layout-executor-policy|no-placeholder-live-acceptance|export-readback|visual-review-intake|color-card-retouch-strategy|color-card-image-probes|c1163-live-acceptance)|smoke-main-image-(design-skill|strategy-contract|strategy-input-builder|dpi-readonly-evidence|asset-hero-strategy|project-style-strategy|design-standards-evidence|design-readiness-report|live-executor-request|live-executor-(checkpoint|runner)|controlled-product-qa-gate|controlled-product-qa-bridge|live-photoshop-adapter-contract|live-adapter-handoff|live-photoshop-tool-adapter|disposable-product-e2e|executor-controlled-product-branch|uxp-toolchain-live|photoshop-tool-capability-matrix|group-hierarchy-contract|variant-placement-strategy|production-structure|production-execution-plan|production-executor-handoff|production-executor-bridge|production-executor-dry-run|copy-strategy|design-concept-plan|design-core|white-bg-sku-material-contract)|smoke-uxp-layer-hierarchy-tools|smoke-uxp-group-export-tool|smoke-text-font-replace-skill|smoke-matte-product-skill|smoke-template-project-observability|smoke-analysis-reference-observability|smoke-find-smart-observability|smoke-text-optimization-contract|smoke-copywriting-framework|design-copywriting-framework|uxp-handlers\/text-handlers|asset-subject-box-service|matting-service|resource-manager-service|smart-layout-service|autonomous-agent\.executor|skill-executors\/index\.ts|skill-executors\/types\.ts|tool-executor\.service|template-knowledge\.service|reference-analysis|skill-param-defaults|skills\/skill-tool-schema|types\.d\.ts|types\/skill\.types/.test(entry.filePath)
  },
  {
    id: 'package-and-deps',
    title: '包配置、依赖与构建配置',
    validation: ['npm install consistency check if dependency versions changed', 'npm run test:sharp-runtime-compatibility', 'npm run build'],
    match: (entry) => /package(-lock)?\.json$|verify-sharp-runtime-compatibility/.test(entry.filePath)
      || entry.filePath.endsWith('webpack.config.js')
  },
  {
