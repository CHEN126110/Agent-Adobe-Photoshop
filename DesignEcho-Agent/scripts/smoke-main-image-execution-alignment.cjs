...
record('watch-when-copy-or-visual-review-is-not-backed-by-evidence', () => {
  ...
  assert(alignment.warnings.some((item) => item.includes('文本写入证据')), 'copy execution warning should be visible');
...
record('blocked-when-context-or-subject-evidence-is-missing', () => {
...
record('aligned-when-plan-dsl-tools-and-review-evidence-are-covered', () => {
  const draft = buildMainImageAgentDraftPlan({
    ...readyDraftInput,
    manualReview: {
      decision: 'approved',
...
record('executor-and-maintenance-wiring-is-present', () => {
  ...
  assertIncludes(executor, 'buildMainImageExecutionAlignment', 'executor missing execution alignment builder');
  assertIncludes(executor, 'mainImageExecutionAlignment', 'executor missing result data exposure');
