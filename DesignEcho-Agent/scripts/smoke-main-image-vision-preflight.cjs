...
function assertNoUngroundedConfidence(text, label) {
  assert(!String(text).includes('confidence'), `${label} should not expose ungrounded confidence`);
  assert(!String(text).includes('置信'), `${label} should not expose ungrounded confidence wording`);
}
...
record('successful-analysis-maps-to-vision-signal', () => {
  ...
  assert(signal.evidence.some((item) => item.includes('mainSubject=堆堆袜')), 'evidence missing subject');
  assertNoUngroundedConfidence(JSON.stringify(signal), 'vision signal');
  return { productType: signal.productType, evidenceCount: signal.evidence.length };
});
...
record('failed-analysis-does-not-create-vision-signal', () => {
  ...
  const evidence = buildMainImageVisionPreflightEvidence({
    plan,
    result: { success: false, error: 'quota exceeded' }
  });
  assert(evidence.resultStatus === 'failed', 'failed result should be failed');
  assert(!evidence.visionSignal, 'failed result must not create signal');
  assertIncludes(evidence.error, 'quota exceeded', 'error should be preserved');
  return { resultStatus: evidence.resultStatus, error: evidence.error };
});
...
record('executor-and-skill-declaration-are-wired', () => {
  ...
  const plannerEvidence = fs.readFileSync(path.join(repoRoot, 'src/renderer/services/skill-executors/design-planner-evidence.ts'), 'utf8');
  const declarations = fs.readFileSync(path.join(repoRoot, 'src/shared/skills/skill-declarations.ts'), 'utf8');
  assertIncludes(executor, 'analyzeAssetContent[main-image-vision-preflight]', 'executor missing preflight tool evidence');
  assertIncludes(executor, 'mainImageVisionPreflight', 'executor missing data exposure');
  assertIncludes(plannerEvidence, 'visionSignal: input.visionSignal', 'planner evidence does not pass visionSignal');
...
record('source-and-output-have-no-mojibake', () => {
  ...
  assertNoMojibake(JSON.stringify(evidence, null, 2), 'preflight evidence');
  assertNoUngroundedConfidence(source, 'main-image-vision-preflight.ts');
  assertNoUngroundedConfidence(JSON.stringify(evidence, null, 2), 'preflight evidence');
  return { sourceLength: source.length, status: evidence.status };
});
