...
function assertAllInputsProvided(evidence) {
  for (const key of requiredInputs) {
    assert(Object.prototype.hasOwnProperty.call(evidence.strategyInputs, key), `missing ${key}`, evidence);
  }
  assert(evidence.missingInputs.length === 0, 'ready context should not miss strategy inputs', evidence);
}
...
function run() {
  ...
  assert(empty.status === 'blocked_missing_strategy_evidence', 'empty context should be blocked by missing evidence', empty);
  ...
  assertNoRawPayload(empty, 'empty strategy input evidence');
  ...
  assertNoRawPayload(ready, 'ready strategy input evidence');
    ...
    knowledgeResults: [
      {
        ...
        evidence: ['Source URL: https://example.com/socks-main-image', 'Boundary: external web knowledge only.'],
  ...
  assert(
    readyWithKnowledge.projectStyleStrategyEvidence.referenceResearchPlan.referenceHintCount === 1,
    'knowledge results should map into main-image reference hints',
    readyWithKnowledge.projectStyleStrategyEvidence.referenceResearchPlan
  );
  assert(
    readyWithKnowledge.copyEvidence.productCopyContext.referenceNotes.some((note) => note.includes('袜子主图参考')),
    'mapped knowledge should reach copy evidence reference notes',
    readyWithKnowledge.copyEvidence.productCopyContext
  );
  assert(
  ...
  assertNoRawPayload(readyWithKnowledge.projectStyleStrategyEvidence.referenceResearchPlan, 'mapped main-image references');
  assertNoConfidence(readyWithKnowledge.projectStyleStrategyEvidence.referenceResearchPlan, 'mapped main-image references');
  ...
  assert(withoutSubject.status === 'blocked_missing_strategy_evidence', 'missing subject bounds should block strategy evidence', withoutSubject);
  ...
  assert(Boolean(withoutSubject.strategyInputs.assetSelectionPolicy), 'asset policy should still be grounded when asset exists', withoutSubject);
  assertNoRawPayload(withoutSubject, 'subject-missing strategy input evidence');

  const draft = buildMainImageAgentDraftPlan({
    ...readyContext,
    strategyReviewGate: buildApprovedGate()
  });
  assert(draft.mainImageStrategyInputEvidence, 'draft should expose generated strategy input evidence', draft);
  assert(draft.mainImageStrategyInputEvidence.status === 'ready_for_strategy_contract', 'draft generated evidence should be ready', draft.mainImageStrategyInputEvidence);
  ...
  assertNoRawPayload(draft.mainImageStrategyInputEvidence, 'draft strategy input evidence');
  ...
  assert(
    draftWithReferenceHints.mainImageStrategyInputEvidence.projectStyleStrategyEvidence.referenceResearchPlan.referenceHintCount === 1,
    'draft plan should pass reference hints into generated strategy evidence',
    draftWithReferenceHints.mainImageStrategyInputEvidence.projectStyleStrategyEvidence.referenceResearchPlan
  );
  ...
  assert(draftWithoutSubject.mainImageStrategyInputEvidence, 'draft without subject should still expose evidence object', draftWithoutSubject);
  ...
  console.log(JSON.stringify({
    ...
    checks: [
      ...
      'knowledge results map into main-image reference evidence without confidence fields',
