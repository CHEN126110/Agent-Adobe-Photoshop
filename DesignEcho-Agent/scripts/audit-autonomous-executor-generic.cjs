...
function run() {
    ...
    const performancePromptBuilder = findFunction(sourceFile, 'buildPerformancePromptSection');
    ...
    const automaticTaskContextFlag = findVariableDeclaration(
        sourceFile,
        'shouldBuildAutomaticTaskContext'
    );
    ...
    const visualScenarioPreparation = findFunction(
        businessVisualContextWrapperSourceFile,
    ...
    const visualScenarioDerivationViolations = [
        ...(identifierCount(visualScenarioPreparation, 'assetIndex') > 0 ? [] : ['missing:reuse-asset-index']),
        ...(identifierCount(visualScenarioPreparation, 'buildProjectVisualSamplingPlan') > 0
            ? []
            : ['missing:derive-visual-sampling-plan']),
        ...(identifierCount(visualScenarioPreparation, 'buildContextSnapshot') > 0
            ? []
            : ['missing:derive-context-snapshot'])
    ];
    ...
    const checks = [
        ...
        {
            id: 'visual-scenario-derived-without-project-rescan',
            description: '业务 Skill 切换视觉采样场景必须优先复用提交时 AssetIndex 派生计划，不得把全项目扫描作为正常路径',
            violations: visualScenarioDerivationViolations
        },
        {
            id: 'adaptive-design-evidence-path',
            description: '开放设计任务必须按 Brief 选择最短充分证据链，不得恢复固定四步法或逐原子截图仪式',
            violations: [
                ['fixed-four-step', sourceText.includes('这四步是顺序不是清单')],
                ['fixed-reference-first', sourceText.includes('先参考搜索')],
                ['screenshot-every-atomic-change', sourceText.includes('每次原子改动后')]
            ].filter(([, present]) => present).map(([name]) => name)
        },
        {
            id: 'automatic-task-context-is-fallback',
            description: '自动 TaskContext 只在已绑定 Runtime 且 Manifest 没有方法论知识时回退构建',
            violations: [
                ['missing-fallback-flag', !automaticTaskContextFlag],
                ['missing-runtime-bundle-condition', automaticTaskContextFlag
                    && identifierCount(automaticTaskContextFlag, 'runtimeContractBundle') === 0],
                ['missing-method-knowledge-condition', automaticTaskContextFlag
                    && identifierCount(automaticTaskContextFlag, 'designMethodKnowledge') === 0]
            ].filter(([, present]) => present).map(([name]) => name)
        },
        {
            id: 'opening-update-is-not-permission-gate',
            description: '用户可见开场只承担进度沟通，不得用固定字数或动作关键词充当执行许可',
            violations: literalTexts(performancePromptBuilder, sourceFile)
                .filter((value) => /12\s*个中文字符|将\/会\/先\/正在\/准备\/开始|明确动作语义/.test(value))
                .map(compactSnippet)
        }
    ];
