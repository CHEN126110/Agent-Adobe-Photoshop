...
async function main() {
  ...
  try {
    ...
    assert(assistantMessages.some((message) => message.hasBusinessVisualEvidenceFeedback), 'assistant message should persist businessVisualEvidenceFeedback.');
    assert(businessPreflightCardTitles.length > 0, 'businessVisualEvidenceFeedback should render as a business preflight card.');
    assert(cardTitles.some((title) => title.includes('业务预检')), `card titles should include 业务预检: ${cardTitles.join(', ')}`);
    assert(!thinkingBlockTitles.some((title) => title.includes('业务预检')), 'business preflight feedback must not render as a thinking block.');
    ...
    const result = {
      ...
      checks: [
        ...
        'Message parser rendered the feedback as a 业务预检 card.',
        '业务预检 did not enter thinkingBlockTitles.',
