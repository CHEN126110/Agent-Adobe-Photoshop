...
function buildReport() {
  ...
  gates.push(buildGate({
    id: 'intent-routing',
    title: 'Agent-first 入口与能力边界',
    goal: '普通自然语言直接进入通用 Agent；Harness 只执行 deny-only 能力边界、结构化续跑和不可逆确认。',
    status: inferStatus([
      ...
      fileIncludes(agentRoot, 'src/renderer/services/design-agent/engine.ts', 'pre_router_bypassed')
    ], [
      '仍需要真实 Provider → Electron → Photoshop E2E 验证开放语义与执行点约束。'
    ]),
    evidence: [
      '普通自然语言不再调用前置 task-classifier，直接进入 autonomous Agent',
      'routing/control-plane 只保留无模型兼容和显式 deny-only 能力边界',
      'ToolDecision / Preflight / HITL 在执行点强制确定性不变量'
    ],
    gaps: [
      '尚未完成真实用户语料回归与 Provider-to-Photoshop E2E。'
    ],
    validations: ['npm run maintenance:validate', 'npm run build']
  }));
