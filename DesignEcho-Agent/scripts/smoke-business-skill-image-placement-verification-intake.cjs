...
function runSharedHelperChecks() {
  const noEvidence = buildBusinessSkillImagePlacementVerificationIntake({
    skillId: 'main-image-design',
    resultData: {}
  });
  assert(noEvidence.status === 'no_placement_evidence', 'missing placement evidence should be explicit', noEvidence);
  assert(noEvidence.userVisible === false, 'placement intake must stay hidden');
  assert(noEvidence.canClaimDesignQuality === false, 'placement intake must not claim design quality');
  assert(noEvidence.requiredNextEvidence.includes('image_placement_plan_required'), 'missing plan should require placement plan');
  ...
  assert(plannedOnly.requiredNextEvidence.includes('photoshop_actual_bounds_required'), 'planned-only should require actualBounds');
  assert(plannedOnly.placementEvidence.hasPlacementPlan === true, 'planned-only should detect placement plan');
  assert(plannedOnly.placementEvidence.hasActualBounds === false, 'planned-only must not fabricate actualBounds');
  ...
  assert(boundsVerified.placementEvidence.verifiedCount === 1, 'bounds verification should count one verified placement');
  ...
  const failedScreenshot = buildBusinessSkillImagePlacementVerificationIntake({
    ...
    resultData: {
      imagePlacementPlan: plan,
      imagePlacementActualBounds: plan.execution.destinationBox,
      imagePlacementScreenshotEvidence: {
        available: true,
        reviewStatus: 'failed',
  ...
  [noEvidence, plannedOnly, boundsVerified, failedScreenshot].forEach((item, index) => {
...
function runSourceChecks() {
  ...
  assert(indexSource.includes('attachBusinessSkillImagePlacementVerificationIntakeToResult'), 'unified executor should attach placement intake');
...
async function run() {
  ...
  console.log(JSON.stringify({
    success: true,
    checks: [
      'business skill image placement verification intake summarizes placement plan and actualBounds evidence',
      ...
      'failed screenshot evidence blocks placement verification',
