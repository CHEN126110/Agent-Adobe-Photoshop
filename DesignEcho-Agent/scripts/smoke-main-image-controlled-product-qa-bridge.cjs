...
function run() {
  ...
  assert(runnerOnly.screenshotQaStage === 'needs_pixel_probe', 'explicit result image evidence should enter screenshot QA', runnerOnly);
  ...
  assert(runnerOnly.resultImageSummary.resultImageNames.includes('main-image-click-800.png'), 'bridge should expose only basename result image evidence', runnerOnly.resultImageSummary);
  ...
  assert(completeBridge.stage === 'passed', 'complete evidence bridge should pass evidence stage', completeBridge);
  assert(completeBridge.screenshotQaStage === 'passed', 'screenshot QA should pass with image, pixel and manual evidence', completeBridge);
  ...
  assertIncludes(screenshotQaSource, 'resultImageEvidence?', 'screenshot QA must accept explicit result image evidence instead of fake quickExport tool results');
  assertIncludes(executorSource, 'buildMainImageControlledProductQaEvidence', 'executor must build controlled product QA bridge evidence');
  ...
  console.log(JSON.stringify({
    success: true,
    checks: [
      'controlled live runner result paths enter screenshot QA via explicit result image evidence',
