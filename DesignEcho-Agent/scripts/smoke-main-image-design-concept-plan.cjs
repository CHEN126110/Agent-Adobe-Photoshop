...
function run() {
  ...
  assertNoUngroundedScore(missing, 'missing concept plan');
  ...
  assert(concept.variantConcepts.every((item) => item.factClaims.length > 0), 'each concept should include grounded fact claims', concept.variantConcepts);
  assert(concept.backgroundDirection === visionSignal.agentDecision.backgroundDirection, 'background direction must come from agent decision instead of keyword inference', concept);
  assert(concept.sharedConcept.tone === visionSignal.agentDecision.recommendedTone, 'tone must preserve agent decision', concept.sharedConcept);
  assert(
    strategy.projectStyleStrategyEvidence.variantPlan.clickImages[0].visualHook === visionSignal.agentDecision.clickVisualHooks[0],
    'click visual hook must come from agent decision',
    strategy.projectStyleStrategyEvidence.variantPlan.clickImages[0]
  );
  assert(
    strategy.projectStyleStrategyEvidence.variantPlan.conversionImages[0].layoutFocus === visionSignal.agentDecision.conversionLayoutFocus,
    'conversion layout focus must come from agent decision',
    strategy.projectStyleStrategyEvidence.variantPlan.conversionImages[0]
  );
  ...
  assertNoUngroundedScore(concept, 'ready concept plan');

  const blocked = buildMainImageDesignConceptPlan({
    designCoreEvidence: strategy.designCoreEvidence,
    projectStyleStrategyEvidence: {
      ...strategy.projectStyleStrategyEvidence,
      status: 'needs_vision',
      projectStyleUnderstanding: {
        ...strategy.projectStyleStrategyEvidence.projectStyleUnderstanding,
        semanticStatus: 'needs_vision',
        visualGrounding: 'none'
      },
      variantPlan: { clickImages: [], conversionImages: [] }
    },
    copyEvidence: strategy.copyEvidence,
    variantPlacementStrategyEvidence: strategy.variantPlacementStrategyEvidence
  });
  assert(blocked.status === 'blocked_missing_visual_grounding', 'concept plan must block metadata-only style evidence', blocked);
  ...
  console.log(JSON.stringify({
    ...
    checks: [
      ...
      'blocked concept plan does not fabricate variants without visual grounding'
    ]
  }, null, 2));
}
