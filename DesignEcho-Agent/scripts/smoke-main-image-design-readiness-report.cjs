...
function buildExecutorReadyStrategy(strategy) {
  return {
    ...
    productionExecutorDryRunEvidence: {
      ...strategy.productionExecutorDryRunEvidence,
...
function run() {
  ...
  assert(missing.status === 'blocked_missing_strategy_evidence', 'missing strategy evidence should block readiness', missing);
  ...
  const waitingCheckpoint = buildMainImageDesignReadinessReport({
    strategyInputEvidence: executorReadyStrategy
  });
  ...
  console.log(JSON.stringify({
    success: true,
    checks: [
      'main-image design readiness blocks missing strategy evidence',
