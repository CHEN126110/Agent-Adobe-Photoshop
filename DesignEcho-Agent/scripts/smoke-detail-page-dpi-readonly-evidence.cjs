...
function buildFixture() {
  const assetIndex = buildProjectAssetIndex({
    ...
    files: [
      {
        path: 'D:/A1 neveralone flagship/C-1160/素材/ZQL_0001.jpg',
      ...
      {
        path: 'D:/A1 neveralone flagship/C-1160/素材/白色短袜细节.jpg',
...
function run() {
  ...
  const planner = buildDetailPagePlannerEvidence({
  ...
  assert(dpi.candidates[0].visualEvidence.status === 'cached_insight', 'detail-page DPI should use cached visual insight', dpi.candidates[0]);
  ...
  assert(dpi.candidates[0].caseEvidence.matchedCaseIds.includes('eagle-case:eagle-detail-1'), 'detail-page DPI should attach Eagle case metadata when tags match', dpi.candidates[0].caseEvidence);
