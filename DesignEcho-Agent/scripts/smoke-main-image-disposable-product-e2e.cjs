...
const {
  buildMainImageLiveAdapterHandoffEvidence
...
function buildQaReport(strategy) {
  const selectedAsset = strategy.assetHeroStrategyEvidence.selectedAsset || selectedAssetBase;
  ...
  const visionPreflight = buildMainImageVisionPreflightEvidence({ plan: visionPlan });
...
function buildValidatedToolchainEvidence(contract, options = {}) {
  return {
...
function buildProductPathEvidence(input = {}) {
  ...
  const handoff = buildMainImageLiveAdapterHandoffEvidence({
    adapterContract: contract,
    toolchainEvidence: buildValidatedToolchainEvidence(contract, {
      source: input.connectionSource || 'guarded-fake-smoke'
    })
  });
...
async function runGuardedFakeProductPath() {
  ...
  const productPath = buildProductPathEvidence({ assetPath });
...
async function runLiveProductPath() {
  ...
  try {
    ...
    const assetPath = await writeFixtureImage();
    const productPath = buildProductPathEvidence({
      assetPath,
...
function assertReadyProductPath(productPath) {
  ...
  assert(productPath.strategy.productionExecutionPlanEvidence.status === 'ready_execution_plan', 'production execution plan should be ready', productPath.strategy.productionExecutionPlanEvidence);
  assert(productPath.strategy.productionExecutorHandoffEvidence.status === 'ready_for_dry_run', 'production handoff should be dry-run ready', productPath.strategy.productionExecutorHandoffEvidence);
  assert(productPath.strategy.productionExecutorBridgeEvidence.status === 'ready_for_dry_run_bridge', 'production bridge should be dry-run ready', productPath.strategy.productionExecutorBridgeEvidence);
  assert(productPath.strategy.productionExecutorDryRunEvidence.status === 'completed_dry_run', 'production dry-run should complete without writes', productPath.strategy.productionExecutorDryRunEvidence);
  ...
  assert(productPath.qaReport.stage === 'needs_manual_review', 'QA report should require manual review after fake runner evidence', productPath.qaReport);
  assertNoOverclaim(productPath.strategy, 'strategy evidence');
  assertNoOverclaim(productPath.checkpoint, 'checkpoint evidence');
...
function makeReport(input) {
  ...
  return {
    ...
    statuses: productPath ? {
      ...
      productionExecutionPlan: productPath.strategy.productionExecutionPlanEvidence.status,
      productionExecutorHandoff: productPath.strategy.productionExecutorHandoffEvidence.status,
      productionExecutorBridge: productPath.strategy.productionExecutorBridgeEvidence.status,
      productionExecutorDryRun: productPath.strategy.productionExecutorDryRunEvidence.status,
    ...
    counts: productPath && runnerResult ? {
      plannedOperations: productPath.strategy.productionExecutionPlanEvidence.plannedOperationCount,
      dryRunOperations: productPath.strategy.productionExecutorDryRunEvidence.operationCount,
