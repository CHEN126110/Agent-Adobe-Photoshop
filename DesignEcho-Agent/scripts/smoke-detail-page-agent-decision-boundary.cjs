...
const agentPlans = inferDetailScreenPlans([{
  ...
  agentDecisions: [{
    screenId: 2,
    screenRole: 'detail-proof',
    mainMessage: '用袜口细节证明舒适不勒脚。',
    supportingPoints: ['镜头聚焦袜口纹理', '文案解释穿着痛点'],
    copyStrategy: 'evidence',
...
assert(agentPlans[0].mainMessage === '用袜口细节证明舒适不勒脚。', 'agent main message should pass through', agentPlans[0]);
assert(agentPlans[0].supportingPoints[0] === '镜头聚焦袜口纹理', 'agent supporting point should pass through', agentPlans[0]);
assert(agentPlans[0].copyStrategy === 'evidence', 'agent copy strategy should pass through', agentPlans[0]);
...
assert(ranker.includes('Decision boundary: template and filename rules are candidate evidence only'), 'detail copy prompt should state model decision boundary');
...
console.log(JSON.stringify({
  ...
  checks: [
    ...
    'detail asset ranking treats heuristic rules as candidate evidence',
