...
} = require(path.resolve(__dirname, '..', 'src', 'shared', 'agent-resume-context-gate.ts'));
const {
  buildAgentResumeContextRefreshRun
} = require(path.resolve(__dirname, '..', 'src', 'shared', 'agent-resume-context-refresh-runner.ts'));
const {
  runAgentResumeReadonlyContextExecutor
} = require(path.resolve(__dirname, '..', 'src', 'shared', 'agent-resume-readonly-context-executor.ts'));
const {
...
function assertBoundary(contract) {
  assert(contract.evidenceOnly === true, 'contract must be evidence-only', contract);
...
function assertPolicyBoundary(policy) {
  assert(policy.evidenceOnly === true, 'policy must be evidence-only', policy);
...
function assertContextGateBoundary(gate) {
  assert(gate.evidenceOnly === true, 'context gate must be evidence-only', gate);
...
function assertRefreshRunBoundary(run) {
  assert(run.evidenceOnly === true, 'context refresh run must be evidence-only', run);
...
function assertReadonlyExecutorBoundary(result) {
  assert(result.evidenceOnly === true, 'readonly executor must be evidence-only', result);
...
function assertResumePlanningBoundary(result) {
  assert(result.evidenceOnly === true, 'resume planning must be evidence-only', result);
...
function assertResumeExecutionGateBoundary(result) {
  assert(result.evidenceOnly === true, 'resume execution gate must be evidence-only', result);
...
function assertControlledExecutionRequestBoundary(result) {
  assert(result.evidenceOnly === true, 'controlled execution request must be evidence-only', result);
...
function assertControlledExecutionRunnerBoundary(result) {
  assert(result.evidenceOnly === true, 'controlled execution runner must be evidence-only', result);
...
function assertControlledExecutionFakeRunnerBoundary(result) {
  assert(result.evidenceOnly === true, 'controlled fake runner must be evidence-only', result);
...
function assertControlledExecutionLiveRunnerBoundary(result) {
  assert(result.evidenceOnly === false, 'controlled live runner is not evidence-only after explicit live adapter execution', result);
...
const noStructuredEvidence = buildAgentResumableTaskContract({
  userInput: '继续下一项',
  conversationHistory: [
...
assert(noStructuredEvidence.status === 'ready_for_model_contextual_reply', 'missing lifecycle should only allow contextual reply', noStructuredEvidence);
assert(noStructuredEvidence.canResumeExecution === false, 'missing lifecycle cannot resume execution', noStructuredEvidence);
assertBoundary(noStructuredEvidence);
const noStructuredEvidencePolicy = buildAgentResumeExecutionPolicy(noStructuredEvidence);
assert(noStructuredEvidencePolicy.action === 'reply_with_context', 'missing lifecycle policy should allow contextual reply only', noStructuredEvidencePolicy);
assert(noStructuredEvidencePolicy.mayCallModelForUserReply === true, 'missing lifecycle policy may call model for reply', noStructuredEvidencePolicy);
assertPolicyBoundary(noStructuredEvidencePolicy);
const noStructuredEvidenceGate = buildAgentResumeContextGate({
  policy: noStructuredEvidencePolicy,
  photoshopConnected: true,
  hasDocument: true
});
assert(noStructuredEvidenceGate.status === 'blocked_policy_not_resumable', 'non-resumable policy should block context gate', noStructuredEvidenceGate);
assertContextGateBoundary(noStructuredEvidenceGate);
const noStructuredEvidenceRefreshRun = buildAgentResumeContextRefreshRun({ gate: noStructuredEvidenceGate });
assert(noStructuredEvidenceRefreshRun.status === 'blocked_gate_not_refreshable', 'non-resumable gate should block refresh runner', noStructuredEvidenceRefreshRun);
assertRefreshRunBoundary(noStructuredEvidenceRefreshRun);
...
assert(refreshGate.requiredReadOnlyEvidence.includes('document_snapshot'), 'refresh gate should require document snapshot', refreshGate);
assertContextGateBoundary(refreshGate);
const waitingRefreshRun = buildAgentResumeContextRefreshRun({ gate: refreshGate });
assert(waitingRefreshRun.status === 'waiting_for_readonly_evidence', 'refresh runner should wait when no readonly evidence exists', waitingRefreshRun);
...
assert(waitingRefreshRun.missingEvidence.includes('document_snapshot'), 'refresh runner should list missing document snapshot evidence', waitingRefreshRun);
...
assert(partialRefreshRun.status === 'partial_readonly_evidence', 'refresh runner should reject partial readonly evidence', partialRefreshRun);
assert(partialRefreshRun.receivedEvidence.includes('document_info'), 'partial refresh run should record received evidence', partialRefreshRun);
assert(partialRefreshRun.missingEvidence.includes('layer_hierarchy'), 'partial refresh run should record missing layer hierarchy', partialRefreshRun);
...
assert(completeRefreshRun.status === 'fresh_context_ready', 'complete readonly evidence should prepare resume planning', completeRefreshRun);
assert(completeRefreshRun.canEnterResumePlanning === true, 'complete readonly evidence should allow resume planning', completeRefreshRun);
assert(completeRefreshRun.mustNotRunWriteTools === true, 'complete readonly evidence still must not write tools', completeRefreshRun);
...
const waitingResumePlanning = buildAgentResumePlanningEvidence({
...
const nonExecutableResumePlanning = buildAgentResumePlanningEvidence({
  contract: resumableCandidate,
  policy: resumableCandidatePolicy,
  gate: planningGate,
  refreshRun: planningRefreshRun,
...
const executableResumePlanning = buildAgentResumePlanningEvidence({
  contract: resumableCandidate,
  policy: resumableCandidatePolicy,
  gate: planningGate,
  refreshRun: planningRefreshRun,
...
const unsafeWriteResumePlanning = buildAgentResumePlanningEvidence({
  contract: resumableCandidate,
  policy: resumableCandidatePolicy,
  gate: planningGate,
  refreshRun: planningRefreshRun,
...
const unlistedToolResumePlanning = buildAgentResumePlanningEvidence({
  contract: resumableCandidate,
  policy: resumableCandidatePolicy,
  gate: planningGate,
  refreshRun: planningRefreshRun,
...
async function runEngineReadonlyResumePlanningCase() {
  ...
  assert(resumeExecutionGate.proposedWriteTools.includes('reorderLayer'), 'engine should preserve proposed write whitelist as review evidence', resumeExecutionGate);
...
async function runEngineReadonlyResumePlanningBlockedCase() {
  ...
  assert(refreshRun?.status === 'waiting_for_readonly_evidence', 'engine should not mark context fresh when readonly executor is blocked', refreshRun);
  ...
  assertControlledExecutionRunnerBoundary(controlledExecutionRunner);
  assert(!callModelPurposes.includes('resume_planning'), 'engine must not request model resume planning without fresh readonly evidence', { callModelPurposes });
}

async function runReadonlyExecutorCase() {
  const notReadyExecutor = await runAgentResumeReadonlyContextExecutor({
    refreshRun: noStructuredEvidenceRefreshRun
  });
  ...
  assert(completedExecutor.evidence?.documentInfo?.name === 'C-1141.psd', 'readonly executor should return injected readonly evidence', completedExecutor);
  ...
  .then(() => {
    console.log(JSON.stringify({
      ...
      checks: [
        ...
        'DesignAgentEngine attaches evidence-only readonly context refresh runner to continuation direct responses',
        ...
        'contract, policy, context gate, refresh runner, readonly executor, resume planning, execution gate and controlled runner are evidence-only and cannot run provider or Photoshop by default'
      ]
    }, null, 2));
  })
  .catch((error) => {
