...
function assertFocusLayerToolGuard() {
  ...
  assert(executor.includes('focusEvidence'), 'Auto-focus results must be attached as explicit focus evidence.');
  assert(executor.includes("sendToPlugin(\n            'focusLayer'"), 'Auto-focus must call the real focusLayer tool rather than fake local state.');
}
...
function run() {
  ...
  assert(includesText(bounds.acceptance.evidence, 'plannedBox'), 'Bounds semantics must require plannedBox evidence.');
  assert(includesText(bounds.acceptance.evidence, 'actualBounds'), 'Bounds semantics must require actualBounds evidence.');
  ...
  assert(includesText(imagePlacement.acceptance.evidence, 'placementAudit'), 'Image placement semantics must include placement audit evidence.');
  ...
  for (const item of all) {
    ...
    assert(item.acceptance.evidence.length > 0, `${item.id} should define acceptance evidence.`);
  ...
  assert(summary.includes('验收证据'), 'Summary should include acceptance evidence.');
