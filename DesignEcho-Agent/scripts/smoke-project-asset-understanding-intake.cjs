...
function buildCandidate(id, cacheStatus, cachedInsight) {
  return {
    ...
    requiredEvidence: ['visual model or human inspection required'],
    cachedInsight,
    evidence: []
  };
}

function buildVisualSamplingPlan(cacheStatus = 'miss') {
...
function buildProjectContext(cacheStatus = 'miss') {
  return {
    projectPath: 'D:/demo',
    assetIndex: buildAssetIndex(),
    visualSamplingPlan: buildVisualSamplingPlan(cacheStatus),
    visualInsightCache: buildVisualInsightCache(cacheStatus === 'hit' ? 1 : 0)
  };
}

function buildExecuteParams(cacheStatus = 'miss', userInput = '帮我做主图') {
  return {
    params: {},
    callbacks: {
      onStep: () => undefined,
    ...
    context: {
      ...
      projectContext: buildProjectContext(cacheStatus)
    }
  };
}
...
function runSharedHelperChecks() {
  ...
  assert(missingVisual.requiredNextEvidence.includes('visual_understanding_required'), 'cache miss should require visual understanding');

  const cached = buildProjectAssetUnderstandingIntake({
    skillId: 'main-image-design',
    projectContext: buildProjectContext('hit')
  });
  assert(cached.status === 'ready_with_cached_visual_evidence', 'cache hit should be ready with cached visual evidence', cached);
  assert(cached.canUseCachedVisualEvidence === true, 'cache hit should allow cached visual evidence reuse');
...
async function runExecutorWiringCheck() {
  const cases = [
    ...
    },
    {
      ...
      userInput: '帮我做详情页',
      data: { detailPagePlannerEvidence: { id: 'fixture-detail' } }
    },
    {
      skillId: 'sku-batch',
      userInput: '帮我做 SKU',
  ...
  for (const item of cases) {
    ...
    const result = await executeSkillWithExecutor(item.skillId, buildExecuteParams('hit', item.userInput));
    ...
    assert(
      data.businessSkillProjectAssetUnderstandingIntake.status === 'ready_with_cached_visual_evidence',
      `${item.skillId} attached intake should reflect cached visual evidence`
    );
    ...
    if (data.businessSkillVisualEvidencePreExecutionRun) {
      assert(
        data.businessSkillVisualEvidencePreExecutionRun.executed !== true,
        `${item.skillId} default path must not execute pre-execution visual refresh`
      );
    }
    if (data.businessSkillVisualEvidenceRefreshRun) {
      assert(
        data.businessSkillVisualEvidenceRefreshRun.executed !== true,
...
function runSourceChecks() {
  ...
  assert(indexSource.includes('attachBusinessSkillProjectAssetUnderstandingIntakeToResult'), 'unified executor should attach asset understanding intake');
  assert(
...
async function run() {
  ...
  console.log(JSON.stringify({
    ...
    checks: [
      ...
      'cache hit can reuse cached visual evidence without claiming design quality',
      'unified business skill executor attaches hidden redacted intake evidence',
