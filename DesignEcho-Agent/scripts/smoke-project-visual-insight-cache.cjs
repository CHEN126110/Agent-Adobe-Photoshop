...
async function main() {
  ...
  assert(cached.visualInsightCache.summary.entriesWithInsight === 1, 'runtime build should carry persisted insight');
  assert(cached.visualSamplingPlan.cacheSummary.hit >= 1, 'cached visual plan should reuse valid insight');
  assert(cached.contextSnapshot.visualInsightCache?.summary.entriesWithInsight === 1, 'ContextSnapshot should carry cache summary');
  assert(
    cached.contextSnapshot.evidence.some((item) => item.summary.includes('VisualInsightCache')),
    'ContextSnapshot should expose visual cache evidence'
  );
  ...
  assertNoMojibake({ uncached, written, cached, disabled }, 'visual insight cache smoke result');
  assertNoRawPayloadValue({ uncached, written, cached, disabled }, 'visual insight cache smoke result');
