  ...
  try {
    ...
    const after = await page.evaluate((payload) => (
      window.__DESIGNECHO_CHAT_TEST_BRIDGE__.submit(payload.prompt, {
        image: { data: payload.imageData, type: 'image/jpeg' },
        timeoutMs: 45000
      })
    ), { prompt, imageData });
    ...
    assert(new RegExp(`元素覆盖:\\s*${CASE_CONFIG.expectedCoverage}\\/${CASE_CONFIG.expectedCoverage}`).test(visibleText), `reference replication should cover every ${CASE_CONFIG.label} element: ${visibleText}`);
    ...
    assert(/识别到|分析参考图|analyzeReferenceLayout|生成模板骨架/.test(progressText), `thinking progress should reflect layout-replication flow: ${progressText}`);
    ...
    const result = {
      ...
      checks: [
        ...
        'deterministic layout-replication route executed instead of ordinary chat',
