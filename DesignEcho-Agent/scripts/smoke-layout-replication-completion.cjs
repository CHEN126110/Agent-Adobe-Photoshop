...
try {
  ...
  const templateOverlayEvidence = summarizeTemplateApplyCompletion({
    headingSuccess: '详情页模板骨架已生成',
    headingReview: '详情页模板骨架部分生成，需复核',
    baseSuccess: true,
    screenCount: 2,
  ...
  record(
    'template-overlay-evidence-summary',
    templateOverlayEvidence.success === true
      && templateOverlayEvidence.messageLines.some((line) => line.includes('Overlay 检查: 2 张截图，5 项标注')),
    templateOverlayEvidence
  );

  const templatePixelProbeEvidence = summarizeTemplateApplyCompletion({
  ...
  record(
    ...
    templatePixelProbeEvidence.success === true
      && templatePixelProbeEvidence.completionContract?.status === 'needs_review'
      && templatePixelProbeEvidence.messageLines.some((line) => line.includes('截图探针: watch'))
      && templatePixelProbeEvidence.completionContract?.evidence?.visual?.pixelProbe?.rawImagesRedacted === true
      && templatePixelProbeEvidence.userReport?.status === 'needs_review'
      && templatePixelProbeEvidence.userReport?.evidenceLines.some((line) => line.includes('截图像素探针: watch'))
      && templatePixelProbeEvidence.userReport?.limitations.some((line) => line.includes('不能作为高保真通过结论')),
    templatePixelProbeEvidence
  );
  const pixelProbeUserMessage = formatLayoutReplicationUserReport(
    templatePixelProbeEvidence.userReport,
    templatePixelProbeEvidence.messageLines
  );
  record(
    'template-pixel-probe-user-report-is-readable-and-not-overclaimed',
    pixelProbeUserMessage.includes('参考图复刻需复核')
      && pixelProbeUserMessage.includes('截图像素探针: watch')
  ...
  const templateCoverage = buildTemplateApplyCoverageEvidence({
    blueprintScreens: [{
      index: 1,
      type: '合格证',
      label: '第1屏_合格证',
  ...
  const templateCoverageFromElementResults = buildTemplateApplyCoverageEvidence({
    blueprintScreens: [{
      index: 1,
      type: '合格证',
      label: '第1屏_合格证',
  ...
  record(
    ...
    templateCoverageSummary.success === false
      ...
      && templateCoverageSummary.completionContract?.evidence?.coverage?.applied === 1,
  ...
  const matchCoverage = buildLayoutMatchCoverageEvidence({
    representation: {
  ...
  const matchCoverageRequiresStableId = buildLayoutMatchCoverageEvidence({
    representation: {
      canvas: { width: 460, height: 460 },
      layout: { layoutType: 'certificate' },
      alignmentGroups: [],
