...
function run() {
  ...
  assert(readyPlan.evidenceTier === 'subject-bounds', `expected subject-bounds evidence: ${JSON.stringify(readyPlan)}`);
  ...
  assert(missingSubjectPlan.evidenceTier === 'metadata', `missing subjectBox should be metadata evidence: ${JSON.stringify(missingSubjectPlan)}`);
