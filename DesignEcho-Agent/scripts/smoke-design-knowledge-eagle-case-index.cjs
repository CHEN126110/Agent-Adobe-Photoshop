...
function run() {
  ...
  assert(first.source.filePath === 'D:/Eagle/library/socks-main-image-reference.jpg', 'case should preserve local file reference', first.source);
  assert(first.source.thumbnailPath === 'D:/Eagle/library/.thumb/socks-main-image-reference.jpg', 'case should preserve thumbnail reference', first.source);
  ...
  assert(first.evidence.some((line) => line.includes('Eagle item id: eagle-case-1')), 'case evidence should include source item id', first.evidence);
