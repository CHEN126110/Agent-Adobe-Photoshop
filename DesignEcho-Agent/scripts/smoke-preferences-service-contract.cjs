...
service.learnPreference('font', '思源黑体');
let items = service.listPreferenceItems();
const inferredFont = items.find((item) => item.category === 'font' && item.value === '思源黑体');
assert(inferredFont, 'learnPreference must create a reviewable inferred preference item', items);
assert(inferredFont.sourceType === 'inferred', 'learned preferences must be marked inferred', inferredFont);
assert(inferredFont.status === 'needs_review', 'learned preferences must require review before use', inferredFont);
...
const projectScoped = service.upsertExplicitPreference({
  ...
  value: '高级灰',
  label: 'C-1160 项目主图风格',
  evidenceSummary: '用户明确要求 C-1160 主图优先使用高级灰和低广告感排版。',
...
const sameValueOtherProject = service.upsertExplicitPreference({
  ...
  value: '高级灰',
  label: 'C-1159 项目主图风格',
  evidenceSummary: '另一个项目可以保存同值偏好，但不能覆盖 C-1160。',
...
const confirmedInferred = service.updatePreferenceItem(inferredFont.id, {
  sourceType: 'explicit',
  status: 'active',
  evidenceSummary: '用户在偏好面板中确认该字体可以作为标题候选。',
  scope: { type: 'user' }
});
...
service.learnPreference('color', '暖白');
const inferredColor = service.listPreferenceItems().find((item) => item.category === 'color' && item.value === '暖白');
assert(inferredColor, 'learnPreference must create inferred color preference for confirmation smoke', service.listPreferenceItems());
...
assert(
  confirmedColor.evidenceSummary.includes('确认'),
...
const mergeResult = importer.importPreferences({
  ...
  preferenceItems: [
    ...exported.preferenceItems,
    {
      ...
      sourceType: 'explicit',
      status: 'active',
      evidenceSummary: '用户明确要求文案少夸张词，不写置信度。',
      confidence: 0.99,
      scope: { type: 'brand', id: 'neveralone' }
    }
  ]
}, { mode: 'merge' });
assert(mergeResult.importedCount >= exported.preferenceItems.length, 'importPreferences merge must import valid preferences', mergeResult);
...
importer.importPreferences({
  ...
  preferenceItems: [{
    ...
    sourceType: 'explicit',
    status: 'active',
    evidenceSummary: '畸形导入样例：外部 id 伪装成用户级 id，但 scope 指向项目级。',
    scope: { type: 'project', id: 'C-1160' }
  }]
}, { mode: 'merge' });
...
const replaceResult = importer.importPreferences({
  ...
  preferenceItems: [{
    ...
    sourceType: 'explicit',
    status: 'active',
    evidenceSummary: '用户验收后保留的详情页排版偏好。',
    scope: { type: 'user' }
  }]
}, { mode: 'replace' });
...
service.learnPreference('style', '浅色干净');
...
console.log(JSON.stringify({
  success: true,
  checks: [
    'inferred preferences require review before use',
