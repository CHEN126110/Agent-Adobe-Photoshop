...
function run() {
  const missingContract = buildMainImageLiveAdapterHandoffEvidence({});
  ...
  assert(missingContract.canWritePhotoshop === false, 'handoff evidence must be read-only', missingContract);
  assert(missingContract.canClaimOutputQuality === false, 'handoff evidence must not claim output quality', missingContract);

  const notReadyContract = buildMainImageLiveAdapterHandoffEvidence({
    adapterContract: {
    ...
    toolchainEvidence: validToolchainEvidence
  });
  ...
  const missingToolchain = buildMainImageLiveAdapterHandoffEvidence({
    adapterContract: readyAdapterContract
  });
  assert(missingToolchain.status === 'blocked_missing_toolchain_evidence', 'ready contract without toolchain evidence should block handoff', missingToolchain);

  const failedToolchain = buildMainImageLiveAdapterHandoffEvidence({
    adapterContract: readyAdapterContract,
    toolchainEvidence: {
      ...validToolchainEvidence,
      success: false,
      failedAssertions: ['export file missing']
    }
  });
  assert(failedToolchain.status === 'blocked_toolchain_not_validated', 'failed toolchain evidence should block handoff', failedToolchain);
  ...
  const unsafeCleanup = buildMainImageLiveAdapterHandoffEvidence({
    adapterContract: readyAdapterContract,
    toolchainEvidence: {
      ...validToolchainEvidence,
      cleanup: {
  ...
  const ready = buildMainImageLiveAdapterHandoffEvidence({
    adapterContract: readyAdapterContract,
    toolchainEvidence: validToolchainEvidence
  });
  ...
  assert(ready.toolchainSummary.assertionCount === validToolchainEvidence.assertionCount, 'toolchain assertion count should be preserved', ready);
