...
const keySummary = summarizeChatError(
  '401 Unauthorized: Authorization: Bearer sk-test_should_not_leak_1234567890',
  { isCloud: true }
);
assert(keySummary.includes('API 密钥或账号权限错误'), 'auth errors must keep auth-specific wording');
assert(!keySummary.includes('sk-test_should_not_leak'), 'auth summary must redact secret-looking values');
