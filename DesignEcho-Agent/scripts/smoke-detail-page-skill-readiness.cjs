...
const missingVisualEvidence = buildDetailPageSkillReadiness({
  mode: 'execute',
  template: buildTemplate(),
  project: buildProject({ visualInsightCount: 0, shouldAnalyzeCount: 6 })
});
assert(missingVisualEvidence.status === 'needs_context', 'missing visual insight should request context instead of claiming ready', missingVisualEvidence);
assert(missingVisualEvidence.requiredNextEvidence.includes('VisualInsightCache or visual model analysis'), 'missing visual insight should request visual evidence', missingVisualEvidence);
assert(missingVisualEvidence.canExecute === false, 'missing visual insight should not claim safe execution readiness', missingVisualEvidence);
...
assert(ready.status === 'ready', 'full template and visual evidence should be ready', ready);
assert(ready.canExecute === true, 'ready case should allow execution readiness', ready);
assert(ready.evidence.every((item) => item.status !== 'passed'), 'readiness evidence must not claim design quality passed', ready);

const serialized = JSON.stringify([inspectOnly, missingTemplate, missingVisualEvidence, ready]);
