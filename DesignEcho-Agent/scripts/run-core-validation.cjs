...
const COMMANDS = [
  ...
  { label: "图像 Provider 请求契约", cwd: agentRoot, args: ["run", "test:image-provider-request"] },
  ...
  { label: "语义目标词规范化", cwd: agentRoot, args: ["run", "test:semantic-target-vocabulary"] },
  { label: "GroundingDINO WordPiece", cwd: agentRoot, args: ["run", "test:bert-wordpiece-tokenizer"] },
  { label: "开放词汇候选过滤", cwd: agentRoot, args: ["run", "test:grounding-candidate-filter"] },
  { label: "蒙版连通域拆分", cwd: agentRoot, args: ["run", "test:mask-regions"] },
  ...
  { label: "SKU 姿态统一算法与事务 Provider", cwd: agentRoot, args: ["run", "test:sku-pose-alignment"] },
  ...
  { label: "Sharp 运行时兼容性", cwd: agentRoot, args: ["run", "test:sharp-runtime-compatibility"] },
