...
function run() {
  ...
  assert(result.evidenceLevel === 'local_case', 'memory result should expose local_case evidence level', result);
  ...
  assert(preferenceItems.some((item) => item.source === 'inferred_from_operations'), 'operation-derived preferences should stay evidence-marked', preferenceItems);
  assert(preferenceItems.some((item) => item.source === 'manual_setting'), 'manual settings should stay evidence-marked', preferenceItems);
