...
async function run() {
  ...
  assert(toolCallPlan.status === 'ready_tool_call_plan', 'ready dry-run evidence should compile into a tool-call plan', toolCallPlan);
  ...
  assert(
    ...
    'sampled benchmark should carry measurement evidence without claiming runtime speedup',
    sampledBenchmark
  );
  ...
  console.log(JSON.stringify({
    ...
    checks: [
      ...
      'ready dry-run evidence compiles to an explicit-opt-in reorderLayer tool-call plan',
