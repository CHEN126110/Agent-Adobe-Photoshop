...
const {
  buildAgentAcceptanceEvidenceMatrix,
  formatAgentAcceptanceEvidenceMatrixMarkdown,
  getAgentAcceptanceModeArtifactPath
} = require('../src/shared/agent-acceptance-evidence-matrix.ts');
...
function run() {
  const report = buildAgentAcceptanceEvidenceMatrix({
  ...
  assert(report.version === 'agent-acceptance-evidence-matrix/v0', 'wrong matrix version');
  ...
  assert(desktop.evidenceReady === true, 'desktop fake evidence should be ready');
  ...
  assert(livePreflight.livePhotoshopEvidenceIntake.status === 'failed_or_blocked', 'live preflight should expose blocked live evidence intake');
  assert(livePreflight.livePhotoshopEvidenceIntake.canClaimDesignQuality === false, 'live evidence intake must not claim design quality');
  ...
  assert(liveDisposable.livePhotoshopEvidenceIntake.status === 'needs_live_run', 'stale live artifact should still require live snapshot/focus evidence');
  ...
  const markdown = formatAgentAcceptanceEvidenceMatrixMarkdown(report);
  assert(markdown.includes('# Agent Acceptance Evidence Matrix'), 'markdown title missing');
