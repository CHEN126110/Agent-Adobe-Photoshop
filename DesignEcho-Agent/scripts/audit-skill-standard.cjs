...
const PROTECTED_BUSINESS_SKILLS = new Set(['main-image-design', 'detail-page-design', 'sku-batch']);
...
function auditParameter(param, index, record) {
  ...
  if (!['string', 'number', 'integer', 'boolean', 'array', 'object', 'image'].includes(param.type)) {
...
function run() {
  ...
  const skuSkillIds = SKILL_REGISTRY
    .map((skill) => skill.id)
    .filter((id) => id === 'sku-batch' || id.startsWith('sku-'));
  const skuCase = cases.find((item) => item.id === 'sku-batch');
  if (skuCase && (skuSkillIds.length !== 1 || skuSkillIds[0] !== 'sku-batch')) {
