...
record('ready-plan-has-agent-control-shape', () => {
  ...
  assert(plan.assetVisualUnderstanding && plan.assetVisualUnderstanding.readiness === 'needs_vision', 'ready fixture should still need real vision evidence');
  assert(plan.visualVerification && plan.visualVerification.stage === 'needs_screenshot', 'ready fixture should still need screenshot evidence');
  return {
...
record('selected-project-image-is-evidence-not-vision-claim', () => {
  ...
  assert(plan.assetVisualUnderstanding.source === 'metadata-only', 'selected project image should remain metadata-only until vision evidence exists');
...
record('blocked-invalid-target-size-stays-safe', () => {
  ...
  assert(plan.verificationReport.status !== 'passed', 'verification must not pass without execution evidence');
  return {
    readiness: plan.readiness,
