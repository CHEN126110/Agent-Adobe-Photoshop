...
function run() {
  ...
  assert(completedGate.resultImageSummary.resultImageCount === 1, 'result image should be discovered from export/readback/final snapshot evidence', completedGate.resultImageSummary);
  ...
  assert(
    ...
    'result image names should contain only basename evidence',
  ...
  assert(completeGate.stage === 'passed', 'complete controlled gate can mark evidence gate passed', completeGate);
  ...
  assert(missingExportGate.stage === 'needs_result_image', 'completed runner without output path should require result image evidence', missingExportGate);
  ...
  console.log(JSON.stringify({
    success: true,
    checks: [
      'controlled product QA gate derives result-image evidence from live runner output',
      ...
      'quality claim remains blocked without result image, pixel probe, and manual approved evidence',
