  ...
  switch (healthStatus) {
    ...
    case 'photoshop_plugin_not_connected':
      actions = [
        ...
        '在 UXP Developer Tool 中重新加载插件；仍失败时重启 Photoshop。'
      ];
      break;
    ...
    case 'photoshop_runtime_tool_mismatch':
      actions = [
        '当前 Photoshop 运行时工具列表与本地期望不一致。',
        '重新构建并重载 UXP 插件，确认缺失工具已注册后再进入业务 skill 验收。'
      ];
      break;
    default:
...
async function buildBridgeHealthReport(options = {}) {
  ...
  const missingTools = requiredToolNames.filter((toolName) => !photoshopToolNames.includes(toolName));
  const blockers = [];
  ...
  const bridgeBlockerHints = buildBridgeBlockerHints(systemStatus, photoshopTools);
  ...
  return {
    ...
    diagnostics: {
      ...
      missingTools,
      hostDiagnostics: {
        pendingRequestDiagnosticsAvailable: systemStatus.ok
          && typeof systemStatus.result?.pluginConnectionDiagnostics?.pendingRequestCount === 'number'
          && Array.isArray(systemStatus.result?.pluginConnectionDiagnostics?.pendingRequests)
      },
      bridgeBlockerHints
    },
    blockers,
