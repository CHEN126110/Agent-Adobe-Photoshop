...
async function run() {
  ...
  assert(result.data.businessSkillVisualEvidenceRefreshPlan.status === 'ready', 'executor wiring should attach ready refresh plan');
  assert(result.data.businessSkillVisualEvidenceRefreshRun.status === 'completed', 'executor wiring should attach completed refresh run summary');
  assert(result.data.businessSkillVisualEvidenceRefreshRun.successCount === 2, 'run summary should expose success count');
  assert(!result.data.businessSkillVisualEvidenceRefreshRun.entries, 'run summary must not expose raw cache entries');
  ...
  const source = read('src/renderer/services/skill-executors/index.ts');
  assert(source.includes('await executor.execute('), 'unified executor should call business executor');
  assert(source.includes('await runBusinessSkillVisualEvidenceRefreshAfterExecution'), 'unified executor should call visual refresh runner');
