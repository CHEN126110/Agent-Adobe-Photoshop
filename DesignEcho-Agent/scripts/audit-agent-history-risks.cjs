...
function checkPowerShellAndEncodingGuard() {
  ...
  const coreValidation = read('scripts/run-core-validation.cjs');
  ...
  ok = assertIncludes(
    coreValidation,
    'maintenance:repo-hygiene:check',
    'powershell-and-encoding-guard',
    'Core validation must retain repository hygiene checks.'
  ) && ok;
  ok = assertIncludes(
    coreValidation,
    'check:repository-encoding',
    'powershell-and-encoding-guard',
    'Core validation must retain the repository encoding check.'
  ) && ok;

  if (ok) {
    pass('powershell-and-encoding-guard', 'PowerShell convention and core repository hygiene validation are present.');
  }
}
