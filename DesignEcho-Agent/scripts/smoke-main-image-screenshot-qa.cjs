...
record('blocks-when-export-was-planned-but-no-quick-export-succeeded', () => {
  ...
  assert(qa.blockers.some((item) => item.includes('没有 quickExport 成功证据')), 'missing quickExport blocker absent');
...
record('needs-pixel-probe-when-result-image-exists', () => {
  ...
  assert(qa.resultImageEvidence.resultPaths.includes('C:/Exports/main-image-800.jpg'), 'result path absent');
  assert(qa.warnings.some((item) => item.includes('没有 pixel probe')), 'pixel probe warning absent');
  return { stage: qa.stage, resultPaths: qa.resultImageEvidence.resultPaths.length };
});

record('accepts-explicit-result-image-evidence-without-fake-quick-export', () => {
  const qa = buildMainImageScreenshotQa({
    sizePlans: [{ ...baseSizePlan, quickExportPlanned: false, quickExportOutputPath: undefined }],
    toolResults: passingToolResults.filter((entry) => !String(entry.toolName).startsWith('quickExport')),
    resultImageEvidence: {
  ...
  assert(qa.status === 'needs_review', `expected needs_review, got ${qa.status}`);
  assert(qa.stage === 'needs_pixel_probe', `expected needs_pixel_probe, got ${qa.stage}`);
  assert(qa.resultImageEvidence.resultPaths.includes('C:/DesignEcho/Exports/controlled-main-image-800.png'), 'explicit result path absent');
  assert(qa.resultImageEvidence.sources.includes('controlledProduct.exportGroup.actualResult'), 'explicit result evidence source absent');
  assert(qa.resultImageEvidence.successfulExportCount === 1, 'explicit successful export count should be preserved');
  return { stage: qa.stage, sources: qa.resultImageEvidence.sources };
});
...
record('executor-and-maintenance-wiring-is-present', () => {
  ...
  assertIncludes(executor, 'buildMainImageScreenshotQa', 'executor missing screenshot QA builder');
