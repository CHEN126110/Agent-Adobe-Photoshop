...
function run() {
  ...
  const missingPlan = buildMainImageProductionExecutorHandoffEvidence({});
  ...
  const defaultDryRun = buildMainImageProductionExecutorHandoffEvidence({
    productionExecutionPlanEvidence: executionPlan,
  ...
  const missingCapability = buildMainImageProductionExecutorHandoffEvidence({
    productionExecutionPlanEvidence: executionPlan,
  ...
  const dryRun = buildMainImageProductionExecutorHandoffEvidence({
    productionExecutionPlanEvidence: executionPlan,
    availableToolNames: allRequiredTools,
    outputDir: 'C:/Exports',
    approvedPendingConfirmations: true,
  ...
  const executorHandoff = buildMainImageProductionExecutorHandoffEvidence({
    productionExecutionPlanEvidence: executionPlan,
    availableToolNames: allRequiredTools,
    outputDir: 'C:/Exports',
    approvedPendingConfirmations: true,
  ...
  assert(strategyInputs.productionExecutorHandoffEvidence, 'strategy input builder should expose executor handoff evidence', strategyInputs);
  assert(strategyInputs.productionExecutorHandoffEvidence.status === 'ready_for_dry_run', 'strategy builder should produce dry-run handoff for the confirmed 800/750/1200 structure', strategyInputs.productionExecutorHandoffEvidence);
  assert(strategyInputs.productionExecutorHandoffEvidence.toolRequests.length === strategyInputs.productionExecutionPlanEvidence.plannedOperationCount, 'strategy builder dry-run handoff should mirror planned operations', strategyInputs.productionExecutorHandoffEvidence);
  ...
  console.log(JSON.stringify({
    ...
    checks: [
      ...
      'strategy input builder exposes productionExecutorHandoffEvidence without changing execution'
    ]
  }, null, 2));
}
