...
assert(cases.some((item) => item.id === 'rr-001-fex-certificate-text-layout'), 'temporary FEX case should remain visible until explicitly replaced');
assert(cases.some((item) => item.id !== 'rr-001-fex-certificate-text-layout'), 'FEX must not be the only benchmark case');
...
const result = {
  ...
  boundary: [
    ...
    'The temporary FEX case remains a text-layout validation fixture until replaced.'
  ]
};
