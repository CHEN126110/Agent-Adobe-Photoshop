...
function assertNoUngroundedConfidence(text, label) {
  assert(!text.includes('confidence'), `${label} should not expose ungrounded confidence`);
  assert(!text.includes('置信'), `${label} should not expose ungrounded confidence wording`);
}
...
record('metadata-only-needs-vision-not-visual-claim', () => {
  ...
  assert(visual.warnings.some((item) => item.includes('缺少视觉模型')), 'missing vision warning');
...
record('vision-signal-becomes-reviewable-understanding', () => {
  const visual = buildMainImageVisualUnderstanding({
    ...
    visionSignal: {
      ...
      evidence: ['provider returned structured product summary']
    }
  });
  ...
  assert(visual.productIdentity.grounding === 'vision-model', 'vision grounding should be explicit');
  assert(visual.mainImageFit.status === 'candidate', `unexpected fit ${visual.mainImageFit.status}`);
  assert(visual.mainImageFit.evidenceLevel === 'visual-and-bounds', `unexpected evidence level ${visual.mainImageFit.evidenceLevel}`);
  assertNoUngroundedConfidence(JSON.stringify(visual), 'visual understanding');
  return {
    source: visual.source,
    readiness: visual.readiness,
    product: visual.productIdentity.label,
    fit: visual.mainImageFit.status,
    evidenceLevel: visual.mainImageFit.evidenceLevel
  };
});
...
record('screenshot-without-manual-review-still-needs-review', () => {
  ...
  assert(verification.verificationReport.scope === 'screenshot', 'screenshot evidence should set screenshot scope');
  return {
    stage: verification.stage,
    status: verification.status
  };
...
record('quick-export-output-path-becomes-screenshot-evidence', () => {
  const exportedPlans = [{
    ...sizePlans[0],
    quickExportPlanned: true,
    quickExportOutputPath: 'C:/project/exports/main-image-800.png'
  }];
  const screenshot = buildMainImageScreenshotEvidenceFromSizePlans(exportedPlans);
  assert(screenshot && screenshot.hasImageEvidence === true, 'quick export path should become screenshot evidence');
...
record('source-and-output-have-no-mojibake', () => {
  ...
  assertNoUngroundedConfidence(source, 'main-image-visual-loop.ts');
  assertNoUngroundedConfidence(output, 'main-image-visual-loop output');
  return {
