...
function run() {
  ...
  assert(report.evidence.some((line) => line.includes('bounds 不匹配明细') && line.includes('mismatch')), 'Expected mismatch evidence to expose the first divergent item.');
  assert(report.evidenceReport?.kind === 'reference-replication-visual-qa', 'Expected structured visual QA evidence report.');
  assert(report.evidenceReport.snapshot.rawImagesRedacted === true, 'Expected evidence report to mark raw images redacted.');
  assert(report.evidenceReport.snapshot.hasImageEvidence === false, 'Bounds-only report must not claim image evidence.');
  assert(report.evidenceReport.blockers.some((line) => line.includes('不匹配')), 'Expected mismatch blocker in evidence report.');
  assert(report.evidenceReport.blockers.some((line) => line.includes('首批差异') && line.includes('target')), 'Expected mismatch blocker to include concise diagnostic detail.');
  ...
  assert(overlayFailure.evidenceReport.snapshot.rawImagesRedacted === true, 'Overlay evidence report must keep raw images redacted.');
  assert(overlayFailure.evidenceReport.snapshot.hasImageEvidence === false, 'Zero-snapshot overlay must not claim image evidence.');
  assert(overlayFailure.evidenceReport.blockers.some((line) => line.includes('overlay 截图')), 'Expected zero-snapshot overlay blocker.');
  ...
  assert(overlaySuccess.evidenceReport.snapshot.hasImageEvidence === true, 'Overlay report with screenshots should claim image evidence.');
  assert(overlaySuccess.evidenceReport.blockers.length === 0, 'Clean overlay report should not add blockers.');
  ...
  assert(pixelProbeWatch.evidence.some((line) => line.includes('截图像素探针')), 'Expected visual QA evidence to include pixel probe summary.');
  assert(pixelProbeWatch.evidenceReport.snapshot.pixelProbe?.rawImagesRedacted === true, 'Pixel probe evidence must keep raw images redacted.');
  assert(pixelProbeWatch.evidenceReport.snapshot.pixelProbe?.status === 'watch', 'Expected pixel probe status to be exposed in evidence report.');
  assert(pixelProbeWatch.evidenceReport.warnings.some((line) => line.includes('截图像素探针')), 'Non-ok pixel probe should be a warning, not silent.');
  ...
  assert(executorSource.includes('evidenceReport: visualQa.evidenceReport'), 'Expected overlay result to expose structured evidence report.');
  ...
  assert(qaSource.includes('未获得真实视觉 QA 证据'), 'Placement QA without visual evidence must be explicitly downgraded.');
