...
function run() {
  const profile = buildMainImagePlatformSizeProfileEvidence({
    platform: 'tmall',
  ...
  assertEqual(longVerticalProfile.evidenceLevel, 'user_project_rule', '1200 profile should remain user project rule evidence', longVerticalProfile);
  ...
  assert(production.status === 'ready_production_document_structure', 'grounded style plus profile should produce document structure', production);
  ...
  const blocked = buildMainImageProductionDocumentStructureEvidence({
    platformSizeProfileEvidence: profile,
    projectStyleStrategyEvidence: metadataOnlyStyle
  });

  assert(blocked.status === 'blocked_missing_visual_grounding', 'metadata-only style must block production child groups', blocked);
  assert(blocked.documents.length === 0, 'blocked production structure must not fabricate documents', blocked);
  assert(blocked.blockers.includes('main_image_visual_grounding_required'), 'blocked production structure should explain visual grounding requirement', blocked);
  ...
  assert(strategyInputs.productionDocumentStructureEvidence, 'strategy input builder should expose production document structure evidence', strategyInputs);
  assert(strategyInputs.productionDocumentStructureEvidence.status === 'ready_production_document_structure', 'builder should carry ready production structure evidence', strategyInputs.productionDocumentStructureEvidence);
