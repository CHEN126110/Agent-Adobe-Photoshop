...
function main() {
  ...
  assertIncludes(modelService, "thinking: { type: 'disabled' }", 'ModelService DeepSeek tool mode boundary');
  ...
  assertIncludes(settings, '<optgroup label="🟢 DeepSeek (官方)">', 'SettingsModal model optgroup');
  ...
  assertIncludes(packageJson, '"smoke:model-provider:deepseek"', 'package scripts');
  ...
  console.log(JSON.stringify({
    ...
    boundary: [
      ...
      'DeepSeek chatWithTools currently runs with thinking disabled until the adapter can preserve reasoning_content across tool turns.',
