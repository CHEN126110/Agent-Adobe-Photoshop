...
async function run() {
  ...
  const blocked = await runBusinessSkillVisualEvidenceRefreshAfterExecution(
    baseResult,
    gate,
    buildExecuteParams({
  ...
  assert(blocked.data.businessSkillVisualEvidenceRefreshPlan.status === 'blocked_no_analyzer', 'runtime-absent plan should stay blocked');
  assert(blocked.data.businessSkillVisualEvidenceRefreshRun.status === 'skipped_plan_not_ready', 'runtime-absent runner should skip');
  ...
  assert(executed.data.businessSkillVisualEvidenceRefreshPlan.status === 'ready', 'runtime-present plan should be ready');
  assert(executed.data.businessSkillVisualEvidenceRefreshRun.status === 'completed', 'runtime-present runner should execute');
