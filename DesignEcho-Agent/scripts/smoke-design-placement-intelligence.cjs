...
function buildFixture() {
  const assetIndex = buildProjectAssetIndex({
    ...
    files: [
      ...
      {
        path: 'D:/A1 neveralone flagship/C-1160/主图/800x800/01.jpg',
...
function run() {
  ...
  assert(needsVisual.status === 'needs_visual_evidence', 'missing visual insight should require visual evidence', needsVisual);
  ...
  assert(candidate.visualEvidence.status === 'cached_insight', 'candidate should use cached visual insight', candidate.visualEvidence);
  assert(candidate.caseEvidence.matchedCaseIds.includes('eagle-case:eagle-main-1'), 'candidate should match Eagle visual case by tags', candidate.caseEvidence);
  assert(candidate.scorecard.totalScore > 0, 'candidate should expose explainable scorecard', candidate.scorecard);
  assert(candidate.scorecard.items.some((item) => item.id === 'visual-evidence'), 'scorecard should include visual evidence item', candidate.scorecard);
  ...
  assert(candidate.placementPlan.evidenceTier === 'metadata', 'DPI must not invent subjectBox from visual insight or Eagle tags', candidate.placementPlan);
  ...
  console.log(JSON.stringify({
    ...
    checks: [
      ...
      'DPI requires visual evidence when candidates have no cached insight',
