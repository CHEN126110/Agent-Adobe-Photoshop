...
async function main() {
  ...
  try {
    ...
    assert(identityMessages.some((message) => message.role === 'assistant' && message.contentPreview.includes('测试模型响应')), 'identity sample should use the controlled model response');
    assert(identityMessages.some((message) => message.role === 'assistant' && message.contentPreview.includes('普通对话')), 'identity sample should be treated as conversational intent');
    assert(identityMessages.every((message) => message.thinkingStepCount === 0), 'identity chat should not render fake thinking steps');
    assert(messagesBlockTitles(identityMessages).length === 0, 'identity chat should not render 正在思考 or 工具调用 blocks without provider thinking/tool evidence');
    ...
    assert(ordinaryChatMessages.some((message) => message.role === 'assistant' && message.contentPreview.includes('测试模型响应')), 'ordinary chat sample should use the controlled model response');
    assert(ordinaryChatMessages.some((message) => message.role === 'assistant' && message.contentPreview.includes('普通对话')), 'ordinary chat sample should be treated as conversational intent');
    ...
    assert(messagesBlockTitles(ordinaryChatMessages).length === 0, 'ordinary chat should not render 正在思考 or 工具调用 blocks without provider thinking/tool evidence');
    ...
    assert(closeDocumentBlockTitles.includes('工具调用'), 'close document telemetry should render as 工具调用');
    ...
    assert(saveDocumentBlockTitles.includes('工具调用'), 'save document telemetry should render as 工具调用');
    ...
    assert(failedAcceptanceMessages.some((message) => message.role === 'assistant' && message.executionStatus === 'failed'), 'failed acceptance sample should expose failed executionSummary status');
    assert(failedAcceptanceMessages.some((message) => message.role === 'assistant' && message.executionSummaryPreview?.includes('执行状态：未完成')), `failed acceptance sample should expose an incomplete task report summary: ${JSON.stringify(failedAcceptanceMessages).slice(0, 800)}`);
    assert(failedAcceptanceSummaryOrPreflightBlocked, `failed acceptance sample should expose either failed acceptance count or tool-decision preflight blocker: ${JSON.stringify(failedAcceptanceMessages).slice(0, 800)}`);
    assert(failedAcceptanceMessages.some((message) => message.role === 'assistant' && message.contentPreview.includes('阻断原因')), 'failed acceptance sample should render blocker wording in the assistant message');
    ...
    const result = {
      ...
      checks: [
        ...
        'document operation telemetry rendered as 工具调用 instead of 正在思考',
        ...
        'failed acceptance sample rendered executionSummary failed state or tool-decision preflight blocker instead of optimistic model completion',
