...
async function main() {
  ...
  await record('explicit-result-evidence-flows-into-readiness', async () => {
    ...
    assert(readiness.stage === 'ready_for_pixel_probe', `expected ready_for_pixel_probe, got ${readiness.stage}`);
    assert(readiness.probeTarget.mode === 'reference-image', 'expected explicit evidence to keep reference-image target');
  ...
  await record('executor-and-ipc-wiring-is-present', async () => {
    ...
    assertIncludes(executor, 'buildMainImageScreenshotProbeReadiness', 'executor missing readiness builder');
