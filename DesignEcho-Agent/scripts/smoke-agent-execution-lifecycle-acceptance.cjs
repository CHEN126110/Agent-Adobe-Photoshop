...
assert(
  report.evidence.agentExecutionLifecycleSnapshot?.version === 'agent-execution-lifecycle/v0',
  'acceptance report should expose agent execution lifecycle snapshot',
  report.evidence
);
assert(
  report.evidence.agentExecutionLifecycleSnapshot?.phase === 'completed',
...
assert(
  report.evidence.agentExecutionLifecycleSnapshot?.toolEvidence.toolCallCount === 1,
  'lifecycle snapshot should preserve tool count evidence',
  report.evidence.agentExecutionLifecycleSnapshot
);
assert(
  isAgentExecutionLifecycleBoundaryOk(report.evidence.agentExecutionLifecycleSnapshot) === true,
...
assert(
  report.evidence.agentExecutionLifecycleSnapshot?.isProviderThinking === false,
...
assert(
  ...
  'execution lifecycle evidence must not be persisted as visible thinking',
  bundle
);
