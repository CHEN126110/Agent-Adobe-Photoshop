...
function run() {
  ...
  const disabled = buildBusinessSkillVisualEvidenceRefreshPlan({
    skillId: 'sku-batch',
    plannerEvidence,
    projectPath: 'D:/demo',
    visualSamplingPlan: buildVisualSamplingPlan(),
  ...
  const ready = buildBusinessSkillVisualEvidenceRefreshPlan({
    skillId: 'sku-batch',
    plannerEvidence,
    projectPath: 'D:/demo',
    visualSamplingPlan: buildVisualSamplingPlan(),
  ...
  const blocked = buildBusinessSkillVisualEvidenceRefreshPlan({
    skillId: 'sku-batch',
    plannerEvidence,
    projectPath: 'D:/demo',
    visualSamplingPlan: buildVisualSamplingPlan(),
  ...
  const noNeed = buildBusinessSkillVisualEvidenceRefreshPlan({
    skillId: 'main-image-design',
    plannerEvidence: buildBusinessSkillPreflightPlannerEvidence(noNeedGate),
    projectPath: 'D:/demo',
    visualSamplingPlan: buildVisualSamplingPlan(),
    enabled: true,
    runtimeCanAnalyze: true,
  ...
  const gate = buildBusinessSkillExecutionPreflightGateForSkill(
    'sku-batch',
    buildExecuteParams({ enableVisualEvidenceRefresh: true, visualEvidenceRefreshRuntimeReady: true }),
  ...
  assert(wrapped.data.businessSkillVisualEvidenceRefreshPlan, 'wrapped result should include visual evidence refresh plan');
  assert(wrapped.data.businessSkillVisualEvidenceRefreshPlan.status === 'ready', 'wrapped refresh plan should become ready with opt-in runtime evidence', wrapped);

  const wrapperSource = read('src/renderer/services/skill-executors/business-skill-visual-evidence-gate.ts');
  assert(wrapperSource.includes('buildBusinessSkillVisualEvidenceRefreshPlan'), 'skill wrapper should build business visual refresh plan');
