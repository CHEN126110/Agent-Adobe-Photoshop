...
当前已有 5 个 `reference_captured` case：

- `rr-001-fex-certificate-text-layout`：临时简单文字排版验证，后续应由中性样例替换或移除。
...
这些新增 synthetic case 只用于补齐代表性输入覆盖，不代表真实商业设计质量，不属于工具、skill 或产品功能。所有 case 尚未完成真实 Photoshop 结果截图和人工评分。
