...
当前 benchmark 目录已有 5 个 reference-captured case，已覆盖 `certificate-text-layout`、`poster-layout`、`ecommerce-detail`、`main-image` 四类代表性输入。`rr-001-fex-certificate-text-layout` 仍只是临时简单文字排版验证，后续应由中性文字排版 case 替换或移除；新增海报、详情页和主图 case 都是 synthetic seed，只用于结构覆盖，不代表真实商业设计质量。
