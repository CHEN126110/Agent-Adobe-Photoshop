...
export class FindLayersTool implements Tool {
    ...
    schema: ToolSchema = {
        ...
        parameters: {
            ...
            properties: {
                ...
                withinGroupId: {
                    type: 'number',
                    description: '只在该图层组内查找（可单独使用，返回组内全部后代；不给则全文档）'
                },
                includeBounds: {
                    type: 'boolean',
    ...
    async execute(params: {
        ...
        try {
            ...
            const hasWithinGroupId = params.withinGroupId !== undefined
                && params.withinGroupId !== null;
            if (!nameContains && !nameEquals && !kindFilter && !hasWithinGroupId) {
                return { success: false, error: 'findLayers 需要至少一个条件：nameContains / nameEquals / kind / withinGroupId。全量结构请用 getLayerHierarchy（大文档配 rootLayerId）。' };
            }

            let root: any = doc;
            let searchedWithin = '(整个文档)';
            if (hasWithinGroupId) {
                const groupId = Number(params.withinGroupId);
                if (!Number.isFinite(groupId)) {
                    return { success: false, error: `withinGroupId 必须是有效的图层组 ID，当前值：${String(params.withinGroupId)}` };
                }
                const group = findLayerByIdDeep(doc, groupId);
                if (!group) {
                    return { success: false, error: `未找到 withinGroupId=${groupId} 的图层组。可先用 findLayers({nameContains:'组名', kind:'group'}) 找组 id。` };
                }
                if (resolveKind(group) !== 'group') {
