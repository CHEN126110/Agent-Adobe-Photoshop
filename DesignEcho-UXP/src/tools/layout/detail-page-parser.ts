...
export class DetailPageParser {
    ...
    private parseTextLayer(layer: any, zone: LayerZone): CopyPlaceholder {
        const bounds = this.getBounds(layer);
        // 图层名只描述图层身份，绝不能冒充 Photoshop 中实际保存的文字内容。
        // 空 contents 必须原样回读为空，让 Agent 的 planned-vs-observed 校验失败关闭。
        const text = String(layer?.textItem?.contents ?? layer?.text ?? '').trim();
    ...
    async execute(_params: { includeStructure?: boolean }): Promise<TemplateParseResult> {
        try {
            const observation = await observeActiveDocumentAtHistoryState({
                commandName: 'DesignEcho: 解析详情页模板',
                timeOut: 15,
                unavailableMessage: '无法读取详情页文档的 Photoshop 历史版本，已拒绝返回无法绑定版本的解析结果。',
                changedMessage: '解析详情页结构期间 Photoshop 文档发生变化，已丢弃这次可能过期的解析结果。'
            }, async () => {
            ...
            const status = error instanceof PhotoshopDocumentObservationError
            ...
            return {
                success: false,
                ...(status ? { status } : {}),
                error: String(error?.message || '详情页模板解析失败。'),
                documentName: '',
                documentSize: { width: 0, height: 0 },
                screenCount: 0,
                screens: [],
