...
function findLayerLocation(container: any, layerId: number): LayerLocation | null {
    const layers = Array.isArray(container?.layers) ? container.layers : [];
    for (let index = 0; index < layers.length; index += 1) {
        const layer = layers[index];
        if (Number(layer?.id) === layerId) {
            return { layer, parent: container, index };
        }
...
async function moveReplacementIntoPlaceholderSlot(
    replacementLayer: any,
    placeholderLayer: any
): Promise<void> {
    if (typeof replacementLayer?.move !== 'function') {
        throw new Error('当前 Photoshop UXP 环境无法保持占位图层的原层级。');
    }
    const placement = getPhotoshopElementPlacement(
        constants,
        'PLACEBEFORE',
        'ReplaceImagePlaceholder'
    );
    await Promise.resolve(replacementLayer.move(placeholderLayer, placement));
}
...
export class ReplaceImagePlaceholderTool implements Tool {
    ...
    async execute(params: {
        ...
        try {
            const originalLayerId = Number(layer.id);
            let resultLayerId = originalLayerId;
            let resultLayerName = String(layer.name || '');
            let resultParentLayerId: number | null = null;
            const targetLocation = findLayerLocation(doc, originalLayerId);
            const targetParentLayerId = Number.isFinite(Number(targetLocation?.parent?.id))
                ? Number(targetLocation?.parent?.id)
                : null;
            ...
            await core.executeAsModal(async () => {
                ...
                failureStage = 'preserve-placeholder-hierarchy';
                await moveReplacementIntoPlaceholderSlot(newLayer, layer);
                const replacementLocation = findLayerLocation(doc, Number(newLayer.id));
                resultParentLayerId = Number.isFinite(Number(replacementLocation?.parent?.id))
                    ? Number(replacementLocation?.parent?.id)
                    : null;
                if (resultParentLayerId !== targetParentLayerId) {
                    throw new Error('新图片没有进入占位符所在的原父级，已阻止删除占位符。');
                }
                    ...
                    // 删除原占位符必须是最后一个可能改变文档的步骤。此前任何重命名、
                    // bounds 读取或审计失败都只清理新图层，绝不让原占位符先消失。
                    await layer.delete();
                    placeholderDeleted = true;
                } catch (operationError: unknown) {
                    const originalStillExists = !!findLayerById(doc, originalLayerId);
                    const canRemoveOrphan = !!newLayer
                        && !placeholderDeleted
                        && originalStillExists
                        && Number(newLayer.id) !== originalLayerId;
                    if (canRemoveOrphan) {
                        try {
                            await newLayer.delete();
                        } catch (cleanupError: unknown) {
                            throw new Error(
                                `${describeUnknownError(operationError)}；`
                                + `置入失败后的孤儿图层清理也失败：${describeUnknownError(cleanupError)}`
                            );
                        }
                    }
            ...
            return {
                ...
                targetLayerId: originalLayerId,
                parentLayerId: resultParentLayerId,
                targetParentLayerId,
                hierarchyPreserved: resultParentLayerId === targetParentLayerId,
                placementAudit,
