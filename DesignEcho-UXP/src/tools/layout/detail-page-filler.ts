...
export class DetailPageFiller {
    ...
    private async fillImage(
        ...
        await core.executeAsModal(async () => {
            ...
            const placedRect = layerRect(placedLayer);
    ...
    private async moveLayerAbove(layer: any, targetLayer: any): Promise<void> {
        ...
        const placement = constants?.ElementPlacement?.PLACEBEFORE;
        if (!placement) {
            throw new Error('DetailPageFiller failed: current Photoshop UXP environment does not expose ElementPlacement.PLACEBEFORE.');
        }
