 ...
 * 目标是给 Agent / 开发者提供可复核的验收证据，而不是替代视觉截图。
 */
...
export class GetAcceptanceSnapshotTool implements Tool {
    ...
    schema: ToolSchema = {
        name: 'getAcceptanceSnapshot',
        description: '获取当前 Photoshop 文档的轻量验收快照，只读返回文档、图层、文字、边界、选中状态等结构化证据，用于任务完成后的验证和 Debug。',
        parameters: {
            type: 'object',
            properties: {
                includeHidden: {
    ...
    private collectLayers(args: {
        ...
        const sourceLayers = Array.from(args.container.layers || []) as any[];
        ...
        for (let index = 0; index < sourceLayers.length; index++) {
            ...
            if (!args.includeHidden && layer.visible === false) {
                continue;
            }

            const kind = this.getLayerKind(layer);
            const pathParts = [...args.parentPath, String(layer.name || `Layer ${layer.id}`)];
            const layerInfo: AcceptanceLayer = {
                id: Number(layer.id),
                ...
                path: pathParts.join('/'),
                selected: args.selectedIds.has(Number(layer.id)),
            ...
            if (layer.layers && layer.layers.length > 0) {
                this.collectLayers({
                    ...args,
                    container: layer,
                    parentId: Number(layer.id),
                    parentName: String(layer.name || ''),
                    depth: args.depth + 1,
    ...
    private getLayerEditability(kind: string): AcceptanceEditability {
        if (kind === 'text') {
            return {
                ...
                editable: true,
                reasons: ['文本图层保留 Photoshop 文本对象，可作为结构化可编辑复刻证据。'],
                warnings: []
            };
        }
        ...
        if (editableShapeKinds.has(kind)) {
            return {
                ...
                editable: true,
                reasons: ['形状或矢量填充图层保留结构化形状属性，可作为可编辑复刻证据。'],
                warnings: []
            };
        }
