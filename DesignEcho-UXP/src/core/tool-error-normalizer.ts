...
// 用 type 而非 interface：对象字面量类型才有隐式索引签名，
// 才能赋给 extends Record<string, unknown> 的各工具 Result 接口（interface 无隐式索引签名）。
...
export function createToolFailureResult(input: CreateToolFailureResultInput): ToolFailureResult {
    ...
    const actionableSummary = `${normalized.userMessage} ${normalized.suggestedAction}`.trim();
    return {
        success: false,
        code: normalized.category,
        error: actionableSummary,
        message: actionableSummary,
...
/**
 * Tool 可以自行返回失败对象，但公共执行边界必须保证它带稳定机器码和可读诊断。
 * 这里只补齐缺失字段，不改变 success、取消、确认或重试语义。
 */
export function ensurePhotoshopToolFailureDiagnostics(
    ...
    if (failure.success !== false) return result;
    const existingDetail = readNonEmptyText(failure.error)
        || readNonEmptyText(failure.message)
        || readNonEmptyText(failure.summary)
        || readNonEmptyText(failure.errorDetails?.userMessage)
        || readNonEmptyText(failure.errorDetails?.message);
    // 已有机器码属于 Tool 的公开契约；公共边界只补缺失值，不改写既有 code。
    const code = readNonEmptyText(failure.code)
        || normalizeDiagnosticCode(failure.errorCode)
        || normalizeDiagnosticCode(failure.errorDetails?.category)
        || (failure.cancelled === true
            ? 'task_cancelled'
            : (existingDetail ? 'photoshop_tool_execution_failed' : 'photoshop_tool_failure_diagnostic_missing'));
    const summary = existingDetail
        ? `「${String(toolName || 'Photoshop 工具')}」未完成：${existingDetail} 请根据该原因重新读取目标对象或修正输入后再继续。`
        : `「${String(toolName || 'Photoshop 工具')}」未完成，工具没有返回具体原因。请重新读取当前文档和目标后再处理。`;
    return {
        ...failure,
        code,
        error: readNonEmptyText(failure.error) || summary,
        message: readNonEmptyText(failure.message) || summary,
        summary: readNonEmptyText(failure.summary) || summary
    };
}
...
function buildUserMessage(toolName: string, category: PhotoshopToolErrorCategory, message: string): string {
    switch (category) {
        ...
        case 'missing_target':
            return `${toolName} 缺少可操作的目标对象：${message}`;
