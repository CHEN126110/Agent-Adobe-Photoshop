...
function hasPhotoshopExecutionResult(result: unknown): boolean {
    if (!isRecord(result)) return false;
    if (isRecord(result.photoshopMutationCommit)) return true;
    return isRecord(result.photoshopOperationResult)
        && result.photoshopOperationResult.version === 'photoshop-operation-result/v1';
}
...
function finalizeRawCancelledResult(result: unknown): Record<string, unknown> {
    ...
    if (isRecord(result)
        && (hasPhotoshopExecutionResult(result) || result.success === true)) {
        return {
            ...result,
