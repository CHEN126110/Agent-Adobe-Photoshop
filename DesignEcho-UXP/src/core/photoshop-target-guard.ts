...
import { ensurePhotoshopToolFailureDiagnostics } from './tool-error-normalizer';
...
export async function executeToolWithPhotoshopTargetGuard(
    ...
    if (!guardedParams.hasGuard) {
        ...
        return ensurePhotoshopToolFailureDiagnostics(tool.name, result);
    }
    ...
    const result = await tool.execute(guardedParams.toolParams, guardedContext);
    return ensurePhotoshopToolFailureDiagnostics(tool.name, result);
}
...
function createPhotoshopTargetChangedResult(
    ...
    const error = buildTargetChangedError(reason, expected, actual);
    return {
        success: false,
        code: 'photoshop_target_changed_before_execution',
        error,
