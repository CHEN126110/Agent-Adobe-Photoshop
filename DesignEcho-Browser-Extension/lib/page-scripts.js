...
// 可选的候选图片元数据清单（src/alt/尺寸，像素下载在 service worker 完成）。
export function readPageScript(maxChars, includeElements, includeImages, maxImages) {
  ...
  const imageLimit =
    typeof maxImages === 'number' && maxImages > 0
      ? Math.min(Math.floor(maxImages), 12)
      : 12;
  // 候选图片最小可见尺寸：过滤 1×1 埋点、图标与装饰像素，只留可能承载设计参考的图。
  ...
  if (includeImages) {
    // 只收集元数据：像素由 service worker 用扩展跨域权限 + 登录态下载（见 handlers.js）。
    const seenSrcs = new Set();
    const imageCandidates = [];
    for (const img of Array.from(document.images)) {
      if (imageCandidates.length >= imageLimit) {
        break;
      }
      const src = String(img.currentSrc || img.src || '').trim();
      if (!/^https?:\/\//i.test(src) || seenSrcs.has(src)) {
        continue;
      }
      // 自然尺寸可用（已加载）时以它为准；未加载但布局尺寸足够大也保留，
      // service worker 下载后还会再按解码尺寸过滤一次，双保险防 1×1 占位图混入。
      const naturalWidth = img.naturalWidth || 0;
      const naturalHeight = img.naturalHeight || 0;
      const rect = img.getBoundingClientRect();
      const layoutWidth = Math.round(rect.width) || naturalWidth;
      const layoutHeight = Math.round(rect.height) || naturalHeight;
      if (
        Math.max(layoutWidth, layoutHeight) < MIN_IMAGE_SIDE
        || Math.min(naturalWidth || layoutWidth, naturalHeight || layoutHeight) < MIN_IMAGE_SIDE
      ) {
        continue;
      }
      seenSrcs.add(src);
      imageCandidates.push({
        src,
        alt: collapseText(img.alt || img.getAttribute('aria-label') || '', 120),
        width: naturalWidth || layoutWidth,
        height: naturalHeight || layoutHeight,
      });
    }
    result.images = imageCandidates;
  }
  return result;
}
...
// 只读当前滚动状态（不移动页面；fullPage 拼接前记录初始位置用）。
...
// 精确滚动到指定纵向位置（fullPage 拼接截图用：切片位置必须与截图像素一一对应）。
export function scrollToScript(targetY) {
  window.scrollTo(0, Number.isFinite(targetY) ? Math.max(0, targetY) : 0);
  return {
    scrollY: Math.round(window.scrollY),
    innerWidth: Math.round(window.innerWidth),
    innerHeight: Math.round(window.innerHeight),
